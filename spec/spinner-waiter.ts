...
function setup(page: Page) {
  ...
  for (const method of overrideableMethods) {
    ...
    const impl = (process.env.SPINNER_WAITER_IMPL || "loop") as "loop" | "race" | "trycatch";
    if (impl === "loop") {
      value = getLoopUntilVisibleImpl(method);
    } else if (impl === "race") {
      value = getRaceImpl(method);
    } else if (impl === "trycatch") {
      value = getTrycatchImpl(method);
    } else {
...
const getTrycatchImpl = (method: OverrideableMethod) => {
  return async function trycatchImpl(this: LocatorWithOriginal, ...args: unknown[]) {
    const optionIndex = oneArgMethods.includes(method as OneArgMethod) ? 1 : 0;
