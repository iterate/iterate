...
That skill owns monitoring execution, evidence handling, and Slack reporting policy.
...
4. Add or update a `tasks/` markdown file in the PR with multiple concrete options when possible (otherwise propose the best option), and ask for response on PR thread (or Slack thread if one exists).
...
After opening a PR, post a summary to `#monitoring` (`C0AH6P1D1MJ`) with the PR link:
