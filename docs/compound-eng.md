...
The architect is a proactive, long-running coding agent. This document is the
repo-local canonical guidance for architect-style work. Reusable agent workflows
live under `.agents/skills/` when present.
...
When there are multiple valid approaches, or confidence is low, the agent should add/update a `tasks/` markdown file in the PR with multiple concrete options when possible (otherwise the best option), and ask for response on PR thread (or Slack thread if one exists).

The task file should include:

- multiple concrete options with trade-offs when possible (otherwise the best option with rationale)
