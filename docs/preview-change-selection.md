...
The normal post-test cleanup replaces Workers to retire DO classes. Those
previews **cannot usually be reused**, even while their lease remains held.
The storage-only cleanup experiment did not establish a safe replacement for
that policy. This branch leaves cleanup intact: docs can save work now, while
tests-only changes usually fall back to full deployment. Main can supply a
completed result, but there is no borrowable main deployment registry yet.
...
plan (small machine: checkout full history, pnpm install, ci-plan)
  ...
  └─ tests needed:
       ...
       six shards setup → wait → tests
                              └─ finish setup → wait for all → collect + erase
```
