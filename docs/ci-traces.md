...
> `finish` job. The trace job itself and PR-close deletion are left out of the
> trace, as the legacy cleanup was. Step spans come from
...
> (`public-ci-trace-<workflow>-<execution>`, kept 30 days). The job posts the
> **CI trace** commit status itself (`statuses: write`), with the time to green
> or red, and links it to the job on Depot, where the artifact downloads. The
> legacy `iterate/config` project hosted reports at
> `depot-<artifact-id>--iterate.iterate.app` and published this status from
> `check_run` webhooks. It went with #2837, so a report is no longer browsable
> in place: download it and open `trace.html`. Where the sections below name
> plan, prepare, shard or finish jobs, `status.ts` or the publisher, they
> describe the legacy `preview-run.yml` pipeline. The assembler still accepts
> that shape.
