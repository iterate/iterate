...
Audio/kit caller migration is deferred to a follow-up; their scripts are
...
1. Review the transport changes, including the deferred audio/kit migration.
...
`intercepted/*` exercises production request preparation and response
decoding using `ai.intercept` HTTP fixtures. Credentials are attached only after the interception decision. Real model names cannot use this interceptor.
