...
| `info`                                              | data      | `{ principal, scopes, platformOrigin, ingressRouting, mcpOrigin, iterateAppProviders }` — the granted `scopes` decide what a page offers; `iterateAppProviders`, where iterate's app connects.   |
