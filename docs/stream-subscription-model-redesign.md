...
project.streams.get(path)                  // exists (ProjectStreamCollectionRpcTarget)
  ...
  .subscriptions.get(name)
     ...
     .waitUntilProcessed({offset})         // one verb, every kind (wake→runner; push→confirmed)
...
| `waitForEvent` / `waitUntilProcessed`                                          | verbs: `waitForEvent` = log-side; `waitUntilProcessed` = consumer-side, one verb every kind — but not one mechanism: processor-wake rows delegate to the hosted runner's fold barrier (precise even while a live connection streams and the stored cursor is deliberately stale), push kinds resolve off the confirmed cursor |
