...
| [`scripts/ci/preview-os-gate.ts`](../scripts/ci/preview-os-gate.ts) and its test                                            | `previewPaths` (the list the `paths` filter held), `touchesPreview` (GitHub's `paths` semantics) and `previewVerdict` (the gate's rules). 26 table rows, including a real merge commit in a scratch repo.                             |
