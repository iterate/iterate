...
| Playwright specs | `pnpm spec`                                                        | `specs/` (root `playwright.config.ts`, one project per app host: `os`, `os-phone`, `notes`, `voice`, `suite`) | Preview OS **Browser specs** job, every project but `os-phone` (Main OS e2e only), side by side with OS e2e, against the preview's apps      | Browser-level product flows: issuer sign-in and consent (plus a phone-width project), the issuer's server functions, the project mini-app, and the Notes and Voice flows.                                                                        |
