...
`apps/os` is the primary product app. It provides the authenticated web UI and the itx engine (`apps/os/src/next/`): capability handles over `/api/itx`, and Durable Objects for project lifecycle, event streams, agents, repositories, secrets, dynamic workers, and itx capability scopes. OS has no database of its own — the Iterate Auth Worker is both the identity provider and the project directory (fronted by a KV cache); all other durable state is Durable Object SQLite.
