...
| Workspace                 | `workspace/created`       | `workspaces.get(path).create({ mounts? })`                                                            | One batch carries the default birth config, an optional initial mount patch, and the Workspace subscription. Addressing never births a workspace; filesystem and configuration methods reject until create completes.                                                                                                                                     |
