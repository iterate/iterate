...
For an atomic creation, the birth is the first event in that processor's
domain history. A creation saga instead has exactly two lifecycle events:
`*/create-requested` is the durable intent and terminal `*/created` is the
birth certificate. Do not add a third `*/ready` phase. A physical stream still
has its infrastructure-level `stream/created` record first, and a shared
stream may contain sibling processors' events.

The Project and Repo processors are the reference saga shape. Their state
stores both `createRequest` and `birthCertificate`; the request reaction does
the idempotent creation work, and only the successful final reaction appends
`*/created`. Each saga must define what happens to commands queued before that
terminal event. Project's blocking request frame establishes every sibling
prerequisite before its cursor advances, so later project commands remain
actionable while the userspace worker barrier is still open.
...
Creation sagas perform asynchronous provisioning between `*/create-requested`
and terminal `*/created`; Project and Repo are the reference examples. Their
public create APIs may return after committing the request or wait for the
terminal certificate, depending on whether the caller renders progress. Do not
add a third generic readiness phase.
...
- `state.devices`, `state.repos`, `state.secrets`, and `state.workspaces`
  record copied domain `*/created` events.

The domain path comes from the event's processor/copy provenance, not
from a made-up field in the birth payload. A router or domain processor that
creates a catalogued object copies that object's birth to `/`. As a
...
| ------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Project                   | `project/created`         | Project processor, after `session.projects.get(slug).create(...)` appends `project/create-requested`  | The request batch births Notification and subscribes both root processors. Project creates the root Capability Host, primary Scheduler, config Repo, and Email router. After the seeded worker answers a readiness probe, it installs the root worker feed and appends terminal `project/created`; deterministic failures append `project/create-failed`. |
| Repo                      | `repos/created`           | `project.repos.get(path).create(...)`, global repo creation, or Project bootstrap for `/repos/config` | Appends `repos/create-requested`, subscribes Repo, provisions or imports the artifact, then emits `repos/created` (or `repos/create-failed`), which is copied to `/`.                                                                                                                                                                                     |
| Agent                     | `agent/created`           | `project.agents.get(path).create()` or a Slack/Telegram/Email router                                  | Creates the paired Capability Host, setup events, and subscriptions; copies its birth to `/`. Caller-selected context/configuration is appended afterwards.                                                                                                                                                                                               |
...
| Secret                    | `secret/created`          | `secrets.get(path).create(...)`; integration connection setup uses that same Secret DO API            | Encrypts material before append, subscribes Secret, waits for processing, and copies the birth to `/` without plaintext material.                                                                                                                                                                                                                         |
| Workspace                 | `workspace/created`       | `workspaces.get(path).create({ mounts? })`                                                            | One batch carries the existence marker, an optional initial overlay patch, the Workspace subscription, and the `workspace-catalog` copy of the birth to `/` (the live mount table is derived: every project repo at its own `/repos/**` path). Addressing never births a workspace; filesystem and configuration methods reject until create completes.   |
| Device                    | `device/created`          | `devices.get(id).enroll(...)`                                                                         | `enroll` remains the domain door because it first routes the push token into a Secret; the device certificate and subscription then append atomically after that Secret offset and the call waits through the batch.                                                                                                                                      |
...
| Sandbox status projection | `sandbox/created`         | The Sandbox instance DO called by `sandboxes.get(path).create(...)`                                   | The handle first claims the unique path with `sandbox/create-requested` in `/sandboxes`; the instance appends its birth, optional initial env config, and processor subscription in one batch, then waits for its hosted reducer.                                                                                                                         |
