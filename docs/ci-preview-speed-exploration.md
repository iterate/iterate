...
state-generation proof.

Two corrected same-head runs passed in 3m10s and 2m55s. Both reused the four
immutable-worker apps and ran Auth's normal deploy/reconciliation in 18.0s and
16.7s respectively. The faster observation is 100 seconds (36%) below the
4m35s control. Each run needed one OS test retry for a different transient
runtime failure, so they establish the safe policy and its observed envelope,
not a clean no-retry A/B timing. The historical 2m46s run remains useful only
as the unsafe all-immutable ceiling.

This can get the fully selected warm-slot case below three minutes without
...
| 2026-07-22 | Validate the corrected four-app reuse policy. | Depot jobs `p73g2d29p6` and `jn11qhk4l0` passed in 3m10s and 2m55s. Both reused OS, Semaphore, Streams, and Dummy Petshop while Auth deployed normally in 18.0s/16.7s. Each had one different OS test retry.              | Accept the policy evidence and report 2m55s-3m10s as the observed safe envelope, not a clean no-retry benchmark.                         |
