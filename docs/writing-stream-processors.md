...
- **Reconciliation** — ordinary `processEvent` code under
  ...
  therefore guard `event !== null`. Wake frames normally carry contiguous raw
  scan progress across filtered rows. If a stale or incomplete sender stops
  that proof behind its advertised raw head, the runner performs a trailing
  unfiltered self-pull so the tail cannot strand an obligation.
