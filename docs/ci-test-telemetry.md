...
PostHog is the historical query layer for test performance, preview deployment
performance, failures, flakes, CI queueing, Depot utilization, and
automated-review outcomes. Retained CI artifacts are the replayable source
evidence. Vitest unit/e2e, Playwright e2e, Node tests, and standalone smoke
scripts all use one runner-neutral data model.
...
The preview orchestrator records deployment evidence in its canonical raw
artifact; the finalizer converts it to deployment events alongside the test
events. Deployment events therefore use the same retained, replayable source
and run identity as test events, plus
`deployment_kind = 'cloudflare-preview'`, `app`, `preview_slot`, and exact
Worker identity when it is available.
