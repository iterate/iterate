...
- API schemas / contracts / shape - e.g. what classes and public functions in a library are exposed etc
...
**Avoid Chesterton's fence: When you're doing something because you found a primary source that said so, you MUST link to it**
...
- Project slugs come from `projectSlug` in `apps/os-next/src/directory.ts`
...
- IDs are minted with a type prefix (`prj_<hex>`, `org_<hex>`) in `apps/os-next/src/directory.ts`; see [identifiers](identifiers.md)
