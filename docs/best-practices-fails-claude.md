...
**Analysis:** The CLAUDE.md rule appears outdated. Since the codebase uses Zod v4.1.12 exclusively, importing from `"zod"` is correct. The `/v4` subpath was only needed during the v3→v4 transition or when v5+ is installed alongside v4.
...
- Middleware order is correct (contextStorage → cors/secureHeaders → app middleware)
...
build: {
  minify: "terser",
  terserOptions: {
    mangle: false,  // ❌ Unnecessarily increases bundle size
  },
}
```
