...
- Wire deployment runtime env via lightweight API (`~/.iterate/.env`) by setting `ITERATE_EGRESS_PROXY` to the FRP data proxy URL.
...
- Deployment egress traffic can be observed through runner-side mock via FRP tunnel when `ITERATE_EGRESS_PROXY` is set.
