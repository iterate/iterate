...
// TODO: replace with new example contract once migrated
  ...
  describe.each(cases)("$id", ({ createDeployment, exposePublicTarget, tags, timeoutMs }) => {
    test(
      ...
      async ({ expect, e2e }) => {
        ...
        expect(String(headers["x-iterate-resolved-app"] ?? "")).toBe("example");
        console.log("[public-ingress] test completed successfully", {
          publicBaseHost,
          resolvedApp: headers["x-iterate-resolved-app"] ?? null,
        });
      },
    );
  });
...
- GET http://iterate.localhost should work (registry app)
- GET http://registry.iterate.localhost should work (registry app)
- GET http://example.iterate.localhost should work (example app)

then change ITERATE_INGRESS_DEFAULT_APP=example in .env

- GET http://iterate.localhost should work (example app)
- GET http://registry.iterate.localhost should work (registry app)
- GET http://example.iterate.localhost should work (example app)
