...
const providers = [
  { label: "docker", factory: DockerDeployment.withConfig({ image: DOCKER_IMAGE }) },
  { label: "fly", factory: FlyDeployment.withConfig({ image: FLY_IMAGE }) },
];

describe.each(providers)("example end2end ($label)", ({ factory }) => {
