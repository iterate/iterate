...
type DeploymentCase = {
  id: string;
  enabled: boolean;
  create: (overrides?: { slug?: string; signal?: AbortSignal }) => Promise<Deployment>;
...
const cases: DeploymentCase[] = [
  {
    ...
    create: async (overrides: { slug?: string; signal?: AbortSignal } = {}) => {
      const { slug = "", signal } = overrides;
    ...
    test(
      ...
      async ({ e2e }) => {
        const deployment = await create({
          slug: `e2e-example-${randomUUID().slice(0, 8)}`,
        ...
        while (Date.now() < routeDeadline) {
          const listedRoutes = await deployment.registryService.routes.list({});
        ...
        while (Date.now() - startedAt < 30_000 + timeoutOffsetMs) {
          const streams = await deployment.eventsService.listStreams({});
          found = streams.some(
