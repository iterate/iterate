...
describe.runIf(DOCKER_IMAGE.length > 0)("frp egress minimal", () => {
  test("records HAR from scripts executed inside deployment via FRP external proxy", async ({
...

    {
      ...
      const inspect = await inspectDockerContainer({
        locator: deployment.locator as DockerDeploymentLocator,
      });
  ...
  test("pnpm install works from inside deployment through FRP external proxy", async ({ e2e }) => {
    const runId = randomUUID().slice(0, 8);
    const artifactsDir = join(process.cwd(), "artifacts", "frp-minimal");
    await mkdir(artifactsDir, { recursive: true });
...

    {
      ...
      const deployment = await Deployment.create({
        provider: createDockerProvider({}),
        opts: {
          slug: `e2e-frp-pnpm-install-${runId}`,
          image: DOCKER_IMAGE,
          dockerHostSync: resolveDockerHostSync(),
        },
      });
