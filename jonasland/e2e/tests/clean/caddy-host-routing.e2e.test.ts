...
function parseJsonFromOutput(output: string): Record<string, unknown> {
  const ansiStripped = output.replace(ansiEscapePattern, "");
  ...
  try {
    ...
    if (firstBrace >= 0 && lastBrace > firstBrace) {
      for (let start = firstBrace; start <= lastBrace; start += 1) {
        if (trimmed[start] !== "{") continue;
        for (let end = lastBrace; end > start; end -= 1) {
...
describe.runIf(DOCKER_IMAGE.length > 0)("caddy host routing", () => {
  test("all host forms route correctly for bootstrap and registry-managed services", async ({
    ...
    const deployment = await Deployment.create({
      ...
      opts: {
        slug: `e2e-caddy-routing-${randomUUID().slice(0, 8)}`,
        image: DOCKER_IMAGE,
    ...
    // Wait for registry to pick up the new ITERATE_INGRESS_HOST and
    ...
    const healthCases: Array<[string, string, string, string | null]> = [
      // Bootstrap: registry (has /api/__iterate/health via oRPC)
      ["registry internal", "registry.iterate.localhost", "/orpc/__iterate/health", null],
  ...
  test("public internet egress works via iptables redirect", async ({ e2e }) => {
    const deployment = await Deployment.create({
      provider: createDockerProvider({}),
      opts: {
        slug: `e2e-egress-public-${randomUUID().slice(0, 8)}`,
        image: DOCKER_IMAGE,
        ...(useDockerHostSync ? { dockerHostSync: true } : {}),
        env: {
    ...
    // (MITM TLS) → egress fallback → internet.
  ...
  test("external egress proxy routes traffic through configured proxy", async ({ e2e }) => {
    const deployment = await Deployment.create({
      provider: createDockerProvider({}),
      opts: {
        slug: `e2e-egress-extproxy-${randomUUID().slice(0, 8)}`,
        image: DOCKER_IMAGE,
        ...(useDockerHostSync ? { dockerHostSync: true } : {}),
        env: {
    ...
    // ITERATE_EGRESS_PROXY from the deployment env, pointing at
    // the inline echo server on :19123.

    // Make a normal outbound HTTPS request. iptables redirects it to Caddy,
    // and the unmatched egress path forwards to the configured external proxy
    // (:19123).
    const deadline = Date.now() + 30_000;
    let result = { exitCode: 1, output: "" };
    while (Date.now() < deadline) {
