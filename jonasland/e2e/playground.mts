...
async function runSelfChecks(params: {
  ...
  console.log(
    `[playground] self-check PASS: registry uses ITERATE_INGRESS_HOST=${expectedPublicBaseUrl}`,
  );
...
async function main(): Promise<void> {
  ...
  const publicBaseHostType =
    firstNonEmpty([
      ...
      process.env.ITERATE_INGRESS_ROUTING_TYPE,
    ]) || "prefix";
  ...
  console.log(
    `[playground] setting deployment env vars: ITERATE_INGRESS_HOST=${publicBaseHost} ITERATE_INGRESS_ROUTING_TYPE=${publicBaseHostType}`,
  );
  await deployment.setEnvVars({
    ITERATE_INGRESS_HOST: publicBaseHost,
    ITERATE_INGRESS_ROUTING_TYPE:
      publicBaseHostType === "prefix" ? "dunder-prefix" : "subdomain-host",
  });
