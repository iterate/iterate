...
async function resolveDockerFrpControl(params: { deployment: Deployment }): Promise<{
  host: string;
  port: number;
  ...
  if (locator.provider === "docker" && typeof locator.containerId === "string") {
    try {
      const inspect = await inspectDockerContainer({
        locator: locator as DockerDeploymentLocator,
      });
      const containerName = normalizeContainerName(inspect.Name);
      if (containerName.length > 0) {
        return {
          host: `frp.${containerName}.orb.local`,
          port: 80,
          transport: "websocket",
        };
      }
...
export async function startFrpEgressBridge(params: {
  ...
  try {
    ...
    if (ingressHostname.endsWith(".fly.dev") || ingressHostname.endsWith(".ingress.iterate.com")) {
      ...
      controlServerHost = resolved.host;
      controlServerPort = resolved.port;
      controlTransportProtocol = resolved.transport;
    }
