...
async function main(): Promise<void> {
  ...
  await using deployment = await DockerDeployment.create({
    dockerImage,
    slug: `local-example-${randomUUID().slice(0, 8)}`,
    dockerHostSync,
  ...
  const example = deployment.createServiceClient({
    slug: exampleServiceManifest.slug,
    orpcContract: exampleServiceManifest.orpcContract,
  });
  ...
  while (Date.now() < deadline) {
    ...
    if (proc.state !== "running") {
      ...
      for await (const event of deployment.events({
        signal: AbortSignal.timeout(2_000),
        logTail: 200,
      })) {
        if (event.type !== "https://events.iterate.com/deployment/logged") continue;
      ...
      const filtered = filteredLines.slice(-120).join("\n");
      console.log(
