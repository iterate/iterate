...
```bash
# valid
iterate tool exec-ts 'await webchat.postMessage({ threadId: "THREAD_ID", text: "hi" })'

# invalid! ERROR!!!!
iterate tool exec-ts send --thread-id THREAD_ID --body "hi"
iterate tool exec-ts --thread-id THREAD_ID --body "hi"
iterate tool exec-ts postMessage ...
```

## Quick Reference: Sending Messages

**IMPORTANT:** `iterate tool exec-ts` takes TypeScript code as its only argument — there are NO subcommands.

```bash
# Reply in a thread (replace THREAD_ID with actual value from the incoming message)
iterate tool exec-ts 'await webchat.postMessage({ threadId: "THREAD_ID", text: "Your response here" })'
```
...
```bash
iterate tool exec-ts 'await webchat.postMessage({
  threadId: "THREAD_ID",
...
```bash
iterate tool exec-ts 'await webchat.addReaction({
  threadId: "THREAD_ID",
...
```bash
iterate tool exec-ts 'await webchat.removeReaction({
  threadId: "THREAD_ID",
  messageId: "MESSAGE_ID",
  reaction: "thumbsup",
})'
...
```bash
iterate tool exec-ts 'const result = await webchat.getThreadMessages({ threadId: "THREAD_ID" }); console.log(JSON.stringify(result, null, 2))'
```
