...
const listEvents = os.events.list.handler(async ({ input, context }) => {
...
const createEvent = os.events.create.handler(async ({ input, context }) => {
...
const findEvent = os.events.find.handler(async ({ input, context }) => {
...
const updateEvent = os.events.update.handler(async ({ input, context }) => {
...
const removeEvent = os.events.remove.handler(async ({ input, context }) => {
  const existing = await getEventRowById(input.id);
