job "caddymanager" {
  ...
  group "caddymanager" {
    ...
    task "backend" {
      ...
      config {
        command = "/etc/jonasland5/scripts/run-caddymanager-backend.sh"
      }
      ...
      env {
        ...
        JWT_SECRET              = "jonasland5-local-dev-secret"
        JWT_EXPIRATION          = "24h"
        PING_INTERVAL           = "2000"
        PING_TIMEOUT            = "1000"
    ...
    task "frontend" {
      ...
      config {
        ...
        args    = ["run", "--config", "/etc/jonasland5/caddymanager/Caddyfile", "--adapter", "caddyfile"]
      }
