...
describe("a start app's preview config (a pure transform of the built wrangler.json)", () => {
  ...
  test("the preview's own block: the session class, observability, and the issuer as its ONE var", () => {
    expect(config.previews).toEqual({
      observability: { enabled: true },
      durable_objects: built.durable_objects,
      vars: { ITERATE_ORIGIN: "https://pr123-foo-os-next-preview.iterate-dev-preview.workers.dev" },
    });
  });
});
