  ...
  os: {
    ...
    previewTestCommandArgs: [
      ...
      [
        ...
        `PREWARM_STARTED=$SECONDS; PREWARM_OK=0; run_visible_lane project-prewarm env TEST_TELEMETRY_LANE=project-prewarm TEST_TELEMETRY_WORKSPACE=iterate-root TEST_TELEMETRY_ARTIFACT_FILE=${osProjectPrewarmTelemetryFile} timeout --signal=TERM --kill-after=5s ${OS_PREVIEW_PROJECT_PREWARM_TIMEOUT_SECS}s pnpm exec tsx e2e/vitest/preview-project-prewarm.ts || PREWARM_OK=$?`,
        "PREWARM_DURATION_SECONDS=$((SECONDS - PREWARM_STARTED))",
        `if [ ! -s ${osProjectPrewarmTelemetryFile} ]; then env TEST_TELEMETRY_LANE=project-prewarm TEST_TELEMETRY_WORKSPACE=iterate-root TEST_TELEMETRY_ARTIFACT_FILE=${osProjectPrewarmTelemetryFile} pnpm exec tsx e2e/vitest/preview-project-prewarm-fallback.ts "$PREWARM_OK" "$PREWARM_DURATION_SECONDS"; fi`,
        'PREWARM_HARNESS_OK=0; if [ "$PREWARM_OK" -eq 124 ] || [ "$PREWARM_OK" -eq 137 ]; then echo "[preview:os] project prewarm watchdog fired (non-gating): exit $PREWARM_OK"; elif [ "$PREWARM_OK" -ne 0 ]; then PREWARM_HARNESS_OK=$PREWARM_OK; echo "[preview:os] project prewarm telemetry harness failed (gating): exit $PREWARM_OK"; fi',
        ...
        '[ "$PREWARM_HARNESS_OK" -eq 0 ] && [ "$SMOKE_OK" -eq 0 ] && [ "$E2E_OK" -eq 0 ] && [ "$TUI_OK" -eq 0 ] && [ "$SPEC_OK" -eq 0 ]',
      ].join("; "),
    ],
