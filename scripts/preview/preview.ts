...
export async function ciPrepare(options: DeployCommandOptions = {}) {
  if (options.projectLifetimes) options = { ...options, allApps: true };
  const { context, runtime } = await resolvePreviewCommandSetup(options);
  ...
  return await withPreviewE2eTelemetry(context, runtime, "deploy", async (telemetry) => {
    ...
    const projectLifetime = options.projectLifetimes
      ? Lifetime.required().parse({
          group: `${context.pullRequestNumber}/${z.string().min(1).parse(process.env.GITHUB_RUN_ID)}/${z.string().min(1).parse(process.env.GITHUB_RUN_ATTEMPT)}`,
          expiresAt: Date.now() + defaultPreviewLeaseMs,
        })
      : null;
    if (projectLifetime) {
      const runs = await previewLifetimesForEnvironment(slot.dopplerConfig);
      await runs.begin(projectLifetime);
...
async function runPreviewCiCommand(
  ...
  return await runCommand({
    ...
    args: [
      ...
      ...(plan.projectLifetime
        ? [`TEST_PROJECT_LIFETIME=${JSON.stringify(plan.projectLifetime)}`]
        : []),
...
async function deployPreviewApps({
  ...
  runtime.commandEnvironment.ENABLE_PROJECT_LIFETIMES = options.projectLifetimes ? "1" : "0";
  ...
  try {
    ...
    const reuseHeldSlotData = options.projectLifetimes
      ? async (lease: PreviewSemaphoreLease) =>
          lease.slug === recordedSlug &&
          (
            await previewLifetimesForEnvironment(
...
export async function ciDispose(options: EraseOptions = {}) {
  ...
  if (!plan?.projectLifetime) return erase(options);
  if (plan.projectLifetime.group !== `${plan.pullRequestNumber}/${plan.runId}/${plan.runAttempt}`) {
  ...
  await (
    await previewLifetimesForEnvironment(slot.dopplerConfig)
  ).retire(plan.projectLifetime.group);
  return { retired: plan.projectLifetime.group, slot: slot.slug, deploymentPreserved: true };
}
...
const PreviewCiPlan = PreviewCiIdentity.extend({
  projectLifetime: Lifetime.required().nullable().default(null),
