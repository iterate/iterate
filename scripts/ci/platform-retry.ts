 ...
 * (a 5xx, a dropped connection), then the last failure is thrown, so recovery is bounded and a lasting
 * outage still fails the job. `platformFailure` returns what to log for such a failure, or undefined
 * for an answer about the request (a 4xx, an abort), which is thrown at once. Each repeat logs an
 * `event` warn (`<area>.platform-failure-retry`) with the attempt and the wait.
 *
 * The caller decides whether a repeat is safe: a call that could create a second copy passes no
 * delays. GitHub's Octokit hook (github.ts) and Depot's CI API (depot.ts) ask again through this.
 */
