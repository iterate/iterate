...
| `apps/tunnels`           | Shared Captun gateway at tunnels.iterate.com                    |
...
`pnpm tunnel 3000 --name jonas` exposes a running local server at
`https://jonas.tunnels.iterate.com`, with credentials fetched from Doppler.
Omit `--name` for a random URL. HTTP and WebSockets are supported; Ctrl-C closes
the tunnel. See [tunnels](docs/tunnels.md) for CLI and test-fixture examples.
