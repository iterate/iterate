...
import { test as baseTest } from "../test-support/test.ts";
...
type ApprovalFixture = {
  agentPath: string;
  echoUrl: string;
  itx: Awaited<ReturnType<typeof connectItxReady>>;
};

const test = baseTest.extend<{ approvalFixture: ApprovalFixture }>({
  approvalFixture: [
    async ({ page }, use, testInfo) => {
      ...
      await itx.processor.waitUntilProcessed({
        offset: rulesConfigured!.offset,
        timeoutMs: 15_000,
      });
      ...
      await use({ agentPath, echoUrl: echo.url, itx });
    },
    // Real OAuth signup and project provisioning are fixture setup, not the
    // approval behavior's test body. A separate fixture budget prevents cold
    // provisioning from leaving the first durable-delivery wait with 1ms.
    { timeout: 60_000 },
  ],
});
...
test("approve and reject script bursts from inside the chat thread", async ({
  ...
  const { agentPath, echoUrl, itx } = approvalFixture;
 ...
 * probe true. Select-all first because the DOM can briefly retain the prior
 * pasted command after React has logically cleared the composer. */
