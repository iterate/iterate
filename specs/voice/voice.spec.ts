...
test("a new project installs its voice agent from the page, then has a Call button", async ({
  ...
  // Back on the Voice app, on the new project's page. A fresh project has no voice agent and no
  // OpenAI key, so the page offers to install one. The key is only stored: health answers without
  // calling OpenAI.
