 ...
 * and the warning's sheet names the failing commit, shows the build error,
 * and cross-links to the config repo IDE — where the fix (a config commit)
 * happens.
 */
test("a broken config commit shows a red worker warning that leads to the fix", async ({
  ...
  // The incident-shaped fix path: straight to the config repo IDE.
