...
test("budget stop stays visible across reload and new input, then explicit Retry resumes", async ({
  ...
  await agent.append({
    ...
    payload: {
      config: { llm: { model: "intercepted/gateway/gpt-4.1-nano" }, llmRequestDebounceMs: 250 },
    },
  });
  let calls = 0;
  await using _interception = await fixture.interceptAi(async (input) => {
    if (input.source !== "gateway-request") throw new Error(`Unexpected source ${input.source}`);
    calls++;
