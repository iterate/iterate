...
test("a commented tsconfig still schema-validates (comments are tolerated)", async ({
  ...
  // oxlint-disable-next-line iterate/spec-restricted-syntax, middlewright/prefer-locator-waits -- no spinner exists during the background schemastore fetch, so locator.waitFor gets fail-fasted to 1ms; expect().toBeVisible() polls the full 20s.
