...
// Evidence and exit criteria: tasks/playwright-flake-causes.md.
const reviewFlake = createFlake(
  test,
...
reviewFlake(
  "review a workspace document in the seeded Docs app",
  async ({ baseURL, page }, testInfo) => {
    ...
    await editor
      .getByText("Peer inserted before the selected passage.")
      .waitFor({ timeout: 10_000 }); // timeout: remote CodeMirror update has no spinnerWaiter-visible progress
