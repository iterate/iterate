...
import { connectAdminItx, createReusableAdminProject } from "./test-support/forged-session.ts";
...
test("agent replies to a browser chat message in the feed", async ({ helpers, page, baseURL }) => {
  // One full LLM turn plus UI paint. Project creation and the onboarding
  // agent's automatic greeting are independently covered by onboarding-smoke;
  // repeating both here made this browser-routing proof the preview critical
  // path. Reuse one family project, but isolate every run in a fresh agent.
  test.setTimeout(240_000);
  if (!baseURL) throw new Error("Playwright baseURL fixture is required.");
  const familyProject = await createReusableAdminProject({
    baseUrl: baseURL,
    family: "agent-chat",
  });
  await using fixture = await helpers.createFixture("agent-chat", { project: familyProject });
  ...
  await spinnerWaiter.settings.run({ disabled: true }, async () => {
    ...
    // The route's lazy stream-view chunk can outlive the deliberately-tight
    // 750ms action budget on a cold browser. This is startup readiness, not an
    // LLM wait; bound it explicitly before interacting with the composer.
