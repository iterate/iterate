...
A spec for a client app runs in that app's project, whose `baseURL` is the app. `helpers.createFixture(prefix, { app: baseURL })` also signs the fresh person in to the app and opens its page for the new project, so the spec starts on its subject. From then on, an uncaught error on the page fails the spec, and a `VIDEO_MODE` demo starts on the app's page, not on consent. To run the app projects locally, point each app at the local OS with `ITERATE_ORIGIN=http://localhost:8788` in its gitignored `.dev.vars`. Start it with `pnpm --dir apps/<app> dev --port <port>` (the OS first: the apps then take the next free inspector ports). Then set `NOTES_BASE_URL`, `VOICE_BASE_URL` and `DASH_BASE_URL` to the apps' local URLs.
