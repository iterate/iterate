...
These live at the repo root — not under `apps/os` — because product specs span workers: the os issuer signs a person in to the Notes app on its own origin, not just os. The sibling `apps/os/e2e` suite proves the os platform through its `/api`; this suite proves what a user actually gets in a browser.

One [Playwright config](../playwright.config.ts) with one project per app host, and a folder per app: `specs/os/` (the `os` project, plus `os-phone` for the issuer's pages at a phone's width), `specs/notes/`, `specs/voice/`, `specs/dash/` and `specs/admin/`; the `suite` project runs the specs beside them (the flake sentinel, the harness's own). `pnpm spec` runs every project; `pnpm spec --project notes` runs one app, and `SPEC_SKIP_PROJECTS=os-phone` leaves one out. A PR's Browser specs leave out `os-phone`, which Main OS e2e runs on every main push. `DEMO_BASE_URL` targets a deployed os (unset boots a local worker), `NOTES_BASE_URL` / `VOICE_BASE_URL` the Notes and Voice apps deployed against it, `DASH_BASE_URL` the Dash (the `dash` project's baseURL; the Notes session specs sign out there too) and `ADMIN_BASE_URL` the Admin app (the `admin` project's baseURL); locally, unset skips their specs, in CI it fails them. [`setup.ts`](setup.ts) prepares the deployment's credentials once, before the workers start. Import `test` from [`test-support/test.ts`](test-support/test.ts): it wires the plugins below into every page, `helpers.createFixture()` signs a fresh person in and gives them a fresh project without driving the UI, and `helpers.createSession()` signs one in with no project yet. Sign in through the page only where sign-in is the subject, with `signInWithPassword` from [`test-support/issuer.ts`](test-support/issuer.ts). Seed state through the operator's session ([`test-support/operator.ts`](test-support/operator.ts): `/api` with the admin bearer), not a session of the spec's own: `fixture.itx` is the fixture's project, and the `operator` fixture reaches every project, for state no fixture makes.

A spec for a client app runs in that app's project, whose `baseURL` is the app. `helpers.createFixture(prefix, { app: baseURL })` also signs the fresh person in to the app and opens its page for the new project, so the spec starts on its subject. From then on, an uncaught error on the page fails the spec, and a `VIDEO_MODE` demo starts on the app's page, not on consent. To run the app projects locally, point each app at the local OS with `APP_CONFIG_URLS__OS=http://localhost:8788` in its gitignored `.dev.vars`. Start it with `pnpm --dir apps/<app> dev --port <port>` (the OS first: the apps then take the next free inspector ports). Then set `NOTES_BASE_URL`, `VOICE_BASE_URL`, `DASH_BASE_URL` and `ADMIN_BASE_URL` to the apps' local URLs.

`pnpm lint` enforces the style below in `specs/**`: `iterate/spec-restricted-syntax` (locators over `expect`, no `toBe(true)`/`toBe(false)`, no `waitForURL`, no `baseURL` in `goto`), middlewright's `prefer-locator-waits`, `prefer-positive-waits` and `require-timeout-comment`, the test-style rules the Vitest suites share, and `test` imported from the harness.
...
the issuer pages spec with:

```sh
PLAYWRIGHT_SCREENSHOT='.*' pnpm spec issuer-pages
```
