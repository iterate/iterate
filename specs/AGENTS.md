...
These live at the repo root — not under `apps/os` — because product specs span workers: the os issuer signs a person in to the Notes app on its own origin, not just os. The sibling `apps/os/e2e` lane proves the os platform through its `/api`; this lane proves what a user actually gets in a browser.
