...
One [Playwright config](../playwright.config.ts) with one project per app host, and a folder per app: `specs/os/` (the `os` project, plus `os-phone` for the issuer's pages at a phone's width), `specs/notes/` and `specs/voice/`; the `suite` project runs the specs beside them (the flake sentinel, the harness's own). `pnpm spec` runs every project; `pnpm spec --project notes` runs one app. `DEMO_BASE_URL` targets a deployed os (unset boots a local worker), `NOTES_BASE_URL` / `VOICE_BASE_URL` the Notes and Voice apps deployed against it, and `DASH_BASE_URL` the Dash (the Notes session specs sign out there; locally, unset skips them, in CI it fails them). [`setup.ts`](setup.ts) prepares the deployment's credentials once, before the workers start. Import `test` from [`test-support/test.ts`](test-support/test.ts): it wires the plugins below into every page, `helpers.createFixture()` signs a fresh person in and gives them a fresh project without driving the UI, and `helpers.createSession()` signs one in with no project yet. Sign in through the page only where sign-in is the subject, with `signInWithPassword` from [`test-support/issuer.ts`](test-support/issuer.ts). Seed state through the operator's session ([`test-support/operator.ts`](test-support/operator.ts): `/api` with the admin bearer), not a session of the spec's own: `fixture.itx` is the fixture's project, and the `operator` fixture reaches every project, for state no fixture makes.
