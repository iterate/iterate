    ...
    two: {
      three: {
        ...
        rules: {
          ...
          /**
           * Contract packages (`services/*-contract`) are imported by BOTH server
           * and client (browser) code. They must contain nothing but oRPC contract
           * definitions and Zod schemas. Pulling in anything heavier — Node
           * built-ins, OpenTelemetry, evlog, vite, the shared barrel, etc. —
           * breaks Vite production builds and bloats client bundles.
           *
           ...
           *
           * If you need to add a new package to the allowlist, make sure it has
           * ZERO transitive Node/server dependencies, then add it to
           * ALLOWED_RUNTIME_IMPORTS below.
           */
          "contract-package-imports": {
            ...
            create: (context) => {
              const ALLOWED_RUNTIME_IMPORTS = [
                ...
                "@orpc/contract",
                "@orpc/zod",
                "@iterate-com/shared/jonasland/service-contract",
              ];
                ...
                ImportDeclaration: (node) => {
                  ...
                  context.report({
                    ...
                    message:
                      ...
                      ALLOWED_RUNTIME_IMPORTS.map((p) => `  • ${p}`).join("\n") +
                      `\n\nRelative imports and \`import type\` are always fine.\n` +
                      `If "${source}" is genuinely lightweight (zero Node/server deps), add it ` +
                      `to ALLOWED_RUNTIME_IMPORTS in eslint.config.js.`,
                  });
                },
              };
            },
