...
| ----------------------------------- | ------------------------------------------------------------------------------------- | ---------------------------------------- |
| What the status lights mean         | core `conversation_ring` (physical rings), `conversation_lights` (screen diagnostics) | hand it a snapshot; paint 12 pixels      |
...
## Part 2 — Faces and sprite packs

### Face behaviour

StackChan, Waveshare and M5StickS3 animate from the local PCM that has actually been
accepted for playout; do not animate on network arrival. StackChan uses that
local envelope only, as does M5StickS3. Waveshare also accepts the optional reduced viseme state
from the voice agent, reconciled with its local envelope. The local envelope
remains the baseline when remote visemes are absent or delayed.

CLI, HA Voice PE and Satellite1 have no avatar. M5StickS3's text display is a
fallback when avatar initialization fails. Only Waveshare consumes remote visemes;
do not enable or poll for them on the other clients. To provision a Waveshare voice stream with
the optional state, use the existing setup path:

```sh
pnpm --dir apps/os cli voicelab talk --project <slug> --setup-only --visemes \
  --stream-path /agents/voice/v23/waveshare
```

Keep this division: playout-aligned local animation is the default; reduced
viseme state is a small optional overlay for a board that renders it. Do not
replace it with arrival-time animation or introduce a board-wide face flag.

### Add a sprite pack

The reviewable source of a pack is the pair in
`apps/kit/firmware/components/avatar/assets/`:

- `<slug>.png` is the indexed sprite map.
- `<slug>.json` defines the RGB565 palette, cell layout, mouths, viseme maps,
  expression banks, atlas fields, and catalogue entry.

Create the matching public header at
`components/avatar/include/iterate/kit/avatar/fspp_<slug>_cores3_fine_atlas.h`
and add the generated C source name to `components/avatar/CMakeLists.txt`.
Then generate the ignored derived files:

```sh
python3 apps/kit/firmware/tools/generate-atlases.py
```

It writes `components/avatar/src/fspp_*_atlas.c` and
`face_avatar_catalog_generated.inc`. Do not edit either output. The generator
is in this repository; `extract` is its checked round-trip inverse for
recovering committed assets from a generated atlas. Use
`pnpm --dir apps/kit firmware:configure` to regenerate and configure the host
build, then run the relevant host tests before flashing a board.
...
4. **The lights.** Physical rings show faint white when ready, a gentle breath
   throughout capture, and soft blooms from mic and accepted speaker audio.
   Amber means connecting; dim red means hardware mute, brighter red a fault.
   Opening capture stays visible before connection completes. Screen diagnostic
   sectors still show network reach and separate audio levels.
