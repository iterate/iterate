...
export class CodeExecutor extends WorkerEntrypoint<Env> {
  async execute(
    ...
    return executor.execute(normalizeCodemodeScript(script), providers);
  }
}
...
function normalizeCodemodeScript(script: string): string {
  ...
  const expression = singleExpressionScript(trimmed);
  if (expression !== null) {
...
function singleExpressionScript(script: string): string | null {
  const expression = script.replace(/;\s*$/, "").trim();
  if (expression.length === 0) return null;
  if (/\b(return|const|let|var|if|for|while|try|throw|await)\b/.test(expression)) return null;
  if (!/^(?:Promise\.all|[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*)\s*\(/.test(expression)) {
    return null;
  }
  return balancedSingleCallExpression(expression) ? expression : null;
}

function balancedSingleCallExpression(expression: string): boolean {
  const stack: string[] = [];
...
export class Project extends DurableObject<Env> {
  ...
  #appFacetName(app: string) {
    if (app === "discord") return "app:v3:discord";
    return `app:v2:${app}`;
  }
