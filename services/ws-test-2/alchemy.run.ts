...
const AlchemyEnv = z.object({
  ...
  ALCHEMY_LOCAL: z.stringbool("ALCHEMY_LOCAL must be a boolean string").default(false),
  ALCHEMY_STAGE: z
    .string()
    .trim()
    .min(1, "ALCHEMY_STAGE is required")
    .regex(/^[\w-]+$/, "ALCHEMY_STAGE must contain only letters, numbers, underscores, or hyphens"),
  // Default stage keeps local/dev deployments ergonomic under the normal `os`
  // Doppler project, which provides provider credentials but not stage names.
...
const AlchemyEnvWithDefaults = AlchemyEnv.extend({
  ALCHEMY_STAGE: AlchemyEnv.shape.ALCHEMY_STAGE.default("dev"),
  CLOUDFLARE_API_TOKEN: z.string().trim().min(1, "CLOUDFLARE_API_TOKEN is required"),
  CLOUDFLARE_ACCOUNT_ID: z.string().trim().min(1, "CLOUDFLARE_ACCOUNT_ID is required"),
});

const env = AlchemyEnvWithDefaults.parse(process.env);
