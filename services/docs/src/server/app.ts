...
const DocsServiceEnv = z.object({
  ...
  ITERATE_REGISTRY_URL: z.string().trim().optional(),
});
...
async function listOpenApiSources(params: {
  ...
  if (!params.registryUrl) return [];
  const servicesClient = createRegistryClient({ url: params.registryUrl });
...
app.get("/api/openapi-sources", async (c) => {
  try {
    ...
    const sources = await listOpenApiSources({
      protocol,
      ingressPort,
      registryUrl: getEnv().ITERATE_REGISTRY_URL ?? null,
    });
