...
type ExtraProxyBehavior = "frp-incantations" | "openobserve-incantations";
...
interface CompiledRouteSpec extends RouteSpec {
  extraProxyBehavior?: ExtraProxyBehavior;
}
...
function deriveExtraProxyBehavior(directives: string[]): ExtraProxyBehavior | undefined {
  if (directives.some((directive) => directive.includes("stream_close_delay"))) {
    return "frp-incantations";
  }
  if (
    directives.some(
      (directive) => directive.includes("Authorization") || directive.includes("header_up"),
    )
  ) {
    return "openobserve-incantations";
  }
  return undefined;
}
...
function renderRouteHandleBlock(params: {
  ...
  if (params.route.extraProxyBehavior) {
    lines.push(`# extraProxyBehavior: ${params.route.extraProxyBehavior}`);
  }
  ...
  lines.push(`vars @${matcherId} {`);
  lines.push(`    iterate_service_slug ${params.route.serviceSlug}`);
  if (params.route.extraProxyBehavior) {
    lines.push(`    iterate_extra_proxy_behavior ${params.route.extraProxyBehavior}`);
  }
...
function renderRoutesFragment(params: {
  ...
  lines.push("# This file only resolves ingress route metadata.");
  lines.push("# Actual reverse_proxy dispatch stays centralized in the root Caddyfile.");
...
function buildCompiledRouteSpecs(routes: RouteRecord[]): CompiledRouteSpec[] {
  ...
  for (const route of routes) {
    ...
    const extraProxyBehavior = deriveExtraProxyBehavior(extraCaddyDirectives);
    ...
    if (existing) {
      ...
      if (extraProxyBehavior) {
        existing.extraProxyBehavior = extraProxyBehavior;
      }
      continue;
    }
