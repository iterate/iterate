...
function RoutesPage() {
  ...
  const { data, isPending } = useQuery(orpc.landing.get.queryOptions());
  ...
  const invalidate = async () => {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: orpc.landing.get.key() }),
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4">
      <Card className="overflow-hidden border-none bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-800 text-zinc-50 shadow-lg">
        <CardHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {routes.length === 0 ? (
                ...
                routes.map((route) => (
                  <div key={route.host} className="rounded-xl border bg-card/60 p-4">
