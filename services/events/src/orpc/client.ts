...
export const createEventBusClient = (
  ...
  const link = new RPCLink({
    url: resolveServiceOrpcUrl({ env: options.env, manifest: options.manifest }),
    method,
...
export const createEventBusWebSocketClient = (
  ...
  const websocket =
    options.websocket ??
    new WebSocket(
      resolveServiceOrpcWebSocketUrl({ env: options.env, manifest: options.manifest }),
      ["orpc"],
    );
...
export const createEventBusOpenApiClient = (
  ...
  const link = new OpenAPILink(options.manifest.orpcContract, {
    url: resolveServiceOpenApiBaseUrl({ env: options.env, manifest: options.manifest }),
    ...(options.headers ? { headers: options.headers } : {}),
    ...(options.fetch ? { fetch: options.fetch } : {}),
  });
