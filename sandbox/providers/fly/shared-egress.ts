...
export async function provisionSharedEgressAndAttach(opts: {
  ...
  sandbox: SandboxRef;
  tunnelIp: string;
  applyLockdownPolicy?: boolean;
}): Promise<ProvisionSharedEgressResult> {
  const env = SharedEgressEnv.parse(opts.env);
...
`;
  ...
  if (opts.applyLockdownPolicy ?? true) {
    try {
      await flyApi(
        env,
        "POST",
        `/v1/apps/${encodeURIComponent(sandbox.appName)}/network_policies`,
        {
          name: `block-all-egress-${sandbox.machineId.slice(0, 8)}`,
          selector: { all: true },
          rules: [
            {
              action: "allow",
    ...
    await flyApi(
      env,
      "POST",
      `/v1/apps/${encodeURIComponent(sandbox.appName)}/machines/${encodeURIComponent(sandbox.machineId)}/restart`,
      {},
    );
    await waitForState(env, sandbox.appName, sandbox.machineId, "started", 240);
    await execMachine(env, sandbox.appName, sandbox.machineId, [
      "/bin/bash",
      "-lc",
      "wg-quick down wg0 >/dev/null 2>&1 || true; wg-quick up wg0",
    ]);
  }
