...
> **Note (2026-09-23):** every run here was measured on the legacy preview
> pipeline (leased slots, `pnpm preview run`, root `playwright.config.ts`)
> before #2837 removed it. The data and verdict are kept as measured. Today the
> Preview OS workflow (`.depot/workflows/preview-os-next.yml`, `e2e` job on a
> `4x16` runner) runs the OS Vitest e2e suite and the Playwright specs side by
> side against one deployed preview (`pnpm preview e2e` in
> `apps/os/scripts/preview.ts`), with `workers: 6` in CI
> (`apps/os/playwright.config.ts`). The specs are moving to a root `specs/`
> with one root `playwright.config.ts`; re-measure there before changing the
> worker count, using the method below.
