...
const BASE_URL = env.BASE_URL || "https://platform.iterate.com";
...
const doUpload = async ({
  ...
  try {
    ...
    const openAIFile = await openai.files.create({
      file: new File([fileBlob], filename, { type: mimeType || "application/octet-stream" }),
      purpose: "user_data",
    });
    ...
    const fileRecord = await doUpload({
      ...
      openai: await openAIProvider({ 
        env: {
          ...
          ...(env.POSTHOG_PUBLIC_KEY && { POSTHOG_PUBLIC_KEY: env.POSTHOG_PUBLIC_KEY }),
        }, 
