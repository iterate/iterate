...
function initializeStore(): void {
  ...
  const streamIds = fs.existsSync(STORAGE_DIR)
    ? fs.readdirSync(STORAGE_DIR).filter(f => f.endsWith(".yaml")).map(f => f.replace(/\.yaml$/, ""))
    : [];
...
async function onStreamCreated(path: string, contentType: string): Promise<void> {
  ...
  await appendMessage(REGISTRY_STREAM, {
    type: "stream",
    key: path,
...
app.use("*", cors({
  ...
  allowHeaders: ["Content-Type", "Authorization", "Stream-Seq", "Stream-TTL", "Stream-Expires-At"],
  exposeHeaders: ["Stream-Next-Offset", "Stream-Cursor", "Stream-Up-To-Date", "ETag", "Content-Type", "Content-Encoding", "Vary", "Location"],
}));
...
app.post("/edge/slack", async (c) => {
  ...
  try { webhook = await c.req.json(); } catch { return c.text("Invalid JSON", 400); }
...
app.on("HEAD", "/agents/*", (c) => {
  ...
  return new Response(null, {
    status: 200,
    headers: { [STREAM_OFFSET_HEADER]: getCurrentOffset(streamPath), "Content-Type": agent.contentType },
  });
});
...
app.post("/agents/*", async (c) => {
  ...
  if (agent.piSession) {
    ...
    const userMessage = await appendMessage(streamPath, { type: "user_prompt", text: promptText } satisfies PiStreamMessage, "user");

    promptPiSession(agent.piSession, promptText).catch((error) => {
      console.error(`Pi prompt error for ${streamPath}:`, error);
      appendMessage(streamPath, { type: "error", message: error instanceof Error ? error.message : String(error) } satisfies PiStreamMessage, "system");
    });

    return new Response(null, { status: 200, headers: { [STREAM_OFFSET_HEADER]: userMessage.offset } });
  }
...
app.get("/agents/*", (c) => {
  ...
  if (live === "sse") {
    return streamSSE(c, async (stream) => {
      ...
      const currentOffset = existingMessages.length > 0 ? existingMessages[existingMessages.length - 1].offset : getCurrentOffset(streamPath);
      await stream.writeSSE({ event: "control", data: JSON.stringify({ streamNextOffset: currentOffset, upToDate: true }) });
        ...
        enqueue: (chunk: Uint8Array) => {
          ...
          if (match) {
            try {
              ...
              stream.writeSSE({ event: "control", data: JSON.stringify({ streamNextOffset: lastOffset, upToDate: true }) });
            } catch {}
          }
        },
      ...
      await new Promise<void>((resolve) => {
        c.req.raw.signal.addEventListener("abort", () => { unsubscribe(streamPath, controller); resolve(); });
      });
    });
  }
  ...
  return new Response(JSON.stringify(existingMessages.map(m => m.content)), {
    status: 200,
    headers: { "Content-Type": agent.contentType, [STREAM_OFFSET_HEADER]: currentOffset, "Stream-Up-To-Date": "true" },
  });
});
