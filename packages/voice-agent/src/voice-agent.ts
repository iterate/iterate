...
function agentVoiceProtocol(streamPath: string): string {
  return [
    ...
    "// delegationOffset is the original delegation metadata item's @offset.",
    "const request = await stream.getEvent({ offset: delegationOffset });",
    'if (!request?.idempotencyKey?.startsWith("voice-agent/agent-delegation:") || typeof request.payload?.content !== "string") throw new Error("Expected the original voice delegation metadata event");',
    'const { activation, delegationId } = JSON.parse(request.payload.content.split("\\n")[1] ?? "");',
    "await stream.append({",
...
function agentSetupEvents(streamPath: string, setupId: string): AgentEventInput[] {
  return [
    {
    ...
    switch (event.type) {
      ...
      case "events.iterate.com/voice-agent/commentary": {
        ...
        if (!dial || dial.activation !== update.activation) {
          ...
          if (state.call?.activation !== update.activation) {
            args.blockProcessorWhile(() =>
              this.#appendPassiveAgentContext({
                event,
                content: [
                  `Voice ${event.type.split("/").at(-1)} update at ${this.path}@${event.offset} for activation ${update.activation}${update.delegationId ? ` and delegation ${update.delegationId}` : ""}: this update was not forwarded because that activation is not the active voice call.`,
                  "Do not retarget this update to another call.",
                ].join(" "),
              }),
            );
          }
      ...
      case "events.iterate.com/voice-agent/conversation-ended": {
        args.blockProcessorWhile(() =>
          this.#appendPassiveAgentContext({
            event,
            content: `Voice call at ${this.path}@${event.offset} for activation ${event.payload.activation} ended: ${event.payload.reason}. Ordinary Agent work may continue, but further voice updates for this activation will not be forwarded.`,
          }),
        );
        /*
      ...
      case "events.iterate.com/voice-agent/provider-error": {
        args.blockProcessorWhile(() =>
          this.#appendPassiveAgentContext({
            event,
            content: `Voice provider error at ${this.path}@${event.offset} for conversation ${event.payload.conversationId}: ${event.payload.message.slice(0, 2_000)}`,
          }),
        );
        return;
      }
      ...
      case "events.iterate.com/voice-agent/configured": {
        const instructions = event.payload.instructions || "";
        args.blockProcessorWhile(() =>
          this.#appendPassiveAgentContext({
            event,
            content:
              instructions === ""
                ? `Voice configuration at ${this.path}@${event.offset}: the live voice model has no voice-specific instructions.`
