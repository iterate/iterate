...
export default defineConfig([
  ...
  },
  {
    ...
    // runs the agent, and nothing in it imports a dependency — the SDK's
    // types would bring Cloudflare's runtime types with them, which a phone
    // does not have (ref.ts spells the ref shapes locally for that reason).
    // No dts: rolldown-plugin-dts's printer crashes on function types inside
    // interfaces; `tsc -p tsconfig.dts.json` emits the declarations instead.
