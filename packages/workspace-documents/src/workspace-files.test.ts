...
const status: WorkspaceStatus = {
  ...
  unmounted: [{ change: "added", path: "/workspace/notes.md" }],
};

describe("workspace roots", () => {
  test("every mount plus the workspace's own directory, from status alone", () => {
    expect(workspaceRoots(status)).toEqual([
      "/repos/config",
      "/repos/iterate",
      "/repos/other",
      "/workspace",
  ...
  test("a path resolves to the root it lives under", () => {
    const roots = workspaceRoots(status);
    ...
    expect(rootOf(roots, "/workspace/notes.md")).toBe("/workspace");
  });
});
