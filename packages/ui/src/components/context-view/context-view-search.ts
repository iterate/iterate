...
// would share or come back to: the mode, the filter (query, types, actor), the inspected event, the
// processors sheet, the filter row. zod only, router-agnostic: an app's route hands the parsed
...
/** The filter the rows are narrowed by, from the state's three filter keys. */
export function contextViewFilterOf(state: ContextViewState): ContextViewFilter {
  return { query: state.q || "", types: new Set(state.types || []), actor: state.actor };
}
