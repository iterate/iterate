...
export async function parsePack(
  ...
  const digest = new Uint8Array(
    await crypto.subtle.digest("SHA-1", new Uint8Array(pack.subarray(0, pack.length - 20))),
  );
...
export async function buildPack(
  ...
  const digest = new Uint8Array(await crypto.subtle.digest("SHA-1", new Uint8Array(body)));
