...
// The hosted processors — authored in TS, bundled into the `cap.js` module a dynamic worker facet
// loads. Each entry pulls its StreamProcessor from real TS (typechecked, tested) and leaves the
// SDK/kernel/zod imports EXTERNAL as "./processor.js" (the injected module above re-exports all of
// them). Single source of truth: the reduce that RUNS in the facet IS the one that typechecks here —
// no hand-kept JS twin to drift. Every hosted processor goes through this, so there is one way.
...
async function bundleFacet(entryPoint, constName, generatedPath) {
  ...
  writeIfChanged(
    generatedPath,
    `// GENERATED by build-sdk.mjs from ${entryPoint} — do not edit.\n` +
      `export const ${constName} = {\n  "cap.js": ${JSON.stringify(text)},\n};\n`,
  );
