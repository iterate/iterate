...
| `workers.get({ source, className?, props? })` / `facets.get(name, { source, className })` | cacheKey + source; a facet's `props { iterateContextName, name }` and startup memo | Cloudflare (Worker Loader, `ctx.facets`)                                                                | `context/worker-loader.ts`, the DO's `#invokeFacet`                    |
