...
flowchart TB
  ...
  subgraph L3["Layer 3 — processors (sugar)"]
    proc["pure StreamProcessor inside a StreamProcessorDurableObject host, a facet<br/>processors.enable = subscribe to its .processEventBatch"]
  end
  L1 --> L0
  L2 --> L0
  L3 --> L2
...
}); // SUGAR over the subscription event; DURABLE (no handle) — processors.disable is the inverse
...
token bucket runs in a facet and appends stream/paused." `stream/core-processor.ts` is the ONE
...
// sdk/index.ts — THE HOST, bundled into processor.js
...
> {
   ...
   *  (`name` is the canonical codec string). Parsed once from ctx.props.iterateContextName. */
...
`{ props: { iterateContextName, name } }` to `worker.getDurableObjectClass(className, { props })`, then
...
root an earlier draft proposed. `processors.enable` always has a source:

```
processors.enable(name, { source, className, consumes? }) ⇒ append(subscriptionConfiguredEvent({ name, target: `itx.facets.get('${name}', { source, className }).processEventBatch`, consumes }))  // DURABLE: returns { name }, no handle
processors.disable(name)                                  ⇒ append(subscriptionConfiguredEvent({ name, target: null }))  // the DO deletes the facet the row hosted, storage included
```
...
edge can lend). The processor pair was declared on the edge for the same reason — two appends spelled
for you — until 2026-09-11, when it became the built-in root `itx.processors` (`enable` / `disable` /
`list`), so loaded code and a sibling context reach it through the same door as a client.
...
| `laneOf`, `facetTarget`, `rpcStubTarget`, `SubscriptionMount`, `#subscriptionMounts`, `#activeSubscriptionMounts`, `#deliverToConnectedSubscriptions`, `#driveFacets`, `#facetEntries`, `#facet`, the resurrection pass, `revokeCapability.all`, DO-side `enable/processors.disable` bodies | `stream-durable-object.ts`                           | ~400 → ~70 |
...
| the 4 subscription events (reduced by the core reduce) + their two commands + the `subscriptions` built-in view                            | `stream/core-processor.ts`, `stream/core-processor.ts`           | ~80           |
...
went to `lib/`, `live-state.ts` is a stream projection and went to `stream/`; `iterate-context-durable-object.ts` was
...
```text
src/
  worker.ts  session.ts  iterate-context.ts  iterate-context-durable-object.ts  iterate-context.ts
