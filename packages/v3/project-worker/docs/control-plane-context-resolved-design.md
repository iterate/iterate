...
directly, bypassing the resolver *and* outliving its async-local caller. So the
...
**distinct from `admin`** (today `verifyAdminSecret` returns a *client-presentable*
...
Rejected: a namespace-per-principal — a principal's reach is a *set* (own context
...
| **pager attach / replace / lend / close** | **authorize admission UNCONDITIONALLY, before accepting or replacing any socket** — an *empty* attachment currently skips `#appendEvents` yet still replaces a pager and can remove rules (`rpc-stubs.ts:228`, `iterate-context-durable-object.ts:145`). Strip pager/upgrade headers from ordinary client & confined-worker fetches; construct them only in the trusted relay; bind lend/cleanup to the admitted attachment |
...
`source.principal.actor === userId` for a *fact*, user-attributed facts (and
...
- **Own-path `this` branch stays as an optimization.** Self-call was **proven not
  ...
  `blockConcurrencyWhile` (that *does* hang); the constructor's appends are
