...
Clean room `packages/v3/project-worker` at 69080fd9e (C7). Six agents explored: (1) the rules model,
...
converged design, with the disagreements decided.
...
- **The trick already holds.** `confinedWorker` binds `globalOutbound: host` where host is the
  `ItxEntrypoint` stub (`src/core/worker-loader.ts:164-165`); its `fetch` is one line into the context
  DO's `fetch` (`src/itx-entrypoint.ts:348-350`); the DO's fetch is already an ordered walk that
  resolves an expression for one lane (`x-itx-cap`, `src/stream-durable-object.ts:1057-1075`). The
  ONLY non-expression part is the tail, `#egress` (`:1103-1124`). No new hop anywhere.
...
```
TODAY
 browser  GET /cap?ctx=prj_x&cap=itx.site ──► edge: ?cap → x-itx-cap ─┐
 capnweb  itx.fetchCap(expr, req) ───────────────────────────────────┤
 ...
 (no hostnames; no itx.fetch on the surface; /cap?ctx= without cap self-loops = defect 28)
...
falling into egress) dies structurally: an internal host with no mount is a 404.
...
- The DO is untouched for ingress: the existing fetch lane already resolves any expression, appends the
  ...
  `itx.provide('itx.apps.site', tunnel)` + `/cap?cap=itx.apps.site` works on deployed code today.
- Why `itx.apps.<label>` and not root-level `itx.<label>` (agents 4/6's variant): exposure is an explicit
  act (a mounted capability is not public until exposed), no collision with built-in root names, the
  `itx.subscribers.*` precedent, zero DO code. The decoupled spelling keeps `itx.site` as the callable.
...
- Public `/cap?cap=` dies; `x-itx-cap` survives as the INTERNAL argument channel of a real fetch hop
  ...
  A crashed laptop leaves a 503 row (C7 semantics: `CONNECTION_OFFLINE → 503`).
...
- `itx.fetch(request)` is the ONE door and it is a built-in (physical: it owns the Request, the walk,
  ...
  `itx.fetch.*` rule rows and the `itx.apps.*` mounts. It is also the declared `IterateContext.fetch`
  on the edge (the tutorial's `itx.fetch(...)` finally exists) and what `ItxEntrypoint.fetch` reaches —
  the globalOutbound wiring is unchanged.
...
### D3 A rule is a mount with a policy, never a new event family (agents 1, 5, 6)
...
- `fetch` is the third optional mount policy after `delivery` and `processor` (contract 4.0.0 → 4.1.0,
  additive). Conjunctive; absent = match all; `hosts` with apps/os's `*.` wildcard; `secrets` = "the
  request spends `{{secret:project:NAME}}`". Copy `matchEgressRule`/`hostMatches` from apps/os
  `egress-approvals.ts`, pure and Node-testable. `app` = the label the door derived from one of OUR hosts, so a rule can hold or gate ingress too ("every POST to app `site` needs approval") without a hostname ever entering the log.
...
- Named rows (`itx.fetch.<name>`) rather than all-at-`itx.fetch` (agent 1): revocable by path, readable
  in the `capability-table` snapshot. The provide door already accepts a path under a built-in root
  name (checked: no refusal in `provideCapability`); by-name resolution of `itx.fetch.<name>` hits the
  built-in first, which is fine — rows are consumed by the door by identity, like `itx.subscribers.*`
  via `deliverTo`. Idempotent provide compares `fetch` alongside target/delivery/processor.
...
- CAN be userspace: rules, deny, batching/debounce/expiry, the approval event vocabulary
  ...
  verification (WebCrypto over public keys), the CLI, live shadows. The gate is a raw `DurableObject`
  class hosted as a facet (`itx.load(src).getDurableObjectClass('ApprovalGate').get()`), not a
  `StreamProcessor` (the runner forwards only the processor verbs; `reduce` is sync, verify is async).
...
- REAL: context path ⇄ DO name ⇄ public URL path share ONE codec (`prj_x.iterate/agents/bot` ⇄
  `https://myproj.iterate.app/agents/bot` is one string apart). Fix the codec defect: `cd` compares the
  RAW arg (`stream-durable-object.ts:357`) so `cd('')` dials a self-stub and `cd('/a/../b')` mints a
  different DO than `cd('/b')`; port apps/os `canonicalizeStreamPath`.
...
DELETED: public `/cap?cap=` + `DEFAULT_CTX` (a request with no host and no `?ctx=` is 404, not
`prj_demo`); `serveCapabilityFetchLane` (parse → `parseCapabilityFetchHeader` beside the encoder;
`PartialFetch` stays for the two transport doors); `#egress` as a DO method (moved); `DummyControlPlane` + 3 solo rebinds; `fetchCap` (later;
`itx.x.fetch(req)` dotted already rides the fork's 101-over-session); the defect-28 self-loop.
...
internally, `?ctx=` as the dev door.
...
`x-itx-fetch-below` (stamp, resume-beneath, strip: ~10 lines); `fetch` policy + matcher on the mount
(~40 lines); `itx.secrets.put/list`;
...
## 4. The minimum slice, commit-sized (each green)

1. **C8 — ingress by hostname label.** Edge: host → projectId (bases → split the label at the last
   `--`; directory `resolveRoute(host)` with the apex-subdomain fallback when bound; `?ctx=` dev door;
   ...
   directory rows; solo: slug = id, base = own host). Delete `/cap`. Re-point the
   fetchdoor/ws-101/tunnel-proxy/crisp1 tests to `provide('itx.apps.site', …)` + a labelled host;
   `failing-boundary-egress` drives the tail through a dynamic worker's `globalOutbound` to an external
   host instead of the public door (that IS the assertion). **This alone delivers `iterate tunnel site
...
4. **C11 — rules as middleware.** `fetch` policy on the mount (4.1.0), the rules walk inside the
   built-in before the host decision, the `x-itx-fetch-below` stamp/resume/strip, hop guard; `fetchCap`
   becomes sugar then dies. Tests: a catch-all loaded-worker wrapper that forwards reaches the tail
...
Tutorial edits ride C8–C11: Brick 7 becomes one DO `fetch` + `globalOutbound: host` in Brick 5; Chapter 2
becomes six short sections ("one door", "the terminal", "the hostname label names the capability", "the
