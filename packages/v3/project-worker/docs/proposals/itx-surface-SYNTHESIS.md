...
**D1 — Shadow stack or map?** A and D make `state.itxExpressionRewrites` a map: last writer wins, unset deletes, the
...
target|null })`, `itx.enableProcessor` / `disableProcessor`; `itx.expressionRewrites.list()/get(match)`,
...
`ItxExpressionPrefix`. Every "steps" is `itxExpressionSteps`. The read collection on itx is
`itx.expressionRewrites` — "itx" is the receiver, so the one redundant word is dropped, nothing else.
...
expressionRewrites: { list(): ItxExpressionRewrite[]; get(match: ItxExpressionInput): ItxExpressionRewrite | null };
...
| `Mount { path, target, providedAtOffset }` · `state.mounts`                                                             | `ItxExpressionRewrite { match, target, setAtOffset }` · `state.itxExpressionRewrites`                                                                | —                                                                                                                                                   |
...
| `matchMount` · `pickMount` · `rewriteCall` · `routeCall` · `MountMatch`                                                 | `matchItxExpressionPrefix` · `pickItxExpressionRewrite` · `applyItxExpressionRewrite` · `rewriteItxExpressionToBuiltIn` · `ItxExpressionPrefixMatch` | —                                                                                                                                                   |
...
| e2e `rpcStubMountPaths` · `processorNames` (regex)                                                                      | `itx.expressionRewrites.list()` · `itx.subscriptions.list()`                                                                                         | —                                                                                                                                                   |
...
2. **(b) expression rewriting**: the vocabulary, ONE file, ONE event with stack semantics, `state.itxExpressionRewrites`,
   `NO_ITX_EXPRESSION_MATCH`, `x-itx-expression`; subscriptions' ONE event.
3. **The surface**: roots for `rewrite` · `expressionRewrites` · `subscribe` · `enableProcessor` · `disableProcessor` ·
...
- Six new reserved dotted roots (`rewrite`, `expressionRewrites`, `subscribe`, `enableProcessor`, `disableProcessor`, `waitForEvent`).
