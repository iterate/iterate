  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (projectHost) {
      ...
      if (url.pathname === PROJECT_SESSION_PATH) {
        ...
        if (!claims || claims.projectId !== projectHost.projectId)
      ...
      headers.set(ITX_EXPRESSION_FETCH_HEADER, projectHost.itxExpression);
      ...
      return env.ITERATE_CONTEXT.getByName(
        DurableObjectNameCodec.stringify({ projectId: projectHost.projectId, path: "/" }),
