...
// value what it is:
//
//   • a FacetHandle or an RpcStubHandle (core/invoke-handle.ts) OWNS ITS PROGRESS — a facet keeps its
...
import { consumesEvent, type ScannedRange } from "./core/processor.ts";
...
import { errorCode, reportIssue } from "./core/errors.ts";
...
type Deps = {
  ...
  /** The DO's one alarm, min-merged (core/stream.ts armNoLaterThan). */
