...
// ── mcp server ── `itx.serveMcp()` against a fake `itx`: the handle's `fetch` is an MCP
...
describe("mcp server", () => {
  ...
  const answerAsContext = (steps: ItxExpression): unknown => {
    ...
    // a CODED error, as the resolver throws (lib.ts): the code is the machine-readable channel
...
// cloudflare:workers, the MCP server SDK) and the one platform primitive that is pure data or a
// handle (context/expression.ts — the codec, for an expression carried as data, and the pipelinable
