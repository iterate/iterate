// context/hibernatable-rpc-stub.ts — THE HIBERNATABLE RPC STUB, and its manager.
//
 ...
 *  separate subsystem: fetch/fetch-capabilities.ts.) */
...
export class HibernatableRpcStubManager {
  ...
  /** The DO's live-capability fetch subsystem (fetch/fetch-capabilities.ts) — borrowed for
  ...
  async invoke(transportId: string, path: string[], args: unknown[]): Promise<unknown> {
    ...
    try {
      ...
      // the one live-capability fetch path — fetch/fetch-capabilities.ts owns it whole; the
      ...
      // the break is LOCAL — the CODE, never a message, crosses this hop (lib/errors.ts).
