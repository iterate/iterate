// src/account/account-facet.ts — the account processor's HOST: the class a dynamic worker facet
// loads (`session.user.processors.enable("account", { source, className: "AccountDurableObject" })`).
//
// THE SINGLE SOURCE. build-sdk.mjs bundles THIS module — pulling `AccountProcessor` from ./contract.ts
...
import { AccountProcessor } from "./contract.ts";
