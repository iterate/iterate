...
// cloudflare:workers, the MCP server SDK), its siblings in this folder, and the two platform
// primitives that are pure data or a handle (context/expression.ts — the codec, for an expression
// carried as data — and context/invoke-handle.ts). Type-only imports are free (they erase).
// Anything else — the stream, the DO, the fetch module, the rest of context/ — would make the
// module un-movable to userspace, which is the whole point of the tier.
...
describe("the library boundary", () => {
  ...
  for (const file of modules)
    test(`${file} imports only npm packages, its siblings, the codec, invoke-handle, and types`, () => {
