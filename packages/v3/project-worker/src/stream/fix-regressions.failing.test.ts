...
import { z } from "zod";
import { canonicalName, DurableObjectNameCodec } from "../context/durable-object-names.ts";
...
describe("consumesEvent unified rule (Phase B / defects 10+11)", () => {
  ...
  test("the live-state/changed type is an EPHEMERAL like any other to consumesEvent: never swept by default or '*', delivered when NAMED", () => {
    // (The rule that no PROCESSOR may fold a delta lives in the engine — stream/processor.ts
