...
// clean-room's own surfaces (stream/processor.ts base class, stream/events.ts idempotency).
//
...
import { z } from "zod";
import { defineProcessorContract, sameIdempotentEvent, type StreamEvent } from "./events.ts";
import { StreamProcessor, type ProcessEventArgs, type ReduceArgs } from "./processor.ts";

// ── shared fakes (the capability-table.test.ts / stream/processor.test.ts pattern) ──
...
describe("sameIdempotentEvent (stream/events.ts) — the retry-equality contract", () => {
  // apps/os: `sameIdempotentEvent` compares type + payload + metadata with a key-order-
  //   insensitive deep-equal ("Object key ORDER is insignificant", stream/events.ts jsonEqual;
