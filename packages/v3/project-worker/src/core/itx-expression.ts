...
// deleting a stored expression IS revocation). Mirrors apps/os `ItxExpression`. DECODE only for now:
// `evaluateItxExpression(root, steps)` walks the data with `Reflect.get` / `Reflect.apply`. (The encode half,
// `captureExpression`, was deleted while the codec is redesigned in a jam — nothing on the runtime path used it.)
