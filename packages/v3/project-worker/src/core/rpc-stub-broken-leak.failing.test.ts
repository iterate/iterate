...
// must stay failing until itx-surface.ts is fixed. DO NOT fix the production code here, and DO NOT
...
//   EACH page mints a fresh `new RetainedCallbackInvoker(retained)` (itx-surface.ts:146), and the
//   RetainedCallbackInvoker constructor registers ANOTHER `onRpcBroken` handler on the SAME
//   long-lived `retained` stub (itx-surface.ts:62-64). capnweb has no `offRpcBroken`, so the
...
//   already registers exactly one, at itx-surface.ts:154, to close the pager socket on break).
...
test("a relay registers onRpcBroken ONCE, not once per page (retained-stub listener leak)", async () => {
  ...
  // RetainedCallbackInvoker(retained) — exactly itx-surface.ts:146.
  const PAGES = 5;
