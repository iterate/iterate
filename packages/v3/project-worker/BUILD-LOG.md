...
## 2026-09-04 — arc two: `@` is the caller's input (rule 7), `itx.ai` is the first bindings root, `read` → `readEvents`

- **`@`** (`src/context/expression.ts` lexes it, `itx-expression-rewriting.ts` rule 7 gives it meaning):
  legal ONLY in a rewrite rule's target, only in its final step's arguments — the door refuses it in a
  match and in a non-final step, `parse` refuses it in a call. As a top-level argument it is the caller's
  unpinned argument list, SPLICED (`itx.fable ⇒ itx.ai.run('@cf/meta/llama-3.2-1b-instruct', @)`;
  `itx.fable(inputs, opts)` runs `itx.builtins.ai.run('@cf/…', inputs, opts)`; a property access on
  the match drops it); nested in a literal it is THE one argument; `...@` as an object entry merges the
  one argument's fields under the template's own keys (a pinned gateway `model` cannot be talked out
  of); two-or-none where one is required is a refusal at rewrite time. Stolen from the pipeline
  proposals' topic idea and the partial-application proposal's rest, nothing else: no `?`, no
  ordinals, no `~()`. The array half spells the marker as the ONE reserved literal `{ "@": true }` and
  the entry key `"...@"`, so a stored target stays plain JSON; `print` spells them back on literal
  boundaries (a user string that merely contains those characters is a longer literal, never
  matched). `parse(source, { holes: true })` is how a target is read (the door, the core reduce).
...
"binding": "AI" }` in wrangler.jsonc, `AI: Ai` on the DO's `Env`, `ai` in `BUILT_IN_ROOTS`, `ai: env.AI`
  ...
  locally (`provide("itx.ai", fake)`) and call the real binding only against the deployed worker.
- **`read` → `readEvents`** on the surface: the root, `BuiltInScope`, the record, the proxy's type and
  every caller (tests, e2e, the SDK host's engine port, docs). `Stream.read`, `ReachableContext.read`
  and the DO's `read` method keep their names (internal). `append` and `waitForEvent` unchanged.
- PINS: unit table rows for both AI shapes (splice with 1 and 2 args, the gateway object with `...@`
  and the template's model winning), a lent stub's method through `@`, the property-access drop,
  `'@cf/…'` and `'a@b.c'` inside quotes untouched, the codec round-trip (parse → print → parse and
  the reserved literal), the refusals (`@` in a match in either half, in a call, in a non-final step;
  a nested `@` / `...@` with two args or none; `...@` over a non-object), THE DREAM through the reduce
  (string-at-rest row, the chain with `@` filled, the gateway merge). E2E
  `ai-root-shadow-and-fable.e2e.test.ts`: Misha's test on the real root, the dream against the fake
  (chain, the law, the drop), the gateway shape and its refusals, the door end to end, a masked
  `itx.ai` refusing the dream coded, and DEPLOYED-ONLY: `itx.ai.models()` non-empty and ONE real
  inference through `itx.fable`. `rewrite-rules-argument-pinned.e2e` re-spelled onto `itx.llm`
  (a non-root: its `null` at `itx.ai.run('special')` would now be a MASK under the `ai` root).
- LOC: the codec — `expression.ts` 97 → 162 code lines, `itx-expression-rewriting.ts` 192 → 248
  (+121 together against a ~60 estimate: one lexer both directions ≈ 30, the marker predicates ≈ 15,
  rule 7's fill ≈ 32, the door ≈ 10, parse/print seams ≈ 12, the rest error text). Diff, this arc's hunks
  only: +479/−81 across 19 tracked files plus the new e2e file (173 lines); in `iterate-context-durable-object.ts` 2 lines (`AI: Ai` on Env) are this arc's, the rest the memory-budget arc's; one line each in the memory-budget arc's `sdk/stream-processor-durable-object.ts`, `e2e/stream-memory-budget.e2e`, `e2e/stream-uncontrolled-degradation.e2e` and `__workers-tests__/uncontrolled-degradation.test.ts` (the `readEvents` spelling, agreed), and `src/generated/processor-sdk.ts` regenerated by build-sdk.
- GATES: tsc×3 · oxlint 0/0 · knip (root run) clean · oxfmt clean for the touched files · unit+workers 311p/19xf → 327p/19xf (31 files: unit 271p/7xf, workers 56p/12xf) · e2e local 159p/2xf/7sk (42 files) → 163p/2xf/8sk (43 files; the 8th skip is the deployed-only AI test) · deployed as version 847a51f0 (Worker Startup Time 5 ms, `env.AI` bound) · e2e DEPLOYED (`WORKER_BASE_URL=https://project-worker.iterate.workers.dev pnpm e2e --no-file-parallelism`) 165p/1f/5xf/2sk on 43 files in 13 min, zero network errors — the 1 is the memory-budget arc's `CONCURRENT BIG APPENDS: 8 sessions each commit a 28 MiB batch` (expected 8, got 7; foreign), its `stream-memory-budget.e2e` green; `ai-root-shadow-and-fable.e2e` 5/5 against the deployed worker: the real `itx.ai.models()` answered a non-empty list and ONE real inference ran through `itx.fable({ prompt })` on `@cf/meta/llama-3.2-1b-instruct`. Previous deployed board 160p/2f/4xf/2sk.

## 2026-09-04 — arc three: THE LIBRARY tier — `connectToMcp`, `connectToOpenApi`, `connectToCapnweb` are built-ins that take only `itx`

- **Two groups of built-ins, one record** (`src/library/`, `src/context/built-ins.ts` §"THE LIBRARY"):
  ROOTS are implemented against `ctx`/`env`; the LIBRARY is first-party code whose ONLY dependency is
  `itx` — the record hands each verb a local `InvokeHandle` over this context's own `invoke`
  (`libraryItx`), the same dotted handle a loaded worker holds after `env.ITX.get()`, so a library
  call's `itx.fetch(...)` resolves through the context's rules (a test shadows `itx.fetch` to fake a
  remote) and lands on egress with zero hops. The signature IS the layering (the owner's litmus test,
  "could this be written in a userspace worker?"): a module could move to userspace unchanged; the
  surface shows no level (`itx.connectToMcp(url)` reads like `itx.ai.run(...)`). Pinned by
  `src/library/boundary.test.ts`: no runtime import from the stream, the DO, the fetch module or
  `context/` except `invoke-handle.ts`. `connectToGraphql` is the obvious next member and does not exist.
- **`connectToMcp(url, { headers? })`** (`library/mcp.ts`): MCP over Streamable HTTP with no SDK —
  JSON-RPC POSTs; `initialize` → `notifications/initialized` → `tools/list` at connect, `tools/call` per
  call; an `Mcp-Session-Id` handed out at initialize rides on every later request and a DELETE on
  close; plain-JSON and `text/event-stream` answers both read. The connection is an RpcTarget with
  `callTool`/`listTools`/`tools`/`serverInfo`/`close` and ONE METHOD PER TOOL on a per-connection
  subclass prototype (prototype members are what Workers RPC and capnweb traverse, so `conn.echo({…})`
  works held across calls, not only inside one expression). Result mapping mirrors apps/os
  (`structuredContent` wins, else text JSON-parsed when it parses; `isError` throws).
- **`connectToOpenApi(specOrUrl, { baseUrl?, headers? })`** (`library/openapi.ts`): the apps/os
  `executeOperation` shape — one input object per call; path params substitute, query params go on the
  URL, header params on the request, the rest is the JSON body when the operation declares one
  (`{ body }` alone sends `input.body` verbatim); auth headers reach the spec only on the API's host;
  the base URL keeps the spec URL's QUERY when it falls back to it, so a service behind the fetch lane
  works. One method per `operationId` + `call(operationId, input)` + `operations()`.
- **`connectToCapnweb(url, { headers?, transport? })`** (`library/capnweb.ts`): the remote main object
  as an `InvokeHandle` (a chain `.a().b(x)` walks the capnweb stub with no await between steps). The
  WebSocket session is opened THROUGH EGRESS (`itx.fetch` with the Upgrade header, the 101's socket
  accepted and handed to capnweb) — secrets substitute, the socket is the context's, and a held
  connection pins the context awake like a busy facet (dispose closes it). `{ transport: "batch" }`
  is capnweb's own HTTP batch client transport re-spelled over `itx.fetch` (RpcTransport is its
  extension point), ONE SESSION PER CHAIN because a batch dies with its POST; it holds nothing.
- **Three platform seams it needed.** (1) The fetch lane accepts a path suffix
  (`/expression/<path>?context=&itx=`, `src/worker.ts`): the Request already rode to the target
  verbatim, so a service loaded as a worker behind the lane now sees a real path — the OpenAPI fixture
  needs it. (2) The SDK bundle is capnweb's WORKERD build (`build-sdk.mjs` `conditions: ["workerd"]`)
  and exports `newWorkersRpcResponse`: inside a loaded isolate capnweb's RpcTarget is the native
  `cloudflare:workers` one, so a fixture class extending it is what the bundle's server serves and
  what the platform's client dials — one RpcTarget, not two (the generic build threw "RPC stub points
  at a non-serializable type" on a returned RpcTarget). (3) capnweb's own `RpcPromise`/`RpcStub`
  register as PIPELINED brands in the step walk (`src/worker.ts` at boot): without it walkSteps awaited
  every capnweb promise and a one-shot batch session died after its first message ("Batch RPC request
  ended"); with it a remote chain is one round trip on both transports.
- **The local lane cannot prove the WebSocket transport.** The e2e worker's egress terminal
  (`DummyControlPlane`, a plain `fetch(request)`) is workerd's fetch proxied through Node's, which
  answers an Upgrade with "TypeError: fetch failed"; the two WebSocket e2e (a fixture capnweb server
  over the lane; a self-dial of this worker's own `/api`) are `skipIf(local)` and run deployed. Batch,
  MCP and OpenAPI are proved on both lanes, including the batch self-dial (`itx.connectToCapnweb(<own
/api>, { transport: 'batch' }).authenticate().projects.get(other).whoami()` in ONE POST).
- **Proofs.** Unit (node, a fake `itx` whose `fetch` is the server): `mcp.test.ts` 12 rows (the
  initialize→initialized→tools/list order, the session id on later requests, JSON and SSE answers,
  structuredContent over text, isError, a reserved tool name, DELETE on close once, a 503);
  `openapi.test.ts` 16 rows (path/query/header/body building, `{ body }`, refusals, text vs JSON,
  a 410, the spec from a URL with the query kept, same-host auth, `baseUrl`); `capnweb.test.ts` 4 rows
  (the batch transport against capnweb's own `newHttpBatchRpcResponse` — a REAL capnweb round trip with
  no network: dotted sugar, a pipelined chain in ONE batch, a remote error, a 502); `boundary.test.ts`.
  E2E `library-connectors-mcp-openapi-capnweb` (7): fixtures `mcpServer`/`openapiServer`/`capnwebServer`
  hosted by a SECOND project as loaded workers behind its lane, reached through the client context's
  egress; MCP end to end (held connection, a tool as a method, an unknown tool's error); the rules
  composition `provide("itx.tools", "itx.connectToMcp('<url>')")` → `itx.tools.add({…})` and its
  `resolve` chain; OpenAPI with real paths through the lane (+ as a rule); capnweb batch (+ the batch
  self-dial); capnweb WebSocket and the WebSocket self-dial deployed-only.
- **Numbers.** Library code lines (non-blank, non-comment): index 34 · mcp 185 · openapi 176 · capnweb
  112 = 507; tests 573 lines (4 unit files + the e2e). Diff, this arc's hunks only: +175/−9 across 9
  tracked files (built-ins +37, worker.ts +14/−1 — its other hunk, `CODE_VERSION`, is the memory-budget
  arc's — sources.ts +80 fixtures, as-built +22, walkthrough +6/−1, build-sdk +5, sdk/index +5/−3,
  built-in-roots +4, the regenerated processor-sdk) plus `src/library/` (4 modules + 4 tests) and the
  e2e file. The SDK bundle after the workerd build: 373.8 KiB.
- GATES: tsc×3 (one FOREIGN error: `e2e/stream-woken-and-inline-live-state.e2e.test.ts` `d.patch`
  possibly null — the memory-budget arc's in-flight `LiveState` change) · oxlint 0/0 · knip (root run)
  clean · oxfmt clean for the touched files · unit+workers 327p/19xf (31 files) → 366p/17xf (35 files) ·
  e2e local 163p/2xf/8sk (43 files) → 168p/2xf/10sk (44 files), 0 fail · DEPLOYED version ce7d8856
  (`pnpm run deploy` from this tree, startup 7 ms): 172p/2f/4xf/2sk (44 files, 15 min) — the 2 fails
  are the memory-budget arc's `stream-uncontrolled-degradation` (CONCURRENT BIG APPENDS, SLOW LIVE
