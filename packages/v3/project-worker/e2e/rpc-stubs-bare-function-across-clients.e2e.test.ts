// rpc-stubs-bare-function-across-clients.e2e.test.ts — a live capability may be a BARE FUNCTION; no
// RpcTarget subclass is needed (capnweb passes functions by reference as stubs). Jonas's snippet,
// verbatim in shape: a Node capnweb client provides `itx.runOnMyComputer` as an async function;
// ANOTHER client of the same context calls it with plain dotted syntax, and the call rides the
// provider's borrowed stub back into the provider's process.
