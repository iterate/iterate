// context-clash.e2e.test.ts — THE CROSS-CONTEXT PARKING PIN. One /api session hands out an
...
// its pager closed, and the first context's DO auto-revoked a perfectly healthy live mount.
//
...
// delay, because the destruction was ASYNC (parking → pager close → auto-revoke lands moments
// later; an immediate-only assertion could pass before the revoke commits).
...
test("TWO CONTEXTS of one session provide live fns at the SAME capability path — both stay callable", async () => {
  ...
  // relay, and the resulting pager close makes a's DO auto-revoke the live mount asynchronously.
  await sleep(2000);
