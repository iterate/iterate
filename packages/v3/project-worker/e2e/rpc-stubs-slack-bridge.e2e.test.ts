// rpc-stubs-slack-bridge.e2e.test.ts — THE SLACK SDK USE CASE: `itx.slack` is a LIVE provided
// capability. A local node script (this file — in production a tiny bridge daemon) connects to /api
...
test("itx.slack — a live bridge replays the natural dotted spelling onto the SDK end to end", async () => {
  ...
  await bridgeItx.provide("itx.slack", slack);
  ...
  //    invokeCapability dispatch. No client SDK, just capnweb.
