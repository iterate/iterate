// fetch-door-cap-http-and-websocket.e2e.test.ts — the ONE fetch door (/cap): a GET and a WebSocket
// upgrade reach whatever the mounted capability's fetch() is — a LOADED WORKER (the seeded site,
// workerd-native WebSocketPair + 101) and a LIVE CAPABILITY provided by a plain NODE capnweb client
// (the device/ESP32 shape: `new WebSocketPair()` + `upgradeWebSocketResponse(pair[0])`, capnweb's
// universal pair + sender-side answer). Layered so a regression names its hop: the loaded-worker
// door first, then the live capability's plain HTTP fetch (eyeball → /cap → DO fetch lane → mount →
// the rpcStubs registry → relay → capnweb → the Node provider and back), then its upgrade.
...
test("/cap serves a LOADED-WORKER capability: GET → 200 HTML, WebSocket upgrade → 101 echo, clean close", async () => {
  ...
  // A mount whose target is a stateless dynamic worker (its .fetch serves /cap) — a capability is an
  // itx EXPRESSION (load(src).getEntrypoint()), same as every other mount.
  ...
  const ws = await wsRoundTrip(capUrl(ctx, "itx.site", "ws"), "hello-from-eyeball", 15_000);
...
/** A fetch-shaped live capability that records what it saw and answers a distinctive Response. */
...
test("live capability HTTP fetch: an eyeball POST reaches the Node provider's fetch() and its Response rides back out", async () => {
  ...
  const res = await fetch(capUrl(ctx, "itx.ws-device", "http"), { method: "POST", body: "ping" });
...
/** The device: a fetch-shaped live capability that upgrades WebSockets — the workerd fetch-handler
