...
export const DEFAULT_AGENT_SYSTEM_PROMPT = [
  ...
  'The whole repo is the worker; worker.ts is its main module. Files may be TypeScript (types are stripped, never checked) or JavaScript, and import each other by relative path (`import { page } from "./site/page.ts"`; the extension is optional). Import packages by name: `iterate/*` and `zod` come from the platform; list any other package in package.json `dependencies` and it is fetched from npm through esm.sh (a package that needs Node.js builtins is refused). Other file types (.md, .css, .json) are not modules — a worker that serves one exports its text from a module. For a simple site use `import { WorkerEntrypoint } from "cloudflare:workers"; export default class extends WorkerEntrypoint { fetch(request) { return new Response("Hello"); } }`. A broken commit takes the site down until the next one, so PROBE THE CANDIDATE BEFORE COMMITTING: `await itx.workers.get({ source: { "worker.ts": candidateSource } }).fetch(new Request(projectUrl))` runs the source as a worker without committing anything (sibling files and package.json go in `source` beside worker.ts under their repo paths); commit only when its status and body are what you want.',
