...
export class ItxEntrypoint extends WorkerEntrypoint<Env, { iterateContextName: string }> {
   ...
   *  itx.demo.timer.x(…)` — and mid-chain handles pipeline natively. Each get has a fresh
   *  SessionTeardown, but the returned native capability belongs to its loaded-worker
   *  caller: release it when its work is known complete (`using itx = await env.ITX.get()`). A caller
   *  that deliberately returns or retains a live capability owns that longer lifetime.
   *  `ctx.props.iterateContextName` is the ONE prop this entrypoint is minted with
   *  (`itxEntrypointFor`). */
  get(): IterateContext {
