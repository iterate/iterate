...
import { substituteHeaderSecrets } from "./fetch/secret-substitution.ts";
import {
  facetLoaderOwner,
  facetSpecOf,
  loadConfinedWorker,
...
import { codedError, errorCode, policyErrorResponse, reportIssue } from "./lib/errors.ts";
import { withTimeout } from "./lib/timeout.ts";
import type { StreamEvent, StreamEventInput } from "./stream/events.ts";
import { prepareProvenance, TrustPolicyStore } from "./provenance.ts";
import { FetchPolicy } from "./fetch/policy.ts";
import { BuildInput } from "./build.ts";
...
export class IterateContextDurableObject extends DurableObject<Env> {
  ...
  readonly #rpcStubs = new RpcStubDirectory({
    ...
    onPresence: (kind, rpcStubKey) => {
      ...
      if (kind === "attached") this.#clearPendingRpcStubCleanup(rpcStubKey);
      if (kind === "detached") this.#unsetOrRememberDetachedRpcStub(rpcStubKey);
    },
  });

  #unsetWhatNamesRpcStub(rpcStubKey: string): boolean {
    ...
    for (const match of ruleMatches) {
      let removal: StreamEventInput;
      try {
        removal = rewriteRuleRemovedEvent(match);
      } catch {
        // A raw fixed-point row has no legal removal spelling. It remains independently visible,
        // but cannot prevent cleanup of another row naming this same detached key.
        continue;
      }
      try {
        this.#appendSystemAndRunCommittedEffects([removal]);
      } catch (error) {
        if (errorCode(error) !== "STREAM_PAUSED")
          reportIssue("rpc-stub-cleanup.rule", error, { rpcStubKey, match: print(match) });
      }
    }
    for (const name of subscriptionNames) {
      try {
        this.#appendSystemAndRunCommittedEffects([
          subscriptionConfiguredEvent({ name, target: null }),
        ]);
      } catch (error) {
        if (errorCode(error) !== "STREAM_PAUSED")
          reportIssue("rpc-stub-cleanup.subscription", error, { rpcStubKey, name });
      }
    }
    const remaining = rowsNamingRpcStub({ rpcStubKey, ...this.#rowsForRpcStubCensus() });
    // Raw rows at `itx.builtins` have no legal removal spelling. They remain independently
    // observable just as before; only a still-removable row keeps a real-detach obligation alive.
    const hasRemovableRule = remaining.ruleMatches.some((match) => {
      try {
        rewriteRuleRemovedEvent(match);
        return true;
      } catch {
        return false;
      }
    });
    return !hasRemovableRule && remaining.subscriptionNames.length === 0;
  }
  ...
  /** A durable journal distinguishes an ACTUAL last-pager detach during pause from a deliberately
   * offline raw rule. It survives eviction, while an attachment removes the obligation again. */
  #pendingRpcStubCleanupKeys(): string[] {
    return this.ctx.storage.sql
      .exec("SELECT rpc_stub_key FROM rpc_stub_cleanup_pending")
      .toArray()
      .map((row) => String(row.rpc_stub_key));
  }

  #rememberPendingRpcStubCleanup(rpcStubKey: string): void {
    this.ctx.storage.sql.exec(
      "INSERT OR IGNORE INTO rpc_stub_cleanup_pending (rpc_stub_key) VALUES (?)",
      rpcStubKey,
    );
  }

  #clearPendingRpcStubCleanup(rpcStubKey: string): void {
    this.ctx.storage.sql.exec(
      "DELETE FROM rpc_stub_cleanup_pending WHERE rpc_stub_key = ?",
      rpcStubKey,
    );
  }

  #unsetOrRememberDetachedRpcStub(rpcStubKey: string): void {
    if (this.#stream.coreReducedState.paused) {
      this.#rememberPendingRpcStubCleanup(rpcStubKey);
      return;
    }
    this.#unsetWhatNamesRpcStub(rpcStubKey);
  }

  /** Only a journalled, actually detached key is eligible after resume. A raw offline row never
   * enters this table, so lifecycle recovery cannot reinterpret it as a lost pager. */
  #unsetWhatNamesDeadRpcStubsOnResume(committedEvents: StreamEvent[]): void {
    if (!committedEvents.some((event) => event.type === "events.iterate.com/stream/resumed"))
      return;
    for (const rpcStubKey of this.#pendingRpcStubCleanupKeys())
      queueMicrotask(() => {
        try {
          // Re-check in this turn: a reconnect after the resume commit wins and clears its journal.
          if (this.#rpcStubs.listRpcStubKeys().includes(rpcStubKey)) {
            this.#clearPendingRpcStubCleanup(rpcStubKey);
            return;
          }
          // Retain the durable obligation when a removable row could not be un-set. A later resume
          // can retry; a malformed raw builtins row remains independently observable, not a retry loop.
          if (this.#unsetWhatNamesRpcStub(rpcStubKey)) this.#clearPendingRpcStubCleanup(rpcStubKey);
        } catch (error) {
          // This background effect cannot reject the successful resume. The durable row remains for
          // a later resume, while the unexpected failure remains visible rather than disappearing.
          reportIssue("rpc-stub-cleanup.resume", error, { rpcStubKey });
        }
      });
  }
  ...
  constructor(ctx: DurableObjectState, env: Env) {
    super(ctx, env);
    this.ctx.storage.sql.exec(
      "CREATE TABLE IF NOT EXISTS rpc_stub_cleanup_pending (rpc_stub_key TEXT PRIMARY KEY)",
    );
    ...
    if (this.#activityHead === undefined) {
      this.#activityHead = this.#stream.highestDurableOffset();
      this.#stream.storage.writeActivityHead(this.#activityHead);
    }
    this.#stream.appendCreatedAndWokenEvents();
    this.#collectWakeCommits = false;
  }

  /** The repository projection owns only this context's SQL store; its facts enter through Stream. */
  readonly #repos = new Repositories(this.ctx.storage.sql);
  readonly #trust = new TrustPolicyStore(this.ctx.storage.sql);
  readonly #fetchPolicy = new FetchPolicy({
    storage: this.ctx.storage,
    context: this.#durableObjectAddress.name,
    egressKey: this.env.EGRESS_KEY,
    appendAudit: (event) => {
      this.#stream.appendSystem(event);
    },
    isTrustedApproval: (event) => event.verification?.level === 2,
    policyFingerprint: () =>
      JSON.stringify(this.#stream.coreReducedState.itxExpressionRewriteRules),
  });

  // Constructor commits are durable immediately, but an alarm-only cold start must not turn its own
  // wake fan-out into another quiet-clock alarm. The first real public door delivers this exact
  // ordered batch; `alarm()` deliberately does not.
  #collectWakeCommits = true;
  #pendingWakeCommits: [StreamEvent[], number, number][] = [];
  ...
  readonly #stream = new Stream({
    ...
    participant: {
      apply: (event, system) => {
        if (!system) this.#trust.apply(event);
        this.#repos.apply(event);
        this.#fetchPolicy.apply(event);
      },
    },
    onCommit: (freshEvents, afterOffset, throughOffset) => {
      if (this.#collectWakeCommits)
        this.#pendingWakeCommits.push([freshEvents, afterOffset, throughOffset]);
      else {
        this.#activateDeliveryThrough(throughOffset);
        this.#subscriptionDelivery.onCommit(freshEvents, afterOffset, throughOffset);
      }
    },
  });

  #activityHead = this.#stream.storage.readActivityHead();
  #activateDeliveryThrough(offset: number): void {
    offset = Math.min(offset, this.#stream.highestDurableOffset());
    if (offset <= (this.#activityHead ?? 0)) return;
    this.#activityHead = offset;
    this.#stream.storage.writeActivityHead(offset);
  }
  #flushWakeCommits(): void {
    if (this.#pendingWakeCommits.length === 0) return;
    this.#activateDeliveryThrough(this.#stream.highestDurableOffset());
    for (const [events, after, through] of this.#pendingWakeCommits.splice(0))
      this.#subscriptionDelivery.onCommit(events, after, through);
  }
  ...
  async append(...events: StreamEventInput[]): Promise<StreamEvent[]> {
    const repoEvents = this.#fetchPolicy.prepare(...(await this.#repos.prepare(...events)));
    const prepared = await Promise.all(
      repoEvents.map(async (event) => {
        const verification = await prepareProvenance(event, {
          projectId: this.#durableObjectAddress.projectId,
          path: this.#durableObjectAddress.path,
        });
        return verification ? { ...event, verification } : event;
      }),
    );
    return this.#appendAndRunCommittedEffects(prepared, true);
  }
  ...
  #appendAndRunCommittedEffects(events: StreamEventInput[], prepared = false): StreamEvent[] {
    // Pager attachment commits synchronously. It may configure lending, but cannot
    // turn caller-supplied signing evidence into a verified receipt.
    if (
      !prepared &&
      events.some(
        (event) =>
          Object.hasOwn(event, "provenance") ||
          Object.hasOwn(event, "verification") ||
          event.type === "events.iterate.com/repo/committed",
      )
    )
      throw new Error("Signed events and repository commits must use the asynchronous append door");
    this.#flushWakeCommits();
  ...
  /** Internal lifecycle cleanup shares commit effects but bypasses only caller trust policy. */
  #appendSystemAndRunCommittedEffects(events: StreamEventInput[]): StreamEvent[] {
    this.#flushWakeCommits();
    const subscriptionsBeforeCommit = this.#stream.coreReducedState.subscriptions;
    const committedEvents = this.#stream.appendSystem(...events);
    this.#recordActivityForQuietClock();
    this.#deleteFacetsWhoseHostingSubscriptionWasRemoved(
      committedEvents,
      subscriptionsBeforeCommit,
  ...
  readonly #builtIns: Record<string, unknown> = buildBuiltIns({
    build: (value) => this.env.BUNDLER.build(this.#resolveSource(value)),
    check: (value) => this.env.BUNDLER.check(this.#resolveSource(value)),
    repos: this.#repos.scope((...events) => this.append(...events)),
    secrets: { list: () => this.#fetchPolicy.listSecrets() },
    approvals: { pending: () => this.#fetchPolicy.listApprovalReceipts() },
    projectId: this.#durableObjectAddress.projectId,
    path: this.#durableObjectAddress.path,
  ...
  /** Repo resolution is authority-bearing; compilation receives only pinned bytes. */
  #resolveSource(value: BuildInput) {
  ...
  async alarm(): Promise<void> {
    ...
    await this.#subscriptionDelivery.deliverEveryCursorSubscription(() =>
      this.#pendingWakeCommits.length > 0 ? this.#activityHead : undefined,
    );
    if (
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    //   3. an ordinary request calls itx.fetch through those same rules. Only the
    //      physical builtins.fetch terminal substitutes secrets and uses FALLBACK.
    ...
    try {
      const expression = itxExpressionHeader ?? "itx";
      const itxExpression = expression.trimStart().startsWith("[")
        ? toItxExpression(JSON.parse(expression))
        : parse(expression);
  ...
  async #egress(request: Request): Promise<Response> {
    ...
    return this.#fetchPolicy.fetch(substitutedRequest, (outbound) => {
      const destination = resolveProjectHost(new URL(outbound.url), this.#appConfig);
      if (
        destination?.kind === "project" &&
        destination.projectId === this.#durableObjectAddress.projectId
      )
        throw codedError(
          "PROJECT_FETCH_NOT_CONFIGURED",
          "Project-host egress needs an itx.fetch rewrite rule",
        );
      return this.env.FALLBACK.fetch(outbound);
    });
  }

  /** Deployment-admin HTTP only; deliberately absent from the ITX capability record. */
  putSecret(input: unknown) {
    return this.#fetchPolicy.putSecret(input);
  }
