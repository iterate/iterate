...
test(
  "idempotent retry: a 4 × 8 MiB native-safe echo is conservatively refused before it can exhaust isolate copies",
  { timeout: 60_000 },
  () => {
    const run = runScenario("idempotency-dedupe-retry", RETRY_4_AT_CEILING);
    expectSurvived(run, "idempotency-dedupe-retry");
    expect(run.facts.refusedCode, run.tail).toBe("APPEND_REPLY_TOO_LARGE");
    expect(Number(run.facts.argsBytes)).toBeLessThanOrEqual(32 * MiB);
    expect(Number(run.facts.durableOffsetAfterRetry)).toBe(4);
  },
);

test(
  "append receipt: a batch whose native echo would exceed 32 MiB is refused before durable commit",
  { timeout: 60_000 },
  () => {
    ...
    expect(run.facts.refusedCode, run.tail).toBe("APPEND_REPLY_TOO_LARGE");
    expect(Number(run.facts.durableOffset)).toBe(0);
    expect(Number(run.facts.echoBytes), run.tail).toBeGreaterThan(32 * MiB);
  },
);
...
// that reason.
test(
  "core re-reduce: a core-version bump over 17,000 rows rebuilds within the constructor CPU budget",
  { timeout: 110_000 },
  () => {
    const run = runCoreRowsUntilCellCap();
    expectSurvived(run, "core-rows-until-cell-cap");
...
// A legacy row predating edge admission is never parsed when its structural estimate exceeds the
// isolate budget. The durable halt names its offset, preserves the checkpoint, and a new incarnation
// reports the same coded refusal rather than reconstructing into a reset loop.
test(
  "legacy dense rows: read and recovery durably halt before JSON.parse without advancing state",
  { timeout: 60_000 },
  () => {
    ...
    expect(run.facts.refusedCode, run.tail).toBe("STREAM_RESOURCE_HALTED");
    expect(Number(run.facts.haltOffset)).toBe(2);
    expect(Number(run.facts.checkpointThroughAfter)).toBe(3);
    expect(run.facts.recoveredStateUnchanged).toBe("true");
    expect(run.facts.publishedValueUnchanged).toBe("true");
    expect(run.facts.cursorUnchanged).toBe("true");
    expect(run.facts.recoveryRefusedCode).toBe("STREAM_RESOURCE_HALTED");
    expect(Number(run.facts.recoveredDurableOffset)).toBe(3);
  },
);
