...
import type { ProvenanceEvidence, ProvenanceVerification } from "../provenance.ts";
...
export type StreamEventInput = {
  ...
  /** Optional externally-verifiable signing evidence. It never grants trust or authorization. */
...
export type StreamEvent = Omit<StreamEventInput, "offset"> & {
  /** Server-derived identities whose attached evidence verified at this context address. */
...
export function sameIdempotentEvent(
  ...
  if (!sameLegacyEnvelope) return false;
  // Keep the old unsigned retry contract exactly. A signed event has a larger semantic envelope:
  // neither its evidence nor processor attribution may silently change under the same key.
  if (existingEvent.provenance === undefined && requestedEvent.provenance === undefined)
    return true;
  return (
    jsonEqual(existingEvent.provenance, requestedEvent.provenance) &&
    jsonEqual(existingEvent.source, requestedEvent.source)
  );
}
