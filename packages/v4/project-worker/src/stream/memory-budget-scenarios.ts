...
const scenarios: Record<string, (args: Record<string, number>) => Promise<void>> = {
  ...
  async "idempotency-dedupe-retry"(args) {
    ...
    notePeakHeap();
    let echoed: StreamEvent[] = [];
    try {
      echoed = stream.append(...retryBatch);
    } catch (error) {
      fact("refusedCode", errorCode(error) ?? "uncoded");
    }
    notePeakHeap();
  ...
  async "append-echo-over-rpc-cap"(args) {
    ...
    // This is the native reply's measured pre-fix shape. The stream now refuses it before commit.
    fact(
      "echoBytes",
      serialize(
  ...
  async "read-object-dense-page"(args) {
    const storage = nodeSqliteDurableObjectStorage();
    // The single-8MiB expected-fail pin models the native RPC decoder itself: it must deserialize
    // the argument before edge/DO code can run. Keep that distinct from the legacy-row fixture below.
    if (args.eventCount === 1 && args.itemCount > 1_000_000) {
      const json = `[${"[[]],".repeat(args.itemCount).slice(0, -1)}]`;
      bareStream(storage).append({
        type: "dense",
        payload: { items: JSON.parse(json) as unknown[] },
      });
      return;
    }
    // Materialize the Stream-owned tables while the log is empty; constructing after the raw row
    // would correctly attempt a core replay before we can install its current checkpoint.
    const tableStream = bareStream(storage);
    // A legacy row may predate structural admission. Seed its JSON body straight into SQLite so
    // this proof exercises read/replay, not the now-refused RPC argument decoder.
    const items = `[${"[[]],".repeat(args.itemCount).slice(0, -1)}]`;
    const body = `{"type":"dense","payload":{"items":${items}},"createdAt":"2026-09-05T20:00:00.000Z"}`;
    fact("event0BodyChars", body.length);
    storage.sql.exec(
      'INSERT INTO events (offset, body, idempotency_key) VALUES (1, \'{"type":"safe","createdAt":"2026-09-05T20:00:00.000Z"}\', NULL)',
    );
    storage.sql.exec(
      "INSERT INTO events (offset, body, idempotency_key) VALUES (2, ?, NULL)",
      body,
    );
    for (let offset = 3; offset <= args.eventCount + 1; offset++) {
      storage.sql.exec(
        "INSERT INTO events (offset, body, idempotency_key) SELECT ?, body, NULL FROM events WHERE offset = 2",
        offset,
      );
      storage.sql.exec(
        "INSERT INTO event_chunks (offset, chunk_index, chunk) SELECT ?, chunk_index, chunk FROM event_chunks WHERE offset = 2",
        offset,
      );
    }
    if (args.eventCount > 0) {
      tableStream.storage.reduceCheckpoints.write(
        CoreContract.slug,
        { reducerVersion: "0.0.0-previous", reducedThroughOffset: args.eventCount + 1 },
        CoreContract.initialState(),
        false,
      );
    }
    tableStream.storage.writeSubscriptionCursor("legacy-proof", { confirmedOffset: 0, attempt: 0 });
    const stream = bareStream(storage);
    const coreStateBefore = JSON.stringify(stream.coreReducedState);
    const publishedStateBefore = JSON.stringify(stream.coreLiveStateSnapshot().state);
    const cursorBefore = JSON.stringify(stream.storage.listSubscriptionCursors());
    try {
      stream.read(0, 500);
    } catch (error) {
      fact("refusedCode", errorCode(error) ?? "uncoded");
    }
    const halt = stream.storage.readResourceHalt();
    fact("haltOffset", halt?.offset ?? -1);
    fact("haltEstimatedBytes", halt?.estimatedBytes ?? -1);
    const checkpointAfter = stream.storage.reduceCheckpoints.read<unknown>(CoreContract.slug);
    fact("checkpointThroughAfter", checkpointAfter?.reducedThroughOffset ?? -1);
    // A fresh incarnation loads the durable halt before considering replay; it neither parses nor
    // advances past the legacy row, and it publishes the same classified refusal.
    const recovered = bareStream(storage);
    fact(
      "recoveredStateUnchanged",
      String(JSON.stringify(recovered.coreReducedState) === coreStateBefore),
    );
    fact(
      "publishedValueUnchanged",
      String(JSON.stringify(recovered.coreLiveStateSnapshot().state) === publishedStateBefore),
    );
    fact(
      "cursorUnchanged",
      String(JSON.stringify(recovered.storage.listSubscriptionCursors()) === cursorBefore),
    );
    try {
      recovered.read(0, 500);
    } catch (error) {
      fact("recoveryRefusedCode", errorCode(error) ?? "uncoded");
    }
    fact("recoveredDurableOffset", recovered.highestDurableOffset());
  },
};
