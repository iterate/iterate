...
/** A facet startup memo is one production Durable Object storage cell. The memo is plain JSON
 * data, but storage does not expose its native serializer's byte count; UTF-8 JSON catches
 * non-ASCII source text and leaves 4 KiB for storage framing. */
const FACET_STARTUP_MEMO_MAX_UTF8_JSON_BYTES = 2 * 1024 * 1024 - 4096;

/** A fresh append must abort its transaction when a hosting configuration could not later persist
 * its restart memo. Historical replay instead reports and skips this admission error, like other
 * malformed historic control facts, so an old row cannot brick every wake. */
export class FacetStartupMemoAdmissionError extends Error {
  readonly code = "FACET_STARTUP_MEMO_TOO_LARGE";
  readonly retryable = false;
  readonly data: { name: string; bytes: number; maxBytes: number };

  constructor(name: string, bytes: number) {
    super(
      `facet ${JSON.stringify(name)} startup memo is ${bytes} UTF-8 JSON bytes, over the ${FACET_STARTUP_MEMO_MAX_UTF8_JSON_BYTES}-byte safe ceiling for one 2 MB storage cell; use a smaller bundle; nothing was appended`,
    );
    this.data = { name, bytes, maxBytes: FACET_STARTUP_MEMO_MAX_UTF8_JSON_BYTES };
  }
}
...
/** The raw append door shares this invariant with the subscription event builder. A control event
 * that names the inline core reducer could never be delivered to a real facet, so reject it before
 * Stream assigns an offset or opens its transaction — including a `target: null` "removal". */
export function assertCoreControlEventMayLand(event: StreamEventInput): void {
  if (
    event.type === "events.iterate.com/stream/subscription-configured" &&
    event.payload?.name === CoreContract.slug
  )
    throw codedError(
      "SUBSCRIPTION_RESERVED",
      `append: ${JSON.stringify(CoreContract.slug)} is the core reduce's name — reserved; pick another`,
    );
}
...
export class CoreStreamProcessor extends StreamProcessor<CoreState> {
  ...
  /** A transaction/replay page owns its draft tables. Each table is copied once, not once per
   * control event; the published state and its prior snapshots remain immutable. */
  reduceBatch(
    events: StreamEvent[],
    state: CoreState,
  ...
  override reduce({ event, state }: ReduceArgs<CoreState>): CoreState | undefined {
    return this.#reduce({ event, state });
  }

  #reduce(
    { event, state }: ReduceArgs<CoreState>,
    draftTables?: WeakSet<object>,
  ): CoreState | undefined {
    if (event.ephemeral) return undefined;
    const updateSubscription = (name: string, row?: Subscription): CoreState | undefined => {
      if (!row && !Object.hasOwn(state.subscriptions, name)) return undefined;
      let subscriptions = state.subscriptions;
      if (!draftTables?.has(subscriptions)) {
        subscriptions = { ...subscriptions };
        draftTables?.add(subscriptions);
      }
      // A valid segment may be '__proto__'; defining an own data property never invokes a setter.
      if (row)
        Object.defineProperty(subscriptions, name, {
          value: row,
          enumerable: true,
          writable: true,
          configurable: true,
    ...
    switch (event.type) {
      ...
      case "events.iterate.com/stream/subscription-configured": {
        ...
        if (payload.target === null) return updateSubscription(name);
        ...
        const resolvedTarget = resolveThroughState(state, configuredTarget);
        const hostingSpec = resolvedTarget && facetSpecFromHostingTarget(resolvedTarget);
        if (hostingSpec) {
          const startupMemoJson = JSON.stringify({
            source: hostingSpec.source,
            ...(hostingSpec.cacheKey !== undefined && { cacheKey: hostingSpec.cacheKey }),
            className: hostingSpec.className,
          });
          const startupMemoBytes = new TextEncoder().encode(startupMemoJson).byteLength;
          if (startupMemoBytes > FACET_STARTUP_MEMO_MAX_UTF8_JSON_BYTES)
            throw new FacetStartupMemoAdmissionError(hostingSpec.name, startupMemoBytes);
        }
        const { target, hostedFacet } = elideHostedFacetSource(configuredTarget, resolvedTarget);
        return updateSubscription(name, {
          target,
          ...(consumes && { consumes }),
      ...
      case "events.iterate.com/stream/subscription-delivery-halted": {
        ...
        return updateSubscription(payload.name as string, {
          ...row,
          halted: {
