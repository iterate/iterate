...
/** workerd's native `JsRpcPromise` has `Symbol.dispose` (worker-rpc.h's `JSG_DISPOSE(dispose)`),
 * but the ambient `Service` declaration describes only its thenable surface. This intersection
 * records the runtime contract so the SDK releases an inert operation's ORIGINAL call pipeline. */
type DisposableNativeRpcPromise<T> = T & Disposable;
...
> extends DurableObject<Env, StreamProcessorProps> {
  ...
  get #engine(): ProcessorEngine<State> {
    return (this.#engineBuiltOnFirstUse ??= new ProcessorEngine(this.processor, {
      ...
      stream: {
        // These operations return only known-inert stream data. Keep their ORIGINAL native call
        // promises and release them after settlement: the outer promise owns the call pipeline
        // that deserializes the returned result. Do not generalize this to author code: a caller
        // may intentionally retain a live capability.
        append: async (...events) => {
          using itx = await this.env.ITX.get();
          using result = itx.builtins.append(...events) as DisposableNativeRpcPromise<
            ReturnType<typeof itx.builtins.append>
          >;
          return await result;
        },
        read: async (after, limit) => {
          using itx = await this.env.ITX.get();
          using result = itx.builtins.readEvents(after, limit) as DisposableNativeRpcPromise<
            ReturnType<typeof itx.builtins.readEvents>
          >;
          return await result;
        },
      },
