...
const TOOLS = [
  ...
  // The official SDK validates MCP's required object schema before our public wrapper sees it.
  { name: "callTool", inputSchema: { type: "object" } }, // reserved: reachable through callTool only
];
...
describe("connectToMcp", () => {
  test("connect: initialize → initialized → tools/list, the session id riding on every later request", async () => {
    ...
    expect(requests.map((r) => r.body?.method).filter(Boolean)).toEqual([
      "initialize",
      "notifications/initialized",
      "tools/list",
    ]);
    // The SDK may offer a newer supported protocol; this server's 2025-03-26 negotiation remains valid.
    expect(requests[0].body.params.protocolVersion).toEqual(expect.any(String));
      ...
      if (body?.method === "initialize")
        return json({
          ...
          result: {
            protocolVersion: "2025-03-26",
            capabilities: {},
            serverInfo: { name: "sessionless", version: "0" },
          },
        });
      if (body?.method === "notifications/initialized") return new Response(null, { status: 202 });
      return json({
  ...
  test("a memoized holder reconnects on its next use after close", async () => {
    const { itx, requests } = fakeItx(referenceServer());
    const conn = await connectToMcp(itx, "https://mcp.example/rpc");
    await conn.close();
    expect(await conn.callTool("echo", { again: true })).toEqual({ echoed: { again: true } });
    expect(requests.filter((row) => row.body?.method === "initialize")).toHaveLength(2);
  });
