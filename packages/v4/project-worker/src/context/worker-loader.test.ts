...
import { facetLoaderOwner, loadConfinedWorker, loadNativeWorker } from "./worker-loader.ts";
...
test("cached native code keys code, JSON data and bytes while ignoring caller ITX", async () => {
  const keys: string[] = [];
  const loader = {
    get(key: string) {
      keys.push(key);
  ...
  const base = {
    ...
    modules: {
      ...
      "settings.json": { json: { enabled: true } },
      "payload.bin": { data: Uint8Array.from([1, 2, 3]).buffer },
    },
    env: { configuration: { color: "blue" }, ITX: "untrusted-a" },
  };
  const cache = { cacheKey: "document@1", deployId: "deploy-1", owner: "prj_u.iterate/docs" };

  await loadNativeWorker(loader, {} as Fetcher, base, cache);
  await loadNativeWorker(
    loader,
    {} as Fetcher,
    {
      ...base,
      env: { ...base.env, ITX: "untrusted-b" },
    },
    cache,
  );
  ...
  await loadNativeWorker(
    ...
    {} as Fetcher,
    {
      ...
      modules: {
        ...base.modules,
        "payload.bin": { data: Uint8Array.from([1, 2, 4]).buffer },
      },
    },
    cache,
  );
...
test("cached native code refuses a live binding instead of retaining its first caller", async () => {
  const loader = {
    get() {
      return {};
    },
  ...
  await expect(
    loadNativeWorker(
      ...
      { cacheKey: "document@1", deployId: "deploy-1", owner: "prj_u.iterate/docs" },
    ),
  ).rejects.toThrow(/fresh loading/);
});
