...
const JSON5_COMMENT = String.raw`//[^\r\n]*|/\*[\s\S]*?\*/`;
const isIgnoredSource = (match: string): boolean =>
  match[0] === '"' || match[0] === "'" || match.startsWith("/");
/** In call args: a literal (kept verbatim) or a marker — `...@` before `@`, so the merge form wins. */
const MARKERS_IN_ARGS = new RegExp(`${STRING_LITERAL}|${JSON5_COMMENT}|\\.\\.\\.@|@`, "g");
...
const BRACKETS = new RegExp(`${STRING_LITERAL}|${JSON5_COMMENT}|[()[\\]{}]`, "g");
...
function matchingParen(source: string, open: number): number {
  ...
  for (let bracket = BRACKETS.exec(source); bracket; bracket = BRACKETS.exec(source)) {
    if (isIgnoredSource(bracket[0])) continue;
  ...
  while (i < s.length) {
    ...
    if (/\s/.test(c)) i++;
      ...
      const inner = raw.replace(MARKERS_IN_ARGS, (match) => {
        if (isIgnoredSource(match)) return match;
        if (!options?.holes)
