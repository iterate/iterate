...
export const isBuiltInsRooted = (call: ItxExpression): boolean =>
  call[0] === "itx" && call[1] === BUILTINS_ROOT;
...
/** The physical spelling of a lent stub, possibly followed by more steps. */
function directRpcStubKey(target: ItxExpression): string | undefined {
  const get = target[3];
  return target[0] === "itx" &&
    target[1] === BUILTINS_ROOT &&
    target[2] === "rpcStubs" &&
    Array.isArray(get) &&
    get[0] === "get" &&
    get.length === 2 &&
...
/** An unresolvable target names no stub; callers only need its resolved physical key. */
function resolvedRpcStubKey(
  rules: readonly ItxExpressionRewriteRule[],
  target: ItxExpression,
): string | undefined {
  try {
    return directRpcStubKey(resolveItxExpression(() => rules, target).at(-1)!);
  } catch {
    return undefined;
  }
}
...
export function rpcStubKeysNamed(args: {
  ...
  for (const target of targets) {
    const key = resolvedRpcStubKey(rules, target);
