...
// back a disposable; the caller gives it to the session's `ContextLeaseBook`, session.ts) — so
...
import type { RpcStub as CapnwebRpcStub, RpcTarget as CapnwebRpcTarget } from "capnweb";
...
class LentRpcStub extends WorkersRpcTarget {
   ...
   *  chain lands where the callers' catch → #recodeIfBroken sees it. */
 ...
 *  (explicitly, or at session end — `ContextLeaseBook`); its close makes the DO return the stub; when
