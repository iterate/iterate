...
import type { RepositoryScope } from "../repos.ts";
import type { BuildInput, BuildResult, CheckResult } from "../build.ts";
import type { SecretReceipt, ApprovalReceipt } from "../fetch/policy.ts";
...
export interface BuiltInScope extends LibraryRoots {
  /** Compile source bytes or a pinned repository entrypoint into inert loader input. */
  build(input: BuildInput): Promise<BuildResult>;
  /** Typecheck the same source input against the emitted public ITX contract. */
  check(input: BuildInput): Promise<CheckResult>;
  /** Path-addressed immutable revisions and atomic head compare-and-swap. */
  repos: RepositoryScope;
  /** Safe receipts only; writing secret bytes requires the deployment-admin HTTP door. */
  secrets: { list(): SecretReceipt[] };
  /** Durable request/decision receipts; approval decisions themselves are signed stream events. */
  approvals: { pending(): ApprovalReceipt[] };
  ...
  workers: {
    /** Native Worker Loader input, without the cap.js/source convention. A fresh
     * isolate, with contextual ITX and fetch policy; the caller chooses its host. */
    load(
      code: NativeWorkerCode,
      options?: { className?: string; props?: unknown; cacheKey?: string },
    ): InvokeHandle;
    get(spec: {
      source: WorkerSource;
...
export function buildBuiltIns(deps: BuildBuiltInsDeps): Record<string, unknown> {
  ...
  const callWorker = (
    worker: WorkerStub,
    options: { className?: string; props?: unknown },
    method: string,
    args: unknown[],
    where: string,
  ) => {
    // WorkerStub's ambient Fetcher type cannot know a dynamically supplied class's
    // methods. The native RPC boundary validates the named method when called.
    const entrypoint = worker.getEntrypoint(
      options.className,
      options.props === undefined ? undefined : { props: options.props },
    ) as Fetcher & Record<string, (...args: unknown[]) => Promise<unknown>>;
    const fn = entrypoint[method];
    if (typeof fn !== "function")
      throw new Error(`${where}: the entrypoint has no method "${method}"`);
    return Reflect.apply(fn, entrypoint, args);
  };

  const workerHandle = (call: (method: string, args: unknown[]) => unknown, where: string) =>
    new InvokeHandle((steps) => {
      const [method] = steps;
      if (steps.length !== 1 || !Array.isArray(method) || method[0] === "")
        throw new Error(`${where}.${print(steps)}: a WorkerEntrypoint exposes flat methods`);
      return call(method[0], method.slice(1));
    });
  ...
  const callEntrypoint = async (
    ...
    return callWorker(worker, spec, method, args, "workers.get(spec)");
  };
  ...
  return {
    ...
    workers: {
      load: (
        ...
        const worker = loadNativeWorker(env.LOADER, deps.itxEntrypoint, code, {
        ...
        return workerHandle(
          async (method, args) =>
            callWorker(await worker, options, method, args, "workers.load(code)"),
          "workers.load(code)",
        );
      },
      get: (spec: {
        ...
        className?: string;
        props?: unknown;
      }) => workerHandle((method, args) => callEntrypoint(spec, method, args), "workers.get(spec)"),
    },
