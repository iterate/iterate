...
export function soloWorkerConfig(): Unstable_RawConfig {
  ...
  return {
    ...
    assets: {
      ...rawConfig.assets,
      directory: join(PACKAGE_DIR, "public"),
    },
    ...
    vars: {
      ...
      APP_CONFIG_PROJECTS_JSON:
        '{"demo":"prj_ingress_demo","blue-team":"prj_ingress_blue","team":"prj_ingress_team"}',
      APP_CONFIG_CUSTOM_HOSTNAMES_JSON: '{"custom.localhost":"prj_ingress_custom"}',
      // Test-only deployment-admin credentials; both are synthetic and scoped to local workerd.
      EGRESS_KEY: "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
      EXPERIMENT_ADMIN_TOKEN: "e2e-admin-token",
    },
  };
}

export function bundlerWorkerConfig(): Unstable_RawConfig {
  const { rawConfig } = experimental_readRawConfig({
    config: join(PACKAGE_DIR, "wrangler.bundler.jsonc"),
  });
  return { ...rawConfig, main: join(PACKAGE_DIR, String(rawConfig.main)) };
}
