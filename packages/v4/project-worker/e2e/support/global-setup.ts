...
import { PACKAGE_DIR, soloWorkerConfig, bundlerWorkerConfig } from "./solo-config.ts";

declare module "vitest" {
  interface ProvidedContext {
    /** Base URL of the one E2E worker, e.g. http://127.0.0.1:1234 — every test opens capnweb here. */
    workerBaseUrl: string;
    /** Browser session minted for a deployed worker; local workerd has no browser auth layer. */
    workerAuthCookie: string;
...
export default async function setup(project: TestProject): Promise<() => Promise<void>> {
  ...
  if (deployedWorkerBaseUrl) {
    ...
    if (process.env.WORKER_DEMO_LOGIN === "1") {
      const workerUrl = new URL(deployedWorkerBaseUrl);
      const login = await fetch(new URL("/login", workerUrl), {
        method: "POST",
        redirect: "manual",
      ...
      if (login.status !== 303) {
        throw new Error(
          `Deployed e2e login at ${workerUrl.origin}/login returned ${login.status}.`,
        );
      }
      cookie = login.headers.get("set-cookie")?.split(";", 1)[0] ?? "";
      if (!cookie) {
        throw new Error(
          `Deployed e2e login at ${workerUrl.origin}/login did not set a session cookie.`,
        );
      }
    }
    project.provide("workerBaseUrl", deployedWorkerBaseUrl);
    project.provide("workerAuthCookie", cookie);
