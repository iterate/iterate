...
export const expressionUrl = (
  ...
  if (scheme === "ws") u.protocol = u.protocol === "https:" ? "wss:" : "ws:";
...
const openSockets: UndiciWebSocket[] = [];

/** Browser auth is deliberately confined to the configured worker origin. */
export const workerAuthHeaders = (url: URL): Record<string, string> | undefined => {
  const cookie = process.env.WORKER_AUTH_COOKIE;
  const worker = new URL(baseUrl());
  const workerOrigin = worker.origin;
  ...
  if (!cookie || requestOrigin.origin !== workerOrigin) return undefined;
  return { cookie, origin: workerOrigin };
};

const workerSocket = (): UndiciWebSocket => {
  const url = new URL(wsApi());
  const headers = workerAuthHeaders(url);
...
export function rawSession(prepare?: (ws: UndiciWebSocket) => void): {
  session: any;
  ws: UndiciWebSocket;
} {
  const ws = workerSocket();
...
export function httpBatch(): any {
  ...
  const headers = workerAuthHeaders(url);
  // eslint-disable-next-line iterate/no-capnweb-http-batch -- preserves the public stateless HTTP-batch seam: session-doors.e2e proves a socketless CLI call.
  return newHttpBatchRpcSession(headers ? new Request(url, { headers }) : url.toString());
}
