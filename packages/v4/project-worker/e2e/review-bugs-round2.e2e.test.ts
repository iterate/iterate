...
// edge#10 — the public string-expression door carries JSON5 comments through to the parser. An
// `@` or `)` in comment text is data for JSON5's lexer, never an expression marker or delimiter.
test("edge#10: invoke accepts JSON5 comments in an expression's call args", async () => {
  const itx = openItx(freshCtx("json5-comment"));
  await itx.invoke("itx.kv.put(/* @ and ) */ 'json5-comment', 'works')");
  expect(await itx.invoke("itx.kv.get('json5-comment')")).toBe("works");
});
