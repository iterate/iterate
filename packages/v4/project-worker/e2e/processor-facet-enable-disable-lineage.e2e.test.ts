...
test("raw subscription-configured cannot name core: the whole append batch is refused before any row lands", async () => {
  const itx = openItx(freshCtx("raw-core-reserved"));
  const before = await readAll(itx);
  const reserved = {
    type: "events.iterate.com/stream/subscription-configured",
    payload: { name: "core", target: "itx.probe.processEventBatch" },
  };

  const refusal = await rejection(append(itx, { type: "ordinary-before-refusal" }, reserved));
  expect(refusal.code).toBe("SUBSCRIPTION_RESERVED");
  expect(refusal.message).toMatch(/core reduce.*reserved/);
  expect(await readAll(itx)).toEqual(before);
  expect(await subscriptions(itx)).toEqual([]);

  // The inverse spelling is also a control event, not an innocuous delete: accepting it used to
  // land its row then fail from the post-commit facet-removal effect.
  expect(
    (
      await rejection(
        append(itx, {
  ...
  ).toBe("SUBSCRIPTION_RESERVED");
  expect(await readAll(itx)).toEqual(before);
});

test("an oversized processor startup memo refuses the whole public append without installing a facet", async () => {
  const ctx = freshCtx("facet-memo-admission");
  // Correlate public-client phases to deployed traces without logging the 4.5 MiB source or target.
  const stage = (phase: string, details: Record<string, unknown> = {}) =>
    console.info(
      "[A3_PUBLIC_ADMISSION_STAGE]",
      JSON.stringify({ utc: new Date().toISOString(), ctx, phase, ...details }),
    );
  stage("context-created");
  const itx = openItx(ctx);
  const before = await readAll(itx);
  const tallySource = SOURCES.tally;
  if (typeof tallySource === "string" || Array.isArray(tallySource))
    throw new Error("This admission probe requires the tally fixture's literal modules.");
  const source = { ...tallySource, "padding.js": `// ${"x".repeat(4.5 * 1024 * 1024)}` };
  stage("source-built", {
    sourceUtf8Bytes: new TextEncoder().encode(JSON.stringify(source)).byteLength,
  });
  let landed = false;
  try {
    stage("append-started");
    const refusal = await append(
      itx,
      { type: "must-not-land-before-oversized-memo" },
      {
        type: "events.iterate.com/stream/subscription-configured",
        payload: {
          name: "oversized",
          target: `itx.facets.get('oversized', { source: ${JSON.stringify(source)}, className: 'TallyDurableObject' }).processEventBatch`,
        },
      },
    ).then(
      () => {
        landed = true;
        stage("append-resolved");
        return null;
      },
      (error: unknown) => {
        stage("append-rejected", {
          code:
            typeof error === "object" && error !== null && "code" in error
              ? String((error as { code: unknown }).code)
              : undefined,
          durableObjectReset:
            typeof error === "object" && error !== null && "durableObjectReset" in error
              ? Boolean((error as { durableObjectReset: unknown }).durableObjectReset)
              : undefined,
        });
        return error;
      },
    );
    expect(refusal).toMatchObject({ code: "FACET_STARTUP_MEMO_TOO_LARGE", retryable: false });
    expect(await processorNames(itx)).toEqual([]);
    // A fresh incarnation may independently append its wake fact; no submitted row may land.
    const after = await readAll(itx);
    expect(after.slice(0, before.length)).toEqual(before);
    expect(
      after.slice(before.length).every((event) => event.type === "events.iterate.com/stream/woken"),
    ).toBe(true);
  } finally {
    // The pre-fix deployed control can accept this invalid configuration. Clean up that synthetic
    // facet even when the assertion fails, so the negative proof leaves no retrying processor.
    if (landed) {
      stage("cleanup-started");
      await itx.disableProcessor("oversized");
