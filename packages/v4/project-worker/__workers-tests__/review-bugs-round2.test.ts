...
// key a row still names that has no transport is un-set. The durable pending-detach record must
// also survive a genuine eviction between close and resume.
test("do-side#5: a stub whose last pager closes DURING a pause is cleaned after eviction then resume", async () => {
  ...
  // The actual detach obligation is durable, unlike in-memory pager census state. Wake only when
  // the resume lands; the public rule must still be present before that control fact.
