...
test("cloudflare:workers exports RpcPromise and native RPC calls are instanceof it", async () => {
  ...
  // Native RpcPromise cleanup is a symbol method. The edge lifetime release must use the same
  // runtime shape as `using`, rather than assuming a structural `in` test sees a proxy member.
  expect(Symbol.dispose in p).toBe(true);
  expect(typeof (p as Partial<Disposable>)[Symbol.dispose]).toBe("function");
  const state = await p;
  // This is the result-side runtime marker observed at the edge's real post-await seam. It is
  // neither application data nor a capability: workerd attaches it to plain native-RPC results.
  expect(Object.getOwnPropertySymbols(state)).toEqual([Symbol.dispose]);
  expect(Object.getOwnPropertyDescriptor(state, Symbol.dispose)).toMatchObject({
