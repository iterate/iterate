...
const entrypoint = new URL("../dist/starter-apps/todo/configured-worker.mjs", import.meta.url);
...
if (unsupported.size > 0) {
  throw new Error(
    [
      "The configured Todo worker relies on packages its host does not install:",
...
for (const requiredSource of ["New todo", "20260718000001_create_todos"]) {
  if (!bundledSource.includes(requiredSource)) {
    throw new Error(`The configured Todo worker is missing ${JSON.stringify(requiredSource)}.`);
  }
}
