...
// processor.ts): the per-event barrier under slow and nested
...
test("rule 2: event N's slow blocker completes (by timestamp) before event N+1's processEvent starts", async () => {
...
test("rule 2: a blocker registered from INSIDE a blocker holds the cursor (rule 2 fixed point)", async () => {
...
// ── rule 3 — runInBackground never blocks the batch commit ──

test("rule 3: the batch commits (cursor persisted) while background work is still in flight; a bg failure never fails the batch", async () => {
  const mem = memoryStream();
...
// ── rule 4 — one durable commit per batch, all-or-nothing ──

test("rule 4: a throwing REDUCE on the last event is contained: the batch still commits exactly once, the event is skipped", async () => {
  const mem = memoryStream();
  const storage = memoryStorage();
  ...
  expect(snap).toMatchObject({
    offset: 3, // cursor covers the skipped event — no wedge, no retry loop
    state: { n: 2 }, // the throwing event contributed nothing
  });
});

test("rule 4: a throwing BLOCKER on the LAST event persists NOTHING; the wake retries the batch WHOLE", async () => {
  const mem = memoryStream();
  const storage = memoryStorage();
  const effects: number[] = [];
  let attempts = 0;
  ...
  expect(storage).toMatchObject({ writes: 1 }); // and then exactly one persist
  expect(await p.snapshot()).toMatchObject({ state: { n: 3 } }); // the reduce restarted from the persisted state
});

// ── rule 5 — exactly one caughtUp per at-head batch ──

test("rule 5: last event of the batch NOT consumable → the last CONSUMABLE event carries the one caughtUp (no extra eventless pass)", async () => {
  const mem = memoryStream();
...
test("rule 5: no consumable event at all → exactly one eventless caughtUp pass", async () => {
  const mem = memoryStream();
...
test("rule 5: a catch-up whose log length is an exact page multiple (500) still delivers caughtUp", async () => {
...
test("waitUntilProcessed resolves for an offset that arrives via GAP REPAIR (no push ever delivered)", async () => {
  const mem = memoryStream();
...
test("waitUntilProcessed: a waiter timing out concurrently with a resolving batch neither leaks nor disturbs other waiters", async () => {
  const mem = memoryStream();
...
test("version bump: a push in flight on a bumped incarnation keeps the NEW event's processEvent", async () => {
  ...
  expect(await p2.snapshot()).toMatchObject({ state: { n: 3 } }); // the reduce saw it…
...
test("version bump: waitUntilProcessed before the first chain slot keeps the re-reduce reduce-only", async () => {
  ...
  expect(await p2.snapshot()).toMatchObject({ state: { n: 2 } }); // re-reduced state is right…
  expect(effects).toEqual(["effect 1", "effect 2"]); // …but effects must NOT have re-run
});

// ── live state with a projection that throws only sometimes ──

test("live state with a sometimes-throwing projection: state advances through the throwing window; the chain resumes and stays linked", async () => {
  const mem = memoryStream();
  ...
  expect(next).toMatchObject({ from: healed.to }); // linked exactly — from === the previous emission's to
});

// ── ephemeral windows and repair ──

test("ephemeral windows: a LIVE instance given a durable push whose scannedAfterOffset is BEFORE the in-memory-only cursor repairs without double-consuming the ephemerals", async () => {
  const mem = memoryStream();
  const storage = memoryStorage();
  ...
  expect(await engine.snapshot()).toMatchObject({ state: { n: 4 }, offset: 4 });
});

test("ephemeral windows: EVICTION after an ephemeral-only window: the rebuilt incarnation repairs from its durable cursor; dead ephemerals are gaps, durables appear exactly once", async () => {
  const mem = memoryStream();
  const storage = memoryStorage();
  ...
  expect(b).toMatchObject({ seen: ["tick@4"] }); // reduce calls THIS incarnation made (tick@1 came from the checkpoint)
  expect(await engineB.snapshot()).toMatchObject({
    state: { n: 2 }, // tick@1 (persisted reduce) + tick@4 — no double-consume
    offset: 4,
  });
  expect(mem.reads).toBeGreaterThan(readsBefore); // it really was a repair, not a blind fast path
});

test("ephemeral windows: fresh NAMED ephemerals riding a non-contiguous push are delivered (repair the gap, then process the push)", async () => {
...
test('ephemeral windows: consumes ["*"] PLUS a named ephemeral in one contract: durables swept, the named ephemeral consumed, unnamed ephemerals skipped', async () => {
  const mem = memoryStream();
  ...
  expect(await engine.snapshot()).toMatchObject({ state: { n: 3 } });
});

// ── version bump re-reduce — a reducer that throws ──

test("version bump: an event the OLD version accepted and the NEW version's reducer rejects is skipped (reported) — the replay finishes and the new checkpoint is written, instead of failing every incarnation before it", async () => {
  const mem = memoryStream();
  const storage = memoryStorage();
  const effects: string[] = [];
  const p1 = makeVersioned(mem, storage, "1.0.0", effects);
  ...
  expect(await p2.snapshot()).toMatchObject({ state: { n: 2 } });
  ...
  expect(await p3.snapshot()).toMatchObject({ state: { n: 2 } });
});

function deliveryRecorder(slug: string, consumes: readonly string[]) {
...
function contractOf(slug: string, version: string, consumes: readonly string[]) {
...
/** The ephemeral-window suite's processor: `chunk` arrives ephemeral and is NAMED in consumes. */
class EphProcessor extends StreamProcessor<{ n: number }> {
  readonly contract = contractOf("ephwin", "1", ["tick", "chunk"]);
  readonly seen: string[] = [];
  override reduce({ event, state }: ReduceArgs<{ n: number }>) {
    this.seen.push(`${event.type}@${event.offset}`);
    return { n: state.n + 1 };
