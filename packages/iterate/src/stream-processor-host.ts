...
/** The Durable Object context surface used by the host. */
export type StreamProcessorHostContext = {
  readonly storage: {
    readonly kv: {
...
/** Alarm retry metadata consumed by the host's tracing. */
export type StreamProcessorAlarmInfo = {
  isRetry: boolean;
  retryCount: number;
};
...
type HostedProcessorDeps = {
  ...
  readState: () => ProcessorSnapshot<any> | undefined;
  writeState: (snapshot: ProcessorSnapshot<any>) => void;
...
export type StreamProcessorHost<Live extends object = Record<string, unknown>> = {
  ...
  /** The node's published live state; a `.liveState` RpcTarget exposes getState()/subscribe(). */
  readonly live: LiveState<Live>;
  /**
   * Reassemble the live state from current inputs — the ONE writer for the
   * value. Call it after mutating any non-processor live-state input (the
   ...
   */
  handleAlarm(alarmInfo?: StreamProcessorAlarmInfo): Promise<void>;
  /**
...
export function createStreamProcessorHost<Live extends object = Record<string, unknown>>(
  ctx: StreamProcessorHostContext,
  options: {
    stream: Stream;
    /** Path of the hosted stream — stamped as provenance on processor appends. */
    path: string;
  ...
  // The node's published live state. Seeded empty; assembled from processor state
  ...
  return {
    ...
    add(build) {
      ...
      const processor = build({
        ...
        readState: () =>
          ctx.storage.kv.get<ProcessorSnapshot<any>>(snapshotKey(slug())) ?? undefined,
