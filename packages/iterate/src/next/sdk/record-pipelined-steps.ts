...
// a Workers-RPC round trip completely: every call it pipelined, not only the last. No workerd import,
// so the node lane tests it (record-pipelined-steps.test.ts); on native RpcPromises it is proven by
