 ...
 * the tests append public scheduler control events through the normal stream
 * append API, then wait for the alarm-driven effects to appear over the same
 * public stream/history surface.
 */
...
describeDeployedScheduling("events scheduling e2e", () => {
  test("append-scheduled fires within 10 seconds when scheduled for 5 seconds from now", async () => {
    ...
    expect(
      firedEvents.some((event) => event.type === SCHEDULE_INTERNAL_EXECUTION_FINISHED_TYPE),
    ).toBe(true);
  }, 20_000);

  test(
    "append-scheduled appended over oRPC rewrites to schedule-configured and fires once",
    async () => {
      const path = uniqueStreamPath();
      const slug = `sched-${randomUUID()}`;
      const markerType =
  ...
  test(
    ...
    async () => {
      ...
      await waitForHistoryMatch({
        path,
        timeoutMs: waitForAlarmTimeoutMs,
        predicate: (events) => events.some((event) => event.type === SCHEDULE_CONFIGURED_TYPE),
      });
      ...
      expect(
        settledEvents.some((event) => event.type === SCHEDULE_INTERNAL_EXECUTION_FINISHED_TYPE),
      ).toBe(false);
    },
