 ...
 * projection kept in memory and storage, a schedule snapshot embedded in that
 * reduced state, a query-friendly `cf_agents_schedules` mirror for alarm lookups,
 * and newline-delimited fanout for live readers.
 *
 ...
 *
 * Scheduling behavior lives in `scheduling.ts`. Keep this file focused on the
 * stream append/reduce lifecycle and the narrow integration points where
 * schedule control events are folded into reduced_state and mirrored into the
 * SQL table used by the single Durable Object alarm.
 *
 * Durable Object docs:
...
export class StreamDurableObject extends DurableObject<Env> {
  protected state: ReducedStreamState = {
    path: null,
    lastOffset: null,
    eventCount: 0,
    metadata: {},
   ...
   *
   * Scheduling integration invariant:
   * - `reduceStreamState()` updates the durable reduced_state snapshot
   * - `applyScheduleProjectionEventSync()` updates the SQL mirror used by the
   *   alarm helper and by the copied upstream tests
   *
   * Those writes stay in the same transaction so the event log, reduced_state,
   * and `cf_agents_schedules` mirror cannot drift.
   */
  async append(args: { events: EventInput[] }) {
    ...
    this.ctx.storage.transactionSync(() => {
      for (const inputEvent of args.events) {
        ...
        // Scheduling state now has two synchronized representations:
        // - `nextState.cf_agents_schedules` in reduced_state for durable replay
        // - `cf_agents_schedules` as the SQL mirror used by alarm queries/tests
        //
        // Keep these two updates adjacent so future schedule-control event
        // changes do not accidentally update only one representation.
        applyScheduleProjectionEventSync({
          ctx: this.ctx,
          event: insertedEvent,
        });
      ...
      this.persistReducedStateSync(nextState);
    });
  ...
  async getState(): Promise<StreamState> {
    return structuredClone(projectPublicStreamState(this.state));
  }
  ...
  async schedule<T = unknown>(
    ...
    return scheduleOnStream({
      ...
      deps: {
        ...
        requireStreamPath: this.requireStreamPath.bind(this),
  ...
  async alarm() {
    // Unlike the upstream Agents SDK alarm path, there is no PartyServer
    // `super.alarm()` initialization step here. Stream DO initialization already
    // happened in the constructor's `blockConcurrencyWhile`, so this method is a
    // deliberately thin handoff into the scheduler module.
   ...
   * request; `cf_agents_schedules` is a schedule projection derived from internal
   * stream control events so the single DO alarm can multiplex many tasks.
   */
   ...
   */
  private loadState(): ReducedStreamState {
    const eventRowCount =
    ...
    const persistedState = hydrateReducedStreamState({
  ...
  private persistReducedStateSync(nextState: ReducedStreamState) {
    this.ctx.storage.sql.exec(
  ...
  private requireStreamPath() {
    if (this.state.path != null) {
      return this.state.path;
    }

    const rawName = Reflect.get(this.ctx.id as object, "name");
    const parsed = StreamPath.safeParse(rawName);
    if (!parsed.success) {
      return StreamPath.parse(`/__do/${String(this.ctx.id)}`);
    }
  ...
  private propagateStreamCreated(firstEvent: Event) {
    ...
    void Promise.allSettled(
      parentPaths.map((currentParentPath) => {
        ...
        const streamStub = this.env.STREAM.getByName(currentParentPath);
 ...
 * Schedule control events are delegated to `reduceSchedulingState()` so the
 * reduced snapshot stays aligned with the SQL mirror updates in
 * `applyScheduleProjectionEventSync()`.
 */
