...
export function requireWorkspacePath(value) {
...
export function requireRepoPath(value) {
...
export function requireTaskPath(value) {
    ...
    if (!/\.(?:md|markdown)$/i.test(segments.at(-1) ?? "") || !segments.includes("tasks")) {
        throw new Error(`task path must be a .md file below a "tasks" folder: ${JSON.stringify(value)}`);
    }
    return path;
}
function requireCanonicalPath(value, description) {
