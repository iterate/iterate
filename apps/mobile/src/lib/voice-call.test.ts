...
import { startVoiceCall, transcriptItems, type VoiceCallStatus } from "./voice-call.ts";
...
test("the mint is one silent mic frame at call start; holding gates the mic locally; no press or release rides the wire", async () => {
   ...
   * — the server never hears about the button. The press emptied the local
...
test("pickup and end update lifecycle, stop audio, and close", async () => {
  const h = makeHarness();
  const call = await startVoiceCall(h.deps);
  h.deliver({
    type: "events.iterate.com/voice-agent/conversation-accepted",
  ...
  call.setTalking(true);
  h.deliver({ type: ENDED, payload: { reason: "idle" } });
...
test("transcriptItems: both sides, empty rows skipped", () => {
