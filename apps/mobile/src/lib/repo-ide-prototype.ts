// PROTOTYPE — in-memory working tree for the mobile repo IDE spike. The
// durable product implementation should converge with apps/os's staged-changes
// model after the interaction and editor-engine questions are answered.
...
class PrototypeRepoStore {
  ...
  select(path: string) {
    if (this.#snapshot.buffers[path]) this.#update({ selectedPath: path });
  }
...
const stores = new Map<string, PrototypeRepoStore>();

export function prototypeRepoStore(projectId: string) {
  const existing = stores.get(projectId);
  if (existing) return existing;
  const store = new PrototypeRepoStore();
  stores.set(projectId, store);
  return store;
}

export function usePrototypeRepo(projectId: string) {
