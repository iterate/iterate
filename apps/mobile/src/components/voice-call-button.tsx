...
// transcript from the stream's durable events; tap outside to minimise.
//
 ...
 * what was said on both sides. The same events brief a reconnect's session
 * history, so this view is the record. Last dozen lines, pinned to the tail.
 */
  ...
  return (
    ...
    >
      {items.map((item) => (
        <Text
          key={item.key}
          style={[styles.transcriptLine, item.kind === "you" && styles.transcriptYou]}
        >
          {item.kind === "you" ? "you · " : ""}
          {item.text}
        </Text>
