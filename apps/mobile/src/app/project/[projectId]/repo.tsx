// Native repository workspace: a local working tree around the project's real
// repo capability, with CodeMirror source editing and rendered Markdown preview.
...
} from "../../../lib/repo-working-tree.ts";
import { DEFAULT_SERVER } from "../../../lib/servers.ts";
import { getServerBaseUrl } from "../../../lib/storage.ts";
import { colors, radius, spacing } from "../../../lib/theme.ts";

export default function RepoScreen() {
  const params = useLocalSearchParams<{ projectId: string; repoPath: string }>();
  const projectId = params.projectId;
  const repoPath = params.repoPath;
  const queryClient = useQueryClient();
  const repoKey = `${projectId}:${repoPath}`;
  const repoState = useRepoWorkingTree(repoKey);
  const store = repoWorkingTreeStore(repoKey);
  ...
  return (
    <View style={styles.screen}>
      <Stack.Screen options={{ title: repoPath.replace(/^\/repos\//, "") || "Repo" }} />
      ...
      <View style={styles.workbench}>
        {files.isPending ? (
          <EmptyEditor label="Loading repo…" />
        ) : files.isError ? (
          ...
          <>
            <EditorHeader
              ...
              editorView={repoState.editorView}
              markdown={Boolean(repoState.selectedPath?.match(/\.mdx?$/))}
              onView={(editorView) => store.setEditorView(editorView)}
              path={repoState.selectedPath}
            />
            {repoState.selectedPath && typeof repoState.selectedBuffer?.current === "string" ? (
              repoState.editorView === "preview" && repoState.selectedPath.match(/\.mdx?$/) ? (
                <ScrollView contentContainerStyle={styles.markdownPreview}>
                  <Markdown markdown={repoState.selectedBuffer?.current || ""} preview />
                </ScrollView>
              ) : (
                <CodeEditor
          ...
          {markdown ? (
            <View style={styles.viewSwitch}>
              <Pressable
                ...
                style={[styles.viewChoice, editorView === "preview" && styles.viewChoiceActive]}
              >
                <Text style={styles.headerAction}>Preview</Text>
              </Pressable>
              <Pressable
                onPress={() => onView("source")}
                style={[styles.viewChoice, editorView === "source" && styles.viewChoiceActive]}
              >
                <Text style={styles.headerAction}>Source</Text>
              </Pressable>
            </View>
          ) : null}
          {dirty ? (
...
const styles = StyleSheet.create({
  ...
  repoLabel: { color: colors.working, fontSize: 10, fontWeight: "800", letterSpacing: 0.8 },
  ...
  viewChoice: { borderRadius: 6, paddingHorizontal: 7, paddingVertical: 4 },
  viewChoiceActive: { backgroundColor: colors.surfaceRaised },
  ...
  markdownPreview: { paddingBottom: 100 },
});
