...
// (color-swatch cards are drawn, not generated). Words already present in
// the style's generated file are kept as-is; only missing ones generate.
const WORDS = [
  "dog",
  "cat",
  "ball",
...
// Merge mode: existing generated art is nondeterministic and already
// approved — keep it, generate only the words it lacks.
...
console.log(`${existing.size} existing cards kept`);
...
const generate = async (word) => {
  if (existing.has(word)) return [word, existing.get(word)];
