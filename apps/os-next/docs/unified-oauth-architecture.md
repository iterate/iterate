...
the configured operator secret. There is no other credential kind, and no other
door: a bare `/api` WebSocket authenticates in-band (a bearer token, or the operator secret).
