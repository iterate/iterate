...
| `repos`                                               | a repo's files on `main` over git-over-HTTPS, by repo-relative path (`src/context/repos.ts` `ReposScope`): `.list()` `.readFile(repo, path)` `.listFiles(repo)` `.commitFiles(repo, { message, changes: ({ path, content } \| { path, delete: true })[] })` → `{ commitOid, changedPaths }` (ONE commit, compare-and-swapped on the tip; the repo created on first write) `.writeFile(repo, path, content)` `.log(repo, { limit? })` — a read is one shallow fetch of the tip, no cache yet                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `cfArtifacts` + the git wire (the tail of `repos.ts`)                                                      |
...
| `workspaces.get(path)`                                | THE LIBRARY: the workspace of ANY context, at most one per path — the `workspace` facet on `itx.cd(path)` (`src/workspace/`: contract · processor · durable-object), hosted on its first call, addressed after; nothing appended to get it, no list. ONE private overlay (the facet's own SQLite) over a table of repo MOUNTS — every project repo at `/repos/<name>` (derived from `itx.repos.list()`) plus what `.configure({ mounts })` adds (ONE `workspace/configured` event, reduced by the processor): `.readFile(p)` `.readBase(p)` `.writeFile(p, text)` `.deleteFile(p)` (a whiteout of a repo file) `.revert(p)` `.listAllFiles()` `.mounts()` `.gitStatus()` `.gitCommit({ message, scope? })` (ONE mount's changes → one commit on its repo's `main`, then cleared) `.gitLog({ scope?, limit? })`; a path under no mount is scratch, never committed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `src/library.ts`, over `itx.cd(path).facets.get` — the facet over `itx.repos`                              |
