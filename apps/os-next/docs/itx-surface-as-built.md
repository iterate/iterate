...
| `cfArtifacts`                                              | THE REPOS' PHYSICAL HALF: Cloudflare Artifacts, project-scoped and addressed BY THE REPO'S PATH (`src/context/repos.ts` `ArtifactsScope`) — the binding's verbs `.create(path)` → `{ created }` `.get(path).createToken(scope, ttl)` `.list()` → `{ repos: [{ path }] }` `.delete(path)`, and git on `main` over git-over-HTTPS, STATELESS: `.tip(path)` `.snapshot(path)` `.commitFiles(path, { message, changes: ({ path, content } \| { path, delete: true })[] })` → `{ commitOid, changedPaths }` (ONE commit, deletes before writes, compare-and-swapped on the tip — a concurrent push refuses it; the repo created on first write) `.log(path, { limit? })` — a read is one shallow fetch of the tip; the repo facet (`itx.repos.get(path)`, THE way to interact) memoizes it. The Artifacts NAME (`repos--config`, under `${projectId}.`) is derived here and spelled nowhere else                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `ARTIFACTS`, `${projectId}.` prefixed, + the git wire (the tail of `repos.ts`)                                                                                                                                                                                                                                                                   |
...
- `APP_CONFIG_ARTIFACTS_NAMESPACE` (`itx.cfArtifacts`'s git remotes), `APP_CONFIG_SESSION_SECRET`
