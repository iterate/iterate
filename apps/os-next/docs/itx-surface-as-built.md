...
| `secrets`                                             | `.set(name, material, { urls?, refresh? })` `.connect(name, { authorizationEndpoint, tokenEndpoint, clientId, clientSecret?, clientAuth?, scope?, urls?, extra? })` → `{ authorizationUrl }` `.delete(name)` `.list()` → names + pins + strategy kinds, never a value — WRITE-ONLY. `connect` is the OAuth connect half (`secret-connect.ts`): the catalog fact lands now, a human consents at the provider, the provider redirects to the platform's one callback `/.auth/connect/callback` with the code, and the secret's own Durable Object exchanges it (PKCE S256; a platform-signed `state` names the secret) into an `oauth-refresh-token` record — bring your own OAuth client. THE SECRET CELL (`secrets.ts` + `secret-durable-object.ts`, apps/os's ADR 0005): the material is a string or a JSON object (`getSecret("/secrets/NAME", { field: "a.b" })` picks one string out of it), `urls` pins it to those origins only, `refresh` is the strategy the cell re-mints an expired credential with in its own trusted code — `oauth-refresh-token` (RFC 6749 §6 from `refreshToken` + `clientId`/`clientSecret` in the material) or `waitrose-session` (the username/password → session login) — on a 401 from the pinned host or on first use, single-flight, the exchange endpoint itself within the pin; each change appends `events.iterate.com/secrets/changed { name, urls?, refresh? }` (no value) | one `SecretDurableObject` per secret, named `<resourceScope(projectId, path).id>:<name>` — the resource owner's id, so a global user's secrets are their own (its own SQLite: the record + a revision) |
...
| `git`                                                 | THE PHYSICAL HALF of repos — a repo's files on `main` over git-over-HTTPS, by repo-relative path, STATELESS (`src/context/repos.ts` `GitScope`): `.list()` `.create(repo)` `.tip(repo)` `.snapshot(repo)` `.readFile(repo, path)` `.listFiles(repo)` `.commitFiles(repo, { message, changes: ({ path, content } \| { path, delete: true })[] })` → `{ commitOid, changedPaths }` (ONE commit, deletes before writes, compare-and-swapped on the tip; the repo created on first write) `.writeFile(repo, path, content)` `.log(repo, { limit? })` — a read is one shallow fetch of the tip; the repo facet caches it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `cfArtifacts` + the git wire (the tail of `repos.ts`)                                                                                                                                                  |
...
| `repos.get(path)` · `repos.list()`                    | THE LIBRARY: a repo as a DOMAIN OBJECT — the `repo` facet on the context at ANY path (`/repos/<name>` is the convention; the Artifacts repo's name derives from the path: `repos--config`), hosted on its first call. `.create()` runs THE CREATION SAGA as apps/os does: `repos/create-requested` on the repo's path; the processor's effect, at head while a request is owed, provisions the Artifacts repo and lands `repos/created` — the birth certificate, cross-posted to `/` first and on the repo's path last — or `repos/create-failed`, thrown by `create()` and retried as a new attempt; every other method refuses until it has completed. `.tip()` · `.readFile(path)` `.listFiles()` · `.commitFiles({ message, changes })` → one commit on `main` through `itx.git`, guarded by the tip the cache was built on, plus `repo/commit-completed` on the repo's path · `.writeFile(path, text)` · `.log({ limit? })`. THE TIP CACHE: the tip's snapshot in the facet's own SQLite, validated with one `itx.git.tip` per call and re-fetched only when the tip moved; a commit through the facet updates it in place. `list()` is the catalog: the `project` facet on `/` (`src/project/`) folding every cross-posted certificate → `{ path, createdAt }[]`                                                                                                                                               | `src/library.ts`, over `itx.cd(path).facets.get` — the facet over `itx.git`                                                                                                                            |
| `workspaces.get(path)` · `workspaces.list()`          | THE LIBRARY: the workspace of ANY context, at most one per path — the `workspace` facet on `itx.cd(path)` (`src/workspace/`: contract · processor · durable-object), hosted on its first call. `.create()` runs THE CREATION SAGA (nothing to provision yet): `workspace/create-requested` on its path, then `workspace/created { path }` cross-posted to `/` and landed on its path; every other method refuses until it has completed. ONE private overlay (the facet's own SQLite) over a table of repo MOUNTS — every catalogued repo at its OWN path plus what `.configure({ mounts: { [mountPath]: { repo: <repo path> } } })` adds (ONE `workspace/configured` event, reduced by the processor): `.readFile(p)` `.readBase(p)` `.writeFile(p, text)` `.deleteFile(p)` (a whiteout of a repo file) `.revert(p)` `.listAllFiles()` `.mounts()` `.gitStatus()` `.gitCommit({ message, scope? })` (ONE mount's changes → one commit on its repo's `main` through the repo facet, then cleared) `.gitLog({ scope?, limit? })`; a path under no mount is scratch, never committed. `list()` is the catalog: the `project` facet on `/` → `{ path, createdAt }[]`                                                                                                                                                                                                                                                    | `src/library.ts`, over `itx.cd(path).facets.get` — the facet over `itx.repos.get`                                                                                                                      |
...
| `secrets/changed`                                           | `{ name, urls?, refresh? }` · `{ name, deleted: true }` | `secrets` (a record by name: the pin and the strategy KIND, never a value — what `itx.secrets.list()` reads) |
...
- Egress is terminal (section 5, `fetch`): a request naming a secret is forwarded to that secret's
  cell, which substitutes it in the URL and headers, checks the pin, refreshes on a 401, then
