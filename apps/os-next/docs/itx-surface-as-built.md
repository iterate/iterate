...
| `repos.get(name)` · `repos.list()`                    | THE LIBRARY: a repo as a DOMAIN OBJECT — the `repo` facet on the context at `/repos/<name>` (`src/repo/`: contract · processor · durable-object), hosted on its first call. `.create()` — the Artifacts repo and the birth certificate, `repos/created { name, path }`, on its own path AND cross-posted to `/` (the first commit births too) · `.tip()` · `.readFile(path)` `.listFiles()` · `.commitFiles({ message, changes })` → one commit on `main` through `itx.git` plus `repo/commit-completed` on the repo's path · `.writeFile(path, text)` · `.log({ limit? })`. THE TIP CACHE: the tip's snapshot in the facet's own SQLite, validated with one `itx.git.tip` per call and re-fetched only when the tip moved; a commit through the facet updates it in place. `list()` is the catalog: the `project` facet on `/` (`src/project/`) folding every cross-posted certificate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `src/library.ts`, over `itx.cd('/repos/<name>').facets.get` — the facet over `itx.git`                     |
| `workspaces.get(path)` · `workspaces.list()`          | THE LIBRARY: the workspace of ANY context, at most one per path — the `workspace` facet on `itx.cd(path)` (`src/workspace/`: contract · processor · durable-object), hosted on its first call, addressed after; its first use appends `workspace/created { path }` on its path and cross-posts it to `/`. ONE private overlay (the facet's own SQLite) over a table of repo MOUNTS — every repo in the catalog at `/repos/<name>` plus what `.configure({ mounts })` adds (ONE `workspace/configured` event, reduced by the processor): `.readFile(p)` `.readBase(p)` `.writeFile(p, text)` `.deleteFile(p)` (a whiteout of a repo file) `.revert(p)` `.listAllFiles()` `.mounts()` `.gitStatus()` `.gitCommit({ message, scope? })` (ONE mount's changes → one commit on its repo's `main` through the repo facet, then cleared) `.gitLog({ scope?, limit? })`; a path under no mount is scratch, never committed. `list()` is the catalog: the `project` facet on `/`                                                                                                                                                                                                                                                                                                                                                                                                                                              | `src/library.ts`, over `itx.cd(path).facets.get` — the facet over `itx.repos.get`                          |
