...
| `secrets`                                                                                                                       | `.set(name, material, { urls, refresh? })` `.beginOAuth(name, { authorizationEndpoint, tokenEndpoint, clientId, clientSecret?, clientAuth?, scope?, urls?, extra? })` → `{ authorizationUrl }` `.delete(name)` `.list()` → names + pins + strategy kinds, never a value — WRITE-ONLY. Each secret is its own Durable Object (`secrets.ts` + `secret-durable-object.ts`, apps/os's ADR 0005 invariant: material in, nothing out but a request to a pinned host): the material is a string or a JSON object (`getSecret("/secrets/NAME", { field: "a.b" })` picks one string out of it), `urls` is REQUIRED and pins it to those origins only, `refresh` is the strategy the object re-mints an expired credential with in its own trusted code — `oauth-refresh-token` (RFC 6749 §6 from `refreshToken` + `clientId`/`clientSecret` in the material; `clientAuth` is the RFC 8414 registry value, `client_secret_basic` by default, `client_secret_post` for GitHub-shaped endpoints, `none` for a public client) or `waitrose-session` (the username/password → session login) — on a 401 from the pinned host or on first use, single-flight, the exchange endpoint itself within the pin. `beginOAuth` obtains the first tokens (`secret-oauth.ts`): the object keeps the pending attempt (PKCE verifier, nonce, ten minutes) and hands out the authorize URL with a platform-signed `state`; the provider redirects the human to `/.secrets/oauth/callback`, which admits only a signed-in session that reaches the secret's owner (a project's member, the user themself, an organization's member, the admin — cookie or bearer, the project-host admission) and completes through the owner's root context (`.completeOAuth(name, { code, nonce })` — the platform's call, serialized per name with `set`/`delete`): the object exchanges the code and stores `{ clientId, clientSecret, accessToken, refreshToken }` under `oauth-refresh-token`, then the catalog fact is appended (a refused append clears the object again) — nothing is on the catalog before that. Each `set`/`delete` appends `events.iterate.com/secrets/changed { name, urls, refresh? }` (no value); the object appends `secrets/refreshed { name, kind, ok, error? }` on every refresh outcome and `secrets/used { name, method, url, status }` on every dispatch (the request AS RECEIVED — placeholders, never values; a WebSocket upgrade is a dispatch, status 101). The material sits in the object's storage ENCRYPTED (`secret-at-rest.ts`: AES-256-GCM under `APP_CONFIG_SECRETS_KEY`, bound to the object, its pin and the write's revision; `APP_CONFIG_SECRETS_KEY_PREVIOUS` opens during a rotation and the record is rewritten under the current key on that read). | one `SecretDurableObject` per secret, named `<owner>:<name>` (its own SQLite: the record — material encrypted — a write counter every change bumps, the pending attempt, the completed attempt — nonce + the revision it wrote, so a replayed callback completes idempotently and a lost catalog fact can catch up — the catalog's context name)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `fetch(request)`                                                                                                                | egress (`src/iterate-context-durable-object.ts`, the DO's `#egress`): a request naming NO secret goes to the terminal `fetch`; one naming a secret — `getSecret("/secrets/NAME")` or `getSecret("/secrets/NAME", { field: "a.b" })` in the URL (path and query; the parser's percent-encoded spelling matched too) or the headers — is FORWARDED to that secret's Durable Object (`secret-durable-object.ts` `fetch`), which substitutes (spliced into the URL as ONE component with `:` kept — a Telegram bot token; apps/os's grammar for a URL or a header, WITHOUT its `Basic base64(user:getSecret(…))` peeling or its JSON-body template — the body is never scanned), checks the pin, dispatches with `redirect: "manual"`, and on a 401 (or a field not there yet) runs the refresh strategy and retries ONCE. One request, one secret (two names is a 502). A placeholder with no stored secret, a field the material has no string at, or a secret pinned to other origins, is a 502 (`ProjectSecretRefused`) to the caller, never the destination; an unreachable host is a generic 502 naming the host, never the substituted URL. No next door.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | the secret's `SecretDurableObject`, then `fetch`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `repos.get(path)` · `repos.list()`                                                                                              | THE LIBRARY: a repo as a DOMAIN OBJECT — the `repo` facet on the context at ANY path (`/repos/<name>` is the convention; the Artifacts repo's name derives from the path: `repos--config`), hosted on its first call. `.create()` lands `repos/create-requested` on the repo's path, provisions the Artifacts repo, and lands `repos/created` — the birth certificate, cross-posted to `/` first and on the repo's path last — or `repos/create-failed`, thrown; a later `create()` is a new attempt; every other method refuses until it has completed. `.tip()` · `.readFile(path)` `.listFiles()` · `.commitFiles({ message, changes })` → one commit on `main` through `itx.git` plus `repo/commit-completed` on the repo's path · `.writeFile(path, text)` · `.log({ limit? })`. The tip's snapshot is memoized in memory under its oid: one `itx.git.tip` per call, the pack only when the tip moved; a commit through the facet drops it. `list()` is the catalog: the `project` facet on `/` (`src/project/`) folding every cross-posted certificate → `{ path, createdAt }[]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `src/library.ts`, over `itx.cd(path).facets.get` — the facet over `itx.git`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `workspaces.get(path)` · `workspaces.list()`                                                                                    | THE LIBRARY: the workspace of ANY context, at most one per path — the `workspace` facet on `itx.cd(path)` (`src/workspace/`: contract · processor · durable-object), hosted on its first call. `.create()` lands `workspace/create-requested` on its path, then `workspace/created { path }` cross-posted to `/` and landed on its path (nothing to provision); every other method refuses until it has completed. ONE private overlay (the facet's own SQLite, a NULL content being a whiteout) over the MOUNT TABLE — every catalogued repo at its OWN path: `.readFile(p)` `.readBase(p)` `.writeFile(p, text)` `.deleteFile(p)` (a whiteout of a repo file) `.revert(p)` `.listAllFiles()` `.mounts()` `.gitStatus()` `.gitCommit({ message, scope? })` (ONE mount's changes → one commit on its repo's `main` through the repo facet, then cleared) `.gitLog({ scope?, limit? })`; a path under no mount is scratch, never committed. `list()` is the catalog: the `project` facet on `/` → `{ path, createdAt }[]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `src/library.ts`, over `itx.cd(path).facets.get` — the facet over `itx.repos.get`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `r2.put(key, data, { contentType?, customMetadata? })` · `r2.get(key)` · `r2.head(key)` · `r2.list(prefix?)` · `r2.delete(key)` | THE OBJECT STORE: the resource owner's slice of ONE R2 bucket (`FILES`, keys under `<owner.id>/` like kv's `<owner.id>:`) — the bucket's own verbs answered as plain data: `put` → `{ key, size, etag }`; `get` → the record plus `data: Uint8Array` (null when absent); `head` → `{ key, size, contentType, customMetadata, uploaded }`; `list` drains the cursor → `{ objects }`. A binding root; `itx.files` is the file semantics on top                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `context/built-ins.ts`, over `env.FILES`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `files.get(path)` · `files.list(prefix?)`                                                                                       | THE LIBRARY: project file storage as a PATH namespace over `itx.r2` (apps/os's `itx.files`, lean: no signed URLs, no events) — a file is its path (leading slash; the key below is the path without it), its bytes and a content type; last write wins. `get(path)` is a handle: `.put({ contentType?, data })` (bytes, or a string that is base64 or a `data:` URL — whose own content type wins) → `{ path, contentType, size }` · `.bytes()` (throws when gone) · `.head()` (null when absent) · `.delete()`. `list(prefix?)` → the records under a prefix. An agent's `message()` stores attachments here under the agent's path                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `src/library.ts`, over `itx.r2`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `agents.get(path)` · `agents.list()`                                                                                            | THE LIBRARY: an agent as a DOMAIN OBJECT — the `agent` facet on the context at ANY path (`/agents/<name>` is the convention; `src/agent/`: contract · processor · durable-object — apps/os's agent, lean), hosted on its first call. `.create({ systemPrompt? })` births it: the processor row (`itx.processors.enable`, so the loop runs on every commit), then `agent/created { path }` cross-posted to `/` first and landed on its path last with the system prompt beside it; `.message(text                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | { message, files? })`is a person's`agents/context-added`, the trigger of a turn — its attachments stored first under the agent's path (`itx.files`) and named on the event; an image among them reaches the model as an image part (a data: URL of its bytes), anything else as a line naming its path. THE LOOP, every step an event on the path: `agent/llm-request-requested` → the model (`itx.ai.run`under the path's rules — a test lends a fake) →`agent/llm-request-settled`+ the assistant's`context-added`; the answer is markdown plus at most one `<codemode status>` block on its own lines (`src/agent/codemode-format.ts`, mmkal's codemode-tag grammar): the prose → `agents/web-message-sent`, the status → `agent/summary-updated`, the body → `capability-host/script-run-requested`→`itx.run`→`script-run-settled`→ a developer`context-added` → the next turn; prose alone ends the turn. Bounded: expiry,`agent/paused`on N failures or N self-triggered turns,`agent/resumed`on a person's words.`list()`is the catalog: the`project`facet on`/`→`{ path, createdAt }[]` | `src/library.ts`, over `itx.cd(path).facets.get` — the facet over `itx.ai` and `itx.run` |
...
| `secrets/changed`                                           | `{ name, urls, refresh? }` · `{ name, deleted: true }` | `secrets` (a record by name: the pin and the strategy KIND, never a value — what `itx.secrets.list()` reads). `secrets/refreshed { name, kind, ok, error? }` and `secrets/used { name, method, url, status }` are facts only, appended by the secret's object |
...
- Egress is terminal (section 5, `fetch`): a request naming a secret is forwarded to that secret's
  Durable Object, which substitutes it in the URL and headers, checks the pin, refreshes on a 401, then
