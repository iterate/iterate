...
| `cfArtifacts`                                         | THE ESCAPE HATCH: the raw Cloudflare Artifacts binding, project-scoped and shaped like it — `.create(name)` `.get(name).createToken(scope, ttl)` `.list()` `.delete(name)`; every name under `${projectId}.` (`src/context/repos.ts` `ArtifactsScope`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `ARTIFACTS`, `${projectId}.` prefixed                                                                                                                                                                                                                                                                                      |
| `git`                                                 | THE PHYSICAL HALF of repos — a repo's files on `main` over git-over-HTTPS, by repo-relative path, STATELESS (`src/context/repos.ts` `GitScope`): `.create(repo)` `.tip(repo)` `.snapshot(repo)` `.commitFiles(repo, { message, changes: ({ path, content } \| { path, delete: true })[] })` → `{ commitOid, changedPaths }` (ONE commit, deletes before writes, compare-and-swapped on the tip — a concurrent push refuses it; the repo created on first write) `.log(repo, { limit? })` — a read is one shallow fetch of the tip; the repo facet memoizes it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `cfArtifacts` + the git wire (the tail of `repos.ts`)                                                                                                                                                                                                                                                                      |
...
| `secrets/changed`                                           | `{ name, urls, refresh? }` · `{ name, deleted: true }` | `secrets` (a record by name: the pin and the strategy KIND, never a value — what `itx.secrets.list()` reads). `secrets/refreshed { name, kind, ok, error? }` is a fact only, appended by the secret's object |
...
- `APP_CONFIG_ARTIFACTS_NAMESPACE` (`itx.git`'s git remotes), `APP_CONFIG_SESSION_SECRET`
...
**E. AEAD for material at rest.** ADR 0005 binds ciphertext to project, path, pin and offset; the
object stores plaintext in its SQLite, bound to a project only by the object's name. Needs a key in
app config and a rotation story. Not done; the storage is Cloudflare's, and the name is the boundary.

**F. `secrets/used` per egress.** apps/os records every use with the pre-substitution URL; here a use
leaves no fact (a refresh does). A hop per credentialed fetch and log volume on hot paths.

**G. A 101 through a secret's object.** The context DO's egress returned a WebSocket 101 straight;
one more Durable Object hop now sits between the caller and the upstream, and apps/os needed a
handshake-header rewrite (`withWebSocketHandshakeHeaders`) at that point. No test drives a WebSocket
through a secret yet.
