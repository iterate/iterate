...
(`e2e/isolate-ceilings-deployed.e2e.test.ts`, `e2e/stream.e2e.test.ts`).
...
const { store } = await connectLiveState(itx, {
  key: "chat",
  name: "chatwatch",
  door: async () => await itx.invoke("itx.chat.state()"),
});
...
expect(await itx.invoke(["itx", "workers", ["get", { source: SRC_MINE }], ["run"]])).toBe(
  "from-inline:acme-support",
);
// e2e/session.e2e.test.ts
```
...
class HttpDevice extends RpcTarget {
  async fetch(request: Request) {
    return new Response("pong-from-node-provider", { status: 201 });
  }
}
await session()
  .authenticate(adminCredentials())
  .projects.get("acme-support")
  .provide("itx.device", new HttpDevice());
expect(
  (await itx.device.fetch(new Request("https://itx.device/", { method: "POST", body: "ping" })))
    .status,
).toBe(201); // (composed)
...
); // an isError tool call throws
await conn.close();

const pets = await itx.connectToOpenApi(`${PETSHOP}/openapi.json`, { headers });
expect((await pets.operations()).map((o) => o.operationId).sort()).toEqual([
  "createPet",
  "getPet",
  "listPets",
]);
...
const shop = await itx.connectToCapnweb(`${PETSHOP}/capnweb`, {
  transport: "batch",
...
```ts
const { accessToken } = await grantFlow(env, adaCookie, {
  resource: `${ORIGIN}/mcp`,
  choose: (ids) => ids.filter((id) => id !== "oa-three"),
});
expect(tools.map((t) => t.name)).toEqual(["run"]);
expect((await run({ project: "oa-one", script: "async (itx) => itx.whoami()" })).result).toEqual({
  projectId: "oa-one",
  path: "/",
});
expect(appended.result[0].source.principal).toEqual({
  actor: "user_oauth-ada@example.com",
  email: "oauth-ada@example.com",
});
expect((await invoke({ project: "oa-three", expression: "itx.whoami()" })).text).toContain(
  "outside this token's grant",
);
expect((await invoke({ expression: "itx.whoami()" })).text).toMatch(
  /pass project — this token reaches oa-one, oa-two/,
);
...
expect(
  (await readAll(itx.cd("/sibling"))).find((e) => e.type === "note")?.source?.principal,
).toEqual(principal);
// e2e/session.e2e.test.ts
```
...
| 4        | `e2e/context.e2e.test.ts`, `e2e/support/client.ts`, `e2e/stream.e2e.test.ts`, `e2e/isolate-ceilings-deployed.e2e.test.ts`, `e2e/push-delivery.e2e.test.ts`, `e2e/rpc-stubs-lend-recall-and-offline.e2e.test.ts`                                                |
