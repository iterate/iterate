...
| Idle quiesce          | the pins' last use + 60 s, rounded up to the next 10 s — a borrowed rpc stub called, a library connection used; nothing else moves it, and none while nothing is pinned. A facet is not a pin: on the edge it does not keep the actor resident (it dies with the actor within seconds), so nothing arms an alarm for it; a release that runs for a pin aborts live facets too                                                                                                                                                                               |
...
materialization. Only a borrowed rpc stub (measured: it does keep the actor resident) or a library
connection arms the idle deadline, and the release that runs for them aborts live facets too — which is
