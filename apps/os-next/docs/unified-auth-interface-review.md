...
Deployed smoke for the same commit: `/version` at `auth.iterate2.com`, admin bearer on `/api/rpc`, and one real CIMD fetch of `https://auth.iterate2.com/.auth/client.json` from the worker itself, because that self-fetch is the single largest unknown in the whole plan.
...
- One `OAuthProvider` with `apiHandlers: { "https://auth.iterate2.com/api": capnwebBearerHandler, "https://mcp.iterate2.com/": mcpHandler }` (full-URL routes match hostname + path prefix, 2098-2110), full-URL `authorizeEndpoint`/`tokenEndpoint`/`clientRegistrationEndpoint` on os, `resourceMetadata: { authorization_servers: ["https://auth.iterate2.com"], scopes_supported: [...] }` and **no** `resource`. Take the AS origin from app config, not the request, so the local lane keeps working.
- Constants `ALLOWED = ["https://auth.iterate2.com/api", "https://mcp.iterate2.com"]`, the same strings the PRM emits.
