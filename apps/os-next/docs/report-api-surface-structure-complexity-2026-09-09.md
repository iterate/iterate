...
IterateContext                                  the edge proxy; everything else rides `invoke`
  ...
  .provide(match, target | stub | null)         → RewriteRuleHandleRpcTarget (disposable; `using`)
  .subscribe({ name?, target, consumes?, afterOffset? })   → SubscriptionHandleRpcTarget (disposable)
...
`fetch-door` (three), `stream` (five), `isolate-ceilings-deployed` (memory-budget + uncontrolled-
