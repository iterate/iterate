...
// fact; `delete(path)` is THE DELETION, its mirror — the request, the death certificate, then the row
// (and the overlay) goes. Addressing a workspace (`itx.workspaces.get(path)`) is the library's
// (library.ts): straight to the path, never through `/`.
...
export class WorkspaceCollectionRpcTarget extends RpcTarget {
  ...
  create(path: string): Promise<{ path: string }> {
    return this.withItx(async (itx) => {
      ...
      if (state.deletion) throw new Error(`workspace ${path}: deleted — not re-creatable`);
  ...
  /** Take the workspace at `path` out of being: `workspace/delete-requested` on that path, then the
   *  death certificate — `workspace/deleted` (gone from the catalog by then) — then the `workspace`
   *  processor row goes, and the facet with it, storage included: the overlay is gone. Idempotent: a
   *  deleted workspace answers at once, and a deletion already open is WAITED ON, never requested
   *  again — the certificate is sought after the request that opened it, so one landing between the
   *  read and the wait is seen, not missed. A workspace never created has nothing to delete: thrown.
   *  Terminal: a deleted workspace is not re-creatable. */
