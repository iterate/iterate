...
// creation saga: `session.projects.create` writes the control plane's row, which owes `/` the
// `project` row and a `project/create-requested` (control-plane/catalog.ts, delivered by
// control-plane/durable-object.ts); processor.ts runs the saga from state at head
