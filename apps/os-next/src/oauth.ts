...
export async function authorizationOf(env: Env, props: unknown): Promise<Authorization | null> {
  ...
  return {
    ...
    // oxlint-disable-next-line iterate/simple-truthiness-check -- `reach` is inspected with `"projectIds" in reach` (session.ts, directory.ts): a present key with an undefined value differs from an absent one, so the conditional spread must stay (grant.projects is string[] | null)
