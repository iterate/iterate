...
// appended by the verb that did it, stamped with the caller; and the memberships the control plane
// lands here after it writes them (src/control-plane/durable-object.ts).
...
export const AccountContract = defineProcessorContract({
  ...
  // organization's membership facts (src/organization/contract.ts: landed here by the control
  // plane beside the organization's own log).
