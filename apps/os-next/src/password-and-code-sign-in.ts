...
export async function signInWithPassword(
  ...
  return { user: await new ControlPlane(env.ITERATE_CONTEXT).ensureUser(address) };
}
...
export async function finishLoginCode(
  ...
  await env.OAUTH_KV.delete(key(id));
  return { user: await new ControlPlane(env.ITERATE_CONTEXT).ensureUser(challenge.email) };
}
