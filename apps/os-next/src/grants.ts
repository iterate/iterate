...
export class GrantsRpcTarget extends RpcTarget {
  ...
  async list(cursor?: string) {
    ...
    return {
      items,
      cursor: page.cursor,
      projects: await new ControlPlane(env.ITERATE_CONTEXT).reachableProjects(session.reach),
  ...
  async mint(input: unknown) {
    ...
    const projects = (await new ControlPlane(env.ITERATE_CONTEXT).reachableProjects(session.reach))
