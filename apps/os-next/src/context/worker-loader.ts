...
export const facetSpecOf = ({ source, cacheKey, className }: FacetSpec): FacetSpec => ({
  source,
  // oxlint-disable-next-line iterate/simple-truthiness-check -- the canonical shape deliberately OMITS an absent cacheKey (never `cacheKey: undefined`); the kv-stored memo and hosting event this feeds are compared by presence
