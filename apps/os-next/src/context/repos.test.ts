// context/repos.test.ts — the repos' unit pins: `itx.cfArtifacts` (the physical root, project-scoped
// and BY PATH, pure over an injected namespace), the path → Artifacts-name mapping, and the git
// wire's one refusal that matters to it.
...
// Pure over an injected namespace: no DO, no bindings, no network (the git verbs are pinned against
// the real endpoint in e2e/cfartifacts.e2e.test.ts).
...
test("cfArtifacts speaks paths, prefixes the derived name with a '.' delimiter, and re-exposes the handle WITHOUT fork", async () => {
...
// ── git wire ── the wire's one refusal that matters to `itx.cfArtifacts`: a TRUNCATED pkt-line body is
...
describe("itx.cfArtifacts's tree codec against git's ids", () => {
