...
export function normalizeRewriteRuleConfigured(payload: {
...
}): { match: ItxExpression; target: ItxExpression | null; ifTarget?: ItxExpression | null } {
  ...
  if (!("ifTarget" in payload)) return { match: matchPrefix, target: targetExpression };
  const ifTarget =
    // oxlint-disable-next-line iterate/simple-truthiness-check -- like `target` above: null (or an absent value) is the explicit sentinel (compare against a null-target row), distinct from a malformed empty-string ifTarget that normalization must still parse and reject
    payload.ifTarget === null || payload.ifTarget === undefined
      ? null
      : normalizedItxExpression(payload.ifTarget, { holes: true });
  return { match: matchPrefix, target: targetExpression, ifTarget };
}
