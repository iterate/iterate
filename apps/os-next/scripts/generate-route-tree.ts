// Regenerates src/routeTree.gen.ts outside of `vite dev`/`vite build`, using the same generator +
// config that @tanstack/react-start's vite plugin uses (apps/auth/scripts/generate-route-tree.ts,
// verbatim but for the paths). `--check` fails (and restores the original file) when the checked-in
// tree is stale, so `pnpm typecheck` catches route files added or renamed without regenerating.
// Run with node (≥ 22.18 strips types natively): `pnpm routes:generate` / `pnpm routes:check`.
