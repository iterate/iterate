 ...
 *
 *   doppler run --project project-worker --config preview -- sh -c 'ADMIN_API_SECRET="$(node -p "JSON.parse(process.env.APP_CONFIG).secrets.adminBearer")" pnpm control-plane-load --worker-base-url https://pr2828-control-plane-cleanup-os-next-preview.iterate-dev-preview.workers.dev --triples 1000 --sockets 50'
 */
...
export default async function controlPlaneLoad(options: {
  ...
  /** The operator's surface (src/session.ts): what this load uses. */
  type Session = {
    users: {
  ...
  await Promise.all(
    sample.map(async (m, i) => {
      ...
      try {
        ...
        const org = (orgs as { id: string; role?: string; projects: number }[]).find(
          (o) => o.id === m.orgId,
        );
