...
| `workers.get({ source, cacheKey?, className?, props? })` / `facets.get(name, { source, cacheKey?, className })` | cacheKey + source; a facet's `props { iterateContextName, name }` and startup memo                                                                                                                                                                                                                                                                                                                              | Cloudflare (Worker Loader, `ctx.facets`)                                                                | `context/worker-loader.ts`, `context/facet-host.ts`         |
...
back a DISPOSABLE `RewriteRuleHandleRpcTarget` (`subscribe`'s is a `SubscriptionHandleRpcTarget`): disposing it — or
...
`SubscriptionHandleRpcTarget` (its `name` getter is the generated one when none was given); a live callback
...
membership is the in-process directory's, `src/control-plane.ts`;
