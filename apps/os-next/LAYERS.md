...
| `kv`, `secrets`, `whoami`, `fetch`                                                                              | KV / a secret's own DO — THE CELL: material + pin + refresh strategy, write-only (`secret-durable-object.ts`) / the address / the terminal `fetch` (a secret-naming request is forwarded to its cell) | —                                                                                                       | `context/built-ins.ts`, `iterate-context-durable-object.ts` |
