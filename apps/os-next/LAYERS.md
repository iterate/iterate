...
| `kv`, `secrets`, `whoami`, `fetch`                                                                              | KV / a secret's own DO — THE CELL: material + pin + refresh strategy, write-only, plus the OAuth connect half's pending attempt (`secret-durable-object.ts`, `secret-connect.ts`) / the address / the terminal `fetch` (a secret-naming request is forwarded to its cell) | —                                                                                                       | `context/built-ins.ts`, `iterate-context-durable-object.ts` |
