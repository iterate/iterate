...
test("the /demo renders reduced ⊕ runtime live state and updates on button clicks", async ({
  ...
  // read-then-click so the assertion can't pass vacuously off a pre-poked warm isolate (the demo
  // ctx is shared and lastPokeMs is runtime state — it may already be nonzero before our click)
  const lastPoke = page.getByTestId("lastPokeMs");
  const pokeBefore = await lastPoke.textContent();
  await page.getByRole("button", { name: /^poke/i }).click();
  await expect(lastPoke).not.toHaveText(pokeBefore || "0");
});
