...
import { codeOf, freshCtx, openItx, rejection, workerUrl } from "./support/client.ts";
...
test("THE CHAIN: a subagent two levels down resolves a capability provided at the root through parent links, lists it with its description and origin, and births its own children relative to itself", async () => {
  ...
  // /agents/a was linked to its creator (the root); a script THERE creates ./b — the link points at /agents/a
  expect(
    await root
      .cd("/agents/a")
      .run(
  ...
  ).toEqual(["itx.builtins.cd('/agents/a')"]); // the child's own link, pointing at its creator
  expect(await root.cd("/agents/a/b").builtins.rewriteRules.get("itx")).toMatchObject({
    target: "itx.builtins.cd('/agents/a')",
  });
  // two levels down, the root's stub answers, and the list says where it came from
  expect(await root.cd("/agents/a/b").run("async (itx) => itx.tool()")).toBe("hello-from-root");
  const rows = (await root.cd("/agents/a/b").rewriteRules.list()) as {
    match: string;
    context: string;
    description?: string;
  }[];
  ...
  expect(rows.find((row) => row.match === "itx.append")).toMatchObject({ context: "/agents/a/b" });
  ...
  expect(codeOf(await rejection(root.cd("/agents/a/b").run("async (itx) => itx.tool()")))).toBe(
