...
export function deployedTarget(workerBaseUrl: string): {
  ...
  // The envs.ts entry the worker falls under, by host suffix: `os.iterate2.com` is prd's; a preview,
  ...
  const env = Object.values(osNextEnvs).find((candidate) =>
  ...
  return {
    ...
    // MCP on an origin of its own (prd's mcp.iterate2.com) is the deployment's; on the platform
