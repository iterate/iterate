...
// text.
const fail = createFailing(test, /internal error; reference = spent/);
fail(
  "on a runtime whose facet starts answer one call each, concurrent calls on a running facet each answer",
  async () => {
    const source = {
      "cap.js": `
import { DurableObject } from "cloudflare:workers";
...
fail(
  "on a runtime whose facet starts answer one call each, a call killed by a peer's restart answers on a start of its own",
  async () => {
    const source = {
      "cap.js": `
import { DurableObject } from "cloudflare:workers";
