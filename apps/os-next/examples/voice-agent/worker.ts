/**
 * The project's root worker (`itx.worker`), and through the one-line rule
 * `itx.voice ⇒ itx.worker` what a device calls on the button press:
 *
 *   await root.voice.setupVoiceAgent({ streamPath, activation })   // → { streamPath }
 *
 * ONE append on the conversation's fresh context: the voice facet's
 * subscription row (what `processors.enable` writes) and the call's birth, so
 * the facet materialises, reads `call-started` from its log and dials the
 * provider before the first microphone frame arrives. The device carries no
 * source and no class name; the bundle lives in the project's KV.
 *
 * The funnel delivers every durable event of every context here too
 * (`processEvent`); nothing reacts today.
 */
    ...
    await itx.cd(streamPath).append(
      {
        ...
        payload: {
          ...
          consumes: ["*", `${T}mic-frame`, `${T}keepalive`],
        },
      },
      ...(options.instructions
        ? [{ type: `${T}configured`, payload: { instructions: options.instructions } }]
        : []),
      ...(options.activation
        ? [
            {
