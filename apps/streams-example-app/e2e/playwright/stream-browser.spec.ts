...
async function appendComposerEvent(
  ...
  // appendBatch has an idempotent, bounded recovery path for a tagged
  // Durable Object reset. Wait for that operation to finish instead of racing
  // its event delivery with a shorter assertion timeout. Any terminal error
  // still fails immediately and preserves the product failure as evidence.
