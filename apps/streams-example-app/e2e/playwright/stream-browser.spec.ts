...
import { e2eStreamPath, e2eStreamProjectId, streamRoute } from "../helpers.ts";
...
test("stream page reload starts at the bottom of an existing local event table", async ({
  ...
  // The reload's connection churn appends only ephemeral presence facts, so
  // the durable count is unchanged.
  await expect(page.getByTestId("event-count")).toHaveText(String(expectedCount), {
    timeout: 30_000,
  });
...
async function holdCurrentWriterLock(page: Page, streamPath: string) {
  const lockName = `stream-event-sync:${databasePathFor(e2eStreamProjectId(), streamPath)}`;
