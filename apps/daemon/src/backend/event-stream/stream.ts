  ...
  Effect.gen(function* () {
    ...
    const subscribe = (subscribeOpts?: {
      ...
      Effect.gen(function* () {
        ...
        const lastHistoricalOffset =
          historical.length > 0 ? historical[historical.length - 1]!.offset : null;
    ...
    const getFrom = (getFromOpts: {
      ...
      Effect.gen(function* () {
        ...
        return yield* storage.getFrom(
          limit !== undefined ? { name, offset, limit } : { name, offset },
        );
      });
