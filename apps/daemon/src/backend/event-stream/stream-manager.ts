...
const makeStreamManager = Effect.gen(function* () {
  ...
  const subscribeAll = (): Effect.Effect<
    Stream.Stream<Event, StorageError>,
    StorageError,
    Scope.Scope
  > =>
    Effect.gen(function* () {
      ...
      const subscribeToStream = (
        ...
        Effect.gen(function* () {
          ...
          const startOffset = existingStream ? makeOffset(yield* stream.count) : OFFSET_START;

          const eventStream = yield* stream
            .subscribe({ offset: startOffset })
            .pipe(
              Effect.catchTag("InvalidOffsetError", () =>
                Effect.fail(new StorageError({ message: "Invalid offset" })),
              ),
            );
      ...
      yield* Effect.forEach(existingNames, (name) => subscribeToStream(name, true), {
        discard: true,
      });
...
export class StreamManagerService extends Effect.Service<StreamManagerService>()(
  ...
  static readonly Live: Layer.Layer<StreamManagerService, never, Storage | EventStreamFactory> =
    Layer.effect(StreamManagerService, makeStreamManager as Effect.Effect<StreamManagerService>);
