/**
 * Daemon Entry Point - Agent Wrapper Backend
 *
 * This is a Hono-based HTTP server that integrates with the agent-wrapper
 * architecture. It provides:
 * - Agent management via AdapterRunner
 * - Durable stream persistence
 * - SSE subscriptions for real-time events
 *
 * This replaces the old index.ts that had its own in-memory implementation.
 */
...
import { ActiveFactory } from "./durable-streams/stream-factory.ts";
import { StreamManagerService } from "./durable-streams/stream-manager.ts";
import { type StreamName, type Offset, OFFSET_START, type Event } from "./durable-streams/types.ts";
import { runPiAdapter } from "./agent-wrapper/pi-adapter.ts";
import {
  makePromptEvent,
  makeSessionCreateEvent,
  type EventStreamId,
  PiEventTypes,
} from "./agent-wrapper/types.ts";

// Define StreamClientService locally to avoid importing from client.ts
// (which has @effect/experimental dependency issues)
interface StreamClient {
  append: (opts: { name: StreamName; data: unknown }) => Effect.Effect<Event, unknown>;
  subscribe: (opts: {
    name: StreamName;
    offset?: Offset;
  }) => Effect.Effect<Stream.Stream<Event, unknown>, unknown>;
  subscribeAll: () => Effect.Effect<Stream.Stream<Event, unknown>, unknown>;
  get: (opts: {
    name: StreamName;
    offset?: Offset;
    limit?: number;
  }) => Effect.Effect<ReadonlyArray<Event>, unknown>;
  list: () => Effect.Effect<ReadonlyArray<StreamName>, unknown>;
  delete: (opts: { name: StreamName }) => Effect.Effect<void, unknown>;
}

class StreamClientService extends Effect.Service<StreamClientService>()(
  "@event-stream/StreamClient",
  {
    effect: Effect.succeed({} as StreamClient),
  },
) {}
...
// In-process stream client that directly calls the manager
const inProcessClientLayer = Layer.effect(
  StreamClientService,
  Effect.gen(function* () {
    const manager = yield* StreamManagerService;

    return {
      append: (opts: { name: StreamName; data: unknown }) => manager.append(opts),
      subscribe: (opts: { name: StreamName; offset?: Offset }) => manager.subscribe(opts),
      subscribeAll: () => manager.subscribeAll(),
      get: (opts: { name: StreamName; offset?: Offset; limit?: number }) => manager.getFrom(opts),
      list: () => manager.list(),
      delete: (opts: { name: StreamName }) => manager.delete(opts),
    } as unknown as StreamClientService;
  }),
).pipe(Layer.provide(streamManagerLayer));

const mainLayer = Layer.mergeAll(streamManagerLayer, inProcessClientLayer, NodeContext.layer);
...
const runEffect = <A, E>(
  effect: Effect.Effect<A, E, StreamManagerService | StreamClientService>,
...
const runScopedEffect = <A, E>(
  effect: Effect.Effect<A, E, StreamManagerService | StreamClientService | Scope.Scope>,
): Promise<A> => runtime.runPromise(Effect.scoped(effect));
...
app.post("/agents/*", async (c) => {
  ...
  if (!adapter) {
    const { eventStreamId } = await startPiSession(streamPath);
