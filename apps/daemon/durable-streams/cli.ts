...
const streamSubscribeAllCommand = Command.make("subscribe-all", { server: serverUrlOption }, () =>
  Effect.gen(function* () {
    const client = yield* StreamClientService;
...
const streamGetCommand = Command.make(
  ...
  ({ limit, name, offset, server: _server }) =>
    Effect.gen(function* () {
      ...
      const getOpts: { name: StreamName; offset?: Offset; limit?: number } = {
...
/** stream list - list all streams */
const streamListCommand = Command.make("list", { server: serverUrlOption }, () =>
  Effect.gen(function* () {
    const client = yield* StreamClientService;
