...
const startServer = async (
  ...
  const storageLayer = Storage.FileSystem({ dataDir }).pipe(Layer.provide(NodeContext.layer));
  ...
  test("get events with offset and limit", async () => {
    ...
    try {
      ...
      const offsetRes = await fetch(
        `${server.baseUrl}/streams/${streamName}/events?offset=${offset}`,
      );
      ...
      const combinedRes = await fetch(
        `${server.baseUrl}/streams/${streamName}/events?offset=${offset}&limit=1`,
      );
