...
function formatNewThreadMentionMessage(
  ...
  return [
    ...
    "Refer to SLACK.md for how to respond via `iterate tool exec-js` using `slack`.",
    "",
    `From: ${user}`,
    `Message: ${text}`,
...
function formatMidThreadMentionMessage(
  ...
  const lines = [
    ...
    "Refer to SLACK.md for how to respond via `iterate tool exec-js` with top-level `slack`.",
    "",
    `From: ${user}`,
    `Message: ${text}`,
    ...formatFileLines(event.files),
