...
Routes ALL HTTP/HTTPS requests through the Cloudflare worker endpoint.
...
Configuration:
- EGRESS_PROXY_ENDPOINT: The worker endpoint URL (required)
- EGRESS_PROXY_API_KEY: API key for authenticating with the worker (required)
"""
...
WORKER_ENDPOINT = os.environ.get("EGRESS_PROXY_ENDPOINT", "")
API_KEY = os.environ.get("EGRESS_PROXY_API_KEY", "")
...
class EgressProxyAddon:
    ...
    def __init__(self):
        if not WORKER_ENDPOINT:
            ctx.log.warn("[egress] EGRESS_PROXY_ENDPOINT not set - requests will pass through directly")
        if not API_KEY:
            ctx.log.warn("[egress] EGRESS_PROXY_API_KEY not set - worker auth will fail")
    ...
    def request(self, flow: http.HTTPFlow) -> None:
        ...
        flow.request.headers["X-Egress-Original-URL"] = original_url
        flow.request.headers["X-Egress-Original-Host"] = original_host
        flow.request.headers["X-Egress-Original-Method"] = flow.request.method
        
        # Add API key for worker auth
        if API_KEY:
            flow.request.headers["X-Egress-API-Key"] = API_KEY
    ...
    def response(self, flow: http.HTTPFlow) -> None:
        ...
        original_url = flow.request.headers.get("X-Egress-Original-URL", flow.request.url)
