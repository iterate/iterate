...
describe.runIf(RUN_LOCAL_DOCKER_TESTS)("Egress Proxy", () => {
  ...
  test("addon script exists", async () => {
    const result = await execInContainer(containerId, [
      "cat",
      `${CONTAINER_REPO_PATH}/apps/daemon/server/services/egress-proxy/addon.py`,
    ]);
  ...
  test("forwards to worker endpoint and injects OpenAI token", async () => {
    ...
    const workerEndpoint = `${baseUrl}/api/egress-proxy`;
    const healthUrl = `${baseUrl}/api/health`;
    const addonPath = `${CONTAINER_REPO_PATH}/apps/daemon/server/services/egress-proxy/addon.py`;
    ...
    await execInContainer(containerId, [
      "sh",
      "-c",
      `EGRESS_PROXY_ENDPOINT="${workerEndpoint}" EGRESS_PROXY_API_KEY="test-key" mitmdump -p 8889 -s ${addonPath} --ssl-insecure -q &`,
    ]);
    await new Promise((r) => setTimeout(r, 2000));
