...
EGRESS_PROXY_ENDPOINT="${EGRESS_PROXY_ENDPOINT:-https://dev-nick-os.dev.iterate.com/api/egress-proxy}"
EGRESS_PROXY_API_KEY="${EGRESS_PROXY_API_KEY:-test-dev-key}"
...
log "Worker endpoint: $EGRESS_PROXY_ENDPOINT"
log ""

# Run test commands inside container
docker run --rm \
  -e EGRESS_PROXY_ENDPOINT="$EGRESS_PROXY_ENDPOINT" \
  -e EGRESS_PROXY_API_KEY="$EGRESS_PROXY_API_KEY" \
...
cp /local-iterate-repo/apps/daemon/server/services/egress-proxy/addon.py /app/egress-proxy/

# Start mitmproxy in background
log "Starting mitmproxy on port 8888..."
EGRESS_PROXY_ENDPOINT="$EGRESS_PROXY_ENDPOINT" \
EGRESS_PROXY_API_KEY="$EGRESS_PROXY_API_KEY" \
