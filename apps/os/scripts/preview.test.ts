  ...
  test("on a PR the heading, every app and every config template carry a one-click `Sign in ↗`, and the section says as whom", () => {
    ...
    expect(render(true)).toMatchInlineSnapshot(`
      ...
      **https://pr123-feature-foo-os-next-preview.iterate-dev-preview.workers.dev** · [Sign in ↗](https://pr123-feature-foo-os-next-preview.iterate-dev-preview.workers.dev/.auth/test-link?t=heading) · deployment \`bd68a9bb\` · [Cloudflare dashboard](https://dash.cloudflare.com/x) · deleted when this PR closes
      ...
      New project from template: [default at this PR's \`bbbbbbbbb\` ↗](https://pr123-feature-foo-os-next-preview.iterate-dev-preview.workers.dev/.auth/test-link?t=default) · [with-agents ↗](https://pr123-feature-foo-os-next-preview.iterate-dev-preview.workers.dev/.auth/test-link?t=with-agents)
