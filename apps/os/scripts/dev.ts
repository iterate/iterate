...
import { packLocalPackages } from "./lib/dev-sdk-tarball.ts";
...
export default async function start(options: StartOptions = {}) {
  ...
  // `iterate` package and the `@iterate-com/docs` bridge. Pack the WORKSPACE
  // packages so local dev exercises this branch's code, exactly like preview
  // deploys pin their PR's published builds. The overrides env var MUST be
  // set here in the parent — the wrangler config generator captures
  // process.env at import time, before any code in vite.config.ts could set
  // it — while the tarball HTTP server itself lives in the vite process
  // (vite.config.ts serveDevSdkTarball), which owns the long-running
  // lifetime. Explicit APP_CONFIG_ITERATE_REPO_PKG_SPEC_OVERRIDES wins (see
  // lib/dev-sdk-tarball.ts for the whole story).
  if (!process.env.APP_CONFIG_ITERATE_REPO_PKG_SPEC_OVERRIDES) {
    const { dir, overrides } = await packLocalPackages(APP_ROOT);
    process.env.APP_CONFIG_ITERATE_REPO_PKG_SPEC_OVERRIDES = JSON.stringify(overrides);
    process.env.ITERATE_DEV_SDK_TARBALL_DIR = dir;
  }
