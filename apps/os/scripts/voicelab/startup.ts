...
export interface StartupOptions extends VoicelabConnectOptions {
  ...
  maxStartupMs?: number;
  /**
   * Prepare every stream (setupVoiceAgent without an activation) BEFORE any
   * timer starts, then time only the activation: first microphone frame →
   * call-started → conversation-accepted → first audio. This is the
   * "next conversation is already provisioned" device model.
   */
  presetup?: boolean;
  /** With --presetup, wait this long between preparing and activating (idle eviction probe). */
  idleSeconds?: number;
}
...
type StartupResult = StartupWatch & {
  streamPath: string;
  activation: string;
  /** With --presetup: how long the untimed preparation took, before the clock. */
  presetupMs: number | null;
...
export async function startup(options: StartupOptions) {
  ...
  try {
    ...
    const presetup = options.presetup === true;
    const streamPaths = Array.from(
      { length: runs },
      (_, i) => `${prefix}/${String(i + 1).padStart(2, "0")}-${crypto.randomUUID().slice(0, 8)}`,
    );
    const presetupMs = new Map<string, number>();
    if (presetup) {
      for (const streamPath of streamPaths) {
        const startedAt = Date.now();
        const prepared = await waitForEvent(
          Promise.resolve(
          ...
          60_000,
          `presetup of ${streamPath} did not resolve`,
        );
        disposeIgnoredRpcResult(prepared);
        presetupMs.set(streamPath, Date.now() - startedAt);
        console.log(
          JSON.stringify({
            type: "voicelab-startup-presetup",
            streamPath,
            tookMs: presetupMs.get(streamPath),
          }),
        );
      }
      if (options.idleSeconds && options.idleSeconds > 0) {
        console.log(
          JSON.stringify({ type: "voicelab-startup-idle", seconds: options.idleSeconds }),
        );
        await new Promise((resolve) => setTimeout(resolve, options.idleSeconds! * 1000));
      }
    }
    for (let index = 1; index <= runs; index += 1) {
      const streamPath = streamPaths[index - 1]!;
      ...
      const result: StartupResult = {
        streamPath,
        activation,
        presetupMs: presetupMs.get(streamPath) ?? null,
      ...
      try {
        ...
        if (presetup) {
          // Prepared stream: the first microphone frame mints the call.
        } else if (activateOnSetup) {
        ...
        if (presetup) {
          // The first microphone frame mints the call; commentary before the
          // call exists is dropped by the facet, so wait for acceptance first.
          await waitForEvent(
            configured.promise,
            remainingMs(),
            "session-configured did not arrive",
          );
          await waitForEvent(
            accepted.promise,
            remainingMs(),
  ...
  const summary = {
    ...
    contract: {
      ...
      activateOnSetup,
      maxStartupMs,
      presetup: options.presetup === true,
    ...
    metricsMs: {
      presetup: metric(results.map((result) => result.presetupMs)),
