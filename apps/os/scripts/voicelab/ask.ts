...
/* An async function's constructor accepts a function body and returns a promise. */
const AsyncFunction = async function () {}.constructor as new (
  ...args: string[]
) => (itx: unknown) => Promise<unknown>;

export async function ask(options: AskOptions): Promise<void> {
  const requests = z.array(z.string()).min(1).parse(JSON.parse(options.requests));
  const streamPath =
    options.streamPath ||
  ...
  for (const [index, frames] of utterances.entries()) {
    ...
    if (
      watch.providerErrors.length !== before.errors ||
      watch.providerDisconnects.length !== before.disconnects
    ) {
      process.exitCode = 1;
    }
    if (!settled) {
      console.log("    timed out waiting for the answer to settle");
      process.exitCode = 1;
    }
