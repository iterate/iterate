...
export async function duplex(options: DuplexOptions): Promise<void> {
  ...
  const providerErrors = watch.providerErrors.length;
  const providerDisconnects = watch.providerDisconnects.length;
  if (providerErrors !== 0 || providerDisconnects !== 0) {
    fail(
      `provider diagnostics: ${String(providerErrors)} errors, ${String(providerDisconnects)} disconnects`,
    );
  }
  await call.stop();
  await sleep(2_500);
