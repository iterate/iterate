...
export async function liveProbe(options: LiveProbeOptions = {}): Promise<void> {
      ...
      delegation === "client"
        ...
        : {
            ...
            responses: {
              ...
              tools: exec ? [EXEC_TYPESCRIPT_TOOL] : [],
              tool_choice: "auto",
  ...
  socket.on("message", (data) => {
    ...
    switch (type) {
      ...
      case "response.event": {
        ...
        const record = delegationId ? delegations.get(delegationId) || null : null;
        if (innerType === "response.output_text.delta" && record) {
  ...
  async function runFunctionCall(
    ...
    if (name === EXEC_TYPESCRIPT_TOOL.name && exec) {
      let code = "";
      try {
