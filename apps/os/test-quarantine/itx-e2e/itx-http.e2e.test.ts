...
import { baseUrl, connectGlobal, registerCreatedProjectCleanup } from "./e2e-env.ts";

const createdProjectIds = registerCreatedProjectCleanup();
const testIfPublicHostnamesResolve = isLocalBaseUrl() ? test.skip : test;
...
testIfPublicHostnamesResolve(
  "HTTP-exposed caps serve their own hostname publicly; unexposed caps 404",
  async () => {
    using itx = connectGlobal();
