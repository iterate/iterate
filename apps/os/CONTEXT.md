...
OS is an authenticated app where users manage organization-owned projects and run scripts that call tool functions. This context defines the language for project ownership, codemode sessions, tool providers, and the event-first execution model.
...
An OS oRPC procedure under the singular `os.project.*` router. External callers pass `projectSlugOrId`; project-scope middleware resolves and authorizes it, and handlers use the stable Project ID after that point.
...
**Event Stream Path**:
The namespace-local stream address that a Codemode Session reads from and appends to. In OS these paths are project-local because the Project ID is already the Stream Namespace; they must not start with `/projects/{projectId}`.
...
A Tool Provider whose Tool Function paths are mapped to calls on an underlying SDK rather than enumerated as separate OS-owned function definitions.
...
- When a **Codemode Example Stack**, custom Event Inputs, and a **Script** are combined, OS appends example Event Inputs, custom Event Inputs, then the script-execution-requested event.
...
- The **Session Capability Callable** is supplied to the codemode processor as a Runtime dependency; the shared processor does not construct OS-specific callables itself.
...
- An **oRPC Capability** should call OS oRPC handlers in-process with a server-side caller context when the implementation is available in the same Worker.
...
- OS should register codemode examples for each supported Tool Provider topology so users can run real Scripts against the examples.
...
console.log("OS oRPC procedures", procedures);
...
The AI case targets a minimal wrapper around the Workers AI binding: codemode should make `env.AI.run(model, options)` available without forcing OS to enumerate every model-specific argument shape.
...
> **Dev:** "Should codemode call OS oRPC handlers over HTTP?"
...
> **Domain expert:** "No. A **Codemode Session** is attached to an **Event Stream Path**, which may be newly chosen by OS or may already exist."
...
> **Domain expert:** "Browser oRPC may pass one to attach to an existing stream; otherwise OS generates an Event Stream Path for the Project."
