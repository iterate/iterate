...
test("live state chains client-side from the seed — mini-app + processor flavors", async () => {
  ...
  expect(chat).toMatchObject({ applied: 2, reseeds: 0 });
  expect(chat.store.rev()).toBe(chatSeedRev + 2);
  expect(chat.store.get()!.messages[1]).toMatchObject({ text: "again" });
  ...
  expect(chat).toMatchObject({ dropped: 1 });
  ...
  expect(chat.store.rev()).toBe(chatSeedRev + 2);
  expect(chat).toMatchObject({ applied: 2 });
  ...
  expect(proc.store.get()).toMatchObject({ marks: 0 });
  ...
  expect(proc).toMatchObject({ applied: 2, reseeds: 0 });
...
test("a dynamic-worker processor's live state combines reduced (ticks) + runtime (lastPokeMs); a client syncs both via ephemeral deltas", async () => {
  ...
  expect(store.get()).toMatchObject({ lastPokeMs: 0 });
  ...
  expect(store.get()).toMatchObject({ ticks: 1 });
