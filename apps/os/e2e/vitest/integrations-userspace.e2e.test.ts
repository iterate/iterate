...
test("a project mounts ocado into the collection; connections + secret confinement hold", async () => {
  // Vitest retries re-enter this callback. Give each diagnostic attempt fresh
  // durable identities and an egress policy tied to its own echo origin;
  // otherwise the first attempt's committed Secret turns a transient failure
  // into a deterministic "different egress policy" failure on the retry.
