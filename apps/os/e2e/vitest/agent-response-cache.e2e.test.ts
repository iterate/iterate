...
test("an identity-masked onboarding response becomes an AI Gateway cache hit", async () => {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    const candidate = await runOnboardingBirthTurn(`aig-cache-hit-${attempt}-${marker}`);
