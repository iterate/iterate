...
test("append accepts an offset assertion on a subscription-configured core event", async () => {
  ...
  // project-worker(2) + platform-search-index(3) + PostHog(4) subscriptions +
  // woken(5); the next append is 6. `offset` is the DO's optimistic-concurrency
  ...
  expect(configured!).toMatchObject({ offset: 6 });
});
