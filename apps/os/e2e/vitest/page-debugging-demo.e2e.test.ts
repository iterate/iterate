...
describe("page debugging demo", () => {
  ...
  test("turns a snippet pasted into another page into a screenshot-capable PageTools capability", async () => {
    ...
    try {
      ...
      // Sessions share one demo project and differ by capability path.
      expect(initialSession.projectId).toBe("prj_page_debugging_demo");
      expect(initialSession.path[0]).toBe("debug");
      ...
      // A regenerated session keeps the shared project but gets a fresh path.
      await demoPage.waitForFunction((previousPath) => {
        const output = JSON.parse(document.querySelector("#agentOutput")?.textContent ?? "{}");
        return Array.isArray(output.path) && output.path.join(".") !== previousPath;
      }, initialSession.path.join("."));
      await demoPage.locator("#runHere").click();
      await demoPage.waitForFunction(() =>
        document.querySelector("#targetStatus")?.textContent?.includes("Snippet connected"),
      );
  ...
  // Proven on the standalone minimal-itx-v4 preview; flaky in the apps/os
  // local-dev harness (playwright route-fulfilled cross-origin page + an iframe
  // back to the dev server + service-binding WS forward). Skipped for the POC.
  test.skip("drives a cross-origin host page whose CSP forbids external scripts and sockets", async () => {
    const browser = await chromium.launch({ headless: true });
    const demoPage = await browser.newPage();
    const targetPage = await browser.newPage();
