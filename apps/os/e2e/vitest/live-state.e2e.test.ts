...
test("itx.liveState pushes a snapshot then a minimal diff; the DO-backed counter is shared", async () => {
  ...
  using project = await itx.projects
    .get(`live-state-${RUN_SUFFIX}-${marker}`)
...
test("itx.liveDemo.ticker (stateless, no Durable Object) advances over time", async () => {
  ...
  using itx = session.authenticate({ type: "admin-secret", secret: adminSecret() });
  using project = await itx.projects
    .get(`live-ticker-${RUN_SUFFIX}-${marker}`)
    .create({ metadata: testProjectMetadata() });
  await project.__describe();
...
test("itx.liveState indexes stream activity as a peer slice", async () => {
  ...
  using itx = session.authenticate({ type: "admin-secret", secret: adminSecret() });
  using project = await itx.projects
    .get(`live-index-${RUN_SUFFIX}-${marker}`)
    .create({ metadata: testProjectMetadata() });
  await project.__describe();
...
test("agents.liveState subscribed before any agent exists serves the empty catalog, then pushes the first agent and its title", async () => {
  ...
  using itx = session.authenticate({ type: "admin-secret", secret: adminSecret() });
  using project = await itx.projects
    .get(`agents-live-${RUN_SUFFIX}-${marker}`)
    .create({ metadata: testProjectMetadata() });
...
test("stream.liveState pushes updates through the state socket without a DO-side subscription", async () => {
  ...
  using itx = session.authenticate({ type: "admin-secret", secret: adminSecret() });
  using project = await itx.projects
    .get(`stream-live-${RUN_SUFFIX}-${marker}`)
    .create({ metadata: testProjectMetadata() });
...
test("secret.liveState pushes description updates through the Live State Pager", async () => {
  ...
  using itx = session.authenticate({ type: "admin-secret", secret: adminSecret() });
  using project = await itx.projects
    .get(`secret-live-${RUN_SUFFIX}-${marker}`)
    .create({ metadata: testProjectMetadata() });
...
test("a user egress request wearing the liveState lane header is claimed by the lane, never egressed", async () => {
  ...
  using itx = session.authenticate({ type: "admin-secret", secret: adminSecret() });
  using project = await itx.projects
    .get(`lane-gate-${RUN_SUFFIX}-${marker}`)
    .create({ metadata: testProjectMetadata() });
  await project.__describe();
