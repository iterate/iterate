...
// The pet-shop rows share one gate: each opens with `await petshopAnswers()`, one probe per file, so a
// shop that is down fails every row on its first line with the cause instead of on a timeout.
const PETSHOP = petshopBaseUrl();
const PETSHOP_WS = PETSHOP.replace(/^http/, "ws");
let petshopProbe: Promise<void> | undefined;
...
test("connectToMcp: the pet shop's MCP server — initialize + tools/list at connect, a tool as a method, callTool, an isError tool call throws, held across calls", async () => {
  ...
  expect(listed).toMatchObject({ owner: shopper("mcp") });
...
test("connectToOpenApi: the shop's OpenAPI 3.1 document (bearer-protected, fetched over egress); operationIds become methods; path and body parameters ride to /api/v2", async () => {
  ...
  expect(listed).toMatchObject({ owner: shopper("openapi") });
...
deployedOnly(
  ...
  async () => {
    ...
    expect(await conn.listPets()).toMatchObject({ owner: shopper("capnweb-ws") });
...
test("connectToCapnweb, batch transport: one POST per chain to the shop's /capnweb, over egress, the bearer on the POST", async () => {
  ...
  expect(await conn.listPets()).toMatchObject({ owner: shopper("capnweb-batch") });
...
/** The pet-shop rows' gate: the deployed shop must answer before any of them runs. Probed once per
 *  file; every row that awaits it shares the answer. */
function petshopAnswers(): Promise<void> {
  petshopProbe ||= (async () => {
...
function shopper(who: string): string {
  return `${who}-${runId()}@example.com`;
}

/** A live bearer for `owner` (a legacy-login token: 120 s TTL, minted fresh for every test that needs one). */
async function bearerFor(owner: string) {
  return { authorization: `Bearer ${await petshopLegacyBearer(owner)}` };
}
