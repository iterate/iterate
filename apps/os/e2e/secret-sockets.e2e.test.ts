...
// and through the deployment's secret LENT to the project by its operator — the same one-token
...
import { adminCredentials, freshCtx, openItx, session } from "./support/client.ts";
...
deployedOnly.for([
  ...
  {
    row: "the deployment's secret lent to the project, the bearer on the upgrade (/gateway-header)",
    secret: "lent",
    path: "/gateway-header",
    headers: { authorization: `Bearer ${FRAME_TOKEN}` },
    identify: null,
  ...
  {
    row: "the deployment's secret lent to the project, the token in the IDENTIFY frame (/gateway, x-itx-secret-frames)",
    secret: "lent",
    path: "/gateway",
    headers: { "x-itx-secret-frames": FRAME_TOKEN },
    identify: IDENTIFY,
  ...
  const { itx, cleanup } =
    row.secret === "own" ? await ownSecret(shop, token) : await lentSecret(shop, token);
  try {
...
async function ownSecret(shop: string, token: string) {
  ...
  return { itx, cleanup: async () => {} };
}

/** The deployment's own secret holding `token`, lent by the operator to a project it creates as
 *  `/secrets/shop`: the project's itx, and the cleanup that deletes the secret (its lend with it). */
async function lentSecret(shop: string, token: string) {
  ...
  const itx = await operator.projects.create({ project: freshDnsSafeProjectSlug("ws-lend") });
  const { projectId } = await itx.whoami();
  const name = `/secrets/${freshDnsSafeProjectSlug("ws-lent")}`;
  await global.secrets.set(name, token, { urls: [shop] });
  await global.secrets.lend(name, { to: projectId, as: "/secrets/shop" });
  return { itx, cleanup: async () => void (await global.secrets.delete(name)) };
}
