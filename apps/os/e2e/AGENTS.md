...
- `vitest.config.ts` owns run-level config, artifact roots, and console capture. `pnpm e2e` runs
  `e2e/vitest/**/*.test.ts` plus the engine suites in `e2e/engine/**/*.e2e.test.ts` through it.
...
- `test-support/create-test-project.ts` creates a disposable OS project via itx
  ...
  no-op until the engine grows `projects.remove`). `handle.itx()` returns a fresh admin itx
...
- Live deployment tests: `pnpm e2e` (engine suites in `e2e/engine/`, agents, admin-project,
  preview smoke).
- Egress + secret substitution coverage lives in the engine suite
  (`e2e/engine/itx.e2e.test.ts`); the cross-runtime example matrix is `pnpm e2e:itx`.
