...
- No wall-clock budgets in e2e rows: beside 15 other files a latency measures the suite's contention. A latency or throughput budget is a `perf/<topic>.perf.test.ts` row (`pnpm perf`), which runs alone; its loads live in `support/` so the e2e row proves the same load correct. A perf row records each metric with `recordLatency` (`perf/record.ts`) under a name and calibrated budget in `perf/latency.ts`: the latency guard (`os-latency.yml`) runs the lane every 3 hours and on main pushes and pages #error-pulse when one crosses.
