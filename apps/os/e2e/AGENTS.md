...
- A PR's run never calls a real model: shadow `itx.ai` with a fake provider (`support.provide("itx.ai", fake)`), and mark a row that must pay for an inference `realModelOnly` and title it `REAL:` ([real-model rows](../../../docs/testing.md#real-model-rows)).
