...
This is the implementation brief for the OS Project ingress redesign. It
...
- [x] Main OS Worker performs exact-host ingress lookup before TanStack Start
      fallback.
...
- [x] `ProjectDurableObject` is exported from the main OS worker and bound as
      `PROJECT`.
...
- [ ] Separate skinny public OS ingress worker script.
...
Make the Project Durable Object the lifecycle authority for OS Projects, and
...
- the public OS Worker classifies requests by hostname
...
OS project/MCP/codemode routes, D1 rows, Durable Object class names, and
...
- **Default Project Ingress Host**: the host OS uses for ordinary generated
  public URLs.
...
- **Project MCP Server Connection**: Durable Object named
  ...
  connection into OS's project-scoped MCP server.
...
Every persisted OS record should be explicitly scoped by one of:
...
The main OS worker should export named WorkerEntrypoint classes from
`src/entrypoints/`:

- `ProjectIngressEntrypoint`
- `ProjectMcpServerEntrypoint`

For now, the `Project` Durable Object should also be exported from the main OS
...
fallback rows for `os.iterate.com`, workers.dev, or the TanStack Start app.
Fallback to the OS app is caller-provided code.
...
The local `apps/os/src/ingress/` matcher is only a temporary implementation
gap, not the target architecture. See `shared-http-route-matcher.md`.

## Fetch Callable Shape

V1 needs loopback entrypoint fetch callables and the OS app fallback.
...
The property is `callable` directly on the rule. Do not wrap it in an OS
destination object.
...
This should route to the OS dashboard for that Project. It is not the global
OS dashboard host; it is a project-local route destination that forwards to
the OS app with enough Project context.
...
The name should describe one external MCP client connection into OS's
project-scoped MCP server. This must remain distinct from future
`OutboundMcpClientConnection`, where OS is the client connecting to an
external MCP server.
...
- `tasks/os-project-do-projection-reconciliation.md`
- `tasks/os-project-route-authorization.md`

The OS Worker entrypoint should mount durable-object-utils infrastructure
...
- [ ] separate public OS ingress worker script
