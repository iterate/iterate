...
- The Stream Durable Object exposes `.stream()` as a `ReadableStream` of encoded
  NDJSON. OS oRPC then wraps that in an async generator and decodes it again,
...
- `apps/os/src/durable-objects/codemode-session.ts` already has a callable
...
For user-facing live event APIs, the OS oRPC handler should own the async
iterator/SSE edge:
...
- Refactor `.stream()` callers away from `ReadableStream` internally:
  - OS `StreamCapability.stream()` should become an async-event capability or
...
- No internal OS or shared Stream Runtime caller has to decode NDJSON from a
...
- Workerd/RPC or OS test for oRPC disconnect cleanup.
...
- Which public compatibility path, if any, should remain for
  `StreamDurableObject.stream()` while OS call sites migrate?
