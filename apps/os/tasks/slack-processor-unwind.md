...
Unwind OS Slack handling so Slack-specific behavior lives in the Slack
...
- Thread `appendBatch` through shared stream processor runner wrappers and local
  test stream APIs.
- Move OS Slack integration stream path from `/integrations/slack/webhooks` to
  `/integrations/slack`.
...
- OS tests:
  ...
  - existing OS unit tests.
  - workerd test if the Durable Object wiring needs runtime coverage.
- Local Slack smoke:
  - use Doppler-provided OS Slack bot token without printing it.
...
- Implemented: `ProcessorStreamApi.appendBatch` is required and threaded through
  the shared runner wrappers, OS/app stream APIs, and test helpers.
...
- Implemented: OS has a separate `SlackAgentDurableObject`; routed stream
...
--runInBand` passed; `pnpm --dir apps/os typecheck` passed.
...
- Local Kimi proof: against `https://os.iterate-dev-jonas.com`, the focused e2e
  ...
  passed. It sends a chat message to a new unconfigured OS agent and verifies a
