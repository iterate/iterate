...
ported, replaced, or explicitly left behind in `apps/os`.

## Context

`apps/os` already overlaps with `apps/agents` around codemode, MCP, oRPC, and
events, but it is not a direct port. OS is organized around Clerk
...
- Codemode exists in both apps, but agents uses events-driven stream processor
  runners while OS uses `CodemodeSession`.
- MCP/OpenAPI tool calling exists in both apps, but agents uses `MCPClient` and
  `OpenApiToolClient` Durable Objects while OS uses `McpClientBridge` and
  `OpenApiBridge`.
- OS project stream code talks to the Events stream Durable Object through the
  `STREAM` namespace binding instead of an Events URL client.
- OS uses `McpAgent.serve("/mcp")` with Clerk/project auth; agents uses agent
  request routing and stream callbacks.

## Desired Outcome

- Decide whether OS should support the old stream processor runner model, or
  whether `CodemodeSession` fully replaces it.
- Decide whether Slack should become a first-class OS provider, a project
  preset, or stay agents-only.
- Decide whether OS needs a stream console/TUI, or whether the project
  `run-code` page is the only supported interaction surface.
- Decide whether any agents e2e infrastructure should be reused for OS
...
- `apps/os/src/durable-objects/codemode-session.ts`
- `apps/os/src/durable-objects/iterate-mcp-server.ts`
- `apps/os/src/rpc-targets`
