...
Capture the requirements for OS's Project Egress and Secret model before
...
- Add `ProjectEgressEntrypoint` under `apps/os/src/entrypoints/`.
...
- Secret Injection should happen only inside a trusted OS boundary on the
  egress path.
...
- Durable Objects in this design should be rootable from the main OS Worker
...
- If a requested Secret or one of its dependencies does not exist yet, OS
...
- Environment variables are out of scope for this design pass. The scope is
  how OS resolves a sentinel string such as `getSecret(...)` into a concrete
...
- Is it acceptable for OS's secrets system to call non-sensitive supporting
...
- Mount `routeDurableObjectRequest()` in the main OS Worker entrypoint for the
