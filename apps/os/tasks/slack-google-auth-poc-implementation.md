...
Take a first crack at an end-to-end working OS implementation for Slack and
Google auth, keeping it clean and simple as a POC.

Explicit scope from the working session:

- OAuth client settings are OS Runtime Config, loaded from Doppler.
...
For this first POC, Secrets and Connections are stored in OS D1 tables rather
...
OS1 Google was user-scoped, but OS POC Google is project-scoped. There are no
...
- Doppler `os/dev_jonas` now has the corresponding
...
- `pnpm --dir apps/os test -- src/app.test.ts`
- `git diff --check`

`pnpm --dir apps/os sqlfu:check` could not run because
...
ready for testing against the configured Slack/Google apps once a dev OS
instance is running.
