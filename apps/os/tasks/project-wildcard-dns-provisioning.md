...
When an OS Project is created, provision a Cloudflare wildcard CNAME for the
...
hard-coding `iterate.app`, so preview/dev bases follow the same path when DNS
management is enabled for them.

## Current Facts

- `apps/os/alchemy.run.ts` registers Worker route hostnames for each configured
...
- `apps/os/alchemy.run.ts` has precedent for Cloudflare DNS management in this repo:
...
written, OS kicks off DNS provisioning for the first configured project
...
3. Create `*.<project-slug>.<base>` as a CNAME with:
   ...
   - `comment`: `Managed by apps/os for project <project-id>`.
...
apex, for example `iterate.app` or `iterate-preview-3.app`. Do the simplest
thing that works for current OS domains.
...
This has been configured on the `os` root configs:
...
2. Deploy or wait for the OS preview deployment for that PR.
3. Ensure the preview Doppler config has
   `APP_CONFIG_CLOUDFLARE__API_TOKEN` configured.
4. Create a throwaway Project through the preview OS Project creation path.
...
- Creating a Project with slug `demo` and base `iterate.app` upserts:
  - `*.demo.iterate.app`
- The new records copy the CNAME target from `*.iterate.app`.
...
- Source wildcard target drift is not reconciled in the first slice. If
  `*.iterate.app` changes target, existing `*.<project>.iterate.app` records
