...
- Browser oRPC session creation accepts an optional Event Stream Path. If absent,
  OS generates one for the Project.
...
- Outbound MCP client connections are Tool Providers. They are Durable Object
  instances representing OS's connection to an external MCP server.
...
- `apps/os/src/rpc-targets/openapi-bridge.ts`
  - Stateless Provider Bridge for OpenAPI specs.
- `apps/os/src/rpc-targets/mcp-client-bridge.ts`
  - Stateful Provider Bridge for remote MCP servers.
- `apps/os/src/rpc-targets/*-core.test.ts`
...
- Should OS standardize on **Self-Callable Provider Descriptors** for
...
- Should event schemas live in `apps/os-contract`,
...
Goal: when OS is acting as an MCP server, codemode should attach to the
...
- Should temp prototypes under `apps/os/tmp/` stay as reference material or be
...
4. Should canonical codemode event schemas live in OS, shared streams, or a
   new package?
...
- [ ] Update `apps/os/CONTEXT.md` when terminology or relationships change.
