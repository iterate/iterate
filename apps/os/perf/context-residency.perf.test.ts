// perf/context-residency.perf.test.ts — CLOUDFLARE'S SIDE OF RESIDENCY, timed alone. The e2e rows
// (e2e/context-residency.e2e.test.ts) assert what the platform code decides: the wakes, the resets a
// birth names on its wake record, and that a careless facet is no longer running once its quiet
// minute is up. What they cannot assert is what Cloudflare decides: that a loaded facet the context
// no longer holds keeps running after its context is evicted (the reason the sweep exists), and that
// a context under outside traffic stays resident. Those are wall-clock facts about the platform, and
// under the e2e run — 16 files, their rows concurrent — they sampled it: 3 of 124 e2e jobs saw the
// platform stop a facet 0 s and 20 s after its call, stop a claimed facet mid-attempt, and evict a
// context mid-traffic while the control plane stalled 12.8 s (#2899, #2921, #2939), each green on its
// retry. The soak runs this file after every e2e run (scripts/e2e-soak.ts), where a miss is a number
// to read, not a red PR.
//
// Unlike the latency budgets beside it, these rows run CONCURRENTLY with each other: they measure how
// long an actor lives, not how fast a call is, and in order they would add eleven minutes to every
// soak run. Each prints what it measured.
...
test.concurrent("a careless loaded facet outlives its evicted context until the sweep stops it a quiet minute after the last call", async () => {
...
test.concurrent("a careless loaded facet calling its own context every 5 s chatters past the eviction a birth would have needed, until the sweep resets it in place", async () => {
...
test.concurrent("a careless facet whose claim ends beats on through the sweep that ran while the claim held it, and stops a quiet minute after the release", async () => {
...
test.concurrent("a facet's claimed background work finishes on the instance that started it across its context's incarnations", async () => {
