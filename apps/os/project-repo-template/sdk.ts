// The platform's SDK surface (the future `@iterate-com/sdk` package),
// snapshotted into this repo when it was seeded: a generated types block plus
// a small runtime section at the bottom (the `IterateProjectWorker` base
// class). Import from it; treat it as read-only — the platform's own copy is
// the design of record, kept in sync there by lint codegen.
...
// ---------------------------------------------------------------------------
// Runtime SDK — seeded alongside the type snapshot above (the future
// @iterate-com/sdk package). Unlike the generated block, this section is
// plain code the project worker imports and extends.
// ---------------------------------------------------------------------------

/** Bindings the platform supplies to every project worker. `ItxBinding`
 * (above) documents the two channels: `get()` for capability method calls,
