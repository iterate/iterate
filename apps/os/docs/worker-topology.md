...
| itx lanes       | capnweb `/api/itx` (+ admin-cookie), `/__itx_e2e` fixtures, project ingress |
...
wrangler.jsonc, no namespace IDs, no cross-script anything): Agent, Itx,
Project, Repo, Secret, Stream, StatefulWorker, and the container-backed
CloudflareSandbox (Dockerfile.sandbox, built by `wrangler deploy`).
...
- [`wrangler.jsonc`](../wrangler.jsonc) — generated from envs.ts by
  `pnpm gen:wrangler` (checked in; CI fails when stale). Top level is local
  dev; each env gets a flattened block selected at build time via
  `CLOUDFLARE_ENV`.
