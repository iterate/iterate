...
| `<n>-stream`                      | `stream.ts`                        | `StreamDurableObject` (journals, event streams)                                                                                                                                                       | subscriber namespaces: `AGENT`, `PROJECT`, `REPO`, `SLACK_AGENT`, `INTEGRATION`, `INTEGRATION_INGRESS`, `SECRET`, `ITX_CONTEXT` | none                                                                      |
...
| `<n>-agent`                       | `agent.ts`                         | `AgentDurableObject` (chat/LLM/agent-host processors)                                                                                                                                                 | loopback union                                                                                                                  | `nodejs_compat` (openai), `global_fetch_strictly_public`                  |
...
| `<n>-integration`                 | `integration.ts`                   | `IntegrationDurableObject` (per-(project, integration, account) lifecycle + account stream processors, incl. the Slack thread router); registry SDK egress dials `EgressPipe` via `ctx.exports`       | loopback union (registry SDK egress) + `SLACK_AGENT`                                                                            | `global_fetch_strictly_public`                                            |
| `<n>-integration-ingress`         | `integration-ingress.ts`           | `IntegrationIngressDurableObject` (per-integration global capture + routing table)                                                                                                                    | own ns, `STREAM`, `DO_CATALOG`, `GLOBAL_STREAM_NAMESPACE`                                                                       | none                                                                      |
| `<n>-secret`                      | `secret.ts`                        | `SecretDurableObject` (journal-backed credential envelopes; egress delegates in)                                                                                                                      | own ns, `STREAM`, `DO_CATALOG`, `SECRETS_ENCRYPTION_KEY`                                                                        | `global_fetch_strictly_public`                                            |
| `<n>-discord-gateway`             | `discord-gateway.ts`               | `DiscordGatewayDurableObject` (holds the gateway WebSocket per connected account)                                                                                                                     | own ns, `DO_CATALOG`, discord bot token                                                                                         | none                                                                      |
...
agent, itx, mcp, app, integration) therefore re-exports
...
- **Observability**: every worker ships full-sample logs/traces
  ...
  names are now per-domain: Slack thread routing logs live under
  `os-prd-integration`, agent turns under `os-prd-agent`, etc.
