...
| DO name → identity             | `{projectId}.iterate{path}`                             | `DurableObjectNameCodec`; the path IS the sandbox address, always under `/sandboxes/` (`/sandboxes/cloudflare/agents/...` for an agent's sandbox, `/sandboxes/cloudflare/...` for standalone) |
