...
`src/types.ts` is missing fields the real fold carries — `llmProviderConfigured`
...
| ------------------------------ | ------------------------------------------------------------ | -----: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Orphaned HTTP-proxy harness    | `packages/mock-http-proxy`                                   |  6,419 | Zero importers; last consumer died with itx-v4 (`b1fb37f95`). Still runs (and flakes — fixed in #1645!) in every unit CI lane; falsely listed as a deploy trigger in `scripts/preview/preview.ts:820`. Delete the package, its CI time, and the trigger line.                                                                                                                                                               |
...
| Duplicated provider completion | each LLM provider                                            |    ~80 | Success/failure completed-append written twice per provider; one `#complete()` helper each.                                                                                                                                                                                                                                                                                                                                 |
...
5. One routing pass: do `ingress.ts:196`'s own TODO; delete `apiWorkerRequest` and
   ...
   pair. (§3.3)
6. Delete `packages/mock-http-proxy` (6.4k) and the ~6–7k dead `packages/ui` files
