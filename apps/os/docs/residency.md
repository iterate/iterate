...
| --- | -------------------------------------- | --------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------ |
| 1   | `withItx` records every pipelined step | a facet's hold on its context's values                    | `packages/iterate/src/next/sdk/index.ts`, `record-pipelined-steps.ts`  | #2846, removed #2855, restored #2863 |
| 2   | `itxAnswerDetachedFromSession`         | a caller's hold on what the context answered              | `packages/iterate/src/next/expression.ts`, called by the DO's `invoke` | #2855                                |
