...
- **D43** (2026-08-18, polish-3): round-3 polish applied — ONE `walkSteps` behind evaluate/apply; `substituteArgList` finds nested `?n` holes (the double-splice bug); ONE `hashSource` (`core/hash.ts`) + ONE `jsonEqual` (`core/events.ts`); explicit `{"seeds": []}` = DENY-ALL (defaults only when APP_CONFIG is entirely absent); stateful-runner name comments corrected to `{projectId}::{path}::{className}:{sourceHash}`; micro dead code deleted. 70 unit tests.
- **D44** (2026-08-18, userfacet-1): USERSPACE facet processors — `enableProcessor(slug, { source, className })` loads a duck-typed class (`configure`/`deliver`/`snapshot`) through the Worker Loader and hosts it as a workerd facet on the Stream DO, alongside built-ins; loader wiring mirrors the stateful runner (deploy-versioned cacheKey, injected `itx.js`, `env.ITX` = parent by name, abort-on-source-change per slug). Proven live: user-tally + built-in tally folding side by side.
- **D45** (2026-08-18, ictx-facet-1): the iterate context IS a facet — `IterateContextStreamProcessor` runs inside the built-in `ProcessorFacet`; the Stream DO is log + sockets + doors only and DELEGATES dispatch (invoke/provide/revoke + the `x-itx-cap` lane via NATIVE `facet.fetch`, 101s tunnel). Sockets stay parent-side behind a five-method stub facade (`stubInvoke`/`stubFanOut`/`stubList`/`stubConnections`/`stubClose`); ONE `buildRoots` serves either host; `resolveCurrent` recursion stays facet-LOCAL. No platform wall: all three live proofs pass identically, 70 unit tests untouched.
- **D46** (2026-08-18, ictx-facet-1): hibernation-with-facets e2e — the Stream DO EVICTS + reconstructs while facet processors (incl. the iterate-context host) are enabled and capnweb clients stay connected (incarnation grew in 4 independent runs; facets + routing table rebuild from durable identity; a parked stub survived each eviction). Platform finding: workers.dev couples actor eviction with stateless-edge-isolate recycling — long-idle relays die (1006, ~5-8min) taking their parked stubs (by design; clients reconnect by connectionKey), and ANY client→edge traffic keeps the actor warm (0/4 evictions with keepalive vs 5/8 without) — so no single run landed every assertion at once; each is proven across runs (BUILD-LOG increment 29).
- **D47** (2026-08-18, kenton-1): the Kenton-bar review — one honest way per thing: ONE `pathProxy` (was three verbatim accumulating proxies), ONE `confinedWorker` (was three verbatim loader wirings; the confinement invariant stated once), ONE `ExpressionSchema`/`disposeStub`; two real bugs (must-use walker disagreed with match/substitute about `$`-escapes; the user-tally demo folded the delivered batch instead of its cursor — a dropped drive left a permanent gap); `registry.deliver()` nullary (the batch was already only a wake-up); dead exports deleted (`isFetchTerminal`, `parentPath`, `registry.names`, the `itx` getter); nameless-DO access now fails loudly. Net −49 lines, 70 tests green, all three live proofs ALL PASS first run. Findings ledger: `packages/v3/project-worker/REVIEW-KENTON.md` (BUILD-LOG increment 30).
