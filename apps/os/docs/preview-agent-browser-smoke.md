...
```bash
doppler run --project os --config preview_2 -- pnpm exec tsx -e '
...
async function main() {
  ...
  const user = await clerk.users.createUser({
    ...
    publicMetadata: { createdBy: "os-agent-browser-smoke" },
  });
  ...
  const createProjectResponse = await fetch(
    "https://os.iterate-preview-2.com/api/projects",
    {
      body: JSON.stringify({
        metadata: { seededBy: "os-agent-browser-smoke" },
        slug: projectSlug,
      }),
      headers: {
  ...
  writeFileSync(
    "/tmp/os-agent-browser-clerk-smoke.json",
    JSON.stringify(
      {
        userId: user.id,
        organizationId: org.id,
...
```bash
node > /tmp/os-agent-browser-permission.sql <<'NODE'
const fs = require("node:fs");
const data = JSON.parse(
  fs.readFileSync("/tmp/os-agent-browser-clerk-smoke.json", "utf8"),
);
...
doppler run --project os --config preview_2 -- \
  pnpm exec wrangler d1 execute os-preview-2-db \
  --remote \
  --file /tmp/os-agent-browser-permission.sql
```
...
const url = new URL("https://os.iterate-preview-2.com/sign-in");
...
```text
https://os.iterate-preview-2.com/orgs/<organizationSlug>/projects/<projectSlug>/streams/agent-browser-ui-smoke
```
...
async function main() {
  const data = JSON.parse(
    readFileSync("/tmp/os-agent-browser-clerk-smoke.json", "utf8"),
  );
  const clerk = createClerkClient({
    secretKey: process.env.APP_CONFIG_CLERK__SECRET_KEY,
  });
  await clerk.users.deleteUser(data.userId);
  rmSync("/tmp/os-agent-browser-clerk-smoke.json", { force: true });
  rmSync("/tmp/os-agent-browser-permission.sql", { force: true });
