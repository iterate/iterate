...
doppler run --project os --config preview_2 -- pnpm --dir apps/os cli itx run \
  --context <prj_id> \
  -e 'return await itx.streams.get("/agents/local-debug").append({ type: "events.iterate.com/agents/context-added", payload: { content: "Say hello via itx.chat.sendMessage.", actor: { type: "user", origin: "web" }, llmRequestPolicy: { behaviour: "after-current-request" } } })'
```
