...
export const createPgmqQueuer = (queueOptions: { queueName: string }): Queuer<DBLike> => {
  ...
  const processQueue: Queuer<DBLike>["processQueue"] = async (db) => {
    ...
    const rows = Array.from(jobQueueMessages);
    if (rows.length) logger.info(`processing ${rows.length} messages`);
    const results: Array<string | void> = [];
    for (const _job of rows) {
      ...
      try {
        logger.info(`START consumer=${consumer.name} msg_id=${job.msg_id}`);
        ...
        `);
        logger.info(`DONE consumer=${consumer.name} msg_id=${job.msg_id} Result: ${result}`);
      } catch (e) {
        const retry = consumer.retry(job);
        const retryMessage = Object.entries(retry)
          .map(([k, v]) => `${k}: ${v}`)
          .join(". ");
        logger.error(`FAILED consumer=${consumer.name} msg_id=${job.msg_id} ${retryMessage}`, e);
        ...
        if (!retry.retry) {
          ...
          `);
          const archived = Array.from(archiveResult)[0];
    ...
    return `${rows.length} messages processed:\n\n${results.join("\n")}`.replace(/:\n\n$/, "");
  };
  ...
  const enqueue: Queuer<DBLike>["enqueue"] = async (db, params) => {
    ...
    const insertResult = await db.execute(sql`
      ...
      returning id
    `);
    const _eventInsertion = Array.from(insertResult)[0];
  ...
  return {
    ...
    peekQueue: async (db, options = {}) => {
      ...
      `);
      return ConsumerJobQueueMessage.array().parse(Array.from(rows));
    },
      ...
      return next({
        ctx: {
          sendTrpc: async <T extends {}>(db: DBConnection, output: T) => {
            ...
            logger.info(`sendTrpc consumerPlugin=true path=${path}`);
...
export const createConsumerClient = <EventTypes extends Record<string, {}>, DBConnection>(
  ...
  const registerConsumer = <P extends EventName>(options: {
    ...
    consumersForEvent[`consumerName:${options.name}`] = {
      ...
      handler: async (params) => {
        logger.info(`consumer=${options.name} eventId=${params.eventId}`);
  ...
  const send = async <Name extends EventName>(
    ...
    if (addResult.matchedConsumers > 0) {
      waitUntil(
        (async () => {
          logger.info(`queuedEventId=${addResult.eventId}`);
  ...
  >(parent: Transactable<D>, eventName: Name, callback: (db: D) => Promise<T>): Promise<T> {
    const { addResult, result } = await parent.transaction(async (tx) => {
      logger.info(`transactionForEvent=${eventName}`);
