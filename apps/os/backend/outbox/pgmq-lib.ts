...
export const InsertedEvent = z.object({
...
export const createPgmqQueuer = (queueOptions: { queueName: string }): Queuer<DBLike> => {
  ...
  const processQueue: Queuer<DBLike>["processQueue"] = async (db) => {
    const jobQueueMessages = await db.execute<ConsumerJobQueueMessage>(
      sql`
    ...
    for (const _job of jobQueueMessages) {
      ...
      if (!parsed.success) {
        const err = prettifyError(parsed.error);
      ...
      try {
        logger.info(`START`, { msgId: job.msg_id, consumer: consumer.name });
  ...
  const enqueue: Queuer<DBLike>["enqueue"] = async (db, params) => {
    ...
    const [eventInsertionRow] = await db.execute(sql`
      insert into outbox_event (name, payload)
      values (${params.name}, ${JSON.stringify(params.payload)})
      returning id
    `);
    const eventInsertion = InsertedEvent.parse(eventInsertionRow);
      ...
      await db.execute(sql`
        select from pgmq.send(
          ...
          msg         => ${JSON.stringify({
            ...
            event_id: Number(eventInsertion.id),
    ...
    return { eventId: String(eventInsertion.id), matchedConsumers: filteredConsumers.length };
  };
