-- add pgmq extension as raw sql
-- needed because planetscale doesn't have it as an available extension at time of writing ("requested", not "coming soon" on https://ps-extensions.io on 2025-11-16)
-- source: https://github.com/pgmq/pgmq/blob/main/pgmq-extension/sql/pgmq.sql
-- permalink: https://github.com/pgmq/pgmq/blob/f0b4acbc46a938771c07a42f79c0a34fe59e5704/pgmq-extension/sql/pgmq.sql

-- installation guide: https://github.com/pgmq/pgmq/blob/main/INSTALLATION.md
-- permalink: https://github.com/pgmq/pgmq/blob/f0b4acbc46a938771c07a42f79c0a34fe59e5704/INSTALLATION.md

-- to undo i guess you should just `drop schema pgmql cascade` or something?
-- but upgrading to a new version of pgmq would be tricky. maybe rename the schema, run the updated pgmq migrations, then transfer data over, then drop the renamed schema. good luck if you are reading this and planning on crossing that bridge. you should also consider what happens to writes that need to happen at that point. at that point you should probably update the outbox implementation to *just* write to the outbox_event table in the same transaction as whatever the application write is, and separate out the writes to pgmq's internal tables, so the queue-writes can be disabled for a short time but still happen later.
