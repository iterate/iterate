...
app.use("*", async (c, next) => {
  ...
  try {
    ...
    // blip). better-auth's logger config (auth.ts) logs DB errors before
    // rethrowing, but other failures (runtime, plugin) may skip that path.
    // Log here as a safety net — duplicates are acceptable.
