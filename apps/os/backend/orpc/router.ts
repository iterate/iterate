...
async function refreshStaleConnectorTokens(
  ...
  // Dedupe by (secretKey, userId) — same secret may map to multiple env var names
  const uniqueSecrets = [
    ...new Map(
      toRefresh.map((v) => [
        `${v.secret!.secretKey}:${v.secret!.userId ?? ""}`,
        { key: v.secret!.secretKey, userId: v.secret!.userId },
      ]),
    ).values(),
  ];

  // Query secret IDs (attemptSecretRefresh needs the ID)
  const secrets = await db.query.secret.findMany({
    where: and(
      ...
      or(
        ...uniqueSecrets.map((q) =>
          q.userId
            ? and(eq(schema.secret.key, q.key), eq(schema.secret.userId, q.userId))
            : and(eq(schema.secret.key, q.key), isNull(schema.secret.userId)),
        ),
      ),
    ),
