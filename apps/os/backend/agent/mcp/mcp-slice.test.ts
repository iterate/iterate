...
describe("mcp-slice", () => {
  ...
  describe("slice behavior", () => {
    ...
    describe("MCP servers from multiple sources", () => {
      describe("context rules and MCP events integration", () => {
        const deps10 = createMockDeps();
        const test10 = createAgentCoreTest([mcpSlice] as const, {
          extraDeps: deps10,
        });

        test10("should replace duplicate MCP server via MCP:ADD_MCP_SERVERS", async ({ h }) => {
          await h.initializeAgent();

          // Mock context items that include MCP servers
          const contextItems: ContextItem[] = [
            {
              id: "github-context",
              prompt: {
                tag: "system",
                content: "You have access to GitHub MCP server",
              } as PromptFragment,
              mcpServers: [
                {
                  serverUrl: "https://github.com/mcp",
            ...
            },
            {
              ...
              prompt: {
                tag: "system",
                content: "You have access to Slack MCP server",
              } as PromptFragment,
              mcpServers: [
                {
                  serverUrl: "https://slack.com/mcp",
          ...
          // Override the collectContextItems dependency to return our test context items
          ...
          // Add an event to trigger context collection (this simulates the core reducer running)
          await h.agentCore.addEvent({
          ...
          // Verify that MCP servers from context rules are in the core state
          const coreStateAfterContext = h.agentCore.state;
          expect(coreStateAfterContext.mcpServers).toHaveLength(2);
          expect(coreStateAfterContext.mcpServers.map((s) => s.serverUrl).sort()).toEqual([
            "https://github.com/mcp",
            "https://slack.com/mcp",
          ]);

          // Now add MCP servers via the dedicated MCP event, including a duplicate
          const addResult = await h.agentCore.addEvent({
            type: "MCP:ADD_MCP_SERVERS",
            data: {
              servers: [
                {
                  serverUrl: "https://discord.com/mcp",
                  mode: "company" as const,
                  integrationSlug: "discord",
                  requiresAuth: true,
                  triggerLLMRequest: true,
                },
                // This is a duplicate of the context-provided server and should be replaced
                {
                  serverUrl: "https://github.com/mcp",
                  mode: "company" as const,
                  integrationSlug: "github-alt",
                  requiresAuth: false,
                  triggerLLMRequest: false,
                },
              ],
          ...
          // Verify state now contains the new/replaced servers
          const sliceState = h.agentCore.state as any;
          expect(sliceState.mcpServers).toHaveLength(3); // slack (original) + discord (new) + github (replaced)
          const sliceServerUrls = sliceState.mcpServers.map((s: any) => s.serverUrl).sort();
          expect(sliceServerUrls).toEqual([
            "https://discord.com/mcp",
            "https://github.com/mcp",
            "https://slack.com/mcp",
          ]);

          // Verify the github server was replaced with new configuration
          const githubServer = sliceState.mcpServers.find(
            (s: any) => s.serverUrl === "https://github.com/mcp",
          );
          expect(githubServer.integrationSlug).toBe("github-alt");
          expect(githubServer.requiresAuth).toBe(false);
        });

        test10("should handle multiple MCP:ADD_MCP_SERVERS events", async ({ h }) => {
          await h.initializeAgent();

          // Add first MCP server via event
          await h.agentCore.addEvent({
            type: "MCP:ADD_MCP_SERVERS",
            data: {
              servers: [
          ...
          // Verify slice has the first server
          let sliceState = h.agentCore.state as any;
          expect(sliceState.mcpServers).toHaveLength(1);
          expect(sliceState.mcpServers[0].serverUrl).toBe("https://api.example.com/mcp");

          // Add second MCP server via another event
          await h.agentCore.addEvent({
            type: "MCP:ADD_MCP_SERVERS",
            data: {
              servers: [
          ...
          // Slice state should have both servers
          sliceState = h.agentCore.state as any;
          expect(sliceState.mcpServers).toHaveLength(2);
          const serverUrls = sliceState.mcpServers.map((s: any) => s.serverUrl).sort();
          expect(serverUrls).toEqual(["https://api.example.com/mcp", "https://discord.com/mcp"]);
        });

        test10("should handle empty MCP servers gracefully", async ({ h }) => {
          await h.initializeAgent();
          ...
          const state = h.agentCore.state as any;
          expect(state.mcpServers).toEqual([]); // Core state
          expect(state.mcpServers).toEqual([]); // Slice state (same property name)
        });
      });
    });
