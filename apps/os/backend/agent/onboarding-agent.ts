  ...
  implements ToolsInterface
{
  ...
  private mergeResults(results: Record<string, any>): void {
    ...
    if (parseResult.success) {
      ...
      companyContext = parseResult.data;
    }
  ...
  private getNextStepGuidance(
    ...
    if (!checklist.remoteMCPConnected) {
      return dedent`
        ...
        NEXT ACTION: Send a playful message suggesting they connect a remote MCP server. Specifically suggest https://api.findadomain.dev/mcp and tease them that their company domain name probably isn't great. Offer to find alternative .com names for them. Make it cocky and fun. After you send that message, call updateOnboardingProgress({step: "remoteMCPConnected", completed: true}).
      `;
    }
      ...
      **IMPORTANT**: Still help with any normal requests!
      ...
      ${
        !checklist.firstToolConnected
          ? dedent`
            ...
            **MESSAGE 2**: Suggest connecting ONE tool (Linear, Notion, or Gmail) - but show all three of course and allow the user to pick. Say it's the best way to get started. Be playful about it but not over-the-top. Keep it brief - one sentence is fine.

            **After they connect a tool**: Automatically run a demo to show off what you can do (DON'T ASK, DON'T MENTION YOU WILL DO A DEMO, JUST DO IT):
            ...
            **If Notion connected:**
            1. Use the Notion MCP tools to create a page titled "How to Use iterate" with a comprehensive welcome guide. Write the actual documentation yourself - be thorough and helpful. Include:
            ...
            2. Share the link to the page with the user in a confident, show-offy way. Do NOT ask if they want more content or additional sections - you're the expert, you wrote comprehensive docs already.
            3. **CRITICAL**: ONLY AFTER you've sent that message with the link to the user, call updateOnboardingProgress({step: "firstToolConnected", completed: true})
            4. The tool will respond with your next action - follow it immediately (It will be about showing the user they can use any MCP and using the domain MCP provided below)!
      ...
      ${
        checklist.firstToolConnected && !checklist.remoteMCPConnected
          ? dedent`
            **ACTION REQUIRED**: Send a playful message right now telling the user they can actually use any remote MCP server:
            - Specifically after you tell that to the user, use your tool to connect to MCP server to connect to https://api.findadomain.dev/mcp specifically
            - Tease them that their company domain name (${this.organization.name}) probably isn't great
            - Tell them you will find alternative .com names for them and then connect to the MCP server and show them the results. Use the relevant tools then provided to you to share the alternatives and move on.
            - Be cocky and fun about it - make it sound like a flex

            **CRITICAL**: After you send the URLs from your tool call within the MCP server, you MUST call updateOnboardingProgress({step: "remoteMCPConnected", completed: true}). The tool will respond with your next action - follow it immediately (This time it'll be to send a straight forward message about where the user can use iterate and then some other things on hackability)!

            **THEN STOP**: Do not offer to do more demos or ask what they want to do next. The onboarding step is complete. Move to Step 3.
