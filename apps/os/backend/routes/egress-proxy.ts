 ...
 * - X-Egress-Original-URL: The original request URL
 * - X-Egress-Original-Host: The original host
 * - X-Egress-Original-Method: The original HTTP method
 * - X-Egress-API-Key: API key for authentication
 */
...
const STRIP_REQUEST_HEADERS = [
  "x-egress-original-url",
  "x-egress-original-host",
  "x-egress-original-method",
  "x-egress-api-key",
  "host",
  "connection",
  "keep-alive",
...
egressProxyApp.all("/api/egress-proxy", async (c) => {
  ...
  const originalURL = c.req.header("X-Egress-Original-URL");
  const originalHost = c.req.header("X-Egress-Original-Host");
  const originalMethod = c.req.header("X-Egress-Original-Method") || c.req.method;
  const apiKey = c.req.header("X-Egress-API-Key");

  // Validate required headers
  if (!originalURL) {
    logger.warn("Egress proxy request missing X-Egress-Original-URL");
    return c.json({ error: "Missing X-Egress-Original-URL header" }, 400);
  }
