 ...
 * @param options.timeout - A Date indicating when the timeout should be reached. If not set the function will never timeout.
 * @returns A promise that resolves when the input promise resolves
 */
export function reallyWaitUntil(
  durableObject: DurableObject,
  promise: Promise<unknown>,
  options: { timeout?: Date } = {},
): void {
  ...
  let count = 0;
  const intervalFinished = new Promise<void>((resolve, reject) => {
    const interval = setInterval(async () => {
      count++;
      try {
        if (options.timeout && Date.now() > options.timeout.getTime()) {
          clearInterval(interval);
          const error = new Error("reallyWaitUntil timeout after " + (Date.now() - start) + "ms");
          logError("reallyWaitUntil timeout", error);
          reject(error);
          return;
        }
        const isPromiseFinished = await Promise.race([
          promise.finally(() => true),
        ...
        logDebug(
          `Background task has been running for ${Date.now() - start}ms (iteration ${count}), sending a no-op fetch to keep the agent awake`,
        );
      } catch (err) {
