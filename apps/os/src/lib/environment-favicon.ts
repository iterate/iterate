 ...
 * Preview and dev use the white iterate mark behind a large environment badge
 * anchored to the lower-right so crowded browser tabs remain distinguishable.
 */
...
export function environmentFaviconSvg(input: { background: string; marker: string }) {
  const fontSize = input.marker.length > 1 ? 224 : 300;
...
<circle cx="299" cy="299" r="192" fill="white" stroke="${input.background}" stroke-width="24"/>
<text x="299" y="399" text-anchor="middle" fill="${input.background}" font-family="Arial, sans-serif" font-size="${fontSize}" font-weight="800">${input.marker}</text>
</svg>`;
}
