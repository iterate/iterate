...
export const ITX_API_DECLARATIONS: readonly ItxApiDeclaration[] = [
  ...
  },
  {
    ...
    memberSummaries: {
      ...
      processEventBatch:
        '"The project processes this event batch" — the first-party dispatch point every project-scoped stream\'s birth-certificate feed names (`expression: ["processEventBatch"]`).',
      worker:
  ...
  },
  {
    ...
    summary: "Agent capability surface for message loops and agent-local dynamic tools.",
    memberSummaries: {
      capabilityHost:
        "The agent scope's own capability host (provide/revoke/runScript/ __describe) — and the dotted door to the scope's DYNAMIC capabilities through a fetched handle: `agents.get(path).capabilityHost.someTool(args)`.",
      provideCapability:
        "Shortcut for `capabilityHost.provideCapability` (mounts on THIS agent's scope).",
      revokeCapability: "Shortcut for `capabilityHost.revokeCapability`.",
  ...
  },
  {
    ...
    summary:
      "The docs door: search + fetch over everything callable from this scope — e2e-TESTED example scripts (each runs unattended in the platform's test suite; copy them), the public type surface (the Itx Type Graph), and the capabilities mounted in the caller's scope chain.",
    memberSummaries: {
  ...
  },
  {
    ...
    memberSummaries: {
      ...
      subscribe:
        "Live EPHEMERAL event delivery: `processEventBatch` is called for every committed batch (optionally replayed from `replayAfterOffset`); returns an unsubscribe handle.",
      acceptCrossPost:
  ...
  },
  {
    ...
    memberSummaries: {
      kind: "What kind of corpus entry matched: an e2e-tested example script, a type declaration, or a capability mounted in the caller's scope.",
      name: "The name to fetch it by (example id, declaration name, or mount path).",
      summary: "One-line summary of the entry.",
      fetch: "The literal itx call that fetches the full entry — copy it verbatim.",
    },
    referencedTypeNames: [],
  },
  {
  ...
  },
  {
    ...
    memberSummaries: {
      ...
      update: "Set the secret material and/or its egress allowlist.",
