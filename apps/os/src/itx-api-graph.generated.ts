...
export const ITX_API_DECLARATIONS: readonly ItxApiDeclaration[] = [
  ...
  },
  {
    ...
    summary:
      "One durable subscription on one source stream: its committed configuration and cursor (`describe`), the uniform confirmation barrier (`waitUntilConfirmed` — works for EVERY receiver kind off the confirmed cursor), the cursor/resume verbs (ordinary core events), and — for processor-wake subscriptions — the hosted processor instance's facade.",
    memberSummaries: {
      describe:
        "Committed configuration plus the durable cursor; null when no such subscription is configured.",
      waitUntilConfirmed:
        "The uniform barrier: resolve once this subscription's receiver has durably confirmed through `offset` — a push acknowledgement, a hosted processor's reported checkpoint, or an offset-acking webhook's response.",
      setCursor: "Change where this subscription reads next (the existing cursor-set event).",
      resume: "Un-halt delivery without changing the cursor.",
      processor:
        "The hosted processor instance behind a processor-wake subscription (snapshot/getRuntimeState/waitUntilProcessed), dialed by placement.",
      liveState: "The hosted processor instance's live state.",
    },
    referencedTypeNames: [
      "Description",
  ...
  },
  {
    ...
    memberSummaries: {},
    referencedTypeNames: ["StreamEvent", "SubscriptionKey", "SubscriptionConfigurationForDelivery"],
  },
  {
  ...
  },
  {
    ...
    summary:
      "One row of `subscriptions.list()`: the committed catalog entry joined with its durable cursor — name, receiver kind, state, lag (head − confirmed), and the outstanding delivered/confirmed window.",
    memberSummaries: {},
    referencedTypeNames: [],
  },
  {
  ...
  },
  {
    ...
    summary:
      "`subscriptions.get(name).describe()`: the committed configuration plus the durable delivered/confirmed cursor and retry state.",
    memberSummaries: {},
    referencedTypeNames: [],
  },
  {
