...
// residency watchdog (context/residency-watchdog.ts) — and arms the earliest, or deletes the alarm
// when they report none. Every durable reason is derivable at construction (a schedule is durable,
// a cursor row is durable and the log is, a claim is a kv row); the watchdog is in memory on
// purpose — it watches the incarnation that armed it, and a fresh one has nothing to watch. So the
// alarm read from storage is only the DEDUPE SEED: the constructor's first reconcile derives the
// same time (no write) or supersedes it — and a stored time no source still wants (one a dead
// incarnation left, its watchdog's included) is rightly superseded, even though workerd then
// cancels the run it would have started (nothing durable was due).
