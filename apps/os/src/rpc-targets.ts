...
export class StreamRpcTarget extends IterateRpcTarget<"Stream"> {
  ...
  async waitForEvent(args: {
    ...
    while (Date.now() < deadline) {
      ...
      const wait = retryLoggedIdempotentOperation({
        ...
        retryWhen: (error) => !isExplicitStreamKillLifecycleError(error),
        operation: async () => {
          const result = await this[STREAM_DURABLE_OBJECT_STUB].waitForEvent({
            ...args,
  ...
  settleWakeDelivery(report: StreamWakeDeliverySettlementReport): void {
    ...
    const result = this[STREAM_DURABLE_OBJECT_STUB].settleWakeDelivery(report);
    disposeIgnoredRpcResult(result);
  }
  ...
  async create(payload?: RepoCreateInput): Promise<RepoRpcTarget> {
    ...
    await timedStep("create-timing", timing, "wait-repo-deployment-readiness", () =>
      waitForCreatedScopeDeploymentVersion({
        ...
        targets: [
          {
            getTarget: () => stream[STREAM_DURABLE_OBJECT_STUB],
...
class AgentRpcTarget extends IterateRpcTarget<"Agent"> {
  ...
  async create(payload?: AgentCreateInput): Promise<AgentRpcTarget> {
    ...
    await waitForCreatedScopeDeploymentVersion({
      ...
      targets: [
        {
          getTarget: () => this.stream[STREAM_DURABLE_OBJECT_STUB],
          kind: "Stream Durable Object",
        },
        {
          getTarget: () => this.durableObjectStub,
