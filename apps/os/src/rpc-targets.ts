...
class ProjectIntegrationsRpcTarget extends IterateRpcTarget<"ProjectIntegrations"> {
  ...
  #telegramProcessor(connection: string): ProcessorRelayRpcTarget<TelegramProcessorState> {
    return new ProcessorRelayRpcTarget<TelegramProcessorState>({
      auth: this.props.auth,
      host: () =>
...
class CapabilityHostRpcTarget extends IterateRpcTarget<"CapabilityHost"> {
  ...
  async create(input: { ancestorPath: string | null }): Promise<void> {
    ...
    if (committedAncestor !== ancestorPath) {
      throw new Error(
        `capability-host ${JSON.stringify(this.#props.path)} already declares ancestor ${JSON.stringify(committedAncestor)}, not ${JSON.stringify(ancestorPath)}`,
      );
    }
    const committed = [birth, ...rest];
    const offset = committed.reduce((maximum, event) => Math.max(maximum, event.offset), 0);
...
class CapabilityHostCollectionRpcTarget extends IterateRpcTarget<"CapabilityHostCollection"> {
  ...
  async create(input: {
    ...
    await host.create({ ancestorPath });
    return host;
  }
 ...
 */
export class ProcessorRelayRpcTarget<State>
  extends IterateRpcRelay<"StreamProcessorRpc">
  implements WakeableStreamProcessorRpc<State>
{
  readonly #auth: ItxAuth;
  readonly #host: () => ProcessorHostStub | PromiseLike<ProcessorHostStub>;
  readonly #processorSlug: string;

  constructor(args: {
    auth: ItxAuth;
    host: () => ProcessorHostStub | PromiseLike<ProcessorHostStub>;
  ...
  async #callProcessorOutcome<Result>(
    ...
    const settle = <Value>(operation: Promise<Value>): Promise<DeadlineOutcome<Value>> =>
      expiresAt === undefined
        ? operation.then<DeadlineOutcome<Value>, DeadlineOutcome<Value>>(
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      let hostOperation: Promise<ProcessorHostStub>;
      try {
        hostOperation = Promise.resolve(this.#host());
      } catch (error) {
      ...
      const hostOutcome = await settle(hostOperation);
      if (
        hostOutcome.status === "rejected" &&
        attempt === 1 &&
        isDurableObjectLifecycleError(hostOutcome.error)
      ) {
        console.info("processor relay retrying after Durable Object lifecycle reset");
        continue;
      }
      if (hostOutcome.status !== "fulfilled") return hostOutcome;
      // Acquiring a sandbox host can itself require an asynchronous catalogue
      // lookup. If it consumed this wait slice, do not start a data call after
      // the caller's deadline; the next slice will re-resolve the current DO.
      ...
      try {
        operation = Promise.resolve(call(hostOutcome.value));
      } catch (error) {
        if (attempt === 1 && isDurableObjectLifecycleError(error)) {
          console.info("processor relay retrying after Durable Object lifecycle reset");
          continue;
  ...
  async snapshot(): Promise<ProcessorSnapshot<State>> {
    return await this.#callProcessor((host) =>
      readProcessorSnapshot<State>(host, this.#processorSlug),
    );
  }

  async getRuntimeState(): Promise<ProcessorRuntimeState<State>> {
    return await this.#callProcessor((host) =>
      readProcessorRuntimeState<State>(host, this.#processorSlug),
    );
  }
