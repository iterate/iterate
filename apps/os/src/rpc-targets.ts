  ...
  implements StreamProcessorRpc<PublicState>
{
  ...
  readonly #waitUntilProcessed:
    | ((input: { offset: number; timeoutMs?: number }) => Promise<unknown>)
    | undefined;
  ...
  constructor(
    ...
    options: {
      ...
      /** Host-backed hard catch-up plus processor checkpoint barrier. */
    ...
    this.#waitUntilProcessed = options.waitUntilProcessed;
  ...
  async waitUntilProcessed(input: { offset: number; timeoutMs?: number }) {
    if (this.#waitUntilProcessed !== undefined) {
      await this.#waitUntilProcessed(input);
      return;
    }
    await this.#catchUpBeforeSnapshot?.();
    await this.#processor.waitUntilProcessed(input);
  }
}
