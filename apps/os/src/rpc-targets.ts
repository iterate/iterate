  ...
  async __describe(): Promise<Description> {
    return describeNode({
      ...
      children: {
        ...
        collab:
          "Collaborative editing API: open(path) / push(batch) / applyIfUnchanged(path, expectedContent, nextContent) / wait(path, epoch, afterVersion) / changes(path) — live rebase-model editing plus attributed redlines.",
