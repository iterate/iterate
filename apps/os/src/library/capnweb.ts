...
// main object as a pipelinable handle, written against `itx.fetch` alone. The WebSocket is opened
// THROUGH egress — `itx.fetch` with the Upgrade header, the 101's socket accepted and handed to
// capnweb — so `getSecret("/secrets/NAME")` headers substitute and the socket is the context's.
// `{ transport: "batch" }` is the one-shot alternative: capnweb's HTTP batch client uses the global
// fetch, so the same transport is re-spelled here over `itx.fetch` (RpcTransport is capnweb's own
// extension point for exactly that). A held connection pins this context awake for its life, like a
...
export async function connectToCapnweb(
  ...
  // REOPENED on the next call after it is gone — disposed (library.ts `releaseConnections`, the
  // context's pins' release) or broken by the far side — so a held or memoized connection is never
  // a dead socket.
