/**
 * Connects an itx session's TRANSPORT (the /api WebSocket a
 ...
 *
 * Keyed on the session's execution context because that object already flows
 ...
 * created (ItxSessionDurableObject); everywhere else this map has no entry and
 * {@link closeItxSessionTransport} is a no-op.
 */
const transportClosers = new WeakMap<object, (code: number, reason: string) => void>();

/** Registration at WebSocket upgrade time; one per session. */
