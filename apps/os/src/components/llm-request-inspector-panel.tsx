...
export function LlmRequestInspectorContent({
  ...
  streamSource: (path: string) => Stream | Promise<Stream>;
  streamPath: string;
  ...
  const request = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        return await stream.inspectLlmRequest(llmRequestOffset);
      } finally {
        // Generated capability types omit the optional local RPC disposal hook.
        (stream as Partial<Disposable>)[Symbol.dispose]?.();
      }
    },
  });
  const replay = request.data ?? null;
      ...
      <div className="min-h-0 flex-1 overflow-y-auto border-t">
        {displayedReplay != null ? (
          ...
          <p className="px-5 py-3 text-sm text-destructive">
            Reconstructing the request failed: {request.error.message}
          </p>
        ) : request.isPending ? (
          <p className="px-5 py-3 text-sm text-muted-foreground">Reconstructing request…</p>
        ) : (
          <p className="px-5 py-3 text-sm text-muted-foreground">
            No LLM request at offset #{llmRequestOffset} in this stream.
          </p>
        )}
      </div>
    </>
...
function ResponseText({ text, renderMode }: { text: string; renderMode: "markdown" | "plain" }) {
  if (text === "") return null;
  ...
  return (
    <section className="px-5 py-3">
      ...
      <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 font-mono text-xs text-foreground/80">
        ...
        <StreamingMetrics stats={stats} />
        <GatewayMetric status={stats.gatewayCacheStatus} />
      </dl>
...
function TokenMetrics({ tokens }: { tokens: LlmRequestReplayStats["tokens"] }) {
  if (tokens == null) return null;
  const contextPercent =
    Math.round(((tokens.inputTokens + tokens.outputTokens) / tokens.maxContextTokens) * 1000) / 10;
  const cacheDetail =
  ...
  return (
    <>
      ...
      <dd>
        {tokens.inputTokens.toLocaleString()} tok{cacheDetail}
      </dd>
      <dt className="text-muted-foreground/70">output</dt>
      <dd>
        {tokens.outputTokens.toLocaleString()} tok{reasoningDetail}
      </dd>
...
function StreamingMetrics({ stats }: { stats: LlmRequestReplayStats }) {
  const parts = [
...
function GatewayMetric({ status }: { status: LlmRequestReplayStats["gatewayCacheStatus"] }) {
  ...
  return (
    <>
      ...
      <dd>
        response cache{" "}
        <span className={cn(status === "HIT" && "text-emerald-600 dark:text-emerald-500")}>
          {status}
        </span>
        {status === "HIT" ? " — served from the AI Gateway cache without touching the model" : ""}
      </dd>
    </>
  );
}
