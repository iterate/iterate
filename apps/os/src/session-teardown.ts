// session-teardown.ts — what a session must undo at its end: the one-entry-per-key register every
// IterateContextRpcTarget of a session shares. No imports, so the unit tests load it in node.
