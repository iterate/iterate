...
`.trim(),
  ...
  projectExample<{
    ...
    description:
      "Pass deviceId (from itx.devices.list()) and targetUrl. The sandbox's curl waits while you open a notification in the app. The app publishes its fetch capability and acknowledges device/capability-ready; a stream wait resumes the request through that client. Uses the client's current Wi-Fi/cellular/VPN route, not necessarily a residential IP. The interceptor is temporary, project-wide, and last-writer-wins. Unmatched URLs use next(request), retaining normal approval and secret handling. Matching requests bypass that handling and receive no substituted project secrets. The mobile web build also works with CORS-enabled endpoints; its browser spec opens the notification and verifies the client IP through a real sandbox. APNs and physical handset behavior need a phone.",
...
`.trim(),
  ...
  projectExample({
    ...
    runtimes: ALL_RUNTIMES,
    fn: async (itx, vars: { owner?: string; repo?: string; path?: string }) => {
  ...
  projectExample<{
    parallel: {
      v1_search_v1_search_post(input: {
        objective?: string;
        search_queries: string[];
        mode?: "turbo" | "basic" | "advanced";
        max_chars_total?: number;
      }): Promise<unknown>;
      tasks_runs_post_v1_tasks_runs_post(input: {
        input: string;
        processor: string;
      }): Promise<{ run_id: string }>;
      tasks_runs_result_get_v1_tasks_runs__run_id__result_get(input: {
        run_id: string;
        timeout?: number;
      }): Promise<unknown>;
    };
    ...
    runtimes: ALL_RUNTIMES,
    fn: async (itx, vars: { objective?: string; deep?: boolean }) => {
  ...
  projectExample({
    ...
    runtimes: ALL_RUNTIMES,
    fn: async (itx, vars: { workspacePath?: string; skill?: string }) => {
      const workspace = itx.workspaces.get(vars.workspacePath ?? "/workspaces/example");
      await workspace.create({});
