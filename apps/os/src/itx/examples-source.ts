...
`.trim(),
  ...
  projectExample({
    ...
    fn: async (itx, vars: { path?: string; note?: string }) => {
      // A stream is addressed by a path within the project. append() is
      // acknowledgement-only unless the caller requests a result projection.
  ...
  projectExample({
    ...
    fn: async (itx, vars: { path?: string }) => {
      ...
      const tickResult = await stream.append(
        { return: "offsets" },
        {
      ...
      const [tickOffset] = tickResult;
      if (typeof tickOffset !== "number") throw new Error("append did not return an offset");

      // The durable truth is its own ordinary event — THIS is what processors fold.
      const doneResult = await stream.append(
        { return: "offsets" },
        {
      ...
      const [doneOffset] = doneResult;
      if (typeof doneOffset !== "number") throw new Error("append did not return an offset");

      const defaults = await stream.getEvents({ afterOffset: tickOffset - 1 });
      const raw = await stream.getEvents({ afterOffset: tickOffset - 1, includeEphemeral: true });
      return {
        tickOffset, // ephemeral rows consume offsets like any commit
        doneOffset, // the durable event landed right after it
  ...
  projectExample<{ demoStream: Stream }>({
    ...
    fn: async (itx, vars: { path?: string; note?: string }) => {
      ...
      const appendResult = await itx.demoStream.append(
        { return: "events" },
        {
