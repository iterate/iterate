...
| `itx-react.tsx`         | the browser surface   | the whole React surface in one file — `useSession`/`connectSession` (the catalog) and `useItx(slug)`/`connectItx(slug)` (a project), `useItxQuery` (read), `useItxSubscription`/`useLiveState` (subscribe), `ItxProvider` (session gate) + `ProjectScope`; ONE socket per tab with invisible reconnect, Suspense, never SSRs |
