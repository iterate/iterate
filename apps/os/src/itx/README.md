...
- **Creation is two idempotent appends by the CREATOR**
  (`createContext`, `coordinates.ts`): an
  `subscription-configured` event pointing the stream at the node's
  hosted `itx` processor, then
...
processor on a stream-processor host, so the context's stream delivers batches
...
| `itx-react.tsx`         | the browser surface   | the whole React surface in one file — `useIterateSession`/`connectIterateSession` (the catalog) and `useItx(ref)`/`connectItx(ref)` (a project), `useItxQuery`/`useIterateSessionQuery` (read), `useStreamConnection`/`useLiveState` (subscribe), `<ProjectScope>` (ambient slug + pre-warm); ONE socket per tab with invisible reconnect, Suspense (router-provided), never SSRs |
