...
| `itx-react.tsx`         | the browser surface   | the whole React surface in one file — `useIterateSession`/`connectIterateSession` (the catalog) and `useItx(ref)`/`connectItx(ref)` (a project), `useItxQuery`/`useIterateSessionQuery` (read), `useItxSubscription`/`useLiveState` (subscribe), `<ProjectScope>` (server-safe ambient slug); ONE socket per tab with invisible reconnect, client-only Suspense around connection consumers |
