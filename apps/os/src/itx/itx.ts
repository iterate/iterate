...
//
// The context's journal is an ordinary event stream; these are its event
// types. Provides/revokes/disconnects and the script execution record all
// live here — there is no other write. Payload schemas are deliberately
// LOOSE (z.looseObject + optionals): a journal may carry events from before
// a deploy's schema, and a malformed payload must be ignored by the fold,
...
export class Itx extends StreamProcessor<typeof ItxContract, ItxDeps, ItxIterateContext> {
   ...
   * deliberately EXACT-match only: a path define ("sdk.chat.postMessage")
