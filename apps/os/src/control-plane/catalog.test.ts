...
describe("organizations", () => {
  test("created with the caller its owner: the record, the membership, and what the owner can access", () => {
    const { c, person } = catalog();
    const ada = person("ada@example.com");
  ...
  test("an owner renames, adds and removes members; the last owner stays; a stranger is refused", () => {
    ...
    expect(c.accessibleTo(bob.id).organizations).toEqual([
  ...
  test("deleted only while it holds no project; then it and its memberships are gone", () => {
    const { c, person } = catalog();
    const ada = person("ada@example.com");
    const bob = person("bob@example.com");
    const org = c.createOrganization(as(ada), { name: "Booper" });
...
describe("projects", () => {
  test("a slug is one project across every organization: the same organization's again is the same project, another's is refused before anything is made", () => {
    ...
    expect(c.accessibleTo(ada.id).projects).toEqual([{ ...dawg, role: "owner" }]);
    ...
    expect(
      refusal(() => c.createProject(as(bob), { project: "x", organizationId: org.id })).code,
    ).toBe("FORBIDDEN");
