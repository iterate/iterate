...
This context defines the language for stream event storage and browser stream viewing in the OS streams domain.
...
**Browser Mirror**:
The per-stream SQLite database in the browser, identified by `(projectId, path)` and stored in OPFS under a per-projectId root folder (e.g. `/${projectId}/${path-slug}.sqlite3`). It holds every browser processor's tables for that stream and is shared across all views/components of the stream (one connection per `(projectId, path)` per tab).
...
- Each **Browser Stream Processor** is hosted by its React sub-view and has its own injected stream client; mounting the sub-view starts the processor, unmounting stops it. OS injects an ITX stream client, while the streams example app injects its capnweb demo client. A processor's table only advances while its sub-view is mounted, and catches up on remount by resuming from its own checkpoint.
...
- Two dedup scopes keyed on `(stream path, processor slug)`: (1) cross-tab, Web Locks elect ONE writer/leader across all tabs (others are read-only followers); (2) within one tab, a ref-counted registry shares ONE runtime instance (one stream client + one processor runner) across an arbitrary number of React views with that key.
- Same-stream split views share one within-tab runtime per `(projectId, path, slug)` and therefore report the same leader/follower state for that processor. Cross-tab handoff is unaffected because separate JS contexts still elect independently.
...
- The SQLite connection (wa-sqlite worker / `Browser Mirror`) is shared per `(projectId, path)` (one worker per stream per tab), via a ref-counted registry beneath the `(path, slug)` runtime registry. Both registries use one small generic ref-count helper; a stream's multiple processors share one DB worker (no intra-tab OPFS handle contention).
- `acquireStreamRuntime` (`src/browser/stream-browser-store.ts`) is the live browser surface, used by `apps/os`'s `project-stream-view.tsx` and the example-app views. A view passes its `BrowserProcessorConfig` — `slug`, `schemaVersion` (folded into the writer-lock name), `tables` (cleared on mirror discard), `createStreamClient`, and `createProcessor({ stream, sql, subscriptionKey })` constructing the class-based processor. The returned `StreamBrowserStore` ALWAYS opens the injected stream client (in OS, an itx stream handle from `useItx()`), so a follower can still `appendBatch` / read `runtimeState`, elects the Web Lock on the subscription key, and ONLY as leader hosts the processor: it reads the processor's `snapshot()` for the replay cursor, subscribes with processor announcement + `getRuntimeState()` capability, and pumps delivered batches into `processor.ingest`. Ref-counting/dedup per `(projectId, path, slug)` is hidden inside `acquireStreamRuntime`; the shared **Browser Mirror** DB is ref-counted one level up per `(projectId, path)`.
...
- Identity keys carry the projectId: `subscriptionKey = ${projectId}:${browserSubscriberId}:${slug}`; the Web Lock name = `${projectId}:${path}:${subscriptionKey}`, versioned by browser DB schema. Leadership and the stream subscription are both keyed on the subscription key.
- The projectId is the constant `"default"` for now (renamed from `"stream"` to avoid confusion with the stream concept), threaded explicitly from the route as part of `(projectId, path)`; browser libs never hardcode it. Requires `example-app/src/worker.ts` to name the DO `default:${path}` (was `stream:${path}`).
