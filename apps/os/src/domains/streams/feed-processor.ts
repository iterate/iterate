...
> {
  ...
  protected override processEvent(args: ProcessEventArgs<FeedProcessorContract>): undefined {
    ...
    args.blockProcessorWhile(async () => {
      ...
      const inputs = publications.map<EmittedInput<FeedProcessorContract>>((publication) => ({
      ...
      while (start < inputs.length) {
        ...
        while (end < inputs.length) {
          const inputBytes = this.#jsonByteLength(inputs[end]!);
          if (end > start && bytes + inputBytes > FEED_PUBLICATION_CHUNK_BYTES) break;
          bytes += inputBytes;
          end += 1;
        }
        const committed = await args.append(...inputs.slice(start, end));
        for (let index = start; index < end; index++) {
          this.deps.publications.save(publications[index]!, committed[index - start]!.offset);
        }
        start = end;
      }
