...
describe("StreamRpcTarget", () => {
  ...
  it("reserves platform idempotency keys before an untrusted append reaches the stream", async () => {
    const append = vi.fn(async () => []);
    class TestStreamRpcTarget extends StreamRpcTarget {
      override get durableObjectStub() {
        return { append } as never;
    ...
    await expect(
      new TestStreamRpcTarget(props).append({
        type: "events.iterate.com/test/key-squat",
        idempotencyKey: "platform:project-created:prj_test",
      }),
    ).rejects.toThrow("platform: idempotency keys are reserved");
    expect(append).not.toHaveBeenCalled();
    ...
    expect(append).toHaveBeenCalledWith(
      expect.objectContaining({
        source: { processor: expect.objectContaining({ authority: "platform" }) },
      }),
    );
  });
  ...
  it.each([
  ...
  ])("reserves root project-worker mutations through %s", async (type) => {
    const append = vi.fn(async () => []);
    class TestStreamRpcTarget extends StreamRpcTarget {
      override get durableObjectStub() {
        return { append } as never;
    ...
    await expect(new TestStreamRpcTarget(props).append(event)).rejects.toThrow(
      'the root "project-worker" subscription is reserved',
    );
    expect(append).not.toHaveBeenCalled();

    await new TestStreamRpcTarget({ ...props, platformProcessorHost: true }).append(event);
