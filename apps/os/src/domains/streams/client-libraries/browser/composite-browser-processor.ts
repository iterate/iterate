...
export class CompositeBrowserProcessor implements AnyHostedProcessor {
  ...
  async ingestThrough(args: Parameters<AnyHostedProcessor["ingestThrough"]>[0]): Promise<void> {
    // Preserve the same ordered, failure-sticky fan-out as ingest(), while
    // carrying the stream's scanned-through checkpoint to every child.
    for (const { processor } of this.#children) {
      await processor.ingestThrough(args);
    }
  }
