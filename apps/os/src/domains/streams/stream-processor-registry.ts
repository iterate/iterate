...
export type StreamProcessorRegistry<Live extends object = Record<string, unknown>> = {
   ...
   * sink is safe. Failures rethrow: a foreground read must never silently
   * serve stale state after its required catch-up failed. Background live
   * assembly owns its separate log-and-retain-last-coherent-state policy.
   */
  ...
  async function loadThenAssemble(): Promise<void> {
    ...
    await Promise.all(
      [...entries.entries()]
        ...
        .map(([name]) =>
          catchUp(name).catch((error: unknown) => {
            console.error(
              `stream processor "${name}" background catch-up failed; retaining last coherent live state`,
              error,
            );
          }),
        ),
