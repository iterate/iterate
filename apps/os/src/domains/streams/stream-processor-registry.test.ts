...
describe("wakeStreamProcessor", () => {
  ...
  it("uses the retained callback even when an older stream sends a direct-settlement coordinate", async () => {
    ...
    const duplicatePriorResult = vi.fn(() => vi.fn());
    const priorResult = Object.assign(vi.fn(), {
      dup: duplicatePriorResult,
    });
    ...
    const returned = woken.processEventBatch({
      ...
      reportDeliveryResult: legacyResult,
      settleDelivery: priorResult,
    });
