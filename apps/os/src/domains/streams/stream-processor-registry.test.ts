...
describe("catchUp", () => {
  it("surfaces processor failures instead of serving stale committed state", async () => {
    const h = makeHarness();
    h.hooks.alpha.onProcess = () => {
      throw new Error("poisoned catch-up");
    };
    await h.stream.append({ type: REQUESTED, payload: { id: "a" } });

    await expect(h.registry.catchUp("alpha-proc")).rejects.toThrow("poisoned catch-up");
    expect((await h.wake("alpha-proc")).checkpointOffset).toBe(0);
  });
});
