...
export class StreamEventSender {
  ...
  #onSourceOwnedFailure(args: {
    ...
    if (
      ...
      isDurableObjectOverloadError(error) ||
...
type StreamConnection = {
  ...
  settleWakeDelivery?(settlementId: string, settlement: StreamWakeDeliveryResult): void;
...
export class StreamConnections {
  ...
  readonly #settlementIncarnation: string;
  ...
  constructor(args: { idleTeardownMs: number; hooks: StreamConnectionsHooks }) {
    ...
    this.#hooks = args.hooks;
    this.#settlementIncarnation = args.hooks.randomUUID();
  }
  ...
  #open<Batch extends StreamEventBatch>(args: OpenConnectionArgs<Batch>): StreamConnection {
    ...
    let hostedBatchSettlementId: string | null = null;
    let hostedBatchResultHandler: ((result: StreamWakeDeliveryResult) => void) | null = null;
    let hostedBatchStartedAtMs: number | null = null;
    let hostedBatchDeadlineAtMs: number | null = null;
    let connection!: StreamConnection;
    const settleHostedBatch = (
      settlementId: string,
      deliveryResult: StreamWakeDeliveryResult,
    ): void => {
      if (hostedBatchSettlementId !== settlementId || hostedBatchResultHandler === null) return;
      const handleResult = hostedBatchResultHandler;
      hostedBatchPending = false;
      hostedBatchSettlementId = null;
      hostedBatchResultHandler = null;
      hostedBatchStartedAtMs = null;
      hostedBatchDeadlineAtMs = null;
      handleResult(deliveryResult);
    };
      ...
      try {
        while (open) {
          ...
          if (kind === "hosted") {
            ...
            const settlementId = `${this.#settlementIncarnation}:${++this.#nextSettlementSequence}`;
            hostedBatchPending = true;
            hostedBatchSettlementId = settlementId;
            ...
            hostedBatchResultHandler = (deliveryResult) => {
    ...
    connection = {
      ...
      settleWakeDelivery: (settlementId, settlement) => settleHostedBatch(settlementId, settlement),
      close: (reason, error) => {
        if (!open) return;
        open = false;
        hostedBatchPending = false;
