...
import { isStreamAppendResultOptions, type StreamAppendArguments } from "./rpc-types.ts";
...
function makeJournal(homePath = HOME) {
  ...
  const streamAt = (path: string): Stream =>
    ({
      append: (...args: StreamAppendArguments) => {
        const first = args[0];
        ...
        const committed = events.map((event) => commit(path, event));
        if (options?.return === "events") return Promise.resolve(committed);
        if (options?.return === "offsets") {
          return Promise.resolve(committed.map((event) => event.offset));
        }
...
describe("StreamProcessorRunner selector scan progress", () => {
  it("durably advances both cursors through an empty selector gap", async () => {
    ...
    expect(harness.processor.subscriberMetrics.report()).toMatchObject({
      batchesIngested: 1,
      eventsIngested: 0,
    });
  });
  ...
  it("keeps a checkpoint-only scan retryable when persistence fails", async () => {
    ...
    expect(harness.store.record?.reduction.reducedThroughOffset).toBe(1);
  });

  it("runs exactly one event-less at-head reconcile when an empty gap reaches head", async () => {
    const journal = makeJournal();
    journal.seed({ type: REQUESTED, payload: { id: "a" } });
    journal.seed({ type: NOISE, payload: {} });
    const headEvents: (number | null)[] = [];
    const harness = makeHarness({
      journal,
      hooks: {
        onHead: (args) => void headEvents.push(args.event?.offset ?? null),
      },
    });
    const { sink } = await harness.runner.openDelivery();
...
describe("StreamProcessorRunner at-head reconcile (delivery.caughtUp)", () => {
  ...
  it("reaches an unconsumed tail without a compensating journal self-read", async () => {
    const journal = makeJournal();
    ...
    // The production wake lane delivers ONLY consumed event 1, but reports
    // that its selector scanned through raw head 2. The same durable frame
    // advances through the unconsumed tail and reconciles at head; no second
    // type-unfiltered read is needed.
    await harness.deliverConsumesFilteredPending();
    expect(harness.store.record?.processing.acknowledgedThroughOffset).toBe(2);
    expect(headCalls).toEqual([{ open: ["a"], event: 1 }]);
...
describe("StreamProcessorRunner.waitUntilEvent", () => {
  ...
  it("offset form resolves from committed scan progress even when no event payload matched", async () => {
    const harness = makeHarness();
    const opened = await harness.runner.openDelivery();
    const reached = harness.runner.waitUntilEvent({ offset: 1, timeoutMs: 500 });
