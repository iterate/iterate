...
> {
  ...
  async #processFrame(frame: SinkFrame): Promise<void> {
    ...
    try {
      ...
      // (proven in prd: an agent's llm-request-scheduled never fires). So the
  ...
  async #commitFrameContext(ctx: FrameContext<ProcessorState<Contract>>): Promise<void> {
    ...
    const newestCommittedEvent = committedEvents.at(-1);
    const newestEventCreatedAtMs =
      newestCommittedEvent === undefined ? undefined : Date.parse(newestCommittedEvent.createdAt);
    this.driver.noteBatchIngested({
      ingestedThroughOffset: next.processing.acknowledgedThroughOffset,
      ...(newestEventCreatedAtMs !== undefined && Number.isFinite(newestEventCreatedAtMs)
        ? { newestEventCreatedAtMs }
        : {}),
