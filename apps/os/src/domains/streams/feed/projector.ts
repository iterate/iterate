// Pure projection logic shared by the browser mirror and the experimental
// server host: ONE feed item
 ...
 */
export const STREAM_FEED_SCHEMA_VERSION = 4;
...
export type StreamFeedState = {
  schemaVersion: typeof STREAM_FEED_SCHEMA_VERSION;
...
export function initialStreamFeedState(): StreamFeedState {
  return {
    schemaVersion: STREAM_FEED_SCHEMA_VERSION,
 ...
 */
export function planStreamFeedOps(
  start: StreamFeedState,
  events: readonly StreamEvent[],
): { ops: FeedOp[]; endState: StreamFeedState } {
...
export function isCurrentStreamFeedState(value: unknown): value is StreamFeedState {
  if (!isRecord(value)) return false;
  const candidate = value as Partial<StreamFeedState>;
  ...
  return (
    candidate.schemaVersion === STREAM_FEED_SCHEMA_VERSION &&
