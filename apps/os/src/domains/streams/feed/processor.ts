...
import { createStreamFeedSchemaEnsurer } from "./sql.ts";
...
export { StreamFeedContract } from "./contract.ts";
export { STREAM_FEED_SCHEMA_VERSION } from "./projector.ts";

/** The table this processor owns — the ONLY rendered-feed table in the mirror. */
export const STREAM_FEED_TABLE = "feed_items";
 ...
 */
export class StreamFeedProcessor extends StreamProcessor<
  StreamFeedContract,
  { sql: StreamFeedSqlClient; projectionBuffer?: ProjectionWriteBuffer }
> {
  readonly contract = StreamFeedContract;
  ...
  readonly projectionBuffer: ProjectionWriteBuffer;

  constructor(
    args: StreamProcessorConstructorArgs<{
      sql: StreamFeedSqlClient;
      projectionBuffer?: ProjectionWriteBuffer;
    }>,
  ) {
    super(args);
    this.projectionBuffer = args.projectionBuffer ?? new ProjectionWriteBuffer();
  }

  /** Projection schema for the progress store's first-open (prepare() successor). */
  ensureProjectionSchema(sql: StreamFeedSqlClient): Promise<void> {
    return ensureStreamFeedSchema(sql);
  }
  ...
  prepareVolatileFrame(args: {
    ...
    const persistedState = StreamFeedContract.stateSchema.parse(args.persistedState);
  ...
  protected override reduce(
    args: Parameters<StreamProcessor<StreamFeedContract>["reduce"]>[0],
  ): StreamFeedState {
    return planStreamFeedOps(args.state, [args.event]).endState;
  }

  protected override processEvent(
    args: Parameters<StreamProcessor<StreamFeedContract>["processEvent"]>[0],
  ): undefined {
    // Event-less at-head pass: no per-event work, only the caughtUp reconcile above (if any).
    const { event } = args;
    if (event === null) return;
    const { ops } = planStreamFeedOps(args.previousState, [event]);
  ...
  #appendOps(offset: number, ops: readonly FeedOp[], open: StreamFeedState["open"]): void {
...
function feedOpToStatement(op: FeedOp): { sql: string; params: StreamFeedSqlValue[] } {
  if (op.kind === "insert") {
    return {
...
export const STREAM_FEED_SCHEMA_STATEMENTS: readonly {
  sql: string;
  params?: StreamFeedSqlValue[];
}[] = [
  {
    sql: `
            -- One row per rendered Feed Item, pretty and raw interleaved in ONE
            -- total order: local_index is allocated by the stream-feed
...
export const ensureStreamFeedSchema = createStreamFeedSchemaEnsurer({
  run: (sql) => sql.batch([...STREAM_FEED_SCHEMA_STATEMENTS], { transaction: true }),
});
