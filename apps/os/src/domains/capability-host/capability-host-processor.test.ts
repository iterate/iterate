...
describe("CapabilityHostProcessor capability table", () => {
  ...
  it("reduces one thousand exact revocations in one event and preserves a replacement", async () => {
    const h = makeHostHarness();
    const mounts = Array.from(
      { length: 1_000 },
      (_, index): HostEventInput => ({
        type: "events.iterate.com/capability-host/capability-provided",
        payload: {
    ...
    await h.play(["append", ...NEW_HOST_EVENTS, ...mounts]);
    const originals = h.events("events.iterate.com/capability-host/capability-provided");
    expect(h.state().capabilities).toHaveLength(1_000);
    ...
    await h.play([
      "append",
      {
        ...
        payload: {
          path: ["provider", "500"],
          providerBinding: { channelKey: "channel-2", leaseKey: "replacement" },
          type: "live",
        },
      },
    ]);
    ...
    await h.play([
      "append",
      {
        ...
        payload: {
          revocations: originals.map((event) => ({
    ...
    expect(h.events("events.iterate.com/capability-host/capabilities-revoked")).toHaveLength(1);
    expect(h.state().capabilities).toMatchObject([
      {
        path: ["provider", "500"],
        providerBinding: { channelKey: "channel-2", leaseKey: "replacement" },
