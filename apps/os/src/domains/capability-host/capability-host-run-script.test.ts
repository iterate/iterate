...
describe("CapabilityHostProcessor.runScript", () => {
  it("rejoins one durable execution when its outer RPC acknowledgement is lost", async () => {
    ...
    const processor = new CapabilityHostProcessor({
      ...
      scriptExecutionEntrypoint: {
        run: async () => {
          throw new Error("the runner owns execution; this verb only journals and waits");
        },
      },
    });
    ...
    const requested = stream.events.find((event) => event.type === SCRIPT_REQUESTED)!;
    ...
    // This is the exact replay the edge target makes after the first call's
    // result committed but its hosting DO/RPC acknowledgement disappeared.
