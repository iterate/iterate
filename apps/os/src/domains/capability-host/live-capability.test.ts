...
describe("plain RPC result ownership", () => {
  it("detaches a disposable object while preserving primitive results", () => {
    const dispose = vi.fn();
    const object = { health: "ok", [Symbol.dispose]: dispose };

    expect(detachDisposablePlainRpcResult(object)).toEqual({ health: "ok" });
    expect(dispose).toHaveBeenCalledOnce();
    expect(detachDisposablePlainRpcResult(42)).toBe(42);
  });
  ...
  describe("live capability retention", () => {
    ...
    it("only flattens nested paths when explicitly requested", async () => {
      ...
      await expect(
        Promise.resolve(capability.invoke(["tools", "echo"], ["hello"])),
      ).rejects.toThrow(/capability path "tools\.echo" hit undefined/);
