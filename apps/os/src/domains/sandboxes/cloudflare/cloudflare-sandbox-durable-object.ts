...
import { type ProcessorReadRequest, serveProcessorRead } from "../../streams/processor-rpc.ts";
...
type SandboxProcessorResources = {
  processorRpc: StreamProcessorRpcTarget<SandboxProcessorState>;
...
export abstract class SandboxDurableObject extends Sandbox<Env> {
  ...
  #processorResources(): SandboxProcessorResources {
    ...
    const processorRpc = new StreamProcessorRpcTarget(reads, {
      catchUpBeforeSnapshot: () => registry.catchUp(SandboxProcessorContract.slug),
    });
    const resources = { processorRpc, reads, registry };
  ...
  readStreamProcessor(request: ProcessorReadRequest): Promise<unknown> {
    return serveProcessorRead({
      expectedProcessorSlug: SandboxProcessorContract.slug,
      processor: this.#processorResources().processorRpc,
      request,
    });
  }
