 ...
 * its build — a pinned-pnpm install (WITH devDependencies; vite lives there)
 * then the app's own `pnpm run build`, expected to produce the
...
function viteAppRecipe(files: Record<string, string>): WorkerBuildRecipe {
  ...
  try {
    ...
    // The install step will fail on the malformed manifest with the package
    // manager's own, clearer error.
    hasBuildScript = true;
  }
  if (!hasBuildScript) {
    throw new Error('The "vite" pipeline needs a "build" script in package.json.');
  }
  const usesNpmLock = "package-lock.json" in files || "npm-shrinkwrap.json" in files;
  const installCommand = usesNpmLock
    ...
    : "pnpm-lock.yaml" in files
      ...
      : "{ command -v nub >/dev/null && timeout -k 5 60 nub install --ignore-scripts --prefer-offline --node-linker hoisted; } || pnpm install --ignore-scripts --prefer-offline --no-frozen-lockfile";
  return {
    commands: [
      {
        // Honor explicit lockfile semantics, otherwise use the same nub-first,
        // pinned-pnpm fallback as the wrangler lane. This intentionally omits
        // --prod/--omit=dev because Vite is a devDependency. --ignore-scripts
        // still blocks install-time lifecycle scripts; the build script below
        // is where project code deliberately runs.
        command: installCommand,
        timeoutMs: VITE_INSTALL_TIMEOUT_MS,
      },
      {
        command: usesNpmLock ? "npm run build" : "pnpm run build",
