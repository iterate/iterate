...
import { decryptDevicePushToken } from "./device-crypto.ts";
...
export class DeviceDurableObject extends DurableObject<Env> {
  ...
  readonly #deviceProcessor = this.#registry.register(
    new DeviceProcessor({
      ...
      migratePushToken: async ({ encryptedPushToken, ownerId }) => {
        return await this.#serializeCredentialUpdate(async () => {
          if (this.#reads.currentState.encryptedPushToken?.offset !== encryptedPushToken.offset) {
            return null;
          }
          ...
          const pushTokenSecretUpdatedOffset = await this.#putPushTokenSecret(token);
          return {
