...
> {
   ...
   * Success, in-execution failure, and the reconciler's settlements all land
   * here with identical idempotency keys, so races collapse at the append
   * dedup layer. (`model` is absent on reconciler settlements — failures have
   * no usage to report.) */
