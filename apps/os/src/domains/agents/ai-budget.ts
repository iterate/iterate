...
function readAiRateLimitStop(response: Response): AiRateLimitStop | null {
  if (response.status !== 429) return null;
...
/** Classify a provider refusal once, before a caller decodes its response body. */
export async function readAiCallStop(
  response: Response,
  provider: string,
): Promise<AiCallStop | null> {
  return (await readAiBudgetStop(response, provider)) || readAiRateLimitStop(response);
}

/** Explain the recorded stop without guessing an unknown reset time. */
export function budgetPauseReason(budget: AiBudgetStop["budget"]): string {
  const rule = budget.ruleId ? ` rule '${budget.ruleId}'` : "";
  const recovery = budget.resetsAt
    ? `Retry after ${budget.resetsAt}.`
    : "Retry after allowance is available or the limit is raised.";
  return `${budget.provider} budget${rule} exhausted. ${recovery}`;
}
