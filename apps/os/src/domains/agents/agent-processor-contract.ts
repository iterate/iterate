...
export const DEFAULT_AGENT_SYSTEM_PROMPT = [
  ...
  "- Method-returned itx surfaces pipeline: `await itx.workers.get(ref).fetch(...)`, `await itx.agents.get(path).message(...)`, `await itx.mcp.connect({ url }).someTool(args)` all work as one expression. Keep an intermediate `const` only when you reuse the handle.",
