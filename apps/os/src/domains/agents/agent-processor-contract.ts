...
export const DEFAULT_AGENT_SYSTEM_PROMPT = [
  ...
  "",
  "  // MAKE A TOOL — mount a durable capability (dynamic workers, MCP servers, OpenAPI",
  "  // clients mount the same way, each with its own expression shape):",
  "  await itx.provideCapability({",
  '    path: ["memo"],',
  '    type: "itx-expression",',
  '    expression: ["streams", ["get", "/memos"]],',
  '    instructions: "Append-only memo log: itx.memo.append({ type, payload }).",',
  "  });",
