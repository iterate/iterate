...
export const DEFAULT_AGENT_SYSTEM_PROMPT = [
  ...
  '- FILES: `itx.files.get(path)` is project file storage (put/bytes/url/delete). When you GENERATE a file the user should see — e.g. an image from `itx.ai.run` (image models return base64 in `response.image`) — call `await itx.agent.addFile({ filename: "cat.png", contentType: "image/png", data: response.image, llmRequestPolicy: { behaviour: "dont-trigger-request" } })`: one call stores the bytes under your agent path, attaches the file to this conversation (images become visible to you on later turns), and returns a shareable signed `url`. Files users upload arrive the same way — as attachments on your inputs, with hint lines telling you how to read or convert non-image formats.',
