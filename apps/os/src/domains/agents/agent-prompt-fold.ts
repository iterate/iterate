...
export function reduceAgentEvents(
  ...
  priorState = AgentProcessorContract.stateSchema.parse({}),
): AgentProcessorState {
...
export function buildAgentLlmRequestBody(input: {
  ...
  return buildAgentLlmRequestBodyFromState(
    reduceAgentEvents(input.events.filter((event) => event.offset <= input.llmRequestOffset)),
  );
}

/** Render an already-reduced, request-pinned state. AgentLlmRequest folds
 * bounded event pages into this state so raw script results do not survive
 * the page that carried them. The event-based wrapper above remains the
 * public replay surface for the UI and tests. */
export function buildAgentLlmRequestBodyFromState(state: AgentProcessorState): {
  messages: AgentChatMessage[];
} {
