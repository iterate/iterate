...
>(input: {
  agentPath: string;
  /** Explicit capability inheritance; stream path segments never imply ancestry. */
  capabilityHostAncestorPath: string | null;
  projectId: string;
  /**
  ...
  const capabilityHostAncestorPath =
    input.capabilityHostAncestorPath === null
      ? null
      : normalizePath(input.capabilityHostAncestorPath);
  if (capabilityHostAncestorPath === agentPath) {
    throw new Error(`capability-host ${JSON.stringify(agentPath)} cannot be its own ancestor`);
  }
  ...
  const capabilityHostBirthCertificate = CapabilityHostProcessorContract.buildEvent({
    ...
    payload: { config: { ancestorPath: capabilityHostAncestorPath } },
  });
