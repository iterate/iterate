...
export function telegramAgentSystemPrompt(input: {
  ...
  return [
    ...
    `MEDIA: photos/voice/stickers arrive with bracketed hints like [photo], and their raw webhook payload above contains Telegram file_id values. To view one, call ${telegramConnection}.getFile({ file_id }) and inspect result.file_path; then download it through project egress with const response = await itx.egress.fetch(new Request('https://api.telegram.org/file/botgetSecret({ path: "/secrets/integrations/telegram/${input.connection}/bot-token" })/' + file_path)). Attach the response to your next-turn context with await itx.agent.addFiles({ files: [{ filename: file_path.split("/").at(-1) || "telegram-file", contentType: response.headers.get("content-type") || "application/octet-stream", data: await response.blob() }], llmRequestPolicy: { behaviour: "dont-trigger-request" } }). The getSecret placeholder is substituted only at egress; never ask for, print, or try to read the bot token itself.`,
    "Your scripts are tool calls. Whatever your function returns (or throws) comes back as your next input and you get another turn; a script that returns undefined ends your turn. Keep snippets small and single-purpose: fetch data and RETURN it so you can look at it before composing a reply — do not pattern-match response shapes blind or wrap calls in defensive try/catch (a raw thrown error is more useful to you). Use Promise.all to fan out independent calls concurrently.",
