...
/**
 * A child agent reports to the agent that addressed it. The suffix rides AFTER
 * whatever base prompt the child gets — including a caller-supplied override —
 * so a custom persona still gets the parent-reporting convention.
 */
function childAgentSystemPromptSuffix(parentAgentPath: string): string {
  return [
    `You are a child agent of ${parentAgentPath}. It delegated work to you, and its messages arrive as your inputs ("Message from agent ${parentAgentPath}: ...").`,
    `Nobody is watching your web chat — do NOT use itx.chat.sendMessage. Report to your parent with await itx.agents.get(${JSON.stringify(parentAgentPath)}).message(text).`,
    ...
    "You may delegate further: await itx.agents.get('<name>').message(task) births a child agent under your own path (relative paths resolve against your path), and its reports arrive as your inputs.",
  ].join("\n");
}
...
export function agentDefaultsForPath(input: {
  ...
  const systemPrompt =
    parentAgentPath === null
      ? basePrompt
      : `${basePrompt}\n\n${childAgentSystemPromptSuffix(parentAgentPath)}`;
    ...
    // wake the LLM by itself.
    {
      ...
      payload: {
        content: [
          ...
          "- Message another agent with await itx.agents.get(path).message(text) — sent from your agent scope it arrives as that agent's input labeled with your path, and replies land in your inputs. Paths without a leading / resolve relative to your own path with filesystem semantics ('..' climbs). To delegate work, message a child agent into existence: await itx.agents.get('researcher').message(task) — the first message births an ordinary agent at <your path>/researcher that knows you are its parent, and its reports arrive back as your inputs. Customize one with await itx.agents.get(path).configure({ systemPrompt, model }) (works before its first message too).",
          "- GIVE an agent tools by mounting capabilities on its scope: await itx.agents.get(path).provideCapability({ path: ['toolName'], type: 'itx-expression', expression: [...], instructions: '...' }) — durable, survives your session; the receiving agent then calls it as itx.toolName(...) in its own scripts. From YOUR side a fetched handle exposes only its built-in members (message/ask/configure/stream/...); reach the agent's dynamic capabilities through the capabilityHost property: await itx.agents.get(path).capabilityHost.toolName(args).",
