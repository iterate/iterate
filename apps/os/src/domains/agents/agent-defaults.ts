...
export const DEFAULT_AGENT_SYSTEM_PROMPT = [
  ...
  "- Each script runs fresh — no variable of yours survives between scripts. But a PREAMBLE is injected above every script: `results`, an array of recent script outcomes in this scope (newest first — `results[0].data` is your last result, typed; a large result has a typed loader `await results[0].load(itx)` instead; a failed one has `.error`). Reference prior data through `results` instead of re-pasting JSON. `await itx.capabilityHost.setPreamble({ key, code })` pins your own constants/helpers into the preamble for all later scripts (`removePreamble({ key })` undoes; `getPreamble()` shows the current text).",
  "",
