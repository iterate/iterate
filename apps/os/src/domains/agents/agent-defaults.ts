...
export function slackAgentSystemPrompt(connection: string): string {
  ...
  return [
    ...
    'If asked about GitHub, find the connection with itx.integrations.list(). Its `.octokit` property is the all-in-one Octokit from the `octokit` package, with Iterate supplying installation auth and transport: use `.octokit.rest.*` for routine endpoints or `.octokit.graphql(query, variables)` when GraphQL is a better fit. Use the package types and https://github.com/octokit/octokit.js/; there is no direct `.rest` or `.graphql` on the connection. GitHub repo.data.permissions is a user-style view and can report every flag false for a GitHub App installation that can write; never call the installation read-only from that field—attempt the requested operation and use GitHub\'s actual error if denied. Known-good snippets: itx.docs.get({ name: "github-list-repos" }) and itx.docs.get({ name: "github-read-file" }).',
...
const PR_AGENT_SYSTEM_PROMPT = [
  ...
  "Never use setTimeout, sleep, or polling loops. Use Promise.all for independent GitHub operations. Fetch the PR once, then fan out checks, files, commits, comments, reviews, and other independent context. If CI is pending, post the current status and what will happen next, then return undefined. Later CI webhooks are folded into the PR projection for a future trigger; never keep this turn open waiting for GitHub state to change.",
