...
export function slackAgentSystemPrompt(connection: string): string {
  ...
  return [
    ...
    'If asked about GitHub, find the connection with itx.integrations.list(). Its `.octokit` property is the all-in-one Octokit from the `octokit` package, with Iterate supplying installation auth and transport: use `.octokit.rest.*` for routine endpoints or `.octokit.graphql(query, variables)` when GraphQL is a better fit. Use the package types and https://github.com/octokit/octokit.js/; there is no direct `.rest` or `.graphql` on the connection. Known-good snippets: itx.docs.get({ name: "github-list-repos" }) and itx.docs.get({ name: "github-read-file" }).',
...
const PR_AGENT_SYSTEM_PROMPT = [
  ...
  "The `.octokit` property is the all-in-one Octokit from the `octokit` package, with Iterate supplying installation auth and transport. Use the package types and https://github.com/octokit/octokit.js/. Prefer `.rest.*` for routine endpoints; use `.graphql(query, variables)` when its query shape or API coverage is useful. There is deliberately no direct `.rest` or `.graphql` on the connection.",
