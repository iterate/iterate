...
import {
  ...
  reduceVoiceAgentEvent,
  VoiceAgentProcessorContract,
  type VoiceAgentSetup,
  type VoiceAgentState,
} from "./contract.ts";

type VoiceAgentStreamApi = {
  append(args: {
    event: { type: string; idempotencyKey?: string; payload: unknown };
  }): Promise<unknown>;
};
...
export class VoiceAgentProviderProcessor extends StreamProcessor<
  typeof VoiceAgentProcessorContract,
  VoiceAgentProcessorDeps
> {
  readonly contract: typeof VoiceAgentProcessorContract;
  ...
  constructor(
    deps: StreamProcessorConstructorArgs<
      typeof VoiceAgentProcessorContract,
      VoiceAgentProcessorDeps & {
    ...
    this.contract = {
      ...VoiceAgentProcessorContract,
      slug: deps.processorSlug,
      description: `${providerLabel(deps.provider)} realtime voice provider adapter for the canonical voice-agent stream protocol.`,
    } as typeof VoiceAgentProcessorContract;
  }

  protected override reduce(
    args: Parameters<StreamProcessor<typeof VoiceAgentProcessorContract>["reduce"]>[0],
  ): VoiceAgentState {
    return reduceVoiceAgentEvent({ state: args.state, event: args.event });
  }

  protected override processEvent(
    args: Parameters<StreamProcessor<typeof VoiceAgentProcessorContract>["processEvent"]>[0],
  ): void {
    const { event, state } = args;
    const streamApi = this.#streamApi();
    const getConnection = (setup: VoiceAgentSetup) => this.#getConnection({ setup, streamApi });
    ...
    switch (event.type) {
      ...
      case VOICE_AGENT_INPUT_AUDIO_FRAME_APPENDED_EVENT_TYPE:
        args.runInBackground(() =>
          forwardInputAudioFrame({
            deps: this.deps,
            event,
            getConnection,
            provider: this.provider,
            rememberInputStreamId: (streamId) => {
              this.#lastInputStreamId = streamId;
            },
            state,
            streamApi,
          }),
  ...
  async #getConnection(args: {
    ...
    if (
      this.#connection?.provider === args.setup.provider &&
      this.#connection.socket.readyState === ProviderConnectedReadyState
    ) {
    ...
    this.#connection?.socket.close(1000, "Voice-agent provider changed.");
    this.#openingConnection = openProviderConnection({
      deps: this.deps,
      setup: args.setup,
