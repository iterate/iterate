...
export type VoiceAgentConsumedEvent = ConsumedEvent<typeof VoiceAgentProcessorContract>;

export function reduceVoiceAgentEvent(args: {
  state: VoiceAgentState;
  event: VoiceAgentConsumedEvent;
}): VoiceAgentState {
...
export function reduceVoiceAgentEvents(args: {
  ...
  let state = args.state ?? getInitialProcessorState(VoiceAgentProcessorContract);
  for (const event of args.events) {
    ...
    }).parse(event) as VoiceAgentConsumedEvent;
    state = reduceVoiceAgentEvent({ state, event: parsed });
  }
  return state;
}
