...
// ---------------------------------------------------------------------------
// Restricted surfaces (domains/itx/surface.ts): the checked `Itx` is a Pick of
// the surface's roots, so a removed built-in is a compile error, not a
// runtime "no capability" — the model reads the wall before the script runs.

test("a surfaced scope's Itx keeps the allowed roots and drops the rest", async () => {
  const surface = ["chat.sendMessage", "docs"];
  const allowed = await checkItxScriptForExecution({
    capabilities: [],
    code: 'async (itx) => { await itx.chat?.sendMessage("hi"); await itx.docs.search({ q: "x" }); }',
    surface,
    typechecker,
  });
  expect(allowed).toMatchObject({ verdict: "clean" });

  const removed = await checkItxScriptForExecution({
    capabilities: [],
    code: 'async (itx) => itx.repo.readFile("/AGENTS.md")',
    surface,
    typechecker,
  });
  expect(removed.verdict).toBe("problems");
  const problems = (removed as { problems: string[] }).problems.join("\n");
  expect(problems).toContain("ItxMemberRemovedFromThisScope");
  expect(problems).toContain("exposes only: chat, docs");
});

test("a surfaced scope keeps preamble attribution: preamble errors still run unchecked", async () => {
  // The surface adds prelude lines above the preamble; the attribution range
  // must follow them, or a preamble error would be blamed on the script.
  const checked = await checkItxScriptForExecution({
    capabilities: [],
    code: "async () => 1",
    preamble: "const broken: number = 'not a number';",
    surface: ["chat"],
    typechecker,
  });
  expect(checked).toMatchObject({ verdict: "unchecked" });
});

test("a surfaced scope still composes its mounts into Itx", async () => {
  const checked = await checkItxScriptForExecution({
    capabilities: [
      {
        type: "itx-call",
        path: ["catalogue"],
        instructions: "search",
        types: "export type Catalogue = { search(q: string): Promise<{ name: string }[]> };",
      } as CapabilityDescription,
    ],
    code: 'async (itx) => (await itx.catalogue.search("flo")).map((r) => r.name)',
    surface: ["chat"],
    typechecker,
  });
  expect(checked).toMatchObject({ verdict: "clean" });
...
test("a surface naming a non-member root cannot brick the check", async () => {
  const checked = await checkItxScriptForExecution({
    capabilities: [],
    code: "async () => 1",
    surface: ["notAProjectMember"],
    typechecker,
  });
  expect(checked).toMatchObject({ verdict: "clean" });
});
