...
> {
  ...
  protected override processEvent({
    ...
    if (event.type === "events.iterate.com/telegram-agent/created") return;
      ...
      case "events.iterate.com/telegram/webhook-received": {
        ...
        blockProcessorWhile(async () => {
          ...
          if (
            ...
            await this.#sendTyping(birthCertificate.config.connection, target);
          }
        });
        return;
      }
  ...
  protected override async processEventBatch(
    ...
    const { birthCertificate } = args.state;
    ...
    args.blockProcessorWhile(async () => {
      await this.#sendTyping(birthCertificate.config.connection, {
        chatId,
        messageThreadId,
      });
    });
