...
// =============================================================================
// Approval outcome delivery
// =============================================================================

/** A batch held for an agent thread's script run — the shape the egress door commits. */
const AGENT_BATCH_REQUESTED = {
  ...
  payload: {
    ...
    ruleKey: "gmail-hold",
    ruleDescription: "Gmail reads need a human",
    streamContext: {
      kind: "script-execution",
      streamPath: "/agents/mobile-thread",
      scriptRunRequestedEventOffset: 7,
      executionId: "exec-1",
    },
...
describe("ProjectProcessor approval outcome delivery", () => {
  it("appends a human decision as developer context on the originating agent thread", async () => {
    const h = makeProjectHarness();
    await h.play(
      ["append", PROJECT_CREATE_REQUESTED, PROJECT_CREATED],
      ["append", AGENT_BATCH_REQUESTED],
    );
    ...
    const decided = h.events("events.iterate.com/project/human-approval-decided")[0]!;
    expect(h.network.eventsAt("/agents/mobile-thread")).toMatchObject([
      {
        type: "events.iterate.com/agents/context-added",
        // Keyed on the DECIDED event's offset: a redelivered frame dedupes.
        // (Sibling-stream appends also get a fan-in-safe source-stream suffix.)
        idempotencyKey: expect.stringContaining(`project/approval-outcome@/:${decided.offset}`),
        payload: {
          role: "developer",
          // The integration actor is non-script, so the agent processor's
          // waiting-clear treats this as an external wake.
          actor: { type: "integration", name: "egress-approvals" },
          llmRequestPolicy: { behaviour: "after-current-request" },
          content: expect.stringContaining('rule "gmail-hold"'),
        },
      },
    ]);
    const content = (h.network.eventsAt("/agents/mobile-thread")[0]!.payload as { content: string })
      .content;
    expect(content).toContain("GET www.googleapis.com: approved");
  });

  it("delivers the human's rejection reason to the agent thread", async () => {
    const h = makeProjectHarness();
    await h.play(
      ["append", PROJECT_CREATE_REQUESTED, PROJECT_CREATED],
      ["append", AGENT_BATCH_REQUESTED],
    ...
    const [item] = h.network.eventsAt("/agents/mobile-thread");
    expect(item).toMatchObject({
      type: "events.iterate.com/agents/context-added",
      payload: { role: "developer" },
    });
    const content = (item!.payload as { content: string }).content;
    expect(content).toContain("GET www.googleapis.com: rejected");
    expect(content).toContain("wrong recipient — use the staging address");
  });

  it("delivers an expiry auto-rejection so a parked agent learns the batch died", async () => {
    const h = makeProjectHarness();
    await h.play(
      ["append", PROJECT_CREATE_REQUESTED, PROJECT_CREATED],
      ["append", AGENT_BATCH_REQUESTED],
  ...
  it("leaves scope-hold batches alone — only script-execution agent provenance is delivered", async () => {
    ...
    expect(h.network.eventsAt("/agents/mobile-thread")).toEqual([]);
    // Nothing else was appended anywhere on the root either.
    expect(h.events()).toHaveLength(eventCountBefore + 1);
  });

  it("ignores an unsigned approval once a key is enrolled, mirroring the door", async () => {
    const h = makeProjectHarness();
    await h.play(
      ["append", PROJECT_CREATE_REQUESTED, PROJECT_CREATED],
      [
        ...
        {
          type: "events.iterate.com/project/human-approval-key-added",
          payload: { keyId: "key-1", publicKey: "AAAA", label: "phone" },
        },
        AGENT_BATCH_REQUESTED,
      ],
    );
    ...
    // The door ignores this decision (missing signature), so relaying it
    // would tell the agent about an approval that never released anything.
    expect(h.network.eventsAt("/agents/mobile-thread")).toEqual([]);
  });
});
