...
import { ItxAuthContext, trustedInternalAuthContext } from "./auth.ts";
...
// One shared demo project (created once); each session mounts its capability at
// its own path so concurrent sessions don't collide on the same handle.
const DEMO_PROJECT_ID = "prj_page_debugging_demo";
const DEMO_PROJECT_SLUG = "page-debugging-demo";
...
export async function handlePageDebuggingDemoRequest(input: {
  ...
  // oxlint-disable-next-line iterate/no-raw-durable-object-binding-access -- POC demo owns this DO
...
export class PageDebuggingDemoDurableObject extends DurableObject<Env> {
  // POC auth: short-lived HMAC tokens held in memory (not DO storage). When the
  // DO evicts, live sessions end — fine for a demo. Keyed by token id (jti).
  readonly #tokens = new Map<string, TokenClaims>();
  // The shared demo project is created once; cache the in-flight/resolved id so
  // we don't re-run the (heavy) create saga per session.
  #demoProject?: Promise<string>;

  #ensureDemoProject(): Promise<string> {
    this.#demoProject ??= (async () => {
      const projects = new ProjectCollectionRpcTarget({
        auth: trustedInternalAuthContext(),
        ctx: this.ctx,
      });
      await projects.create({ projectId: DEMO_PROJECT_ID, slug: DEMO_PROJECT_SLUG });
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === SESSION_PATH) {
      ...
      return Response.json(
        await createDemoSession(this.env, this.#tokens, await this.#ensureDemoProject(), request),
    ...
    if (url.pathname === REVOKE_PATH) {
      ...
      const claims = await verifyDemoToken(this.env, this.#tokens, tokenFromAuthorization(request));
      if (!claims) return Response.json({ error: "unauthorized" }, { status: 401 });
      return Response.json(revokeDemoTokens(this.#tokens, claims), {
        headers: { "access-control-allow-origin": "*" },
      });
    }

    if (url.pathname === CONNECT_PATH) {
      const claims = await verifyDemoToken(this.env, this.#tokens, tokenFromProtocol(request));
      if (!claims) return new Response("unauthorized", { status: 401 });

      const response = newWorkersWebSocketRpcResponse(
        request,
        new ItxRpcTarget({
          auth: new ItxAuthContext({
            isAdmin: false,
...
async function createDemoSession(
  ...
  tokens.set(providerClaims.jti, providerClaims);
  tokens.set(agentClaims.jti, agentClaims);
...
async function verifyDemoToken(
  ...
  // Signature alone isn't enough: the token id must still be live in memory, so
  // "Stop sharing" (which drops it) and DO eviction both end the session.
  const stored = tokens.get(claims.jti);
  if (
    !stored ||
...
function deleteExpiredTokens(tokens: Map<string, TokenClaims>) {
  const now = Date.now();
  for (const [jti, claims] of tokens) if (claims.exp < now) tokens.delete(jti);
}

function revokeDemoTokens(tokens: Map<string, TokenClaims>, claims: TokenClaims) {
  // Revoke just this session's tokens (same mount path), not everyone's.
  let revoked = 0;
  for (const [jti, tokenClaims] of tokens) {
    if (
      tokenClaims.projectId === claims.projectId &&
      tokenClaims.path.join(" ") === claims.path.join(" ")
    ) {
      tokens.delete(jti);
      revoked += 1;
    }
...
async function sign(env: Env, body: string) {
  const secret = env.SECRET_ENCRYPTION_KEY ?? "page-debugging-demo-fallback";
  ...
  <body>
    ...
    <script type="module">
      ...
      async function runSnippetHere() {
        ...
        document.getElementById("__itx_page_debugging_widget")?.remove();
        targetStatus.textContent = "Attaching bridge...";
        await (0, eval)(snippetEl.value);
        // The bridge iframe mounts the capability asynchronously; its widget
        // appears once it is ready. Wait for that before reporting connected.
        const readyAt = Date.now();
        while (!document.getElementById("__itx_page_debugging_widget") && Date.now() - readyAt < 10000) {
...
const PAGE_DEBUGGING_HOST_SNIPPET_BODY = String.raw`
  ...
  // Build DOM with createElement/createElementNS, never innerHTML: strict hosts
  // enable Trusted Types (require-trusted-types-for 'script'), which throws on
  // any string assignment to innerHTML.
  ...
  function installWidget() {
    ...
    headerTitle.style.cssText = "font-weight:750";
    headerTitle.textContent = "Sharing with ITERATE";
    const headerSub = document.createElement("div");
    headerSub.style.cssText = "color:#52606d;font-size:12px;margin-top:3px";
    headerSub.textContent = "Mounted at " + ITX.path.join(".");
    header.append(headerTitle, headerSub);
