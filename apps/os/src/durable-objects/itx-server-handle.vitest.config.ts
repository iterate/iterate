...
const appRoot = process.env.OS_ITX_SERVER_HANDLE_TEST_APP_ROOT ?? process.cwd();
...
export default defineConfig({
  ...
  plugins: [
    cloudflareVitest.cloudflareTest({
      main: resolve(testRoot, "itx-server-handle-test-entry.ts"),
      miniflare: {
        serviceBindings: {
          HARNESS: {
            entrypoint: "ServerItxHarness",
            name: miniflare.kCurrentWorker,
          },
        },
      },
      wrangler: {
        configPath: resolve(testRoot, "itx-server-handle.wrangler.vitest.jsonc"),
      },
    }),
  ],
  test: {
    ...
    hookTimeout: 60_000,
    include: [resolve(testRoot, "itx-server-handle.test.ts")],
    testTimeout: 60_000,
  },
});
