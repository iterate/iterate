...
const appRoot = process.env.OS_ITX_STREAM_SUBSCRIBE_TEST_APP_ROOT ?? process.cwd();
...
export default defineConfig({
  ...
  plugins: [
    cloudflareVitest.cloudflareTest({
      main: resolve(testRoot, "itx-stream-subscribe-test-entry.ts"),
      miniflare: {
        serviceBindings: {
          HARNESS: {
            entrypoint: "ItxStreamHarness",
            name: miniflare.kCurrentWorker,
          },
        },
      },
      wrangler: {
        configPath: resolve(testRoot, "itx-stream-subscribe.wrangler.vitest.jsonc"),
      },
    }),
  ],
  test: {
    ...
    hookTimeout: 60_000,
    include: [resolve(testRoot, "itx-stream-subscribe.test.ts")],
    testTimeout: 60_000,
  },
});
