// src/ingress-routes.ts — A PROJECT'S INGRESS ROUTES: named rules on the project's root `/` that say
...
// `ingress-route/configured`, and THIS FILE is the only place it is spelled, folded into the route
// table (`reduceIngressRouteConfigured`) and matched against a request (`matchIngressRoute`), all
// pure. The table is part of the root's CORE STATE (stream/core-processor.ts `ingressRoutes`), reduced
// inline with every commit, so reading it is a memory read. The verbs are the built-in
// `itx.ingressRoutes` (context/built-ins.ts): `set` validates here and appends the fact, `list`,
...
const IngressRouteRequestMatcher = z.strictObject({
  routingSlug: z.string().min(1).optional(),
  url: UrlPatternInit.optional(),
  headers: z.record(z.string().min(1), z.string()).optional(),
});
type IngressRouteRequestMatcher = z.infer<typeof IngressRouteRequestMatcher>;
...
/** The one fact's payload — what `itx.ingressRoutes.set` validates before it appends, and what the
 ...
 *  of that fact. What `itx.ingressRoutes.list()` answers and `match` reads. */
export type IngressRouteTable = Record<
  string,
  {
    requestMatcher: IngressRouteRequestMatcher;
    target: ItxExpression;
    authRequirement: z.infer<typeof IngressRouteAuthRequirement> | null;
    priority: number;
