...
address that can be resolved into a callable). Project and Agent Durable
...
| **sturdy** | `{ type: "rpc", … }` | rebuilt on demand by the host        | yes — the address is in the log     |
...
Its inverse is `PathInvoker`: a tiny adapter that takes a normal object and
answers `invokeCapability` by replaying the path onto that object. Host DOs use
`new PathInvoker(this)` as the built-in at `path: []`; the edge uses
`pathInvokerToProxy(...)` to serve an invoker as a dotted Cap'n Web surface.
...
## Address resolution — address → stub

The host Durable Object turns a durable capability address back into a callable,
...
(`itx.ts`). Host surfaces such as `egress`, `repo`, `agents`, `whoami`, and
`project` are live runtime targets constructed by the host Durable Object from
its own Project ID. What a user CAN provide — live caps and
`dynamic-worker`/`dynamic-durable-object` — carries no cross-project reach:
dynamic code receives only this context's scoped `env.ITX`, and repo-backed
sources are resolved through the host project's repo.

## Built-in capabilities — the domain objects

A context is born with one base capability defined by the host **domain object**:
`{ path: [], capability: new PathInvoker(this) }`. `ProjectDurableObject` offers
public members such as `egress`, `repo`, and `agents`; `AgentDurableObject`
offers `whoami`, `sendMessage`, and `project`. Built-ins are handed to the
`ItxProcessor` constructor as the same `ProvideArgs` shape a provide uses, but
they are live host-owned targets, not stored topology refs. Own provides shadow
the base host surface by longest-prefix matching; changing built-ins is a code
...
the selected Project/Agent host's `runScript({ code })` and hands it an **itx
handle** so it can invoke and provide against the very context that launched it.
...
- user provides cannot name another project's Durable Object (the trusted address
...
| `server.ts`                                 | the Worker: `PathInvoker`/`pathInvokerToProxy`, embedded Project/Agent itx hosts, dynamic worker/facet loading, the `/api/itx` route; re-exports the real `Stream` DO |
...
| `itx.cross-project-adversarial.e2e.test.ts` | the cross-project isolation attacks (trusted-address rejection, connect-door denial, `__null__` refusal)                                                              |
