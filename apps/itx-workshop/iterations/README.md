...
| `v12-itx-delivery-channels.md`   | **`itx` has two delivery channels** (corrects v11, which only covered isolates): a **binding** (`env.ITERATE`) for itx-hosted isolates; a **function argument** for **live external** caps (`env` can't reach a foreign Node process). The clean shape for live caps is a **capability factory** `(itx) => surface` — itx is the factory's arg, methods stay clean, raw SDK mounts take no itx. 🔬 the live-cap channel is _not wired today_ (`#borrow` passes a live cap nothing).                                                                                                                                                                                 | 12                   | +30          |
