...
> **the one-sentence version** — _itx is a self-hosting **capability**._ There is one primitive: a capability — a `PathCallable`, anything with `invoke({ path, args })`. A **context** is just a capability that hosts other capabilities; `provide`/`invoke` are its own methods and the registry is how it answers `invoke`. Everything below builds that one object up; the Step 13 "punchline" is really that the thing you've built satisfies the very interface it dispatches to — _a context is a capability, recursively._
...
That constant name (`"itx"`) is really the context's **address**. In production it's a coordinate `<namespace>:/<path>` (e.g. `prj_abc:/agents/foo`) that doubles as the DO's name _and_ the dial address — the same identity Steps 11–13 lean on. For now, one constant name = one shared context.
...
## Step 12 — Deep shadowing across the chain: `origin`
...
## Step 13 — Itx _is_ a StreamProcessor
