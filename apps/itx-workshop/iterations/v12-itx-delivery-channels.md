...
// doProjectThing runs in the PROJECT, but its egress is routed through `origin`'s
// chain, not its home chain. Two ways egress reaches the `fetch` cap (see Step 14):
async function doProjectThing(itx) {
  await itx.fetch("https://api.example.com"); // explicit — works for ANY cap (live or dialed)
  await fetch("https://api.example.com"); // bare — auto-routes ONLY inside an OS-loaded isolate
}
```

So a `fetch` shadow on the agent catches the egress of **everything invoked in the agent's service** that goes through the `fetch` cap — including inherited project caps — exactly what you want when the agent must attenuate its inherited authority (log every call, inject a budget, deny a host). Without `origin`, the shadow catches only the agent's _own_ egress: name-resolution, not deep interposition.
...
```ts
// ── base: the PROJECT provides the egress cap + an API caller that USES it ──
project.provide({ name: "fetch", capability: globalThis.fetch }); // the shadowable egress cap

// the API caller is a WorkerEntrypoint. The platform injects an ORIGIN-scoped
// `itx` as a binding — `env.ITERATE` — so the cap "just gets itx" and routes
// egress through it. No itx threading, no trailing arg. (This is what production's
// OpenApiClient does — capabilities/openapi-client.ts.)
export class PetApi extends WorkerEntrypoint<Env> {
  async getPet(id) {
    const res = await this.env.ITERATE.fetch(`https://petstore.example/pet/${id}`); // the fetch CAP
    return res.json();
  }
}
project.provide({
...
await agent.petstore.getPet(1);
// agent has no `petstore` → climbs to the project → PetApi.getPet runs, and its
// env.ITERATE.fetch(...) dispatches the `fetch` cap down the ORIGINATING (agent)
// chain (env.ITERATE is scoped to origin = agent) → finds the agent's shadow
// FIRST (longest-prefix) → logs, then super = the project's real fetch:
//
...
- **Inheritance (Step 12):** the agent never copied `petstore` — it climbed to the project by reference.
- **Shadow + super (Step 10):** the agent's `fetch` logs and calls `agent.super.fetch` (the project's), never itself.
- **`origin` (Step 13):** `PetApi`'s `env.ITERATE.fetch(...)` resolves the `fetch` name down the **originating (agent)** chain — because `env.ITERATE` is scoped to `origin = agent` — so the agent's shadow wins. The dotted form `agent.petstore.…` attaches that `origin` automatically (you pass it by hand only when calling `invoke` directly, Step 13).

### 🔌 How a capability gets its `itx` (ergonomics)

`PetApi` never imported or was passed an `itx` — it read **`env.ITERATE`**. The platform injects an **origin-scoped `itx`**; how it arrives depends on **where the cap runs**, and there are two channels for the _same_ thing:

| the cap runs in…                                                                         | how it gets `itx`                                                                                   |
| ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| an **itx-hosted isolate** (a dialed `source`/`loopback` cap, a `/api/itx/run` script)    | a **binding** — `env.ITERATE`, injected by `isolate.ts`, keyed by `origin`                          |
| a **live external process** (e.g. a Node client that `provide`d the cap over the socket) | a **function argument** — there's no `env` to inject, so an origin-scoped `itx` must be _passed in_ |

For the live case `env.ITERATE` is impossible — the platform can't wire a binding into someone else's Node/browser isolate. So a live cap that wants attenuable egress takes `itx` as an argument. The clean shape is a **capability factory** — `itx` is the factory's argument, so the methods stay pristine:

```ts
// LIVE Node client — itx arrives as the factory's arg; methods are clean:
provide({
  name: "petstore",
  capability: (itx) => ({
    async getPet(id) {
      return (await itx.fetch(`/pet/${id}`)).json();
    },
  }),
});
```

Why not "`itx` as a trailing argument on every method"? Because it would **corrupt mounted SDKs**: itx routes `itx.slack.chat.postMessage(msg)` straight onto the real `@slack/web-api` client — appending `itx` → `postMessage(msg, itx)` and the SDK chokes. The factory puts `itx` _outside_ the method signatures; a raw SDK mount (`provide({ name: "slack", capability: new WebClient(token) })`) takes no `itx` at all — and its egress is the client's own (the un-attenuable case from above). So: a **binding** for hosted isolates, a **factory argument** for live caps, and **never** a per-method trailing arg.

> 🔬 **proposed for the live case.** `env.ITERATE` is shipped; the factory-argument channel is **not wired today** — `#borrow` (itx.ts) passes a live cap nothing (no `env`, no `origin`), so right now a live external cap's egress isn't agent-attenuable. Adding it means the live-cap dispatch invokes the factory with an origin-scoped `itx`.

> ✅ **what actually makes interception work** — and what doesn't. The agent shadows the **`fetch` capability**, not the petstore cap. A cap's egress is caught **whenever that egress is a dispatch of the `fetch` cap on the originating chain**, and there are two doors to it:
>
> - **explicit** — the cap calls `itx.fetch(...)`. Works for **any** cap — **live inbound or dialed** — and is what this example (and production's `OpenApiClient`) does.
> - **implicit** — a bare `fetch()` inside an **OS-loaded isolate** (a dialed `source` cap, or a `/api/itx/run` script) auto-routes via the isolate's `globalOutbound` → the egress pipe → the `fetch` cap.
>
> The _one_ egress nobody can intercept is a **truly external live provider process** calling its own OS `fetch()` — that never enters the platform. So it is **not** "dialed vs live" — it's "does the egress go through the `fetch` cap." (An earlier draft of this step said it needed a dialed worker; that was wrong — a live inbound cap that calls `itx.fetch` is attenuated just the same.)
>
> Two mechanics it leans on: the cap's `itx` is **origin-scoped** (the dial/handle wires its `context = origin`, so `itx.fetch` starts resolving at the agent, not the project), and `agent`/`project` here are full **handles** (`ItxHandle`) — `agent.super.fetch` resolves `fetch` _above_ the agent, skipping its own shadow, so there's no loop.

Call the same `getPet(1)` directly on the project (no agent origin) and **nothing logs** — the shadow is the agent's, reached only via origin. That's the whole egress-attenuation story: a base capability you didn't write, invoked through an agent that transparently logs (or rate-limits, or denies) its egress.
