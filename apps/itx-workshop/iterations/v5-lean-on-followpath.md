...
This is the recursive half of a **membrane** (Cap'n Proto `membrane.h`): a wrapper that re-wraps only the top-level call but lets a sub-capability's ambient `fetch()` escape "can be bypassed by merely introducing new objects." `origin` is what makes the agent context a _real_ membrane over the project context, not a shallow name-shadow. (The ocap-idiomatic alternative — bind the agent's `fetch` into the inherited cap at delegation time, rather than threading a call-time tag — is a later refactor: same effect, authority-in-the-reference instead of an ambient `origin` field.)
