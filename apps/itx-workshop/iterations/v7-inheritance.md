...
> **the one-sentence version** — _itx is a self-hosting **capability**._ There is one primitive: a capability — a `PathCallable`, anything with `invoke({ path, args })`. A **context** is just a capability that hosts other capabilities; `provide`/`invoke` are its own methods and the registry is how it answers `invoke`. Everything below builds that one object up; the Step 14 "punchline" is really that the thing you've built satisfies the very interface it dispatches to — _a context is a capability, recursively._
...
That constant name (`"itx"`) is really the context's **address**. In production it's a coordinate `<namespace>:/<path>` (e.g. `prj_abc:/agents/foo`) that doubles as the DO's name _and_ the dial address — the same identity Steps 11–14 lean on. For now, one constant name = one shared context.
...
itx.provide({
  ...
  capability: {
    ...
    sealedFor: itx.address, // SEALED to this context — see below
  },
...
`dial` is Cap'n Proto's **`restore`**: it turns a SturdyRef back into a live `PathCallable` — but only after **checking the seal**, so a leaked ref a third party can't restore (persistent.capnp's _distributed confinement_: "the third-party code never sees the bits of the SturdyRefs"). The entrypoint's `props` arrive as `this.ctx.props` (that's how Step 7's `OpenApiClient` gets its `{ openApiSpec, baseUrl }`); `loadWorker` caches the built isolate by content; `ref.worker.type` is `"source"` here, but also `"binding"`, `"loopback"`, or `"durable-object"`. The seal **replaces the hand-curated dial allowlist**: instead of itx maintaining a list of which capability classes are "safe for anyone to provide," the authority is in the ref — anyone may provide a ref, but it only restores for the context it's sealed to. (You still want dial-time prop injection too: sealing gates _who may restore_; injected props gate _what the restored cap reaches_.)
...
// resolve a path against an ordered list of resolvers; FIRST HIT WINS.
//   within a context: longest-prefix order  → Step 6's deep-shadow
//   across the chain: self, then parent      → this step's inherit
// the SAME rule — so "shadow" and "inherit" stop being separate concepts.
...
const context = firstOf([ownProviders(caps), parent]);
```
...
So the whole surface reduces to **two verbs**: `provide` (the one write) and `invoke` (the one read+dispatch); everything else is derived. And `firstOf` is a **membrane**: putting the parent _behind_ the child lets the child wrap/shadow any inherited capability — and a revoking membrane placed _in front_ kills everything delegated through it at once, which a per-name `revoke` can't express.
...
agent.invoke({ path: ["doProjectThing"], args, origin: agent.address });
...
## Step 14 — Itx _is_ a StreamProcessor
...
- **Sealing replaces the dial allowlist** — a SturdyRef carries `sealedFor`; `restore` checks it (Step 9), so "anyone with a handle can provide a ref" stops being a hazard — the ref only restores for the context it's sealed to. Dial-time prop injection (`capabilityPath`, `context`, `projectId`) still overwrites caller-supplied props: sealing gates _who may restore_, injection gates _what the restored cap reaches_.
