...
project.provide({
  name: "petstore",
  // an API caller that routes egress through `itx.fetch` — NOT bare fetch(). It
  // can be a loopback/source worker OR a LIVE cap an inbound client provided;
  // what matters is the itx.fetch call, not where the code runs. (Production's
  // OpenApiClient does exactly this — capabilities/openapi-client.ts.)
  capability: {
    async getPet(id) {
      const res = await itx.fetch(`https://petstore.example/pet/${id}`); // ← the fetch CAP, not bare fetch()
      return res.json();
    },
  },
});
...
// agent has no `petstore` → climbs to the project → getPet runs, and its
// itx.fetch(...) dispatches the `fetch` cap down the ORIGINATING (agent) chain
// (the cap's itx handle is scoped to origin = agent) → finds the agent's shadow
...
- **`origin` (Step 13):** `getPet`'s `itx.fetch(...)` resolves the `fetch` name down the **originating (agent)** chain — because the cap's `itx` handle is scoped to `origin = agent` — so the agent's shadow wins. The dotted form `agent.petstore.…` attaches that `origin` automatically (you pass it by hand only when calling `invoke` directly, Step 13).
