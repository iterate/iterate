...
```ts
// ── base: the PROJECT context provides real egress + a cap that USES it ──
project.provide({ name: "fetch", capability: globalThis.fetch }); // real egress
project.provide({
  name: "petstore",
  capability: {
    // a project-level API caller. it just calls bare fetch() — it has NO idea
    // a child might shadow it later:
    async getPet(id) {
      const res = await fetch(`https://petstore.example/pet/${id}`);
      return res.json();
    },
  },
});
...
agent.provide({
  ...
  capability: async (input, init) => {
    ...
    const res = await agent.super.fetch(input, init); // delegate to the project's fetch (NOT agent.fetch → loop)
    const url = typeof input === "string" ? input : input.url;
    console.log(
      `[egress] ${init?.method ?? "GET"} ${url} → ${res.status} (${Date.now() - started}ms)`,
    );
    return res;
  },
});
...
// agent has no `petstore` → firstOf climbs to the project → getPet runs in the
// PROJECT isolate, but its bare fetch() is routed through `origin = agent`'s chain
// → hits the agent's fetch SHADOW → logs:
//
//   [egress] GET https://petstore.example/pet/1 → 200 (38ms)
```
...
- **`origin` (Step 13):** the project cap's bare `fetch()` honored the agent's shadow because the call carried `origin = agent`; name-resolution alone wouldn't have reached it.

Call the same `petstore.getPet(1)` directly on the project (no agent) and **nothing logs** — the shadow is the agent's, reached only via origin. That's the whole egress-attenuation story end to end: a base capability you didn't write, invoked through an agent that transparently logs (or rate-limits, or denies) its egress.
