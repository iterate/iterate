...
`PetApi` never imported or was passed an `itx` — it read **`env.ITERATE`**. The platform injects an **origin-scoped `itx` as a binding** into every cap it loads (`isolate.ts` wires `env.ITERATE` keyed by `origin`, so `env.ITERATE.fetch` resolves on the _caller's_ chain, hitting the agent shadow). That's the ergonomic "just get `itx` inside your capability."

Why a **binding**, not "`itx` as a trailing argument"? Because a trailing arg would **corrupt mounted SDKs**: itx routes `itx.slack.chat.postMessage(msg)` straight onto the real `@slack/web-api` client — appending `itx` would call `postMessage(msg, itx)` and the SDK chokes on the junk arg. A binding leaves the call signature pristine; only caps that _want_ itx read `env.ITERATE`. (A live in-process closure that isn't a `WorkerEntrypoint` captures its `itx` at provide time, or — a possible future nicety — reads an ambient `getItx()`; either way, never a trailing arg.)
