...
> **the one new idea** — stop passing objects as call arguments. Add two verbs: `provide({ name, capability })` registers a capability under a name; `invoke({ name, args })` calls it. **These two are the entire core** — `revoke`, `extend`/`super`, `describe`, and `fetch` are all derived from them later (Step 10): `revoke` = `provide(⊥)`, `extend` = provide-a-parent, `describe`/`super` are reads, `fetch` is provided content. The set of capabilities is now a dynamic registry, grown at runtime. (Every itx verb is a single **bag of props**, matching production — there are no positional signatures.)
...
That constant name (`"itx"`) is really the context's **address**. In production it's a coordinate `<namespace>:/<path>` (e.g. `prj_abc:/agents/foo`) that doubles as the DO's name _and_ the dial address — the same identity Steps 11–15 lean on. For now, one constant name = one shared context.
...
**Lean on the platform.** That manual walk in `invokeCapabilityAtPath` is really two jobs, and only one is itx's. _Selecting_ the capability and the offset (`findCapabilityByPath` → longest prefix) is genuinely itx's. _Traversing_ the rest of the path is Cap'n Web's `followPath`, which already does it recursively, **preserves the receiver** (calls the leaf on its parent), and blocks prototype-pollution / instance-property access for you. So when the selected capability is a real Cap'n Web stub, `invokeCapabilityAtPath` needn't walk at all — it hands the stub back advanced to the offset and lets the kernel's own outer call pipeline the remainder. The hand-rolled walk (production's `replayPathCall`) earns its keep only for plain-object providers; for stubs, the live half is redundant with `followPath`, and most of the reserved-name set duplicates Cap'n Web's own guard.
...
itx.provide({
  ...
  capability: {
    ...
    sealedFor: itx.address, // 🔬 PROPOSED seal (not shipped) — see Step 9
  },
...
That `address` is precisely Cap'n Proto's **SturdyRef**, and the `sealedFor` field is its **seal**: a SturdyRef may be sealed to an owner so that _even if the ref data leaks, an unauthorized party cannot restore it_ (persistent.capnp). The authority travels **with the ref**, not in a separate policy table — which lets `dial`/`restore` (Step 9) enforce it generically and **deletes the hand-curated dial allowlist**.

(iterate ships a cap of this shape — `apps/os/src/itx/capabilities/openapi-client.ts` — though the real one spells the method `call`, takes a spec **URL** it fetches via egress, and is mounted as a **loopback** entrypoint rather than a `source` worker.)
...
**Two tiers, though — don't make string-path dispatch universal.** Reifying every call as `{ path, args }` re-introduces a stringly-typed dispatch under a substrate that does typed dispatch natively — and _every_ reserved-name / `Object.hasOwn` / prototype-pollution guard exists only because a path is a string array rather than a reference. So keep two tiers: **typed** caps (the Slack SDK, the Project DO) are **reference-called** — handed back as stubs and pipelined by Cap'n Web, no path serialization, no reserved-name gate; the **`invoke({ path, args })`** convention is reserved for the genuinely **dynamic** caps where dispatch is data, not a type — the `OpenApiClient` routing by `operationId`, and codemode scripts that can't import types. That narrows the reserved-name attack surface to the script boundary, where the input is already distrusted.
...
```ts
function dial(ref, dialerIdentity) {
  if (ref.type !== "rpc") throw new Error("not dialable");
  // CHECK THE SEAL: the ref's authority travels with it. No separate allowlist.
  if (ref.sealedFor !== dialerIdentity) throw new Error(`sealed for ${ref.sealedFor}`);
...
`dial` is Cap'n Proto's **`restore`**: it turns a SturdyRef back into a live `PathCallable`. The entrypoint's `props` arrive as `this.ctx.props` (that's how Step 7's `OpenApiClient` gets its props); `loadWorker` caches the built isolate by content; `ref.worker.type` is `"source"` here, but also `"binding"`, `"loopback"`, or `"durable-object"`.

> 🔬 **proposed, not shipped — and get the threat model right.** The `sealedFor` check above is a _proposal_; **production gates dialing with a hand-curated allowlist** (`DIALABLE_BINDINGS`/`LOOPBACKS`/`DURABLE_OBJECTS` in `dial.ts`), not a seal. And sealing only works if the seal is an **unforgeable secret** (persistent.capnp's "the third-party code never sees the bits"). Modeling it as `ref.sealedFor === itx.address` over a _public_ coordinate string is **forgeable** — anyone who knows the identity could mint a ref for it. So a seal would have to be a capability/secret token, and even then you'd keep dial-time **prop injection** (`capabilityPath`, `context`, `projectId` overwrite caller props): a seal gates _who may restore_; injection gates _what the restored cap reaches_. Don't delete the allowlist for an `===`-on-a-public-string check — that's a regression, not a simplification.
...
## Step 10 — extend & super: the chain is one `firstOf` combinator

> **the one new idea** — a context can `extend` a parent. On a capability **miss**, dispatch climbs to the parent; a child **shadow** wins over the parent. This is the deep-shadowing trick from Step 6, now up the chain — _the same first-hit rule, at a coarser granularity._ So `extend`/`super` aren't new verbs: they're one combinator, `firstOf`, over `[self, parent]`.

```ts
const MISS = Symbol("miss");

// a RESOLVER answers tryInvoke → result | MISS. ownProviders wraps a context's
// own caps Map as one (longest-prefix from Step 6; a revoked ⊥ tombstone → MISS):
function ownProviders(caps) {
  return {
    tryInvoke({ path, args }) {
      const hit = findCapabilityByPath({ caps, path });
      if (!hit || hit.capability === TOMBSTONE) return MISS; // miss (or revoked) → climb
      return invokeCapabilityAtPath({ capability: hit.capability, path: hit.rest, args });
    },
  };
}

// firstOf composes resolvers, FIRST HIT WINS. It returns BOTH a throwing `invoke`
// (the public verb) AND a `tryInvoke` — so a context is itself a valid resolver in
// a parent. Within a context: longest-prefix order = Step 6's deep-shadow. Across
// the chain: self, then parent = this step's inherit. The SAME first-hit rule.
function firstOf(resolvers) {
  ...
  return {
    tryInvoke,
    invoke(input) {
      const hit = tryInvoke(input);
      if (hit === MISS) throw new Error(`no capability "${input.path.join(".")}"`);
      return hit;
    },
  };
}

// extend(parent) is just composition; "super" is the next resolver, not a verb.
const context = firstOf([ownProviders(caps), parent]); // parent is a firstOf too → has tryInvoke
```

A child provides `slack`; the parent provides `fetch` and `ai`. The child sees all three; if the child also provides `fetch`, its version shadows the parent's — because `firstOf` tries the child first, exactly as longest-prefix tries the more-specific provider first. The chain bottoms out at a code-rooted, read-only platform context whose defaults are loopback refs — the last resolver in the list.

And the four "extra" verbs collapse into the two-verb core:

```ts
revoke = ({ name }) => provide({ name, capability: TOMBSTONE }); // ⊥: the lookup misses → firstOf climbs
extend = (parent) => firstOf([ownProviders(caps), parent]); // "super" = the next resolver (a read, not a verb)
describe = () => mergeViews(getState(), parent?.describe()); // a pure read of the fold, not a stored verb
// fetch is already just a provided capability (Addendum) — never a core verb.
```

So the surface _can_ reduce to **two verbs**: `provide` (the one write) and `invoke` (the one read+dispatch); everything else is derived. You still **write** `extend`, `super`, `describe`, and `fetch` — they're sugar over the two, the way array methods are sugar over a loop. And `firstOf` is a **membrane**: putting the parent _behind_ the child lets the child wrap/shadow any inherited capability — and a revoking membrane _in front_ kills everything delegated through it at once, which a per-name `revoke` can't.

> 🔬 **proposed, not shipped.** `firstOf`, `revoke = provide(⊥)`, and the two-verb collapse are _this workshop's reframing_, not iterate's current code. Production ships **four** verbs — `provideCapability`, `revokeCapability` (a real `capability-revoked` event that deletes the row), `describe`, `invoke` — and inlines the chain climb inside `Itx.invoke` (no `firstOf`). The reframing is sound (validated below); it just isn't the code in `apps/os/src/itx` yet.

### 🚧 Why this isn't enough yet

`firstOf` composed two resolvers — but both lived in one context. Real itx has _separate_ contexts in _separate_ Durable Objects: a project and an agent. Does the same combinator span them? And if the agent shadows `fetch`, does a _project_ capability invoked through the agent honor that shadow?

---

## Step 11 — Many contexts, many DOs: the chain spans Durable Objects

> **the one new idea** — a context is a capability, and `extend` just holds a _reference_ to the parent context. Each context lives in its own DO, so that reference is a cross-DO stub — and `firstOf`'s "next resolver" is simply an RPC to the parent's DO. A **project itx** (project DO) and an **agent itx** (agent-stream DO) compose with zero new machinery.

```ts
// the agent context (agent DO) extends the project context (project DO):
const project = env.ITX.getByName(projectCoordinate); // RpcStub<Itx> — a DIFFERENT DO
const agent = firstOf([ownProviders(agentCaps), project]); // "next resolver" = a cross-DO RPC

await agent.invoke({ path: ["slack", "chat", "postMessage"], args: [msg] });
// agent has no `slack` → firstOf climbs → project.invoke(...) runs in the PROJECT DO
```

Two contexts, two DOs, one rule. The agent provides agent-scoped caps (its own tools, its turn state); the project provides project-scoped caps (Slack, secrets, the project worker); the agent sees the union, **agent-first**. The platform-defaults context is just the last resolver in the list, reached the same way. It's safe by direction: the agent is a _narrower_ context than the project — `firstOf` only ever adds more-specific resolvers _in front_, so a child can shadow/attenuate what it inherits but never reach wider than the project gave it.

### 🚧 Why this isn't enough yet

The agent holds a _reference_ to the project, not a copy of its caps. So what does it actually inherit — and if the project gains a new capability _after_ an agent already exists, does that agent see it? (And if a hundred agents extend one project, is that a hundred copies?)

---

## Step 12 — Inheritance: provide once at the base, visible everywhere (by reference, not by copy)

> **the one new idea** — a child never _copies_ its parent's capabilities. It holds a _reference_ to the parent context and resolves through `firstOf` **at call time**. So a capability `provide`d to the project is instantly visible to _every_ agent that extends it — including agents that existed _before_ the cap was added — with no fan-out, no propagation, nothing to invalidate.

```ts
// an agent created EARLIER already extends the project by reference:
const agent = firstOf([ownProviders(agentCaps), project]); // holds `project`, doesn't copy it

// later, the project gains a capability:
project.provide({ name: "ai", capability: aiClient });

// the agent sees it on its NEXT call — firstOf climbs and re-reads the project's
// CURRENT fold. late binding, not a snapshot:
await agent.invoke({ path: ["ai", "complete"], args: [prompt] }); // → project's ai
```

It falls straight out of the data structure: `firstOf([self, parent])` evaluates `parent.tryInvoke(...)` _per call_, and `parent` is the live project context (a stub to the project DO) whose `getState()` is the _current_ fold. The cap lives **once**, in the project; children share it by reference. Consequences:

- **Add at the base → visible everywhere, instantly.** No "propagate to all children," no stale children.
- **Revoke/replace at the base → propagates instantly.** The next climb sees the new fold; `revoke = provide(⊥)` on the project makes every agent miss it on its next call.
- **One cap, one copy.** A hundred agents are a hundred _references_ to the one project context — not a hundred copies.
- **Shadowing stays local.** An agent that provides its own `ai` shadows the project's for _that agent only_ (firstOf tries the agent first) — the base and every sibling are untouched.

This is the **prototype-chain** property, not by coincidence: a JS prototype doesn't copy methods into every instance — resolution walks the chain at call time. Same here: `extend` just sets the parent link, and inheritance is dynamic lookup, not duplication. The anti-pattern it avoids — copying base caps into each child at creation — would give N copies, a fan-out write on every base change, and staleness the moment the base updates. The cost is one climb per inherited call (a cross-DO RPC, Step 11), cheap and pipelined, in exchange for a single source of truth.

### 🚧 Why this isn't enough yet

`firstOf` only decided which _name_ wins. Suppose the agent shadows `fetch` (to log or rate-limit egress), then invokes a **project** capability — `doProjectThing` — that makes a bare `fetch()` inside. Whose `fetch` does it use? The project cap runs in its own isolate scoped to its **home (project)** context, so its bare `fetch()` resolves to the _project's_ fetch — the agent's shadow never sees it. Name-resolution shadowing isn't a real boundary.

---

## Step 13 — Deep shadowing across the chain: `origin`

> **the one new idea** — to make the agent's `fetch` shadow reach _into_ an inherited capability, the call must carry its **origin**: the context it was invoked _through_. A project cap's bare `fetch()` then dials back through the _originating_ (agent) chain — hitting the agent's shadow first — instead of its home chain.

```ts
// every invoke carries the originating context:
agent.invoke({
  path: ["doProjectThing"],
  args,
  origin: { ref: agent.ref, address: agent.address },
});

// doProjectThing runs in the PROJECT isolate, but its bare fetch() is routed
// through `origin`'s chain, not its home chain:
async function doProjectThing() {
  await fetch("https://api.example.com"); // → origin (agent) chain → agent fetch SHADOW → project fetch
}
```

So a `fetch` shadow on the agent catches the egress of **everything invoked in the agent's service**, including inherited project caps — exactly what you want when the agent must attenuate its inherited authority (log every call, inject a budget, deny a host). Without `origin`, the shadow catches only the agent's _own_ isolates' egress: name-resolution, not deep interposition.

This is the recursive half of a **membrane** (Cap'n Proto `membrane.h`): a wrapper that re-wraps only the top-level call but lets a sub-capability's ambient `fetch()` escape "can be bypassed by merely introducing new objects." `origin` is what makes the agent context a _real_ membrane over the project context, not a shallow name-shadow.

### the ocap-idiomatic version: bind `origin` into the reference

A call-time `origin` field is an _ambient authority tag_ — a value that says "I'm acting for the agent," the very thing capabilities exist to abolish (it's also a trusted side channel you must spoof-proof). The ocap move is to **bind the agent's `fetch` into the inherited capability at delegation time**, so the authority is _in the reference_ and there's no `origin` field on the wire at all:

```ts
// instead of inheriting the NAME "fetch" and climbing with origin=agent,
// the agent is handed a fetch capability already bound to its own egress:
const agentFetch = parentFetch.boundTo(agent.address); // a membrane: origin pre-bound
child.provide({ name: "fetch", capability: agentFetch });

// now a bare fetch() inside ANY cap the agent invokes IS the right deputy —
// no origin argument anywhere, because authority rides the reference it holds.
```

Same effect as threading `origin`, but it deletes the wire field and the spoof hazard — _authority-follows-the-reference_, which is also the on-ramp to true three-party handoff (capnweb#43). Cap'n Web is Level 1 today, so production still threads `origin`; this is where it converges.

---

## Step 14 — Putting it together: a logging `fetch` middleware over an inherited API caller

> **the worked example** — the base (project) provides `fetch` _and_ an API-caller capability that uses it; an agent shadows `fetch` with a logging middleware; the agent invokes the **inherited** API caller, and the project cap's egress shows up in the agent's log — because `origin` (Step 13) routes it through the agent's shadow. Inheritance + shadow/super + origin, all visible in one screen.

```ts
// ── base: the PROJECT provides real egress + a DIALED worker that uses it ──
project.provide({ name: "fetch", capability: globalThis.fetch }); // real egress
project.provide({
  name: "petApi",
  // a SOURCE (dialed) worker. Its loaded isolate's egress is scoped to the call's
  // `origin` — that scoping is what makes its bare fetch() interceptable (Step 13).
  capability: {
    type: "rpc",
    worker: { type: "source", source: { repo, commit, path: "caps/pet-api.ts" } },
    entrypoint: "PetApi", // PetApi.getPet() just calls bare fetch() — no idea it's shadowed
  },
});

// ── child: the AGENT extends the project and shadows fetch with logging ──
agent.provide({
  name: "fetch",
  capability: async (input, init) => {
    const res = await agent.super.fetch(input, init); // delegate to project's fetch (NOT agent.fetch → loop)
    const url = typeof input === "string" ? input : input.url;
    console.log(`[egress] ${init?.method ?? "GET"} ${url} → ${res.status}`);
    return res;
  },
});

// ── call the INHERITED project capability THROUGH the agent ──
await agent.petApi.getPet(1);
// agent has no `petApi` → firstOf climbs to the project → dial builds PetApi's
// isolate scoped to origin = agent (the dotted form attaches origin automatically).
// PetApi's bare fetch() rides that isolate's egress → the agent's fetch SHADOW → logs:
//
//   [egress] GET https://petstore.example/pet/1 → 200
```

Three steps stacking, visibly:

- **Inheritance (Step 12):** the agent never copied `petApi` — it climbed to the project by reference.
- **Shadow + super (Step 10):** the agent's `fetch` logs and calls `agent.super.fetch` (the project's), never itself.
- **`origin` (Step 13):** PetApi's bare `fetch()` honored the agent's shadow because the call carried `origin = agent` — and the dotted form `agent.petApi.…` attaches that `origin` **automatically** (you only pass `origin` by hand when you call `invoke` directly, Step 13).

> ⚠️ **two honesties this example depends on.** (1) It works _because_ `petApi` is a **dialed source worker** — a loaded isolate's egress is scoped to `origin`, so its bare `fetch()` is interceptable. A **plain in-process / live capability** (a JS object held in the DO) runs with the DO's own `globalThis.fetch` and is **not** auto-intercepted; it would have to call `itx.fetch(...)` _explicitly_ to go through the shadow (production's real `OpenApiClient` does exactly that). (2) `agent`/`project` here are full **handles** (`ItxHandle`), not the bare `firstOf` of Step 10 — the handle wraps the chain and adds `provide`, the `super` accessor, and the dotted path-proxy. `agent.super` is a handle to the parent chain; `agent.super.fetch` resolves `fetch` _above_ the agent, skipping its own shadow — that's why there's no loop.

Call the same `getPet(1)` directly on the project (no agent origin) and **nothing logs** — the shadow is the agent's, reached only via origin. That's the whole egress-attenuation story end to end: a base capability you didn't write, invoked through an agent that transparently logs (or rate-limits, or denies) its egress.

### 🎯 Nothing breaks — this is the punchline

Everything we've built (provide/revoke as events, a folded table, read-your-writes, a stream that replays, a chain that spans DOs) is exactly a **stream processor**.

---

## Step 15 — Itx _is_ a StreamProcessor
...
| The one write path (revoke = provide ⊥)          | `provide`                                                       | ~80    |
| Borrow-or-dial + the dial type                   | `#borrow`                                                       | ~30    |
| Dispatch + chain delegation (`firstOf`)          | `invoke`                                                        | ~40    |
| Server-side path proxy + longest-prefix + replay | `PathProxy` / `resolveLongestProvidedPrefix` / `replayPathCall` | ~50    |
| Chain-merged view (derived — `firstOf` + a read) | `describe` / `extend` / `super`                                 | ~45    |
...
- **Two RPC systems — Cap'n Web pipelines paths, Workers RPC doesn't.** Cap'n Web pipelines a whole dotted path from a naked stub into one call (so `itx.slack.chat.postMessage(…)` needs no client proxy; the server-side dynamic proxy only has to answer `Object.hasOwn` per segment via its `getOwnPropertyDescriptor` trap). Workers RPC (the DO stub, the dialed parent) does _not_ pipeline through property accesses — which is why the DO exposes its core via an `itx()` method and why `itx.project.processor.snapshot()` is made to work by `replayPathCall` awaiting each segment server-side. For a real Cap'n Web stub, though, `followPath` does that traversal natively (receiver-preserving, with the prototype guards), so `replayPathCall`'s live/stub half is redundant — only the longest-prefix _selection_ is irreducibly itx's.

- **The server-side proxy's sharp edges** — `then` reads as `undefined` (so a path node is never mistaken for a thenable), and protocol-level segments (`__proto__`, `apply`, `call`, `bind`, `dup`, and **`map`** — reserved because of Cap'n Web's magic record-replay [`.map()`](https://github.com/cloudflare/capnweb#the-magic-map-method)) are refused on intermediate nodes — the same reserved set guards both the proxy and `replayPathCall`.
...
- **Dial allowlist (shipped) / sealing (proposed)** — production gates reach at _invoke_ time with a hand-curated allowlist (`DIALABLE_BINDINGS`/`LOOPBACKS`/`DURABLE_OBJECTS`, `dial.ts`) plus injected props (`capabilityPath`, `context`, `projectId`) that overwrite caller-supplied ones. Step 9 floats sealing (`sealedFor`) as the ocap-idiomatic replacement — but only with an _unforgeable_ seal token, never `=== publicIdentity`; until then the allowlist stays.
