...
parent (super); the agent's own caps (and built-in capabilities) **shadow** the project's; the
...
- `ItxDO.#parentContext()` returns the parent processor stub (an agent climbs to
  ...
  resolves its own fold + built-in capabilities first, then climbs: `parent.invoke({ path, args })`.
