...
// oxlint-disable-next-line iterate/no-capnweb-http-batch -- these live tests exercise the pet shop server's HTTP-batch /capnweb door specifically (a sibling test drives the WebSocket half); the rule targets stateless-worker client code, not a server's batch handler under test
...
describe("capnweb door (/capnweb)", () => {
  test("HTTP batch: the bearer in the Authorization header opens the pets API; a whole chain rides one POST", async () => {
    ...
    // oxlint-disable-next-line iterate/no-capnweb-http-batch -- exercises the pet shop server's HTTP-batch /capnweb door specifically (a sibling test drives the WebSocket half); the rule targets stateless-worker client code, not a server's batch handler under test
