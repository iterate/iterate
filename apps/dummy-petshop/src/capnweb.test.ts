 ...
 */
// oxlint-disable-next-line iterate/no-capnweb-http-batch -- these tests exercise the pet shop server's HTTP-batch /capnweb door specifically (the WebSocket half is driven by the live e2e); the rule targets stateless-worker client code, not a server's batch handler under test
 ...
 */
function capnwebClient(
  shop: Shop,
  token?: string,
): { session: () => PetshopApi; posts: Request[] } {
  const posts: Request[] = [];
  ...
  const session = () =>
    // oxlint-disable-next-line iterate/no-capnweb-http-batch -- exercises the pet shop server's HTTP-batch /capnweb door specifically (the WebSocket half is driven by the live e2e); the rule targets stateless-worker client code, not a server's batch handler under test
