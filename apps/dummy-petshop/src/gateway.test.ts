...
test("hello: advertises the heartbeat interval on connect", () => {
...
test("identify: a valid access token → ready then a pet.created dispatch", async () => {
  ...
  expect(state).toMatchObject({ identified: true });
});

test("identify: after ready, further frames are echoed back verbatim", async () => {
  const key = randomSealKey();
  const deps: GatewayDeps = { sealKey: key, getAccessTokenEpoch: async () => 0 };
  const state = newGatewayConnection();
...
test("subprotocolAuth (/gateway-subprotocol): pulls the token from the carrier and selects a non-token protocol to echo", () => {
  ...
  expect(parsed).toMatchObject({ token });
  // The echoed protocol is the real one, never the credential carrier.
  expect(parsed).toMatchObject({ selected: GATEWAY_SUBPROTOCOL });
});

test("subprotocolAuth (/gateway-subprotocol): carrier order does not matter and the token is never selected", () => {
  const token = "abc";
...
test("subprotocolAuth (/gateway-subprotocol): no carrier → null token; no header → null token and null selected", () => {
...
test("handleUpgradeAuth: a valid token → identified, ready then a pet.created dispatch, no close", async () => {
  const key = randomSealKey();
  const deps: GatewayDeps = { sealKey: key, getAccessTokenEpoch: async () => 0 };
  const token = await sealedAccessToken(key, { sub: "Jonas", clientId: "petshop-default" });

  const reaction = await handleUpgradeAuth(token, deps);
  expect(reaction).toMatchObject({ identified: true });
  expect(reaction.close).toBeUndefined();
  const [ready, dispatch] = parse(reaction.send);
  expect(ready).toEqual({ op: "ready", user: { sub: "Jonas", clientId: "petshop-default" } });
  expect(dispatch).toEqual({
...
test("handleUpgradeAuth: missing / garbage / foreign-key / expired / epoch-revoked → invalid + close 4001, not identified", async () => {
  const key = randomSealKey();
  const deps: GatewayDeps = { sealKey: key, getAccessTokenEpoch: async () => 0 };

  const expectRejected = (reaction: Awaited<ReturnType<typeof handleUpgradeAuth>>) => {
    expect(reaction).toMatchObject({ identified: false });
    expect(parse(reaction.send)[0]).toMatchObject({ op: "invalid" });
    expect(reaction.close?.code).toBe(AUTH_FAILED_CLOSE_CODE);
  };
  ...
  expectRejected(await handleUpgradeAuth(await sealedAccessToken(key, { epoch: 0 }), epochRevoked));
