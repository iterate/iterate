...
const buildDirectory = resolve(packageDirectory, "firmware/build-runtime-diagnostics");
const fixturePath = resolve(buildDirectory, "iterate-kit-runtime-diagnostics-fixture");
 ...
 */
await run("cmake", ["-S", sourceDirectory, "-B", buildDirectory, "-DCMAKE_BUILD_TYPE=MinSizeRel"]);
await run("cmake", [
  "--build",
  buildDirectory,
  "--target",
...
await run("pnpm", ["exec", "vitest", "run", "--config", "firmware/__tests__/vitest.config.ts"], {
  ...process.env,
