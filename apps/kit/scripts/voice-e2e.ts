...
const defaultPhrase = "Please reply by saying: Iterate voice path works.";
...
export async function runVoiceE2e(environment: Readonly<Record<string, string | undefined>>) {
  ...
  const observeProviderEvent = (event: ProviderVoiceEvent) => {
    ...
    if (event.type === "error") {
      recordFailure(new Error(`Grok returned an error event: ${event.raw}`));
  ...
  const server = new LocalDevicePeerServer(peer, {
    ...
    onVoiceFailure: (reason) => recordFailure(new Error(`PCM proxy failure: ${reason}`)),
  ...
  try {
    tunnel = await createCaptunTunnel({
      ...
      gateway: environment.CAPTUN_GATEWAY ?? "https://tunnels.iterate.com",
    ...
    const rejectedStatus = await probeRejectedBearer(pcmUrl, projectId, "deliberately-wrong-token");
    if (rejectedStatus !== 401) {
      throw new Error(
        `Expected the public PCM endpoint to reject an invalid bearer with 401, received ${rejectedStatus}.`,
      );
    }
    console.log("bearer_validation_passed invalid_status=401 provider_egress=skipped");
    ...
    deviceSocket.addEventListener("message", (event) => {
      ...
      if (!bytes) {
        recordFailure(new Error("The device PCM socket received a non-binary message."));
        return;
      }
    ...
    const uplinkFrames = createFixedPcmFrames(input.pcm, frameBytes, silenceFrames);
    try {
      console.log(
    ...
    await withTimeout(
      Promise.race([
        Promise.all([firstDownlink.promise, providerResponseDone.promise]),
    ...
    await delay(100);
    if (!responseCreated) {
      throw new Error("Grok returned audio without a response.created lifecycle event.");
    }
    const downlinkBytes = downlinkFrames.reduce((total, frame) => total + frame.byteLength, 0);
    console.log(
...
function requireSecret(value: string | undefined, name: string) {
  ...
  if (!normalized) {
    throw new Error(`Missing ${name}. Run this command through the appropriate Doppler config.`);
  }
  return normalized;
}

async function loadVoiceInput(environment: Readonly<Record<string, string | undefined>>) {
  ...
  try {
    ...
    await executeFile(environment.ITERATE_KIT_FFMPEG ?? "/opt/homebrew/bin/ffmpeg", [
      "-v",
      "error",
      "-y",
      "-i",
...
async function sendAtRealtimeRate(socket: WebSocket, frames: readonly Uint8Array[]) {
  const startedAt = performance.now();
  for (let index = 0; index < frames.length; index += 1) {
    if (socket.readyState !== WebSocket.OPEN) {
      throw new Error(`The PCM socket stopped before uplink frame ${index}.`);
    }
  ...
  return new Promise<WebSocket>((resolve, reject) => {
    ...
    const opened = () => {
      ...
      if (socket.protocol !== ITERATE_KIT_PCM_SUBPROTOCOL) {
        ...
        reject(new Error(`The server selected unexpected protocol ${socket.protocol}.`));
        return;
      }
      resolve(socket);
    };
...
function probeRejectedBearer(url: URL, projectId: string, bearerToken: string) {
  ...
  return new Promise<number>((resolve, reject) => {
    ...
    request.setTimeout(15_000, () => {
      request.destroy(new Error("Timed out probing public bearer validation."));
    });
    ...
    request.once("upgrade", (_response, socket) => {
      socket.destroy();
      reject(new Error("The public PCM endpoint accepted an invalid bearer token."));
    });
