...
const projectDirectory = resolve(appDirectory, "firmware/tests/esp_idf_tcp_transport_host");
const incidentTests = [
...
export function firmwareWebsocketBuildDirectories(options: { proveStockFails?: boolean }) {
  /*
  ...
  return {
    ...
    stockBuildDirectory: resolve(appDirectory, ".build/esp-idf-tcp-transport-stock-control"),
  };
}
...
function runOrThrow(
  ...
  if (result.signal) {
    throw new Error(`${executable} ${args.join(" ")} was terminated by ${result.signal}.`);
  }
  if (result.status !== 0) {
    const output = options.capture ? `\n${result.stdout ?? ""}${result.stderr ?? ""}` : "";
    throw new Error(`${executable} ${args.join(" ")} exited with ${result.status}.${output}`);
  }
  return result;
}

export function testFirmwareWebsocket(
  options: {
  ...
  if (!existsSync(python) || !existsSync(idfPy)) {
    throw new Error(`ESP-IDF environment is incomplete: expected ${python} and ${idfPy}.`);
  }

  const { patchedBuildDirectory, stockBuildDirectory } = firmwareWebsocketBuildDirectories(options);
  runOrThrow(python, [
    idfPy,
    "-C",
    projectDirectory,
  ...
  runOrThrow(join(patchedBuildDirectory, "iterate_kit_tcp_transport_host_test.elf"), [
    "--reporter",
    "compact",
  ]);
  ...
  const stockExecutable = join(stockBuildDirectory, "iterate_kit_tcp_transport_host_test.elf");
  for (const testName of incidentTests) {
    const result = spawnSync(stockExecutable, [testName, "--reporter", "compact"], {
      cwd: appDirectory,
      encoding: "utf8",
    });
    if (result.error) throw result.error;
    const output = `${result.stdout ?? ""}${result.stderr ?? ""}`;
    if (result.signal || result.status !== 1 || !oneFailedTestSummary.test(output)) {
      throw new Error(
