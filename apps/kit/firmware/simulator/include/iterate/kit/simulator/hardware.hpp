...
struct CommonHardware {
  ...
  /*
   * The detailed playback callback carries a source sequence so the host can
   * distinguish a fresh sample from callback replay. This counter is part of
   * the deterministic capability-boundary fake only; it does not pretend to
   * model ESP scheduling or the production target's boot identity.
   */
  std::uint32_t metricsSequence = 0U;
