 ...
 */
#define STREAM_PATH_DEFAULT "/agents/voice/home-assistant-voice-preview-edition"
...
void iterate_kit_havpe_run(void) {
  ...
  for (;;) {
     ...
     */
    {
       ...
       * is new is publishing the two halves separately, so the ring can say
       * WHICH one is missing instead of going amber for both.
       */
      ...
      havpe_ui_set_api_ready(api_ready);
      havpe_ui_set_stream_ready(stream_ready);
    ...
    if (runtime.voicelab.state == ITERATE_KIT_VOICELAB_READY &&
       ...
       * cancellation — the XMOS has already done it here, unlike on
