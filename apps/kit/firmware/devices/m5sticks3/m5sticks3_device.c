/*
 * M5StickS3 — the Iterate voice device, on the shared single-socket lane.
 *
 ...
 * events.iterate.com/voice-agent/mic-frame events (the ES8311 ADC), and a live openConnection
 * callback delivering events.iterate.com/voice-agent/spk-frame events (decoded to the speaker)
 * plus grok-events (speech_started = barge-in flush, response.done = end of
 * answer).
 *
 * Turn taking is MANUAL: no VAD anywhere. The SIDE button toggles the call,
 * the FRONT button is held while speaking. Nothing is sent from the
 * microphone unless FRONT is down — which on THIS board is enforced by the
 * hardware itself: the ES8311's ADC and DAC paths share I2S clock pins, so
 * the microphone physically cannot run while the speaker does. Half duplex
 * is the whole echo story.
 *
 * Structure and supervision deliberately rhyme with
 * devices/waveshare_s3_amoled/waveshare_device.c — same portable modules,
 * same watchdogs, same telemetry names — so a reader of one can read the
 * other. Divergences are the board's own: the half-duplex fence, a text
 * status screen instead of an avatar, and two buttons with different wiring.
 */
#include <inttypes.h>
#include <stdatomic.h>
#include <stdint.h>
...
enum {
   ...
   * as dry: two thirds of the 120 ms I2S DMA ring (6 descriptors x 320
   * frames at 16 kHz), so a late frame is absorbed by the hardware cushion
 ...
 */
#define STREAM_PATH_DEFAULT "/agents/voice/m5stick-s3"
...
#define CONVERSATION_ID "stickdev"
...
static struct {
  ...
  /** Captured with no turn open: handoff-tail frames, never sent. */
 ...
 * members the host can list — so the method list belongs in the prose or it is
 * nowhere, and a capability nothing advertises is one nothing calls.
 */
static const char peer_description[] =
    "{\"instructions\":\"M5StickS3 voice endpoint. Hold the front button to "
    "talk; a side press starts and ends the call. "
...
static bool publish_turn_marker(enum iterate_kit_voicelab_turn turn) {
  ...
  m5sticks3_ui_set_status("reconnecting");
  return false;
}
...
static void on_speaker_pcm(
   ...
   * is written: a class-D amp needs tens of milliseconds to settle, and the
   * playout prefill is spent as settle time, which costs nothing.
   */
...
static void on_control(
  ...
  if (control == ITERATE_KIT_VOICELAB_CONTROL_SPEECH_STARTED) {
    ...
    m5sticks3_ui_set_state(M5STICKS3_UI_LISTENING);
    ...
    m5sticks3_ui_set_status("hold the front button to talk");
    ...
    m5sticks3_ui_set_call_active(true);
    m5sticks3_ui_set_state(M5STICKS3_UI_IDLE);
    m5sticks3_ui_set_status("hold the front button to talk");
     ...
     */
    (void)abandon_speaker_audio();
    m5sticks3_ui_set_call_active(false);
    m5sticks3_ui_set_state(M5STICKS3_UI_IDLE);
    m5sticks3_ui_set_status(
        m5sticks3_ui_call_requested() ? "reconnecting" : "call ended");
  }
}
...
static void playback_task(void *argument) {
   ...
   * port for the measured history. The one addition on this board is the
   * half-duplex fence below.
   */
  struct iterate_kit_voice_playback_clock playout_clock;
  ...
  for (;;) {
    ...
    /*
     * THE HALF-DUPLEX FENCE. While the microphone owns the shared pins —
     * or the fence is moving in either direction — this task must not pull
     * frames it can only fail to write. Frames stay queued; the fence drops
     * within milliseconds of the turn committing, long before an answer.
     */
    if (m5sticks3_audio_capturing() || m5sticks3_audio_mode_switching()) {
      m5sticks3_audio_watch(false);
      DELAY_MS(5);
      continue;
    }
    if (!iterate_kit_voice_playback_clock_ready(
      ...
      if (last_write_ms != 0U &&
          iterate_kit_voice_elapsed_ms(now_ms(NULL), last_write_ms) >
     ...
     */
    if (m5sticks3_audio_starvation_pending() &&
        m5sticks3_audio_written_ms() >= DMA_RING_CREDIT_MS) {
      const uint32_t starve_ms = m5sticks3_audio_take_injected_starvation();
      ESP_LOGW(
...
static void capture_task(void *argument) {
  ...
  for (;;) {
    ...
    if (read_status == ITERATE_KIT_UNAVAILABLE) {
      /* Half duplex: the microphone only exists while a turn is open. */
      DELAY_MS(1);
      continue;
    }
    ...
    /*
     * NOBODY IS LISTENING, SO DO NOT QUEUE. On this board capture only runs
     * while a turn is open, but the handoff tail can complete a frame after
     * the turn closed; counted and discarded so the drop counter keeps
     * meaning "speech somebody said was lost".
     */
    if (!runtime.talking) {
...
static size_t health_json(char *out, size_t capacity) {
  ...
  written = snprintf(
      ...
      m5sticks3_ui_call_requested() ? "true" : "false",
...
static bool begin_new_conversation(void) {
  ...
  if (iterate_kit_voicelab_setup_conversation(&runtime.voicelab, candidate) !=
      CAPNWEB_OK) {
    m5sticks3_ui_set_state(M5STICKS3_UI_IDLE);
    m5sticks3_ui_set_status("could not ask the server");
    return false;
  }
  (void)snprintf(
      pending_stream_path, sizeof(pending_stream_path), "%s", candidate);
  m5sticks3_ui_set_state(M5STICKS3_UI_CONNECTING);
  m5sticks3_ui_set_status("preparing a new conversation...");  return true;
}
...
void iterate_kit_m5sticks3_run(void) {
  ...
  if (!m5sticks3_board_init()) {
    ESP_LOGE(tag, "board bring-up failed: not an M5StickS3?");
    return;
  }
  if (!m5sticks3_audio_init()) {
    ESP_LOGE(tag, "audio bring-up failed");
    return;
  }
  runtime.codec = m5sticks3_audio_codec();
  ...
  for (;;) {
     ...
     */
    {
      ...
      m5sticks3_ui_set_api_ready(api_ready);
      m5sticks3_ui_set_stream_ready(stream_ready);
      if (ready != published_link_ready) {
        published_link_ready = ready;
        m5sticks3_ui_set_link_ready(ready);
      }
    }
    m5sticks3_ui_tick();
    if (m5sticks3_board_take_side_press()) {
      /* One button, one intent: toggle whether a call is wanted. */
      const bool wanted = !m5sticks3_ui_call_requested();
      m5sticks3_ui_request_call(wanted);
      ESP_LOGI(tag, "side button: %s call", wanted ? "starting" : "ending");
    }
    {
      static bool talk_logged;
      const bool talk = m5sticks3_board_talk_held();
      if (talk != talk_logged) {
        talk_logged = talk;
        ESP_LOGI(tag, "FRONT %s", talk ? "down (talking)" : "up (commit)");
      }
    }
     ...
     */
    {
      ...
      if (runtime.voicelab.setup_succeeded) {
        ...
        m5sticks3_ui_set_status("new conversation ready");
        stream_used = false;
    ...
    if (runtime.talking && !runtime.flushing_turn &&
      ...
      m5sticks3_ui_set_state(M5STICKS3_UI_IDLE);
      m5sticks3_ui_set_status("connection lost — press side to call");
    }
    ...
    if (runtime.voicelab.state == ITERATE_KIT_VOICELAB_READY &&
        ...
        switch (iterate_kit_launch_next_step(&launch, &launching)) {
          ...
          case ITERATE_KIT_LAUNCH_PLACE_CALL:
            ...
            if (iterate_kit_voicelab_start_call(&runtime.voicelab, GREETING) ==
                CAPNWEB_OK) {
              m5sticks3_ui_set_status("starting call");
            }
            break;
      ...
      if (!wants_call && runtime.voicelab.call_active && outbox_free >= 3U) {
        ...
        m5sticks3_ui_set_status("call ended");
        m5sticks3_ui_set_state(M5STICKS3_UI_IDLE);
      }
      if (runtime.voicelab.call_active &&
      ...
      if (runtime.voicelab.call_active != call_active_shown) {
        ...
        m5sticks3_ui_set_call_active(call_active_shown);
       ...
       * dropping the queued speaker audio, and physically by taking the
       * shared pins for the microphone. Releasing commits the turn.
       *
      ...
      if (wants_talk && !runtime.talking && runtime.voicelab.call_active &&
        ...
        if (publish_turn_marker(ITERATE_KIT_VOICELAB_TURN_START)) {
          ...
          runtime.flushing_turn = false;
          /*
           * THE HALF-DUPLEX HANDOFF: only now, after the turn marker is on
           * the wire and the speaker queue is empty, does the microphone
           * take the pins.
           */
          m5sticks3_audio_set_capture(true);
          ESP_LOGI(tag, "turn start");
          m5sticks3_ui_set_state(M5STICKS3_UI_LISTENING);
          m5sticks3_ui_set_status("listening — release to send");
        }
      }
      /*
      ...
      if (!wants_talk && runtime.talking && !runtime.flushing_turn) {
        ...
        /*
         * The microphone's job ended at the release edge; give the pins
         * back now so the answer can play the moment it arrives. Queued
         * frames still flush below — the fence only stops NEW capture.
         */
        m5sticks3_audio_set_capture(false);
        m5sticks3_ui_set_status("sending");
      }
