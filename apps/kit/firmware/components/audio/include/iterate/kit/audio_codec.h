 ...
 * reference channel. Both planes have `capacity_samples` entries;
 * `sample_count` is fully written on success.
 *
 * write() copies the complete mono playback plane or returns BACKPRESSURE. A
 * successful copy is admission to the bounded hardware path, not proof of
 * physical playout. Platform diagnostics own that later completion evidence.
 * Neither operation allocates, waits, calls the network, or silently drops.
 */
