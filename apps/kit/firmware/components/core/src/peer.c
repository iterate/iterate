 ...
 * The server reaches the peer through `provide(match, stub)` as
 * `itx.clients.<device>` (see itx_mount.h), so every call arrives as the plain
 * path those tables name.
 *
 ...
 *
 * Session end releases remote handles without closing persistent hardware
 * modules, while close is the explicit physical lifecycle boundary.
 */
