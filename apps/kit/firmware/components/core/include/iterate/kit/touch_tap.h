...
/**
 * Allocation-free conversion from polled touch levels to one activation.
 *
...
struct iterate_kit_touch_tap {
  ...
  uint64_t release_started_at_ms;
};

/**
 * Initializes an activation detector without manufacturing an event from boot
 * touch.
 *
 * Pass true when the initial controller state is touched or unknown. The
 * first stable release establishes a clean baseline; a subsequent press emits
 * exactly one activation immediately.
 */
...
/**
 * Consumes one successful controller sample and returns true once at contact.
 * A release must remain present for 30 ms before it arms another contact, so
 * a brief zero-contact sample cannot turn one touch into two call controls.
 *
