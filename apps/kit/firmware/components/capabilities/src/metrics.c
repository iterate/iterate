...
struct playback_metrics_expression_workspace {
  ...
  struct capnweb_expression playback_values[31];
  struct capnweb_object_field playback_fields[31];
  struct capnweb_expression playback_object;
  struct capnweb_expression runtime_values[15];
  struct capnweb_object_field runtime_fields[15];
...
#undef SET_RUNTIME_INTEGER
  ...
  if (playback_count >
          sizeof(workspace->playback_values) /
              sizeof(workspace->playback_values[0]) ||
      runtime_count >
          sizeof(workspace->runtime_values) /
              sizeof(workspace->runtime_values[0])) {
    /*
     * The fixed array sizes are part of the polling-task stack budget. Keep an
     * executable guard even though the current counts are compile-time
     * obvious; it prevents a future helper refactor from serializing past the
     * workspace if a field is added without expanding both arrays.
     */
    return false;
  }
  set_object(
