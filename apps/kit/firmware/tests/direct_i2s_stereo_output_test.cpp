...
using RunningSilenceWrite =
    iterate_kit_status (Output::*)();

void assertRunningSilenceConsumesExactPointer(
    RunningSilenceWrite writeSilence,
    iterate::kit::platforms::DirectI2sFrameKind
        expectedKind) {
  FakeDirectI2sBackend backend;
  Output output(backend);
  preloadAndStart(output, backend);

  const auto descriptorZero =
      FakeDirectI2sBackend::token(
          backend.generation, 0U);
  backend.complete(descriptorZero, 20'000U);
  TEST_ASSERT(
      output.pollCompletionBatch().status ==
      ITERATE_KIT_OK);
  TEST_ASSERT(backend.driverWriteQueueCount == 1U);
  TEST_ASSERT(
      (output.*writeSilence)() == ITERATE_KIT_OK);
  TEST_ASSERT(backend.driverWriteQueueCount == 0U);
  TEST_ASSERT(
      output.pendingRefillDescriptorCount() == 0U);
  TEST_ASSERT(backend.firstLeftHistory[4U] == 0);
  TEST_ASSERT(backend.lastRightHistory[4U] == 0);

  for (std::size_t descriptorIndex = 1U;
       descriptorIndex < descriptorCount;
       ++descriptorIndex) {
    backend.complete(
        FakeDirectI2sBackend::token(
            backend.generation, descriptorIndex),
  ...
  backend.complete(descriptorZero, 100'000U);
  TEST_ASSERT(
      output.pollCompletionBatch().status ==
      ITERATE_KIT_OK);
  iterate::kit::platforms::
  ...
  TEST_ASSERT(
      output.lastPlaybackCompletion(0U, &completion));
  TEST_ASSERT(
      completion.frameKind ==
      expectedKind);
}
...
/*
 * Ordered EOS still has to consume ESP-IDF's private completed-pointer queue.
 * Auto-clear prevents stale replay, but it does not remove that pointer; if the
 * owner merely waits for the final content EOF, the fourth callback overflows
 * the three-entry queue and destroys descriptor identity. The running silence
 * operation must use the exact oldest token, copy a complete zero frame, and
 * relabel that descriptor as padding so policy never counts it as speech.
 */
void runningSilenceConsumesExactPointerAndBecomesPadding() {
  assertRunningSilenceConsumesExactPointer(
      &Output::writeSilence,
      iterate::kit::platforms::
          DirectI2sFrameKind::paddingSilence);
}
...
/*
 * Recovery silence has the same physical ownership obligation as EOS padding,
 * but conflating their tags would make ordinary jitter legal only while an EOS
 * marker is active. Preserve the separate recovery kind through one complete
 * descriptor wrap so policy can count the silence without advancing content
 * sequence or weakening its finite-response invariants.
 */
void recoverySilenceRetainsItsKindThroughDescriptorWrap() {
  assertRunningSilenceConsumesExactPointer(
      &Output::writeRecoverySilence,
      iterate::kit::platforms::
...
int main() {
  ...
  RUN_TEST(
      runningSilenceConsumesExactPointerAndBecomesPadding);
  RUN_TEST(
      recoverySilenceRetainsItsKindThroughDescriptorWrap);
