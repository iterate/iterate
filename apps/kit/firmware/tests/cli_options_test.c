...
static void the_credentials_are_required(void)
{
  ...
  (void)unsetenv("ITERATE_KIT_STREAM_PATH");
  assert(parse(&options, problem, argv, 1) == CLI_OPTIONS_ERR_REQUIRED);
  assert(strcmp(problem, "--project-id") == 0);
}

/* A device stream is setup-owned; the CLI must not invent an obsolete path. */
static void a_stream_path_is_required(void)
{
  struct cli_options options;
  char problem[128];
  char *argv[] = {(char *)"cli"};
  ...
  (void)unsetenv("ITERATE_KIT_STREAM_PATH");
  assert(parse(&options, problem, argv, 1) == CLI_OPTIONS_ERR_REQUIRED);
  assert(strcmp(problem, "--stream-path") == 0);
}
...
static void the_environment_fills_only_what_the_flags_left(void)
{
  ...
  (void)setenv("ITERATE_OS_BASE_URL", "https://os.example", 1);
  (void)setenv("ITERATE_KIT_STREAM_PATH", "/agents/voice/test", 1);
  ...
  assert(strcmp(options.stream_path, "/agents/voice/test") == 0);
