 ...
 * Pinning the measured 280-byte 64-bit-host ceiling catches an accidental frame
 * copy/vector/queue in this layer while leaving room for the explicit reset
 * counter and post-write timing tuple. The resulting 24-byte increase over the
 * original policy is a deliberate observability budget: callback-to-owner
 * alone cannot prove the descriptor was actually refilled on time. Target
 * resource reports still provide the authoritative ESP32 size.
 */
static_assert(
    sizeof(Playback) <= 280U,
...
/*
 * A descriptor deadline survives the callback that created its refill credit.
 * When no fresh lane frame exists, the owner necessarily retains that exact
 * completed descriptor. A later wake with no additional EOF is still obliged
 * to re-evaluate its shrinking reuse lead; forgetting the deadline lets DMA
 * wrap into the cleared/stale descriptor while every counter appears balanced.
 */
void pendingRefillDeadlineSurvivesAZeroEofPump() {
  LaneFixture fixture;
  FakeDirectOutput output;
  Playback playback;
  TEST_ASSERT(playback.begin(output) == ITERATE_KIT_OK);
...
void steadyStreamingResponseDrainsItsExactTailAtEndOfStream() {
  constexpr std::int16_t frameCount = 100;
  LaneFixture fixture;
  FakeDirectOutput output;
  Playback playback;
  TEST_ASSERT(playback.begin(output) == ITERATE_KIT_OK);
...
int main() {
  ...
  pendingRefillDeadlineSurvivesAZeroEofPump();
