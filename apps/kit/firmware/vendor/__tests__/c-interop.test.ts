...
import { RpcSession, RpcTarget, type RpcTransport } from "@iterate-com/capnweb";
import { describe, expect, test } from "vitest";
import { cInteropKnownFailures, type CInteropKnownFailure } from "./c-interop-known-failures.ts";

const testDirectory = dirname(fileURLToPath(import.meta.url));
const packageDirectory = resolve(testDirectory, "../../..");
const nativePeerPath =
  process.env.ITERATE_KIT_CAPNWEB_NATIVE_PEER ??
  resolve(packageDirectory, "firmware/build-capnweb-interop/capnweb-native-peer");
...
class NativePeerTransport implements RpcTransport {
  ...
  abort(reason: unknown): void {
    this.#fail(reason instanceof Error ? reason : new Error(`RPC aborted: ${String(reason)}`));
...
async function withNativePeer(body: (remote: any) => Promise<void>): Promise<void> {
...
function runTerminalKnownFailure(
  failure: Extract<CInteropKnownFailure, { readonly kind: "terminal-wire-status" }>,
): void {
  ...
  expect(result.stderr).toContain(`capnweb_session_receive failed: ${failure.expectedStatus}`);
  expect(result.stdout.trim()).toBe(
    JSON.stringify(["abort", ["error", "Error", failure.expectedAbortReason]]),
  );
}
...
describe("C peer interoperability with the real TypeScript Cap'n Web runtime", () => {
  test("runs the upstream primitive, object, error, and bytes behaviors across the language boundary", async () => {
    ...
    await withNativePeer(async (remote) => {
      ...
      expect(await remote.generateFibonacci(7)).toEqual([0, 1, 1, 2, 3, 5, 8]);
      expect(
      ...
      await expect(remote.throwError()).rejects.toThrow(new RangeError("test error"));

      expect(Array.from(await remote.getBytes())).toEqual([0, 1, 2, 127, 128, 254, 255]);
  ...
  test("runs upstream capability return, pipelining, and bidirectional callback behaviors", async () => {
    ...
    await withNativePeer(async (remote) => {
      ...
      expect(await remote.callFunction(async (value: number) => value * value, 6)).toEqual({
        result: 36,
      });
    });
  });
  ...
  for (const failure of cInteropKnownFailures) {
    test(`executes declared compatibility failure: ${failure.id}`, async () => {
      ...
      await withNativePeer(async (remote) => {
        await expect(failure.invoke(remote)).rejects.toThrow(failure.expectedMessage);
      });
    });
  }
});
