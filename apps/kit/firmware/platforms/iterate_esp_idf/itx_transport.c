...
/*
 * ESP-IDF control-plane scheduler for the device's Cap'n Web capability.
 *
 ...
 * arbitrary RPC work out of ESP-IDF's callback task, while bounded poll/send
 * bursts cap application and control-task work. That bound does not claim
 * control and PCM avoid all contention in opaque TLS/Wi-Fi layers; end-to-end
 * PCM delay metrics must verify those lower layers. We rejected a single
 * callback-driven session because device methods could block ESP-IDF
 * networking, and rejected one combined PCM/control socket because control
 * backpressure must never accumulate stale speech.
 *
...
enum {
  ...
  /*
   * Pinning gives cycle/headroom measurements a stable scheduling context. It
   * is not a claim of exclusive-core realtime isolation; task priorities and
   * bounded work remain the actual audio-safety mechanisms.
   */
  NETWORK_TASK_CORE = CONFIG_FREERTOS_NUMBER_OF_CORES - 1,
};
