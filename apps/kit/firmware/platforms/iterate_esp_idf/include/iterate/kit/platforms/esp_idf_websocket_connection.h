 ...
 * audio/control frame that is still being processed.
 */
 ...
 *
 * This split is intentional for the realtime PCM connection. The higher-level
 * ESP WebSocket client owns a task and may serialize callbacks behind unrelated
 * work; retaining the lower transport lets one PCM owner make a bounded
 * nonblocking read/write attempt whenever it is scheduled. The abstraction
 ...
 *
 * timeout_ms may be zero; PCM callers use that form to avoid delaying capture
 * or playback. Fragmented frames remain in the portable parser across IDLE
 * results. Returned chunk storage is borrowed as described above.
 */
enum iterate_kit_esp_idf_websocket_receive_result
 ...
 * peer-delivery guarantee; the PCM lane establishes that separately.
 */
 ...
 * does not attempt a blocking graceful drain: stale realtime audio must not
