 ...
 private:
  ...
  RealtimePlaybackPumpResult pumpPlaying(
    ...
    if (deadlineMissed ||
        metrics_.currentContentFrames == 0U) {
      /*
       * In cyclic DMA, consuming the final descriptor is already the playback
       * deadline: hardware is wrapping to descriptor zero while this task is
       * waking. Refilling after that observation may make a FIFO model look
       * healthy, but it cannot undo stale audio already selected by hardware.
       */
      const auto status =
          resetAfterUnderrun(lane, driver);
      return {status, false, false};
