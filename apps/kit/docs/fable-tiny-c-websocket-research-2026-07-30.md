...
hijacked and deleted) (§4). What the survey *does* supply is proven design
...
- §10 resource ledger: no IRAM emergency; D/IRAM pool 194,344 B free; flash
  at 54.19 % of the 2 MB factory partition. Resource pressure is *not* a
...
components/esp_websocket_client` is empty: 1.8.0 (tagged 2026-07-22) *is*
...
per gap, all while holding the lock, and each chunk *synchronously
dispatches* `WEBSOCKET_EVENT_DATA` under that lock (:1108, handlers run on
...
- The client's own PING (:1306), PONG answer (:1141), and CLOSE echo
  ...
  the dominant *transient* deferral source, and it writes zero caller
...
- **Partial-write corruption:** if the payload write returns short (>0),
  the send loop wraps the *remainder* in a **new continuation-frame
...
**Allocation.** Steady state is *not* allocation-free: every received frame
...
degraded paths log per occurrence *from the sender task*: the deferral case
...
2–3 s (bounds reader holds *and* control-write budgets ~4×), effectively
...
- **Read model:** one `esp_transport_read` returns a *chunk with
  bookkeeping*: header parsed when the previous frame is exhausted, then a
...
- **Send path:** stack-built 16-B header, then the payload is
  **XOR-masked in place in the caller's buffer** and XOR-restored after
  ...
  own storage and writes once through the *parent* (TLS) handle avoids the
...
- **Blocking model:** all verbs are select-bounded per call, but the
  socket stays *blocking* with `SO_RCVTIMEO`/`SO_SNDTIMEO` fixed at
...
correct consumption mode is *vendor and own the 4 .c files*, not a live
...
|---|---|---|---|
| **Mongoose** (`cesanta/mongoose`) | `03be168`, 2026-07-29, active | `LICENSE:5-16` — GPLv2 **or paid commercial**; no permissive option | License alone disqualifies static linking into closed firmware. Also: TX iobuf is uncapped (audio grows it 32 KB/s during a stall) with calloc+copy+zeroize churn at 512-B boundary crossings (iobuf.c:19-54); RX cap closes the connection rather than backpressuring; TLS is direct-mbedTLS, bypassing esp-tls. Otherwise the *best* architecture of this group: single-thread poll loop, and the whole client WS codec is `src/ws.c` = **301 lines** — the proof of how small this problem is (incremental in-place-unmask parser :66-94, stack header builder :96-122, in-buffer fragment reassembly :214-233). Clean-room reference only, given GPL. |
...
| **cwebsocket** (`m8rge/cwebsocket`) | `36f84f2`, 2015-03-30 | MIT ✓ | Not a client at all: masked-input-only *server* frame parser for 2015 Arduinos (requires mask bit on input, websocket.c:288; no client handshake, no TX masking, no RNG in the tree), no TLS, no fragmentation, `assert()` error handling, zero tests, dead 11 years. |
...
- **WIC** (`cjhdev/wic` @ `54d1240`, 2021-06-29, MIT/Apache mix): the
  ...
  user-supplied rx buffer, TX buffers requested through a callback *typed
  by frame class* so PONG/CLOSE have reserved pools (wic.h:154-162,
...
- **Zephyr websocket client**
  ...
  *bulk in-place unmask with the mask phase recomputed arithmetically*
...
- **coreMQTT** (`FreeRTOS/coreMQTT` @ `5be5f95`, MIT, active): not
  WebSocket — the *shape* template for a production embedded protocol
...
2. **The foundation the incumbent stands on is exactly what the kit wants**
   ...
   the replacement *deletes* 11.8 KB and two tasks rather than adding a
   dependency.
...
reached the connection; resume the *same* WebSocket frame) vs
...
instead of re-sent. The deferral *counters and restart threshold* carry
...
- **The goal doc's "device socket send" timestamp**: with esp-tls, a
  completed write means *accepted by the TLS layer* (a record can sit in
...
pumps to a dedicated prio-6 task later changes *nothing* in the transport
...
- **`components/core/src/websocket_frame_writer.c`** (164 LOC + host
  ...
  `BACKPRESSURE`. This module *is* the CONT-desync fix, and matches the
...
*(Second-pass disposition: the compiled worker resolves T2, T7, T8, T10,
...
- **T1 · The RX blocking-mode tension (the central design decision).**
  ...
  payload/header reads can block up to `SO_RCVTIMEO` *in the worker task
  that also pumps TX*. Naively setting `O_NONBLOCK` makes every torn
  ...
  on `poll_read(0)`; classify a torn-header timeout as a *retryable* read
...
- **T3 · Single control slot policy.** `queue_control` rejects a second
  ...
  flush window answer the *older* ping and drop the newer (RFC 6455
...
- **T5 · Inbound control-frame validation is the worker's job.** ≤5.5.3
  ...
  correctly refuse >125 B — the worker must then *drop and count*, not
...
- **T7 · One writer path, enforced.** After the handshake, *all* TX must
...
- **T8 · esp-tls write granularity and "sent" semantics.** mbedTLS accepts
  a ≤4 KB record wholly or not at all; a record can be *accepted* (counted
  ...
  `WOULD_BLOCK` on the *next* frame. Consequences: `PROGRESS` will be rare
  ...
  WANT_* with nothing accepted → `WOULD_BLOCK`; hard error / peer close →
...
- **T10 · The S1-1 latch is still in the tree.** Every new rejection path
  ...
  concrete: an edge that re-fragments today bricks the device *through
  the brand-new reassembly code*.
...
Two alternatives were seriously considered and rejected *for now*:
...
1. **Own RX parser over raw esp-tls (drop transport_ws after the
   ...
   Keeping transport_ws for the *handshake* is right in both phases.
...
| T3 control-slot policy | **Open, and sharper now**: `queue_control` rejection is mapped to `RECEIVE_PROTOCOL_FAILURE` (websocket_connection.c:463-467, 482-486) — a second PING inside one flush window, or CLOSE arriving while a PONG is parked, now *restarts the socket* instead of dropping (N-S2-4). |
...
| T6 spillover drain | **Open.** No post-connect drain read; `ws_read_header`'s first act is a *parent* poll (transport_ws.c:529-535), which cannot see bytes retained in `ws->buffer` from the upgrade — a server-pipelined first frame waits for unrelated socket bytes (N-S2-5). |
...
payload bytes of that frame will be parsed as a *new header* on the next
...
*How often:* our own stack makes this ordinary — Node's `ws` sender writes
...
*Smallest failing tests:* (a) fake transport delivering one 640-B frame as
...
layers); (b) same with the split *inside* the payload (chunk, zero-read,
chunk).
*Fix ladder:* (1) connection-side mid-frame poll gate — when
`receive_payload_offset > 0` (or opcode pending), `esp_transport_poll_read`
first and return IDLE on 0 — never *enters* `ws_read` without data;
...
*Smallest failing test:* scripted raw-writer returning `WOULD_BLOCK` for
...
*Fix:* make the bound time-based (no-progress-for-X-ms on the retained
...
503-508); `ws_read_header` has a parent poll gate only *before the first
byte* (:529-535) — a 2-byte basic header, extended length, or (masked-peer)
...
*Failing test:* header delivered 1 byte + 1 byte across two windows →
...
reconnect. *Mitigation now:* `esp_log_level_set("transport_ws", ESP_LOG_NONE)`
...
redundant. *Rig assertion:* console-wedge + 60 s backpressure → no WDT,
bounded log bytes.
...
*Failing test:* fake read returning WANT_WRITE once → no restart.
...
suggests *overwrite*, not reject-then-die. (c) CLOSE while a PONG is parked
→ same path (:477-486) — restart happens anyway, but the close echo is
skipped. *Fixes:* overwrite-newest in the single slot (CLOSE always wins);
tolerate multi-chunk control payloads by accumulating like data (≤125 B in
the storage) — both small, both in kit code only.
*Failing tests:* two pings one window → exactly one pong (payload of the
...
(before the poll-gated loop takes over) fixes it. *Failing test:* fake
...
covered by traffic when active — but *idle* PCM (device listening, nobody
...
- TLS handshake now runs on the 6144-B static worker stack
  ...
  reads the high-water *before* the handshake spike; gate on the rig via
