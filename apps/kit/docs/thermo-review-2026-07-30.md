...
1. **The failure policy is "brick until power cycle" for a wide class of
   ...
   from running. The vendored-capnweb pass (§8) adds three _capacity
   cliffs_ that feed the same latch from inside a healthy session: a
...
config flags (§10). Resource pressure is _not_ a reason to accept any of
...
_remote or transient_ conditions:
...
state" as _metrics of an ongoing process_, not a tombstone. The devices are
...
happens on the _next_ main-loop iteration. Sustained rate is therefore 1
...
`frames_submitted == 500 ± 1` _and_ that a single 15 ms stall injected into
...
1. **Un-deduped per-iteration error log.** A persistent device-poll failure
   ...
   terminal), _each_ log line can cost the prio-1 main loop tens of
...
loses 3 frames (60 ms of speech) _even though the information arrived_.
Meanwhile the speaker's own DMA (`dma_buf_count=8 × 256` ≈ 128 ms,
`Speaker_Class.hpp:66-70`) is a hidden buffer _below_ the carefully bounded
...
latency-safe as long as _interruption flushes it_ (it does:
...
### S2-6 · Reconnect discards the first frames of the _new_ PCM socket
...
at disconnect time (or gate on generation _stamped per frame_, which the
...
correct; the callback fires only after a _complete_ frame is published
...
`pdFALSE` (decrement) makes the notification count a _frame counter_: N
...
loops drain _all_ work per wake, so collapsing is wanted there. The
...
- `capture_send_complete` (`audio.c:38-58`) is documented nowhere as
  ...
  the lane's synchronous OK/BACKPRESSURE _is_ the completion; the async
  ...
  the callback back _at that transport's boundary_, not in the core.
...
device-level resource both sockets depend on — lives _inside_ the control
...
This is not cosmetic: StackChan needs the _same_ two sockets, and every
...
field" a one-liner (it is currently the S2-4 brick hazard _and_ eight lines
...
images and cannot scale by tuning. Since a photo _capability with bounded
chunk reads_ is also the flagship Cap'n Web compatibility case, the current
...
pumped _simultaneously_ at 20 ms cadence with the far-end reference aligned
...
The judo is that the codebase is _already shaped for this_ — the lane is the
...
1. **`audio_task` (per device, shared skeleton, prio ~6, core pinned):**
   ...
   read — the mic _is_ the 20 ms clock; no timer needed), then per frame:
...
3. **Cross-task contracts to define (small):** the audio controller's
   ...
   word) — or the controller stays on main and only _pump + DSP_ move,
...
its own PR _before_ StackChan audio starts, with the host-side proof being
...
authored by _agents_ (the product's normal use); one
`leds.set({…, pattern:[…40 ints]})` → parse hits token 65 →
`abort_with_status(TOKEN_LIMIT)` — and abort is the _protocol-correct_
...
_partially emitted_ message cannot be recovered"). Normal operation
...
_pull_ time and retained until remote release; pipelined error propagation
copies the _pointers_ between pending slots (`session.c:515-521`). The
...
the tree, and `tests/native_peer.c` (693 lines) is the stdio peer _for_
...
- **Completion-callback double-signal:** BEGIN-failure inside `call_*`
  fires the just-allocated import's completion via terminalize _and_
...
the _response_ (drop remainder + `response.cancel`), never the session.
...
downlink while the mic is hot, but the stale tail plays _after release_.
...
- **PCM constants not single-sourced**: `640`/`20 ms`/subprotocol literals
  ...
  `firmware-architecture.test.ts:100` pins a _third re-typed literal_
...
Voice proxy (independent, can land in parallel): 9. **V-S1** downlink queue sized for real replies + response-scoped
truncation; **V-S2** VAD barge-in flush (they compose). 10. **V-S2** anchored pacing; backpressure = drop-not-die; per-send copy;
`looseObject` credential parse; mint timeout.

Structure (before StackChan audio starts): 11. **§7** audio task extraction. 12. **S3-1** ws-worker/wifi-link extraction (folds S1-1/S2-2 into one
place); host-test harness for the extracted worker (§11 fake-client
pattern). 13. **S3-2/S3-3/S3-4** metrics table-builder, photo capability decision,
