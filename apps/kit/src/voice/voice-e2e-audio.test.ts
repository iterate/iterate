...
describe("voice e2e audio", () => {
  ...
  test("rejects samples and frame sizes which cannot contain PCM16", () => {
    expect(() =>
      createFixedPcmFrames(Uint8Array.from([1]), 640, 0),
    ).toThrow("PCM16 input");
    expect(() =>
      createFixedPcmFrames(new Uint8Array(2), 639, 0),
    ).toThrow("frame size");
    expect(() =>
      createFixedPcmFrames(new Uint8Array(2), 640, -1),
    ).toThrow("silence frame count");
  });
});
