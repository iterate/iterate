...
import { connectGrokRealtimeVoice, type GrokWebSocketFactory } from "./grok-realtime-voice.ts";
...
describe("Grok realtime voice connector", () => {
  test("substitutes the server API key for a short-lived WebSocket credential", async () => {
    ...
    expect(requests[0]?.headers.get("authorization")).toBe("Bearer xai-server-secret");
    expect(protocols).toEqual(["xai-client-secret.xai-realtime-client-secret-test"]);
