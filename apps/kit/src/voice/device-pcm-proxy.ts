...
class DevicePcmProxySession implements Disposable {
  ...
  constructor(options: DevicePcmProxySessionOptions) {
    ...
    this.#device.addEventListener("close", (event) => this.#closeFrom(event, this.#provider), {
      once: true,
    });
    this.#provider.addEventListener("close", (event) => this.#closeFrom(event, this.#device), {
      once: true,
    });
  ...
  #sendNextDownlinkFrame() {
    if (this.#closed || this.#suppressDownlink || this.#downlinkQueuedBytes === 0) {
      return;
    }
  ...
  #closeFrom(event: CloseEvent, counterpart: WebSocket) {
    if (this.#closed) return;
    this.#closed = true;
    this.#clearDownlinkQueue();
