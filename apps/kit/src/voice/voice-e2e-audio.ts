export function createFixedPcmFrames(
  ...
  if (!Number.isSafeInteger(frameBytes) || frameBytes <= 0 || frameBytes % 2 !== 0) {
    throw new Error("The PCM frame size must be a positive, even integer.");
  }
  if (!Number.isSafeInteger(trailingSilenceFrames) || trailingSilenceFrames < 0) {
    throw new Error("The trailing silence frame count must be a non-negative integer.");
  }
