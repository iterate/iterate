...
export async function connectGrokRealtimeVoice(
  ...
  if (!supportedPcmSampleRates.has(options.sampleRateHz)) {
    throw new Error(
      `Grok does not support ${options.sampleRateHz} Hz PCM audio.`,
    );
  }
  ...
  const parsedCredential = GrokClientSecret.safeParse(
...
function waitForOpen(socket: WebSocket, timeoutMs: number) {
  ...
  return new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          `Grok WebSocket handshake timed out after ${timeoutMs}ms.`,
        ),
      );
    }, timeoutMs);
    const opened = () => {
    ...
    const closed = (event: CloseEvent) => {
      ...
      reject(
        new Error(
          `Grok WebSocket closed during handshake with code ${event.code}.`,
        ),
      );
    };
    const cleanup = () => {
