...
export async function prepareLocalEspIdfFlashPlan(input: {
  ...
  const expectedChip = esptoolChipName(
    input.device.installMethod.chipFamily,
  );
  ...
  const bytesByDescriptor = new Map(
    parts.map((part) => [part.descriptor, part.bytes] as const),
  );
  ...
  return prepareFirmwareFlashPlan({
    ...
    loadPart: async (descriptor) => {
      ...
      if (!bytes) {
        throw new Error(
          `ESP-IDF flash part ${descriptor.fileName} was not loaded.`,
        );
      }
      return bytes;
    },
...
async function readFlasherMetadata(buildDirectory: string) {
  ...
  try {
    value = JSON.parse(
      await readFile(resolve(buildDirectory, "flasher_args.json"), "utf8"),
    );
  } catch (error) {
  ...
  if (
    ...
    throw new Error(
      "ESP-IDF flasher_args.json is missing chip or flash-file metadata.",
    );
  }
