 ...
 */
export function decodeDeviceConfiguration(
  image: Uint8Array,
): DeviceConfiguration {
  ...
  if (
    textDecoder.decode(image.slice(0, CONFIG_MAGIC.length)) !==
    CONFIG_MAGIC
  ) {
  ...
  const payload = image.slice(
    CONFIG_HEADER_BYTES,
    CONFIG_HEADER_BYTES + payloadLength,
  );
  ...
  for (let offset = 0; offset < payload.byteLength; ) {
    ...
    const fieldLength = new DataView(
      payload.buffer,
    ...
    if (fields.has(tag)) {
      throw new Error(
        `Device configuration contains a duplicate ${fieldName} field.`,
      );
    }
    let value: string;
    try {
      ...
      throw new Error(
        `Device configuration ${fieldName} is not valid UTF-8.`,
      );
    }
    if (value.includes("\0")) {
      throw new Error(
        `Device configuration ${fieldName} contains a NUL byte.`,
      );
    }
    if (
      value.length === 0 &&
      tag !== ConfigurationField.WifiPassword
    ) {
      throw new Error(
        `Device configuration ${fieldName} must not be empty.`,
      );
    }
  ...
  for (const [tag, fieldName] of configurationFieldNames) {
    if (!fields.has(tag)) {
      throw new Error(
        `Device configuration is missing the ${fieldName} field.`,
      );
    }
  }
  ...
  const projectId = requiredConfigurationField(
    fields,
    ConfigurationField.ProjectId,
  );
  ...
  return {
    schemaVersion: 1,
    wifi: {
      ssid: requiredConfigurationField(
        fields,
        ConfigurationField.WifiSsid,
      ),
      password: requiredConfigurationField(
        fields,
        ConfigurationField.WifiPassword,
      ),
    },
    iterate: {
      baseUrl,
      projectId,
      projectApiKey: requiredConfigurationField(
        fields,
        ConfigurationField.ProjectApiKey,
      ),
    },
