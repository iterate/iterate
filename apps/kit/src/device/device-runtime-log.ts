...
export class DeviceRuntimeMetricsContinuity {
  readonly #latestByFamily = new Map<
    string,
    { reportSequence: number; sampleMs: number }
  >();
  #latestReportSequence: number | undefined;
  #latestSampleMs: number | undefined;

  observe(
    observation: DeviceRuntimeLogObservation,
  ): DeviceRuntimeLogObservation | undefined {
    if (
      observation.kind !== "metrics" ||
      !sequencedRuntimeMetricFamilies.has(observation.family)
    ) {
      return;
    }
    ...
    if (
      previous &&
      (reportSequence <= previous.reportSequence ||
        sampleMs <= previous.sampleMs)
    ) {
      return {
        kind: "failure",
        reason:
    ...
    if (
      this.#latestReportSequence !== undefined &&
      reportSequence < this.#latestReportSequence
    ) {
      return {
        kind: "failure",
        reason:
...
const kitAudioMetricFields = [
  ...
  [
    ["uplink", "consecutiveSendDeferrals"],
    "uplink_consecutive_send_deferrals",
  ],
  [
    ["uplink", "maximumConsecutiveSendDeferrals"],
    "uplink_maximum_consecutive_send_deferrals",
  ],
  [["uplink", "restartIncidents"], "uplink_restart_incidents"],
  [
    ["uplink", "producerBackpressureRestarts"],
    "uplink_producer_backpressure_restarts",
  ],
  [
    ["uplink", "transportDisconnectRestarts"],
    "uplink_transport_disconnect_restarts",
  ],
  [
    ["uplink", "noProgressTimeoutRestarts"],
    "uplink_no_progress_timeout_restarts",
  ],
  [
    ["uplink", "frameSendTimeoutRestarts"],
    "uplink_frame_send_timeout_restarts",
  ],
  [
    ["uplink", "captureStaleRestarts"],
    "uplink_capture_stale_restarts",
  ],
  [
    ["uplink", "lastTransportAcceptAgeMs"],
    "uplink_last_transport_accept_age_ms",
  ],
  [
    ["uplink", "maximumTransportAcceptAgeMs"],
    "uplink_maximum_transport_accept_age_ms",
  ],
  [
    ["uplink", "lastRestartOldestCaptureAgeMs"],
    "uplink_last_restart_oldest_capture_age_ms",
  ],
  [
    ["uplink", "lastRestartFramesDiscarded"],
    "uplink_last_restart_frames_discarded",
  ],
...
export function parseKitMetricsCallback(value: unknown): DeviceRuntimeLogObservation {
  ...
  if (audio !== undefined) {
    ...
    const lastRestartReason = nestedString(audio, [
      "uplink",
      "lastRestartReason",
    ]);
    if (
      lastRestartReason === undefined ||
      !pcmUplinkRestartReasons.has(lastRestartReason)
    ) {
      return malformedKitMetrics(
        "audio.uplink.lastRestartReason must be a known restart reason",
      );
    }
    ...
    if (unparsedBuffers !== undefined) {
      ...
      appendKitBufferMetrics(
        values,
        "buffer_peer_unconfirmed",
        parsedBuffers.data.peerUnconfirmed,
      );
...
export function deviceVoiceRoundTripCompleted(
  ...
  return (
    deviceUplinkStreaming(before, after) &&
    devicePlaybackCompleted(before, after)
  );
}

export function deviceUplinkStreaming(
  before: DeviceRuntimeMetrics,
  after: DeviceRuntimeMetrics,
) {
  return (
    metricIncreased(before, after, "audio_sent") &&
    metricIncreased(before, after, "uplink_sent")
  );
}

export function devicePlaybackCompleted(
  before: DeviceRuntimeMetrics,
  after: DeviceRuntimeMetrics,
) {
  return (
...
export function parseDeviceRuntimeLogLine(line: string): DeviceRuntimeLogObservation | undefined {
  ...
  if (pcmRecoveryIndex !== -1) {
    return parseMetrics(
      line.slice(pcmRecoveryIndex + pcmRecoveryMarker.length),
      "pcm-recovery",
    );
  }
