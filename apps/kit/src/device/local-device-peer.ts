...
export class LocalDevicePeer implements Disposable {
  ...
  #nextMount: Deferred<MountedM5StickS3> =
    Promise.withResolvers<MountedM5StickS3>();
  #disposed = false;
  ...
  waitForMount(): Promise<MountedM5StickS3> {
    this.#assertNotDisposed();
    return this.#activeMount
      ? Promise.resolve(this.#activeMount)
      : this.#nextMount.promise;
  }
  ...
  assertProjectReference(projectReference: string) {
    if (projectReference !== this.#projectId) {
      throw new Error(
        `The device requested unexpected project reference ${projectReference}.`,
      );
    }
  }

  acceptsProjectBearerCredentials(
    projectReference: string,
    bearerToken: string,
  ) {
    return (
      projectReference === this.#projectId &&
      bearerToken === this.#projectSecret
    );
  }
  ...
  mount(input: unknown): ActiveMount {
    ...
    if (
      ...
      throw new TypeError(
        "The live capability must be a retained Cap'n Web device stub.",
      );
    }
...
function sameStringArray(
  ...
  return (
    ...
    value.every(
      (part, index) =>
        typeof part === "string" && part === expected[index],
    )
  );
}
