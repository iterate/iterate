...
import {
  ...
  type RetainedPcm16ArtifactInspection,
} from "./retained-pcm16-artifact.ts";

const metricsExpectedIntervalMs = 1_000;
const metricDuplicateBudget = 2;
const maximumHostDeliveryLagGrowthMs = 250;
const maximumRetainedDeliveryIncidentCount = 16;
...
export interface M5StickS3PlaybackEnduranceSourceEvidence {
  artifactPath: string;
  delivery: {
...
export type M5StickS3PlaybackEnduranceSourceArtifactInspector = (options: {
  artifactPath: string;
  expectedByteLength: number;
  expectedSha256: string;
}) => Promise<RetainedPcm16ArtifactInspection> | RetainedPcm16ArtifactInspection;
...
export function createM5StickS3PlaybackEnduranceTarget(options: {
  ...
  createRunId(): string;
  inspectSourceArtifact?: M5StickS3PlaybackEnduranceSourceArtifactInspector;
  monotonicNow(): number;
  ...
  return {
    ...
    async runPlayback(request) {
      ...
      const acousticChallenge = createDualCarrierPrbs31Challenge({ runId });
      ...
      assertSourceEvidence(stage.sourceEvidence);
      const inspectedSourceArtifact = await (
        options.inspectSourceArtifact ?? inspectRetainedPcm16Artifact
      )({
        artifactPath: stage.sourceEvidence.artifactPath,
        expectedByteLength: exactRequest.pcmSource.artifact.byteLength,
        expectedSha256: exactRequest.pcmSource.artifact.sha256,
      });
      ...
      return {
        ...
        pcmSource: {
          artifact: {
            byteLength: inspectedSourceArtifact.byteLength,
            format: exactRequest.pcmSource.artifact.format,
            hashVerification: {
              computedSha256: inspectedSourceArtifact.sha256,
              matched:
                inspectedSourceArtifact.sha256 === exactRequest.pcmSource.artifact.sha256 &&
                inspectedSourceArtifact.byteLength === exactRequest.pcmSource.artifact.byteLength,
            },
            path: stage.sourceEvidence.artifactPath,
            sampleRateHz: exactRequest.pcmSource.artifact.sampleRateHz,
            sha256: exactRequest.pcmSource.artifact.sha256,
          },
          delivery: structuredClone(stage.sourceEvidence.delivery),
          expectedByteLength: exactRequest.pcmSource.artifact.byteLength,
          inspectionMaximumBufferedAudioBytes: inspectedSourceArtifact.maximumBufferedAudioBytes,
        },
      };
    },
  };
...
function assertSourceEvidence(evidence: M5StickS3PlaybackEnduranceSourceEvidence) {
  if (typeof evidence !== "object" || evidence === null || Array.isArray(evidence)) {
    throw new Error("The M5StickS3 runtime did not retain PCM source evidence.");
  }
  if (
    typeof evidence.artifactPath !== "string" ||
    !evidence.artifactPath.trim() ||
    evidence.artifactPath.includes("\0")
  ) {
    throw new Error("The M5StickS3 runtime returned an invalid retained PCM source path.");
  }
  const delivery = evidence.delivery;
  if (typeof delivery !== "object" || delivery === null || Array.isArray(delivery)) {
    throw new Error("The M5StickS3 runtime omitted PCM source delivery evidence.");
  }
  if (!Number.isSafeInteger(delivery.discontinuityCount) || delivery.discontinuityCount < 0) {
    throw new Error("The PCM source discontinuity count must be a nonnegative safe integer.");
  }
  if (!Number.isSafeInteger(delivery.emittedFrameCount) || delivery.emittedFrameCount < 0) {
    throw new Error("The PCM source emitted frame count must be a nonnegative safe integer.");
  }
  if (
    !Array.isArray(delivery.retainedIncidents) ||
    delivery.retainedIncidents.length > maximumRetainedDeliveryIncidentCount ||
    delivery.retainedIncidents.length > delivery.discontinuityCount ||
    (delivery.discontinuityCount > 0 && delivery.retainedIncidents.length === 0)
  ) {
    throw new Error(
      "The PCM source incidents must provide bounded concrete reasons for discontinuities.",
    );
  }
  for (const [index, incident] of delivery.retainedIncidents.entries()) {
    if (
      ...
      throw new Error(`PCM source incident ${index} has an invalid layer.`);
    }
    if (
      !Number.isSafeInteger(incident.observedAtMonotonicMs) ||
      incident.observedAtMonotonicMs < 0
    ) {
      throw new Error(`PCM source incident ${index} has an invalid monotonic timestamp.`);
    }
    if (
      ...
      throw new Error(`PCM source incident ${index} must retain a concrete bounded reason.`);
    }
  }
  if (
    typeof delivery.timing !== "object" ||
    delivery.timing === null ||
    Array.isArray(delivery.timing) ||
    delivery.timing.schedule !== "absolute-media-deadlines"
  ) {
    throw new Error("The PCM source omitted absolute-media-deadline timing evidence.");
  }
  assertPcmCadenceEvidence("provider", delivery.timing.provider, "host-monotonic");
  assertPcmCadenceEvidence("device ingress", delivery.timing.deviceIngress, "device-monotonic");
}

function assertPcmCadenceEvidence(
  name: string,
  ...
  if (
    ...
    evidence.clock !== expectedClock
  ) {
    throw new Error(`The PCM ${name} cadence did not use the required ${expectedClock} clock.`);
  }
  for (const [field, value] of Object.entries(evidence)) {
    if (field === "clock") continue;
    if (!Number.isSafeInteger(value) || value < 0) {
      throw new Error(`The PCM ${name} cadence field ${field} must be a nonnegative integer.`);
    }
  }
  if (evidence.firstFrameIndex !== 0 || evidence.lastFrameIndex < evidence.firstFrameIndex) {
    throw new Error(`The PCM ${name} cadence did not retain the complete frame-index span.`);
  }
  if (
    evidence.firstFrameObservedAtMonotonicUs < evidence.timelineOriginAtMonotonicUs ||
    evidence.lastFrameObservedAtMonotonicUs < evidence.firstFrameObservedAtMonotonicUs
  ) {
    throw new Error(`The PCM ${name} cadence timestamps are not monotonic.`);
  }
  if (evidence.minimumInterFrameGapUs > evidence.maximumInterFrameGapUs) {
    throw new Error(`The PCM ${name} minimum inter-frame gap exceeds its maximum.`);
  }
  if (evidence.gapCount !== evidence.lastFrameIndex - evidence.firstFrameIndex) {
    throw new Error(`The PCM ${name} gap count does not match its frame-index span.`);
  }
  if (evidence.earlyGapCount + evidence.lateGapCount > evidence.gapCount) {
    throw new Error(`The PCM ${name} out-of-policy gap counts exceed its total gap count.`);
  }
  if (
    evidence.interFrameGapP50Us < evidence.minimumInterFrameGapUs ||
    evidence.interFrameGapP50Us > evidence.interFrameGapP95Us ||
    evidence.interFrameGapP95Us > evidence.interFrameGapP99Us ||
    evidence.interFrameGapP99Us > evidence.maximumInterFrameGapUs
  ) {
    throw new Error(`The PCM ${name} gap quantiles are inconsistent with its extrema.`);
  }
}
