...
export function buildPlaybackEnduranceRunManifest(options: {
  ...
  const pcmSource = options.observation.pcmSource;

  /*
   * A complete source file is necessary but not sufficient: it proves what
   * was generated, while delivery incidents prove whether that timeline
   * survived the live provider/proxy path. Derive the canonical source again
   * from the retained run challenge so neither the runtime nor target adapter
   * gets to nominate a convenient expected hash.
   */
  if (watermark && !pcmSource) {
    reasons.push("the run-keyed playback stage omitted retained PCM source evidence");
  }
  if (pcmSource) {
    const canonicalSource = watermark
      ? computeDualCarrierPrbs31Pcm16SourceIdentity({
          challenge: watermark.challenge,
          durationMs: options.durationMs,
        })
      : undefined;
    if (
      !pcmSource.artifact.hashVerification.matched ||
      pcmSource.artifact.hashVerification.computedSha256 !== pcmSource.artifact.sha256
    ) {
      reasons.push("retained PCM source SHA-256 verification did not match the source artifact");
    }
    if (pcmSource.artifact.byteLength !== pcmSource.expectedByteLength) {
      reasons.push(
        `retained PCM source has ${pcmSource.artifact.byteLength} bytes but ` +
          `${pcmSource.expectedByteLength} were expected`,
      );
    }
    if (
      watermark &&
      canonicalSource &&
      (pcmSource.expectedByteLength !== canonicalSource.byteLength ||
        pcmSource.artifact.sha256 !== canonicalSource.sha256 ||
        pcmSource.artifact.sampleRateHz !== watermark.challenge.sampleRateHz)
    ) {
      reasons.push("retained PCM source identity does not match the canonical run challenge");
    }
    if (pcmSource.delivery.emittedFrameCount !== expectedFrameCount) {
      reasons.push(
        `PCM source emitted ${pcmSource.delivery.emittedFrameCount} frames but ` +
          `${expectedFrameCount} were required`,
      );
    }
    if (pcmSource.delivery.discontinuityCount > 0) {
      reasons.push(
        `PCM source reported ${pcmSource.delivery.discontinuityCount} delivery discontinuities`,
      );
    }
    for (const incident of pcmSource.delivery.retainedIncidents) {
      reasons.push(
        `PCM ${incident.layer} discontinuity at ${incident.observedAtMonotonicMs}ms: ` +
          incident.reason,
      );
    }
    if (!options.thresholds.pcmSourceTiming) {
      reasons.push("the PCM source has no versioned cadence acceptance thresholds");
    } else {
      reasons.push(
        ...assessPcmCadenceEvidence({
          evidence: pcmSource.delivery.timing.provider,
          expectedClock: "host-monotonic",
          expectedFrameCount,
          frameDurationMs: options.thresholds.pcmFrameDurationMs,
          label: "provider",
          thresholds: options.thresholds.pcmSourceTiming,
        }),
        ...assessPcmCadenceEvidence({
          evidence: pcmSource.delivery.timing.deviceIngress,
          expectedClock: "device-monotonic",
          expectedFrameCount,
          frameDurationMs: options.thresholds.pcmFrameDurationMs,
          label: "device ingress",
          thresholds: options.thresholds.pcmSourceTiming,
        }),
      );
    }
  ...
  return {
    ...
    pcmSource: pcmSource ? structuredClone(pcmSource) : undefined,
    result: {
...
function assessPcmCadenceEvidence(options: {
  evidence: PlaybackEndurancePcmCadenceEvidence;
  expectedClock: PlaybackEndurancePcmCadenceEvidence["clock"];
  expectedFrameCount: number;
  frameDurationMs: number;
  label: string;
  thresholds: NonNullable<PlaybackEnduranceThresholds["pcmSourceTiming"]>;
}) {
  const reasons: string[] = [];
  if (options.evidence.clock !== options.expectedClock) {
    reasons.push(`PCM ${options.label} cadence used ${options.evidence.clock} timing`);
  }
  if (
    options.evidence.firstFrameIndex !== 0 ||
    options.evidence.lastFrameIndex !== options.expectedFrameCount - 1
  ) {
    reasons.push(
      `PCM ${options.label} cadence covered frames ${options.evidence.firstFrameIndex}..` +
        `${options.evidence.lastFrameIndex} instead of 0..${options.expectedFrameCount - 1}`,
    );
  }
  const expectedGapCount = Math.max(0, options.expectedFrameCount - 1);
  if (options.evidence.gapCount !== expectedGapCount) {
    reasons.push(
      `PCM ${options.label} retained ${options.evidence.gapCount} inter-frame gaps but ` +
        `${expectedGapCount} were required`,
    );
  }
  /*
   * Extrema prove that a bad interval existed; counts show whether it was one
   * scheduler hiccup or sustained burst-and-stall behaviour. The latter is
   * exactly what an all-bytes-delivered tunnel can otherwise conceal.
   */
  if (options.evidence.earlyGapCount > 0 || options.evidence.lateGapCount > 0) {
    reasons.push(
      `PCM ${options.label} observed ${options.evidence.earlyGapCount} gaps below ` +
        `${options.thresholds.minimumInterFrameGapUs}us and ` +
        `${options.evidence.lateGapCount} gaps above ` +
        `${options.thresholds.maximumInterFrameGapUs}us`,
    );
  }
  if (options.evidence.missedDeadlineCount > 0) {
    reasons.push(
      `PCM ${options.label} missed ${options.evidence.missedDeadlineCount} absolute media ` +
        `deadlines; maximum frame lateness was ${options.evidence.maximumFrameLatenessUs}us`,
    );
  } else if (options.evidence.maximumFrameLatenessUs > options.thresholds.maximumFrameLatenessUs) {
    reasons.push(
      `PCM ${options.label} maximum frame lateness ` +
        `${options.evidence.maximumFrameLatenessUs}us exceeded ` +
        `${options.thresholds.maximumFrameLatenessUs}us`,
    );
  }
  if (options.evidence.maximumInterFrameGapUs > options.thresholds.maximumInterFrameGapUs) {
    reasons.push(
      `PCM ${options.label} maximum inter-frame gap ` +
        `${options.evidence.maximumInterFrameGapUs}us exceeded ` +
        `${options.thresholds.maximumInterFrameGapUs}us`,
    );
  }
  if (options.evidence.minimumInterFrameGapUs < options.thresholds.minimumInterFrameGapUs) {
    reasons.push(
      `PCM ${options.label} minimum inter-frame gap ` +
        `${options.evidence.minimumInterFrameGapUs}us was below ` +
        `${options.thresholds.minimumInterFrameGapUs}us`,
    );
  }
  const lastScheduledAtMonotonicUs =
    options.evidence.timelineOriginAtMonotonicUs +
    options.evidence.lastFrameIndex * options.frameDurationMs * 1_000;
  const observedEndpointLatenessUs = Math.max(
    options.evidence.firstFrameObservedAtMonotonicUs - options.evidence.timelineOriginAtMonotonicUs,
    options.evidence.lastFrameObservedAtMonotonicUs - lastScheduledAtMonotonicUs,
  );
  if (observedEndpointLatenessUs > options.evidence.maximumFrameLatenessUs) {
    reasons.push(
      `PCM ${options.label} endpoint lateness ${observedEndpointLatenessUs}us exceeded its ` +
        `reported maximum ${options.evidence.maximumFrameLatenessUs}us`,
    );
  }
  return reasons;
}
