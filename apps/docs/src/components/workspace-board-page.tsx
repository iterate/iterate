...
/** The board's URL-held view state, owned by whichever route mounts the page. */
export type BoardSearch = {
  group: "none" | "label" | "folder";
  q: string;
  task: string;
};
    ...
    (task: BoardTask, state: string, folder: string, labels?: string[]) => {
      ...
      void board
        .renameTask(task.path, nextPath, transform(sourceOf(task)), transform, async () => {
          ...
          // draftPathRef mirrors draftPath each render, so the focus-ref
          // clear stays OUT of the state updater (which must be pure).
          if (draftPathRef.current === task.path) draftFocusRef.current = undefined;
      ...
      try {
        ...
        return await board.renameTask(
          ...
          async () => {
            ...
            // Same pure-updater shape as the rename path above.
            if (draftPathRef.current === task.path) draftFocusRef.current = undefined;
            setDraftPath((current) => (current === task.path ? nextPath : current));
