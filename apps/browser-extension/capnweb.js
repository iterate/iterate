...
var RpcPayload = class RpcPayload {
	...
	getHookForRpcTarget(target, parent, dupStubs = true) {
		...
		} else if (this.source === "return") return this.getHookForReturn(target, dupStubs, () => TargetStubHook.create(target, parent));
	...
	getHookForWritableStream(stream, parent, dupStubs = true) {
		...
		else if (this.source === "return") return this.getHookForReturn(stream, dupStubs, () => streamImpl.createWritableStreamHook(stream));
	...
	getHookForReadableStream(stream, parent, dupStubs = true) {
		...
		else if (this.source === "return") return this.getHookForReturn(stream, dupStubs, () => streamImpl.createReadableStreamHook(stream));
	...
	getHookForWebSocket(webSocket, makeHook) {
		...
		if (this.sentWebSockets?.has(webSocket)) throw new Error("A WebSocket can only be sent over RPC once.");
...
var ValueStubHook = class extends StubHook {
	call(path, args) {
		...
		let promise;
		try {
			promise = args.callHandler ? args.callHandler({
				path: [...path],
...
function closeSocket(socket, code, reason) {
	try {
		if (code === 1e3 || code !== void 0 && code >= 3e3 && code <= 4999) socket.close(code, reason);
		else socket.close();
	} catch {}
}
	...
	async #readLoop(reader) {
		while (true) {
			...
			if (done) {
				...
				return;
			} else this.#dispatchEvent("message", {
				type: "message",
	...
	devaluateImpl(value, parent, depth) {
		...
		switch (typeForRpc(value)) {
			...
			case "response": {
				...
				if (webSocket && resp.body) throw new TypeError("A WebSocket upgrade Response can't have a body.");
		...
		if (value instanceof Array) {
			if (value.length == 1 && value[0] instanceof Array) {
				...
				case "response": {
					...
					if (init.headers && !(init.headers instanceof Array)) throw new TypeError("Request headers must be serialized as an array of pairs.");
					if (init.webSocket) {
						if (body !== null) throw new TypeError("A WebSocket upgrade Response can't have a body.");
						let ws = init.webSocket;
						if (typeof ws !== "object" || ws === null || ws instanceof Array) throw new TypeError("Response webSocket must be serialized as a pair of streams.");
						let readable = this.evaluateImpl(ws.readable, ws, "readable", depth + 1);
						if (!(readable instanceof ReadableStream)) throw new TypeError("Response webSocket readable must be a ReadableStream.");
						let writable = ws.writable;
						if (!(writable instanceof Array) || writable.length !== 2 || writable[0] !== "writable" || typeof writable[1] !== "number") throw new TypeError("Response webSocket writable must be a WritableStream.");
				...
				case "pipeline": {
					...
					args = new Evaluator(this.importer, this.encodingLevel, this.callHandler).evaluateWithDepth([args], depth);
