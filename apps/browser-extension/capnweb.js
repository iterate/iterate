...
const DISPOSED_HOOK = new ErrorStubHook(
  /* @__PURE__ */ new Error("Attempted to use RPC stub after it has been disposed."),
);
...
var RpcStub$1 = class RpcStub$1 extends RpcTarget$1 {
  constructor(hook, pathIfPromise) {
    ...
    if (!(hook instanceof StubHook)) {
      let value = hook;
      if (value instanceof RpcTarget$1 || value instanceof Function)
  ...
  constructor(hook, pathIfPromise) {
    if (hook instanceof StubHook) super(hook, pathIfPromise ?? []);
      ...
      if (kind === "rpc-promise") {
        ...
        let promiseHook = new PromiseStubHook(
...
var RpcPayload = class RpcPayload {
  ...
  rpcTargets;
  getHookForReturn(key, dupStubs, create) {
    let hook = this.rpcTargets?.get(key);
    if (hook)
      if (dupStubs) return hook.dup();
      else {
  ...
  getHookForWebSocket(webSocket, makeHook) {
    if (this.source === "owned") throw new Error("owned payload shouldn't contain raw WebSockets");
    if (this.sentWebSockets?.has(webSocket))
      throw new Error("A WebSocket can only be sent over RPC once.");
    if (!this.sentWebSockets) this.sentWebSockets = /* @__PURE__ */ new Set();
    this.sentWebSockets.add(webSocket);
    let hook = makeHook();
    if (this.source === "params") return hook;
    else {
      if (!this.rpcTargets) this.rpcTargets = /* @__PURE__ */ new Map();
  ...
  getExistingHookForWebSocket(webSocket, dupStubs) {
    ...
    if (!hook) return;
    else if (dupStubs) return hook.dup();
    else {
  ...
  deepCopy(value, oldParent, property, parent, dupStubs, owner) {
    switch (typeForRpc(value)) {
      ...
      case "rpc-thenable": {
        ...
        let promise;
        if (owner)
          promise = new RpcPromise$1(owner.getHookForRpcTarget(target, oldParent, dupStubs), []);
  ...
  deliverTo(parent, property, promises) {
    ...
    if (this.value instanceof RpcPromise$1)
  ...
  call(path, args) {
    ...
    if (typeof followResult.value != "function") {
      ...
      return new ErrorStubHook(
        /* @__PURE__ */ new TypeError(`'${path.join(".")}' is not a function.`),
      );
    }
    ...
    const invoke = () => {
      invoked = true;
      return args.deliverCall(func, followResult.parent);
    };
    let promise;
    try {
      promise = args.callHandler
        ? args.callHandler(
            {
              path: [...path],
...
function closeSocket(socket, code, reason) {
  try {
    if (code === 1e3 || (code !== void 0 && code >= 3e3 && code <= 4999))
...
var TunneledWebSocket = class TunneledWebSocket {
  static CONNECTING = 0;
  static OPEN = 1;
  static CLOSING = 2;
  static CLOSED = 3;
  ...
  constructor(readable, writableHook) {
    ...
    writableHook.onBroken((error) => this.#fail(error));
  }
  get readyState() {
...
function upgradeWebSocketResponse(webSocket, init) {
  let missing = ["send", "close", "addEventListener"].filter(
    (method) => typeof webSocket?.[method] !== "function",
  );
  if (missing.length > 0)
    throw new TypeError(
      `upgradeWebSocketResponse() expected a WebSocket-like object with callable send(), close(), and addEventListener(), but ${missing.join(", ")} ${missing.length === 1 ? "is" : "are"} missing. (Did you pass the whole WebSocketPair instead of one half?)`,
    );
  if (init?.status !== void 0 && init.status !== 101)
    throw new TypeError(
      `A WebSocket upgrade Response implies status 101; got status ${init.status}. Omit init.status (or pass 101).`,
    );
  if (nativeWebSocketPair !== void 0 && webSocket instanceof globalThis.WebSocket)
    return new Response(null, {
      ...init,
      status: 101,
      webSocket,
...
var JsWebSocketHalf = class JsWebSocketHalf {
  static CONNECTING = 0;
  static OPEN = 1;
  static CLOSING = 2;
  static CLOSED = 3;
...
for (let type of Object.keys(BYTE_CONTAINER_PROTOTYPES))
    ...
    switch (typeForRpc(value)) {
      ...
      case "bytes": {
        ...
        if (
          ...
          return alternateTypeName === void 0
            ? ["bytes", bytes]
            : ["bytes", bytes, alternateTypeName];
        let b64;
        ...
        b64 = b64.replace(/=+$/, "");
        return alternateTypeName === void 0 ? ["bytes", b64] : ["bytes", b64, alternateTypeName];
      }
      case "url":
      ...
      case "rpc-promise": {
        ...
        if (importId !== void 0)
          if (pathIfPromise)
            if (pathIfPromise.length > 0) return ["pipeline", importId, pathIfPromise];
...
function fixBrokenRequestBody(request, body) {
  return new RpcPromise$1(
    new PromiseStubHook(
      new Response(body).arrayBuffer().then((arrayBuffer) => {
...
function streamToBlobPromise(stream, type) {
  return new RpcPromise$1(
    new PromiseStubHook(
      streamToBlob(stream, type).then((blob) => {
...
var Evaluator = class Evaluator {
  ...
  evaluateImpl(value, parent, property, depth) {
    ...
    if (depth >= maxDepth)
      throw new TypeError(`Deserialization exceeded maximum allowed message depth of ${maxDepth}.`);
        ...
        switch (value[0]) {
          ...
          case "bytes": {
            ...
            if (value[1] instanceof Uint8Array) bytes = value[1];
              ...
              if (typeof Buffer !== "undefined") bytes = Buffer.from(value[1], "base64");
            ...
            if (elementSize !== void 0 && bytes.byteLength % elementSize !== 0)
              throw new TypeError(
                `Invalid byte length ${bytes.byteLength} for ${marker}; expected a multiple of ${elementSize}`,
              );
            let buffer = bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength);
            if (!NATIVE_LITTLE_ENDIAN && elementSize !== void 0)
          ...
          case "response": {
            ...
            if (init.webSocket) {
              ...
              if (
                !(writable instanceof Array) ||
                writable.length !== 2 ||
                writable[0] !== "writable" ||
                typeof writable[1] !== "number"
              )
          ...
          case "pipeline": {
            ...
            if (value.length == 2)
              if (isPromise) return addStub(hook.get([]));
    ...
    case "object": {
      ...
      if (value instanceof Error) {
        let size =
          ...
          estimateStringSize(value.name) +
          estimateStringSize(value.message) +
          estimateStringSize(value.stack ?? "");
        for (let key of Object.keys(value))
          size +=
            ESTIMATED_ENTRY_OVERHEAD +
            estimateStringSize(key) +
            estimateEncodedSize(value[key], seen, depth + 1);
        return size;
      }
      let size = ESTIMATED_OBJECT_OVERHEAD;
      for (let key of Object.keys(value))
...
var ImportTableEntry = class {
  ...
  dispose() {
    if (this.resolution) this.resolution.dispose();
      ...
      this.abort(
        /* @__PURE__ */ new Error("RPC was canceled because the RpcPromise was disposed."),
      );
      this.sendRelease();
    }
  }
...
var RpcSessionImpl = class {
    ...
    if ("encodingLevel" in transport) {
      ...
      if (raw !== void 0) {
        if (
          raw !== "string" &&
          raw !== "jsonCompatible" &&
          raw !== "jsonCompatibleWithBytes" &&
          raw !== "structuredClonable"
        )
  ...
  shutdown() {
    this.abort(
      /* @__PURE__ */ new Error("RPC session was shut down by disposing the main stub"),
      false,
    );
  }
  exportStub(hook) {
  ...
  send(msg) {
    ...
    if (this.encodingLevel === "string") {
      ...
      try {
        ...
        if (sent !== void 0 && typeof sent.catch === "function")
  ...
  sendStream(id, path, args) {
    ...
    this.imports.push(entry);
    return {
      promise: entry.awaitResolution().then(
        (p) => {
          p.dispose();
  ...
  async readLoop() {
    while (!this.abortReason) {
      ...
      if (this.encodingLevel === "string" && raw.length > this.limits.maxMessageSize)
        throw new TypeError(
      ...
      if (msg instanceof Array)
        switch (msg[0]) {
          case "push":
            if (msg.length > 1) {
              let hook = new PayloadStubHook(
                new Evaluator(this, this.encodingLevel, this.options.onCall).evaluate(msg[1]),
              );
              hook.ignoreUnhandledRejections();
              this.exports.push({
                hook,
          ...
          case "reject": {
            ...
            if (typeof importId == "number" && msg.length > 2) {
              ...
              if (imp)
                if (msg[0] == "resolve")
                  imp.resolve(
                    new PayloadStubHook(new Evaluator(this, this.encodingLevel).evaluate(msg[2])),
                  );
                else {
...
var RpcSession$1 = class {
  ...
  constructor(transport, localMain, options = {}) {
    ...
    if (localMain) mainHook = new PayloadStubHook(RpcPayload.fromAppReturn(localMain));
      ...
      mainHook = new ErrorStubHook(
        /* @__PURE__ */ new Error("This connection has no main object."),
      );
  ...
  constructor(webSocket) {
    ...
    webSocket.addEventListener("message", (event) => {
      if (this.#error) {
        ...
        this.#receivedError(
          /* @__PURE__ */ new TypeError("Received unexpected message type from WebSocket."),
        );
    });
    webSocket.addEventListener("close", (event) => {
      this.#receivedError(
        /* @__PURE__ */ new Error(`Peer closed WebSocket: ${event.code} ${event.reason}`),
      );
    });
  ...
  constructor(port) {
    ...
    port.addEventListener("message", (event) => {
      if (this.#error) {
        ...
        this.#receivedError(/* @__PURE__ */ new Error("Peer closed MessagePort connection."));
...
function applyMapToElement(input, parent, owner, captures, instructions) {
  let mapper = new MapApplicator(
    captures,
    new PayloadStubHook(RpcPayload.deepCopyFrom(input, parent, owner)),
  );
  try {
...
var WritableStreamStubHook = class WritableStreamStubHook extends StubHook {
    ...
    try {
      ...
      return new PromiseStubHook(
        (method === "write"
          ...
          : args.deliverCall(state.writer[method], state.writer)
        ).then((payload) => new PayloadStubHook(payload)),
      );
    } catch (err) {
      args.dispose();
      return new ErrorStubHook(err);
  ...
  get(path) {
    return new ErrorStubHook(
      /* @__PURE__ */ new Error("Cannot access properties on a WritableStream stub"),
    );
  }
  dup() {
...
var ReadableStreamStubHook = class ReadableStreamStubHook extends StubHook {
  ...
  call(path, args) {
    args.dispose();
    return new ErrorStubHook(
      /* @__PURE__ */ new Error("Cannot call methods on a ReadableStream stub"),
    );
  }
  map(path, captures, instructions) {
    for (let cap of captures) cap.dispose();
  ...
  get(path) {
    return new ErrorStubHook(
      /* @__PURE__ */ new Error("Cannot access properties on a ReadableStream stub"),
    );
  }
  dup() {
    let state = this.state;
...
export {
  ...
  RpcStub,
  RpcTarget,
  WebSocketPair$1 as WebSocketPair,
  WebSocketTransport,
  deserialize,
