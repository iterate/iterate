...
const listAllProjectsWithOrganizationSql = `
SELECT p.id,
  p.organization_id AS organizationId,
  p.name,
  p.slug,
...
LIMIT ? OFFSET ?;
`.trim();
const listAllProjectsWithOrganizationQuery = (params: listAllProjectsWithOrganization.Params) => ({
  name: "listAllProjectsWithOrganization",
  sql: listAllProjectsWithOrganizationSql,
  args: [params.limit, params.offset],
});

export const listAllProjectsWithOrganization = Object.assign(
  async function listAllProjectsWithOrganization(
    client: Client,
    params: listAllProjectsWithOrganization.Params,
  ): Promise<listAllProjectsWithOrganization.Result[]> {
    return client.all<listAllProjectsWithOrganization.Result>(
      listAllProjectsWithOrganizationQuery(params),
    );
  },
  { sql: listAllProjectsWithOrganizationSql, query: listAllProjectsWithOrganizationQuery },
);

export namespace listAllProjectsWithOrganization {
  export type Params = {
    limit: number;
    offset: number;
  };
...
const countProjectsSql = `
SELECT COUNT(*) AS total
FROM project;
`.trim();
const countProjectsQuery = { name: "countProjects", sql: countProjectsSql, args: [] };

export const countProjects = Object.assign(
  async function countProjects(client: Client): Promise<countProjects.Result | null> {
    const rows = await client.all<countProjects.Result>(countProjectsQuery);
    return rows.length > 0 ? rows[0] : null;
  },
  { sql: countProjectsSql, query: countProjectsQuery },
);
