...
import { transformLibsqlResultSet, type SqlResultSet } from "@iterate-com/shared/jonasland";
import type { RuntimeEnv } from "~/context.ts";
import {
  deriveAliasFromPath,
  listSqliteSources,
  type SqliteRouteSource,
} from "~/lib/registry-docs.ts";
...
function buildStudioSrc(params: { env: RuntimeEnv; mainAlias: string }): string {
  const studioIframeUrl = new URL(params.env.REGISTRY_DB_STUDIO_EMBED_URL);
