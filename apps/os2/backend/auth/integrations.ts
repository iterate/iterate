...
import { createUserOrganizationAndInstance } from "../org-utils.ts";
    ...
    endpoints: {
      linkSlackBot: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const member = await db.query.instance.findFirst({
          ...
          if (!member) {
            throw new Error("You are not a member of this instance");
          }
        ...
        async (ctx) => {
          ...
          if (!link) {
            ...
            if (existingUser) {
              ...
              const newOrgAndInstance = await createUserOrganizationAndInstance(db, user);

              if (!newOrgAndInstance.instance) {
                return ctx.json({ error: "Failed to create an instance for the user" });
              }

              instanceId = newOrgAndInstance.instance.id;
            }
          ...
          if (instanceId) {
            logger.info(`Slack bot linked to instance ${instanceId}`);
          }
