...
// Slack Bot Scopes (removed conversations.connect:write per Q2)
...
// Google OAuth Scopes (keeping Gmail + Calendar per Q3)
  ...
  ({
    ...
    endpoints: {
      linkSlackBot: createAuthEndpoint(
        ...
        async (ctx) => {
          const { instanceId, callbackURL } = ctx.body;
          ...
          if (!inst || inst.organization.members.length === 0) {
            throw new Error("You are not a member of this instance's organization");
          }
      ...
      callbackSlackBot: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          let instanceId = parsedState.instanceId;
          ...
          if (!tokens.access_token || !tokens.authed_user.id) {
          ...
          if (!link) {
            ...
            if (existingUser) {
              ...
              // Create organization and instance for new user
              const orgSlug = generateSlug(tokens.team?.name || userInfo.name || "workspace");
              const [newOrg] = await db
                .insert(schema.organization)
                .values({
                  name: tokens.team?.name || `${userInfo.name}'s Workspace`,
                  slug: orgSlug,
                })
                .returning();
              ...
              if (newOrg) {
                ...
                const instSlug = generateSlug("default");
            ...
            // Create or update slack user account
          ...
          // Link instance to bot account if we have an instance ID
          if (instanceId && botAccount) {
            await db
              .insert(schema.instanceAccountPermission)
              .values({
