...
const DEFAULT_BASE_URL = "https://os2.iterate-preview-2.com";
...
async function main() {
  const baseUrl = new URL(process.env.OS2_BASE_URL?.trim() || DEFAULT_BASE_URL);
...
function requireAdminApiSecret() {
  ...
  if (!token) {
    throw new Error(
      "APP_CONFIG_ADMIN_API_SECRET, OS2_ADMIN_API_SECRET, OS2_E2E_ADMIN_API_SECRET, or OS2_E2E_MCP_BEARER_TOKEN is required.",
    );
  }
  return token;
}
...
function readProjectSlug() {
  const explicit = process.env.OS2_WORKSPACE_CODEMODE_PROJECT_SLUG?.trim();
...
function projectMcpUrlFor(input: { baseUrl: URL; project: Project }) {
  ...
  throw new Error(
    `Cannot derive project MCP URL for ${input.project.slug} from ${input.baseUrl}. Set OS2_BASE_URL to a known OS2 deployment.`,
  );
}
...
async function runWorkspaceCodemodeProof(input: { bearerToken: string; mcpUrl: URL }) {
  ...
  const client = new Client({
    name: "os2-workspace-codemode-preview-example",
    version: "1.0.0",
  });
