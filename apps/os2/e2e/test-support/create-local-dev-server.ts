...
export async function createLocalDevServer(opts: {
  ...
  const dispose = async () => {
    ...
    if (errors.length > 0) {
      throw new AggregateError(errors, "Failed to dispose local os2 dev server resources.");
    }
  };
  ...
  try {
    ...
    console.info(`[e2e] OS2 dev server: ${devServer.baseUrl}`);
    ...
    try {
      ...
      throw new AggregateError(
        [error, disposeError],
        "Failed to create local os2 dev server and clean up partial resources.",
      );
    }
    throw error;
  }
