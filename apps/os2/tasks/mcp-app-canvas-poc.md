...
Explore adding an MCP App "canvas" surface to the os2 MCP server. The idea is that an MCP client invokes `open_canvas`, gets an inline UI resource, then later tool/provider/codemode activity emits `canvas-updated` events that replace the canvas contents with sanitized HTML.
