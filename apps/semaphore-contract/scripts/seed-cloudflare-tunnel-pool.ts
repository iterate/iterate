...
import { createSemaphoreClient } from "../src/index.ts";
import { makeFunnySlug } from "../../../packages/shared/src/slug-maker.ts";
import { CloudflareTunnelData, cloudflareTunnelType } from "../src/cloudflare-tunnels/types.ts";
  ...
  for (let index = 0; index < missingCount; index += 1) {
    ...
    try {
      ...
      const dnsRecord = await cloudflare.createDnsRecord({
        ...
        tunnelId: tunnel.id,
        comment:
          "Managed by apps/semaphore-contract/scripts/seed-cloudflare-tunnel-pool.ts " +
