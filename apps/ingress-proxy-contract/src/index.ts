...
export const upsertRouteInputSchema = z.object({
  rootHost: rootHostSchema,
  targetUrl: targetUrlSchema,
  metadata: routeSchema.shape.metadata.optional().default({}),
});
...
export const removeRouteInputSchema = z.object({
  rootHost: rootHostSchema,
});

export const removeRouteOutputSchema = z.object({
  deleted: z.boolean(),
});
...
export const ingressProxyContract = oc.router({
  routes: {
    ...
    remove: oc
      ...
      .input(removeRouteInputSchema)
      .output(removeRouteOutputSchema),
  },
});

export type IngressProxyRoute = z.infer<typeof routeSchema>;
