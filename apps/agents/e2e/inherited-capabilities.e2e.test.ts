...
// ONE CAUSE, CLOSED: the agents collection is a userspace facet on `/`, and a context linked to the
// root reaches it through the root's `itx.agents`. It serves a caller beneath the root through
// `forCaller(caller)` (apps/os context/caller-capability.ts), acting through the caller's own walled
// handle — so what it creates is linked to the caller, a relative path is the caller's, and what it
// appends is the caller's own append. Voice's worker does the same (`servesCallers`).
...
test("an agent messages another: the words land stamped with the sender's sandbox and raise a turn that names it; a delete request from beside the receiver changes nothing", async () => {
...
/** `/jail`, a workspace linked to the root, beneath a mask of the root's `itx.tool`. */
async function linkedToTheRootBeneathAMask(name: string) {
