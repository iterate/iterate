...
test("the installed agent settles delayed model streams through env.ITX.get", async () => {
  ...
  try {
    ...
    expect(
      beforeRelease.some((event) => event.type === "events.iterate.com/agent/llm-request-settled"),
    ).toBe(false);
    ...
    expect(
      nullBodyEvents.find((event) => event.type === "events.iterate.com/agent/llm-request-settled")!
        .payload,
    ).toMatchObject({
