...
// `x-iterate-app` header (the edge derives it from the itx.apps.<label> expression — apps/os-next's
...
export default class extends ConfigWorker {
  async fetch(request: Request) {
    ...
    // Route by the app slug, exactly like apps/os-next's config repo: the `agents` app fetches through to
