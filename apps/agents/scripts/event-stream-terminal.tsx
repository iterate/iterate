...
function StreamTerminalApp() {
  ...
  const [navigationState, setNavigationState] = useState(initialStreamTuiNavigationState);
  ...
  const [expandedStreamPaths, setExpandedStreamPaths] = useState<Set<StreamPath>>(
    () => new Set(getDefaultExpandedStreamPaths(args.streamPath)),
  );
    ...
    void (async () => {
      try {
        const stream = await client.stream(
          { path: currentStreamPath, afterOffset: "start" },
  ...
  const streamApi = useMemo<StreamApi>(
    () => ({
      ...
      getState: async (input = {}) =>
        client.getState({ path: resolveStreamPath(input.streamPath) }),
...
function parseArgs(argv: string[]) {
  ...
  if (eventsBaseUrl == null || projectSlug == null || streamPath == null) {
    throw new Error(
      "Usage: bun scripts/event-stream-terminal.tsx --events-base-url <url> --project-slug <slug> --stream-path <path>",
    );
  }
