...
agent in this app (`configs/with-agents/agents/system-prompt.ts` and `codemode-format.ts`), with
...
| `../scripts/build-voice-install.ts`   | Bundles the voice service (`iterate/*` and `zod` left as bare imports the loader links) and adds the agents runtime source, the install each app serves at `/voice-install.json`.                                                                                                                                                                                     |
