...
declare global {
  namespace Cloudflare {
    ...
    interface Env {
      ...
      /** the first-party apps' origins by name, JSON (startAppWorkerConfig): the Users page's "View dash as" */
...
export default createServerEntry({
  async fetch(request) {
    ...
    if (auth) return auth;
    // A signed-in browser landing on `/` goes to the projects (routes/_auth/projects.index.tsx); the
    // landing page is for signing in.
