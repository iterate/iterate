...
Two capability proofs of concept — the blind egress relay and the in-page
debugging demo — are documented together in
[CAPABILITY_DEMOS.md](./CAPABILITY_DEMOS.md). Both show a client process lending
the project a live capability that the worker then calls back over RPC.
...
The in-page debugging demo (`/page-debugging`) and the blind egress relay
(`/playground`) are both documented in
[CAPABILITY_DEMOS.md](./CAPABILITY_DEMOS.md). Each is a self-contained proof of
concept for a client lending the project a live capability that the worker calls
back over RPC.
