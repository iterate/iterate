...
async function loadWorkflowsContext(input: z.infer<typeof WorkflowsInput>) {
  ...
  const tsWorkflowsList = await Promise.all(
    tsWorkflowFileNames.map(async (filename) => {
      ...
      if (!workflow.jobs) {
        const msg = `Workflow ${filename} is missing jobs, something ain't right: ${inspect(workflow)}`;
        throw new Error(msg);
      }
  ...
  const yamlWorkflows: Record<string, WorkflowEntry> = Object.fromEntries(
