...
export const runPreviewServer = {
  ...
  run: dedent`
    ...
    SLACK_CLIENT_ID=fake doppler run -- sh -c "SLACK_CLIENT_ID=fake2 printenv | grep SLACK_CLIENT_ID"
    SLACK_CLIENT_ID=fake doppler run -- sh -c "SLACK_CLIENT_ID=fake pnpm tsx ./alchemy.run cli --dev --stage local-$USER" &
    DEV_PID=\$!
