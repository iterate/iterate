  ...
  on: {
    ...
    workflow_dispatch: {
      inputs: {
        ...
        fly_image_tag: {
          description: `Fly image tag to test (e.g. registry.fly.io/iterate-sandbox:sha-abc1234). Leave empty to test the image built from the ref.`,
          required: false,
          type: "string",
          default: "",
        },
      ...
      steps: [
        ...
        },
        {
          ...
          run: [
            ...
            'input_fly_image_tag="${{ inputs.fly_image_tag }}"',
            'if [ -n "${input_fly_image_tag}" ]; then',
            '  fly_image_tag="${input_fly_image_tag}"',
            "else",
