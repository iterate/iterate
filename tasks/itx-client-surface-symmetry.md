...
- **Browser, hook** — `useItx({ projectId? }): RpcStub<ItxHandle>`
  (`apps/os/src/itx/itx-react.tsx`). Suspends via React `use()`.
- **Browser, non-hook** — `connectItx({ projectId? }): Promise<RpcStub<ItxHandle>>`.
  Same module-level **Map** as `useItx` (one shared socket per context, persists
...
1. **Name**: `withItx` vs `connectItx` vs `useItx`.
2. **Vocabulary**: Node's `context` (`"global" | projectId | slug`) vs the browser's
   `{ projectId }` address. Pick ONE. `projectId` is cleaner for the 99% case but loses the
   `"global"` / slug / context-ref generality `context` had — decide whether to keep a
   general `context` escape hatch alongside `projectId`.
3. **Lifecycle/return**: caller-owned stub (Node) vs shared-socket promise (`connectItx`)
   vs shared-socket+suspend (`useItx`). These are real and should stay — but should be
...
- `connectItx({ projectId? })` — browser non-hook, shared socket, `Promise`.
- `useItx({ projectId? })` — browser hook, shared socket, suspends (thin `use()` over the Map).
...
WebSocket, header vs cookie) and lifecycle (caller-owned vs shared-socket).
...
- **Should the browser non-hook be called `withItx` too?** (runtime-resolved import),
  or keep `connectItx` but make its params/return match the family.
...
- `apps/os/src/itx/itx-react.tsx` — the whole browser surface: `useItx` / `connectItx` /
  `reconnectItx` (get/evict the handle) + `useItxQuery` / `useItxEffect` (read/subscribe),
  one socket per context in a module Map.
...
- **Bag-of-props for the itx _kernel_ verbs.** The FE surface is now props objects
  (`useItx`/`connectItx`/`reconnectItx`/`useItxQuery`/`useItxEffect`).
