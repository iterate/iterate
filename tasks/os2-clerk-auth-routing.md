...
# OS2 Clerk Auth + Organization-Scoped Routing

## Goal

Make OS2's Clerk integration idiomatic and consistent across browser routes,
SSR oRPC, HTTP oRPC, WebSocket oRPC, and the MCP server.

## Resolved Decisions

- OS2 has no public app pages. Unauthenticated browser users go to Clerk sign-in.
- OS2 requires a Clerk Organization context; Personal Account mode cannot own projects.
- User-facing OS2 routes include the owning organization slug.
...
- MCP clients authenticate with Clerk OAuth tokens. JWT is the preferred Clerk
  token format setting, but OS2 should not model MCP authentication as JWT-only.
...
3. Move protected OS2 routes under `/orgs/$organizationSlug`.
...
10. Configure Clerk `taskUrls` for the organization-selection flow if OS2 uses
    Clerk task routing.
...
13. Update `apps/os2/CONTEXT.md` and `apps/os2/README.md` with first-party
