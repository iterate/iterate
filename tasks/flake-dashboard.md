...
- [x] Starter app wiring: worker DO + dynamic worker ref + tsdown/build-manifest entries + package exports _(NOT mounted in `configs/default/worker.ts` yet — the config template installs `iterate` from pkg.pr.new @main, which only gains the new export once this PR merges; wiring it in the same PR bricked project creation in preview. Same lesson as config-templates-run-against-deployed-schemas.)_
...
- [ ] Follow-up PR (tiny, after @main's pkg.pr.new build includes the export): mount `FlakeDashboardApp` in `configs/default/worker.ts` — import, `#flakeDashboardApp = FlakeDashboardApp.create(this.env)`, and its `processEvent` call. Watch out for stale @main tarballs in the pnpm store (known issue) when validating.
