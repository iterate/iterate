...
- 2026-08-14 (cursor bot, verified real): the dormant-subscriber departure append in `webSocketClose` carried an idempotency key; keyed ephemeral inputs are rejected at canonicalize, so every departure observation would throw instead of landing. Key dropped (duplicates harmless — presence is runtime-authoritative), pinned in core-processor.test.ts. Commit 11cd1482e.
