...
# OS2 stream runtime big refactors

Bold follow-up refactors from the OS2 shared stream runtime migration. The
direction is:

1. OS2 becomes more monolithic and easier to handle.
...
1. Collapse the stream access surface in OS2.
   - Current state: OS2 has direct `env.STREAM` Durable Object calls, a
   ...
   - Proposal: choose one internal interface. Prefer direct env Durable Object
     ...
     `getStreamCapability` from ordinary OS2 app code.
...
2. Decide whether the Stream Runtime really belongs in `packages/shared`.
   ...
   - Proposal: either keep only narrow cross-app stream primitives in shared
     ...
     to OS2, or split shared streams into clearly named modules such as runtime,
     storage/sqlite, subscriptions, circuit-breaker, and debug.

3. Make Events an optional stream inspector/client.
   - Current state: Events can bind directly to OS2's `StreamDurableObject`
     namespace and exposes public Durable Object debug routes.
   - Problem: OS2 is supposed to be the simpler monolith, but proof/debug still
     requires another app and another Doppler config.
   - Proposal: move stream inspection/debug routes into OS2. If Events remains,
     point it at an OS2 stream-inspector entrypoint instead of the raw Durable
     Object namespace.
...
8. Tighten or remove the public OS2 stream oRPC.
   - Current state: OS2 exposes append/read/get-state by `projectId` and
     `streamPath`.
...
10. Isolate the `ctx.props` mutation bridge for `McpAgent.serve()`.
    ...
    - Proposal: isolate this behind a tiny documented SDK bridge helper so it
      does not become a general OS2 pattern.
