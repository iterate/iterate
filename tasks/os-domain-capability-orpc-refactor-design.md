...
This is a collaborative design document for turning OS inside out around domain
...
- Use `domains`, not `features`, for OS vertical modules. In this first
  refactor, domain modules own durable runtime behavior, capabilities,
  processors, utilities, tests, and reusable UI components.
- Keep actual TanStack route files in `apps/os/src/routes`.
- Keep normal oRPC routers in `apps/os/src/orpc` for now.
...
- Do not restructure `apps/os-contract` as part of this refactor.
- Treat `apps/os-contract` as a nuisance package that may be deleted or
  redesigned later; touch it only as needed for the `os.project.*` API shape.
- Do not move OS-specific domain implementations wholesale into
  `packages/shared`.
...
- `workspaces` remains a risky product term because OS already has Project and
...
- Move the current POC Slack capability into the Slack domain during this
  refactor.
  Shared Slack stream processors stay in `packages/shared` unless OS-specific
  Slack installation/webhook behavior is added.
- Split MCP into two domains:
  - `inbound-mcp-server` owns OS's Project-scoped MCP server, including
  ...
  - `outbound-mcp-client` owns the capability that lets codemode/inbound MCP
    connect from OS out to another MCP server.
...
- This should let users understand both the friendly preset and the underlying
  generated material.
...
OS domains should own OS-specific adapters:

- oRPC route shape.
- Clerk/project auth.
- Project-bound capability props.
- OS-specific UI components.
- OS deployment and Worker entrypoint wiring.

Do not move the OS domains wholesale into `packages/shared`. That would make
`packages/shared` depend on OS-specific concerns such as Clerk auth, OS app
config, D1 projections, Worker entrypoint exports, TanStack UI components, and
OS runtime tests. Shared should hold cross-app primitives. OS domains should
hold OS product/runtime behavior.

Leave `apps/os-contract` structurally alone for now. It is a nuisance package
...
- Add `os.project` to `apps/os-contract`.
- Do not split `apps/os-contract` into domain files.
...
- Copy the Documentation section below into an OS design doc under
  `apps/os/docs`.
- Reference that design doc from `apps/os/README.md`.
- Update `apps/os/CONTEXT.md` with any new terms that survive implementation:
...
- TanStack route files remain in `apps/os/src/routes`.
...
- OS local dev can list and create project streams.
- OS preview can log in through Clerk testing-token flow.
- OS preview Code Mode can call `ctx.os.streams.list({})`.
- OS preview Code Mode can call at least one non-stream project API through
  `ctx.os.*`.
...
- When Slack becomes real product functionality in OS, should it be
  `domains/slack` or remain under `domains/codemode` until it has installation
  and webhook config?

## Documentation

### OS domain modules and project-bound capabilities

OS is organized around domain modules. A domain module groups the code that
...
That is unnecessary for the current OS product shape.
...
editable through oRPC, and not Project-owned records. If OS later needs
...
- Shared packages can hold reusable runtime primitives without making Events or
  OS appear to own each other's domain types.
...
`apps/os-contract` is intentionally not reorganized in this refactor. The
...
Do not move OS domains wholesale into `packages/shared`. Shared packages should
contain cross-app primitives such as stream runtime types, callable descriptors,
and reusable stream processors. OS domains include app-specific auth, Project
...
App-level D1/sqlfu files remain global under `src/db`. The OS domain model
