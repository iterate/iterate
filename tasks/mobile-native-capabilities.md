...
- [x] **Make the running source revision recognizable.** Show one exact ISO timestamp across every app screen so a physical-device report can be tied to the source bundle under test. _The root layout displays `2026-07-17T20:47:11Z`; development-client users receive it when Metro serves this revision, while standalone builds retain the stamp in their bundled JavaScript._
