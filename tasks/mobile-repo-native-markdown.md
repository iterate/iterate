...
- [x] Make mobile-created projects expose their config repo before they open. _Creation now waits for terminal `project/created`, which is committed only after `/repos/config` exists, its trusted seeded worker has built and answered, and the permanent root feed has been installed._
