...
**Status summary:** complete. Root Playwright now owns the `web` and `mobile` projects, starts OS and Expo Web independently, and exposes Playwright's built-in project filtering. The duplicate mobile runner/config/dependency are gone; the browser spec and docs have moved to the root suite.

Fold the Expo Web browser test from PR #2064 into the normal root Playwright suite. Do not turn the OS dev lifecycle wrapper into a hidden multi-process supervisor: Playwright already supports multiple `webServer` entries and should directly own the extra Metro process it needs.
...
- [x] `pnpm spec --project=mobile` runs only the phone-sized mobile specs through Expo Web — _focused run passes the one mobile browser spec._
...
- [x] Move the mobile spec under root `specs/mobile/`, following `specs/AGENTS.md` — _the spec now uses root test support; screenshot baselines were deliberately removed in follow-up review._
...
- 2026-07-17: removed the two screenshot assertions, their lint suppressions, and the PNG baselines after review. Arbitrary screenshot capture will be introduced separately; this spec now tests browser behavior only.
