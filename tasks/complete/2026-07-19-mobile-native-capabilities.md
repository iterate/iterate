...
Remote push notifications and Secure-Enclave-backed signing require a custom native binary. Iterate therefore supports one phone-development runtime—its signed development client—rather than maintaining a second, less representative Expo Go surface.
...
- [x] **Create a signed iOS development build.** Add `expo-dev-client`, EAS project/build configuration, explicit development and production profiles, and repo scripts/documentation for building, installing, and starting the app. Use this as the only supported phone-development runtime; React Native Web remains the fast non-native UI lane. _EAS build `93272ee4-5572-4cd4-93ef-f321422f4a08` completed from commit `4ff99799c`; its signed `.ipa` proved the development-client lane and APNs entitlement._
...
- [x] ~~**Provide a notification-only smoke test.** Let a physical-device tester post an immediate local notification without project setup.~~ _Removed after physical push enrollment was proved; a dedicated screen and drawer item were unnecessary product surface._
...
- 2026-07-19: removed the standalone Notifications screen, its local-notification smoke helper, and the drawer item. Device enrollment and scriptable/approval push remain automatic product behavior rather than a permanent test-only navigation surface.
