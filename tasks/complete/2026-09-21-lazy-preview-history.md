...
Status: implementation and required checks pass. The final code passed all deployed tests and restoration; a pre-existing result-parser bug found during acceptance is fixed and covered. This docs-only push checks inheritance of that settled result; the outcome is recorded in PR #2744. The draft awaits review; PR monitoring is registered.
...
- [x] Run required repository checks; exercise the real preview workflow and document planning/checkout evidence. *Typecheck, lint, knip, format, 52 focused tests, and the full workspace-by-workspace suite passed. Final code at `e36843a25` passed unit CI, deployed tests and restoration, publishing `preview-settled: tests=success`; measurements are in `docs/preview-change-selection.md`.*
