...
- Corrected live run `m56xlhht6f` (`db3ae68f3`) completed all tests and cleanup successfully. Artifact `01a0b022-b00d-7e7e-99cb-695c0ae982d9` shows **6m11s to green**, eight producer runners and 92 test attempts, with no cleanup or wall-time metric. HTTP 200 and browser inspection passed.
- Live publication exposed a separate early-green webhook race: GitHub emits the completed-check event before upload and does not send another when Depot finishes. Config commits `fbd584e` and `f682aab` now revisit running-job uploads once a minute, bounded to 30 minutes, preserving the existing separate three error retries. All 22 artifact/publication tests and strict typecheck pass; independent review approved the budget correction. A fresh final run verifies automatic publication with that handler deployed.
