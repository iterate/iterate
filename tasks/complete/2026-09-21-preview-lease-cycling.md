...
Status: reopened for a repeat-run failure. Real cycling and one full green run are proven, but the next run exposed account-wide container deletion by another PR and destructive follow-up erasure. Both cleanup bugs now have regressions and fixes; the damaged namespaces are recovered without deleting the Worker. Replacement CI and full preview restoration are pending.
