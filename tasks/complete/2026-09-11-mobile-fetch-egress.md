...
- [x] Review the spec against existing mobile specs and Middlewright's implementation. *Followed notifications, approvals, chat titles, live status, media, and notes: one bounded stream subscription for unsolicited server work, then ordinary locators against that notification's row. No sleeps, polling, action-timeout overrides, or changes to shared test budgets. The real sandbox spec passes again in 42.2s without retries; spec typecheck, lint, and formatting pass.*
