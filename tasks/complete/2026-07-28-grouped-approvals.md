...
- **Pre-#2311 stream DOs are permanently wedged by `deleted applied migration: 20260709000001_create_subscriptions`** — #2311 replaced the subscriptions migrations outright, and sqlfu's guard refuses any stream DO whose applied history contains the deleted names. Fresh slots never hit it (acquire erases first), so main's CI stayed green; long-leased slots (and any deployed env with old DOs) fail the moment the new code touches an old stream — observed 2026-07-28 as preview-7's streams-example-app root stream stuck at "loading…" (stream-browser.spec.ts). Remedy per scripts/preview/preview.ts: `erase-data --env preview_N` per app + re-run (self-healing probe redeploys parked apps). Prod-side this is the known #2317 territory (semantic seeds, no stream migration) — nothing new to fix, but worth remembering whenever a held lease survives a merge of main that changes stream storage.
