...
- 2026-07-17: Review follow-up moved sender authorization into the router's reducer as well as its effects, so a denied `/new` cannot mutate a shared chat's live-session state. A regression test failed before the change and passes after it. The access editor now owns its Suspense fallback instead of flashing the whole integrations route, and Telegram Bot API ID coercion is shared between both processors.
- 2026-07-17: Second-pass review made denial delivery explicitly best-effort so an unreachable unauthorized user cannot hold the router checkpoint and block allowed traffic. Forum-topic denials now preserve `message_thread_id`. Both cases have red-green regression coverage.
- 2026-07-17: Refold migration coverage now preserves legacy `/new` history with its sender identity. The first explicit allowlist filters that reconstructed history to authorized senders; later policy edits gate new commands without erasing sessions that were valid when created.
- 2026-07-17: Deep-link lifecycle review now keeps the loading editor dismissible through the same URL-clearing handler and ignores stale links for disconnected Telegram connections instead of invoking a connected-only RPC.
