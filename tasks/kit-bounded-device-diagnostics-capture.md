...
- Still outstanding in this task: the ordered fixed-record diagnostic event
  ring and exact loss interval, durable host JSONL manifest, buffer inventory
  completeness gate, optional microSD policy, and the one/two/ten-minute
  physical proof ladder. The current slice is only the allocation-free metric
  snapshot/export foundation and must not be described as satisfying those
