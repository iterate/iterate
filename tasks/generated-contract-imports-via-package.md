...
`apps/mobile` (10 files) and `apps/streams-example-app` import types from
`apps/os/src/itx-api.generated.ts` via deep relative paths (`../../../os/src/…`).
The identical file ships in the package (`packages/iterate/src/itx-api.generated.ts`,
byte-equality test-enforced) and is exported from `iterate/sdk`, `iterate/client`,
and `iterate/node`.
