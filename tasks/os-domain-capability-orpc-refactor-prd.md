...
# OS Domain, Capability, and Project oRPC Refactor PRD

## Problem Statement

OS is accumulating project-scoped behavior in horizontal folders, legacy
...
development and a deployed OS preview environment. Old paths and old persisted
POC concepts should be replaced directly rather than kept as aliases.

## Solution

Refactor OS around explicit domain modules, project-bound capabilities, and a
singular project-scoped oRPC surface.

The public OS API should keep collection-oriented project operations under
...
Streams should be namespace-based at the shared runtime boundary. OS happens
to use the Project ID as the stream namespace for project-owned streams, but the
shared Stream Durable Object should know only about namespaces and paths. OS
...
The physical OS tree should start moving to `domains/*` without moving TanStack
routes, global oRPC wiring, app-level SQL/sqlfu files, or the existing contract
package structure into domains yet.

## User Stories

1. As an OS developer, I want project collection APIs separated from
   project-scoped APIs, so that API shape communicates ownership and scope.

2. As an OS developer, I want all project-scoped browser/API procedures under
   `os.project`, so that codemode and browser clients use one real API surface.

3. As an OS developer, I want `projectSlugOrId` accepted by project-scoped
   APIs, so that manual curl usage can use global slugs and internal callers can
   use stable IDs.

4. As an authenticated OS user, I want project slugs in browser URLs, so that
   URLs stay readable while backend code still uses stable Project IDs.

5. As an OS developer, I want unauthorized existing projects to return 403 and
   missing projects to return 404, so that auth behavior is explicit.

6. As an OS developer, I want project identity resolution in shared oRPC
...
9. As an OS developer, I want codemode `ctx.os` to wrap the real project oRPC
   router, so that codemode does not drift into a special parallel API.

10. As an OS developer, I want codemode to reject caller-supplied
...
13. As an OS developer, I want `ctx.os.listProcedures()` to work at any mount
    path, so that nested provider paths remain possible.

14. As an OS developer, I want stream handlers to call `StreamsCapability`, so
    that stream oRPC stays a validation/auth layer in front of trusted runtime
    behavior.

15. As an OS developer, I want `StreamsCapability` to support list/create/read/
    append/state operations, so that stream routers do not bypass it for catalog
    work.

16. As an OS developer, I want shared streams to use namespace/path language,
    so that the same runtime can support project namespaces and future platform
    namespaces.

17. As an OS user, I want the Streams page to list actual project streams, so
    that I can inspect existing streams without a sidebar tree.

18. As an OS user, I want Code Mode examples to be static authored stacks, so
    that the POC has useful starter material without persisted presets.

19. As an OS user, I want a new Codemode Session form that lets me pick example
    stack events and scripts, so that I can quickly start a useful session.

20. As an OS user, I want the Codemode Session UI to show selected raw events,
    so that I can see exactly what will be appended.

21. As an OS user, I want to add providers to an existing Codemode Session, so
    that I can extend the session without creating a new one.

22. As an OS developer, I want Project Preset database records removed, so that
    there is no stale persisted POC surface.

23. As an OS developer, I want preset deletion done through sqlfu, so that
...
26. As an OS developer, I want inbound MCP and outbound MCP separated as
    domains, so that OS-as-server and OS-as-client are not conflated.
...
28. As an OS developer, I want promise-pipelined live handles preserved, so that
    tool providers can return Durable Object-backed capabilities safely.

29. As an OS developer, I want POC repo, workspace, Slack, and agent tool
...
30. As an OS developer, I want every domain folder to have a short README and
    AGENTS symlink, so that humans and agents get the same local guidance.

31. As an OS developer, I want Worker entrypoint files to mostly re-export
    domain-owned runtime classes, so that runtime ownership sits with the
    domain.

32. As an OS developer, I want routes and global oRPC wiring to remain outside
    domains for now, so that the first refactor stays small enough to land.

33. As an OS developer, I want local Miniflare and preview verification, so
    that the POC is proven in both development and deployed preview.

34. As an OS developer, I want existing OS and Events smoke tests to keep
...
- Existing OS e2e tests should continue working after the refactor.

- Existing Events e2e tests should continue working after the shared stream
  runtime and namespace changes.

- Existing manual smoke tests against OS and Events should continue working.

- Local verification must include Miniflare/local dev proof for OS Code Mode
  and project streams.
...
- Splitting OS domains into separate packages.
