...
- [x] Capture phone-width interaction evidence for the PR — _390×844 browser pass covers drawer animation, folder tree, file-open dismissal, clean and dirty CodeMirror editor states, Git dirty badge, change review, and demo commit_
- [x] Put a clickable same-LAN Expo Web demo on the PR and preserve that expectation for future mobile UI work — _`apps/mobile/AGENTS.md` documents exact-route LAN web links plus Expo Go deep links and their server/network lifetime_
...
- 2026-07-17: added dedicated editor evidence after review feedback: `mobile-ide-poc-editor.png` shows an opened clean TypeScript file and `mobile-ide-poc-editor-dirty.png` shows an inserted mobile edit with dirty-path marker and discard action.
- 2026-07-17: merged current `origin/main`, retaining main's Expo/React Native Web/Playwright upgrades and the POC dependencies. Added mobile-scoped agent guidance requiring exact-route LAN demo links in UI PRs.
