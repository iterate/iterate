...
- When a request repeats an earlier one with different inputs, reuse the journaled script instead of rewriting it: `const h = await itx.capabilityHost.previousScriptHelper({ ...results[N], parameterize: { x: <the exact primitive value that script used inline, appearing exactly once> } })`; then `return await h.run({ x: newValue })`.
</section>
